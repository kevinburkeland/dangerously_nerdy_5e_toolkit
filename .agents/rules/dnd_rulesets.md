...
| **Heavy Weapon / Power Attack** | -5 attack penalty for +10 flat damage (2014 legacy) | Adds Proficiency Bonus (PB) damage to heavy weapon attacks on hit (2024) |
...
- **No WotC Product Identity:** Never generate code, tests, fixtures, or mock data referencing Product Identity monsters, characters, or locations.
  ...
  - Permitted: SRD 5.1 & SRD 5.2.1 monsters and subclasses (e.g., Red Dragon, Knight, Wolf, Skeleton, Zombie, Ogre, Mage, Champion, Evoker, etc.).
...
- **2014 RAW Subclass Milestones:**
  ...
  - **Level 3:** Barbarian, Bard, Fighter, Monk, Paladin, Ranger, Rogue (and custom round-up half-casters).
...
- **Single-Class Half & Third Casters:**
  - Single-class half-casters (Paladins, Rangers), round-up half-casters (for imported or custom class definitions), and 1/3-casters (Eldritch Knight, Arcane Trickster) MUST use their dedicated class tables (`_halfCasterSlots2014`, `_halfCasterSlots2024`, `_thirdCasterSlots`, `_artificerSlots`).
...
- **Class Starting Tool Proficiencies & Custom Tools Selection:**
  - Classes and imported archetypes with tool choices or fixed starting tools receive appropriate tool grants (e.g. Rogues with Thieves' Tools, Druids with Herbalism Kit, or custom classes with tool packages).
  - `CharacterBuilderScreen._buildClassToolsPrompt` renders an interactive artisan/tool selection chip group while displaying granted tools.
  - `SkillTraitResolver.resolveTools` and `CharacterDraft.reconcile` ensure starting tools for classes (including Thieves' Tools for Rogues and Herbalism Kit for Druids, as well as imported tool packages) are never dropped.

## 10. Class Starting Skills & Attributes-First Flexible Lineage Architecture

- **Class Starting Skills Ingestion:**
  - Class definitions ingest starting skill proficiencies via `CompendiumClassParser.parseClassSkills` from external compendium `startingProficiencies.skills` (handling `choose.from`, `any`, lists, and `{@skill}` tags) into `customProperties['allowedSkills']` and `customProperties['skillChoiceCount']`.
