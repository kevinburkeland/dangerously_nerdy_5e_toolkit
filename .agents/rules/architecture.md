...
### 1.5. Clean Room Anti-Corruption Layer (ACL) & Capability-Driven Mechanics
- **Zero Third-Party Tool (5etools) Coupling:** No hardcoded 5etools collection keys (e.g. `monsterfluff`, `racefluffmeta`, `subclassfeature`), third-party schema markers, or splatbook names in core business logic.
- **Agnostic Structural Ingestion:** Ingestion adapters (`GithubIngestorAdapter`) dynamically traverse structures and identify entities using generic structural indicators (`name`, `entries`, `hitDie`, `stats`, `type`, `level`, actions) rather than third-party tool bundles.
- **External Text Parsing Isolation:** External pipe-delimited syntax (`Feature|Class|Source|Level`, `Spell|Source#c`) must be isolated in dedicated infrastructure ACL parsers (`CompendiumPipeParser`), keeping domain models clean.
- **Capability-Driven `FeatureGrant` Architecture:** All mechanical capabilities, substitutions, and combat rules resolve via domain `FeatureGrant`s (`GrantType.attackAbilitySubstitution`, `GrantType.capabilityFlag`) or normalized capability flags (`flags['initiativeBonusMode']`, `flags['mediumArmorDexCapBonus']`, `flags['passivePerceptionBonus']`). Never pattern match on non-SRD string names.
