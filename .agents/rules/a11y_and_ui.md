...
## 5. Tabular UI Rendering & Card Horizontal Scrolling

- Tables in cards (`DmRuleCard`, `ItemCard`, `FeatCard`, `SpellCard`, `RaceCard`) must render via `ScrollableMarkdownTable` / `FormattedMarkdownText`.
- Tables must be enclosed in horizontal `SingleChildScrollView` with interactive `Scrollbar` (visible thumb/track).
- `ScrollConfiguration` must include `PointerDeviceKind.mouse` alongside touch, trackpad, and stylus to support mouse drag-scrolling on web and desktop.
- Cards must use `clipBehavior: Clip.antiAlias` to prevent table content from bleeding outside rounded card corners.
- Horizontal table dragging must NOT trigger card tap actions (`InkWell.onTap`).
