...
- **Timestamp-Gated Snapshot Reconciliation:** In `RoomSyncOrchestrator._handleIncomingPayload`, incoming `room_sync_full` payloads are dropped if their timestamp is older than or equal to `_lastProfileSyncTimestamp` (`profileTs <= _lastProfileSyncTimestamp && _lastProfileSyncTimestamp != 0`), guaranteeing that late-arriving full snapshots do not overwrite more recent state.
