...
import 'package:vtt_engine_core/storage/models/storage_checksum.dart';
...
void main() {
  group('EngineProfile Value Object Invariants', () {
    test('Token-based user agent detection without regex', () {
      ...
      final iosTabProfile =
          UserAgentParser.parse(iosSafariUa, isStandalonePwa: false);
      ...
      final iosPwaProfile =
          UserAgentParser.parse(iosSafariUa, isStandalonePwa: true);
      ...
      final ffPwaProfile =
          UserAgentParser.parse(firefoxUa, isStandalonePwa: true);
  ...
  group('StorageTelemetryReport Permutations & Risk Properties', () {
    test(
        'Evaluates isUnprotected strictly against WebKit eviction risk and persistence',
        () {
  ...
  group('StorageSnapshotBundle Cryptographic Integrity', () {
    test('Validates authentic bundle with constant-time SHA-256 seal', () {
      final payload =
          Uint8List.fromList(utf8.encode('{"campaign":"Staging Node"}'));
      ...
      final expectedChecksum =
          StorageChecksum.computeBundleChecksum('vault_alpha', payload);
    ...
    test('Rejects tampered payloadBytes or altered vaultId', () {
      ...
      expect(() => tamperedChecksumBundle.validateOrThrow(),
          throwsA(isA<StateError>()));
    });

    test('Round-trip envelope serialization preserves cryptographic seal', () {
      final payload =
          Uint8List.fromList(utf8.encode('{"characters": ["c1", "c2"]}'));
      final bundle = StorageSnapshotBundle.create(
        vaultId: 'vault_delta',
        payloadBytes: payload,
        schemaVersion: '1.2.0',
