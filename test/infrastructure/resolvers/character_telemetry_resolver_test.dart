...
void main() {
  ...
  group('CharacterTelemetryResolver', () {
    test('formatSlugToTitle converts kebab and snake case to clean Title Case',
        () {
      expect(CharacterTelemetryResolver.formatSlugToTitle('blood-hunter'),
          'Blood Hunter');
      expect(
          CharacterTelemetryResolver.formatSlugToTitle(
              'order_of_the_ghostslayer'),
          'Order Of The Ghostslayer');
      expect(
          CharacterTelemetryResolver.formatSlugToTitle('homebrew-shadow-mage'),
          'Homebrew Shadow Mage');
      expect(
          CharacterTelemetryResolver.formatSlugToTitle('champion'), 'Champion');
      expect(CharacterTelemetryResolver.formatSlugToTitle(''), 'Unknown');
    });

    test(
        'resolves canonical SRD species, background, class, subclass, feats, and equipment',
        () async {
      final dto = CharacterTelemetryDto(
        ...
        classPointers: const [
          ClassLevelPointerDto(
              classSlug: 'fighter', subclassSlug: 'champion', level: 5),
        ],
        currentHp: 45,
        maxHp: 50,
        tempHp: 5,
      ...
      expect(resolved.unresolvedSlugs, isEmpty,
          reason: 'Unresolved slugs: ${resolved.unresolvedSlugs}');
      ...
      expect(resolved.equippedItemNames.any((i) => i.contains('Plate Armor')),
          isTrue);
    ...
    test(
        'gracefully falls back to title case with hasUnresolvedPointers for unknown slugs',
        () async {
      final dto = CharacterTelemetryDto(
        id: 'homebrew-hero',
        name: 'Kallista',
    ...
    test(
        'evaluates combat status getters (hpPercent, isConscious, isDying, isDead)',
        () {
      // 1. Healthy character
    ...
    test(
        'resolves Breastplate to exact Breastplate entity without inheriting Plate Armor (Full Plate)',
        () async {
      final dto = CharacterTelemetryDto(
        id: 'skirmisher-hero',
        name: 'Ranger Dave',
      ...
      expect(
          resolved.equippedItemNames.any(
              (name) => name.contains('Full Plate') || name == 'Plate Armor'),
          isFalse);
