...
void main() {
  group('Variant Lineage & Nested Species Ability Ingestion Tests', () {
    final customArtificeLineageJson = <String, dynamic>{
      'name': 'Human (Lineage of the Forge)',
      ...
      'entries': [
        ...
        {
          'name': "Smith's Intuition",
          'entries': [
            'When you make an Arcana check or an ability check with artisan tools, you can add 1d4.'
          ],
        },
      ],
      'customTrait': "Smith's Intuition",
      'lineageType': 'Forge',
    };

    final customWardingLineageJson = <String, dynamic>{
      'name': 'Human (Lineage of the Shield)',
      'source': 'CUSTOM_LINEAGE',
      'raceName': 'Human',
      'raceSource': 'SRD',
      'ability': [
      ...
      'entries': [
        "Shield's Intuition: When you roll Initiative or make an Insight or Perception check, add 1d4.",
      ],
    };
    ...
    test(
        'correctly classifies subraces and entries with raceName/subrace as race',
        () {
    ...
    test(
        'extracts nested static stat bonuses and choice pools into normalizedData',
        () {
      final dtoArtifice = HomebrewEntityDto.fromJson(
        customArtificeLineageJson,
        ruleset: RulesetVersion.srd2014,
      ...
      final abilities =
          dtoArtifice.normalizedData['abilities'] as Map<String, dynamic>;
      ...
      final flex = dtoArtifice.normalizedData['flexibleAbilities']
      ...
      expect(dtoArtifice.unparsedPayload['customTrait'],
          equals("Smith's Intuition"));
      expect(dtoArtifice.unparsedPayload['lineageType'], equals('Forge'));
    });
    ...
    test('extracts multiple fixed ability modifiers from nested ability map',
      ...
      final abilities =
          dtoWarding.normalizedData['abilities'] as Map<String, dynamic>;
    ...
    test('combines root-level stats and nested choice pools seamlessly', () {
      ...
      final flex =
          dto.normalizedData['flexibleAbilities'] as Map<String, dynamic>;
      expect(flex['count'], equals(1));
      expect(flex['from'], containsAll(['dex', 'int']));
    });

    test(
        'HomebrewIngestor maps DTO into Domain Race with accurate bonuses and choice pools',
        () {
      final dtoArtifice = HomebrewEntityDto.fromJson(
        customArtificeLineageJson,
        ruleset: RulesetVersion.srd2014,
      ...
      expect(race.name, equals('Human (Lineage of the Forge)'));
      ...
      final flexProp =
          race.customProperties['flexibleAbilities'] as Map<String, dynamic>;
      expect(
          flexProp['from'], containsAll(['str', 'dex', 'con', 'wis', 'cha']));
      ...
      expect(race.traitsMarkdown, contains("Smith's Intuition"));
    });

    test(
        'HomebrewIngestor.parseCustomRaces batch ingests multiple species without error',
        () {
      final batch = [
        customArtificeLineageJson,
        customWardingLineageJson,
      ...
      expect(races[0].name, equals('Human (Lineage of the Forge)'));
      expect(races[0].fixedAbilityBonuses['int'], equals(2));
      expect(races[1].name, equals('Human (Lineage of the Shield)'));
