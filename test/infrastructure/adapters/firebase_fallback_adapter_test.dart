...
void main() {
  group('FirebaseFallbackAdapter Relay Tests', () {
    ...
    test(
        'Initializes and broadcasts payload to relay messages collection with expireAt TTL',
        () async {
    ...
    test(
        'disconnect closes incoming payloads stream and re-initialization reopens cleanly',
        () async {
      ...
      expect(isDone, isTrue,
          reason: 'Incoming payloads stream should be closed on disconnect');
      await sub.cancel();
      ...
      final sub2 =
          adapter.watchIncomingPayloads().listen(receivedAfterReconnect.add);
    ...
    test(
        'tolerates clock skew with 30-second query threshold and deduplicates payload hashes',
        () async {
    ...
    test(
        'enforces maxProcessedPayloadHashes LRU bound and clears tracking on disconnect',
        () async {
      ...
      expect(adapter.processedPayloadHashes.length,
          equals(FirebaseFallbackAdapter.maxProcessedPayloadHashes));
    ...
    test(
        'cleanUpPeerRelay prunes only specified peerId hashes preserving multi-peer session (N >= 3)',
        () async {
      await adapter.initializeRoom('ROOM-MESH', 'local-node');

      const payloadPeerA = '{"from":"peer-A","data":"attack"}';
      const payloadPeerB = '{"from":"peer-B","data":"cast_spell"}';
      final hashA = CryptoUtils.sha256Hex(payloadPeerA);
      final hashB = CryptoUtils.sha256Hex(payloadPeerB);

      adapter.emitIncomingPayload(payloadPeerA,
          messageId: 'msg-a', senderId: 'peer-A');
      adapter.emitIncomingPayload(payloadPeerB,
          messageId: 'msg-b', senderId: 'peer-B');
      await Future<void>.delayed(Duration.zero);

      expect(adapter.processedPayloadHashes.contains(hashA), isTrue);
      expect(adapter.processedPayloadHashes.contains(hashB), isTrue);

      // Prune peer-A only
      adapter.cleanUpPeerRelay('peer-A');

      // peer-A hash must be evicted, but peer-B must remain tracked
      expect(adapter.processedPayloadHashes.contains(hashA), isFalse);
      expect(adapter.processedPayloadHashes.contains(hashB), isTrue);
    });
  });
}
