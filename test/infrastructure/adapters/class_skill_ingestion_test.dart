...
void main() {
  group('Class Skill Ingestion & Normalization Tests', () {
    test('Detects class entity from hitDie and startingProficiencies without explicit entityType', () {
      final json = <String, dynamic>{
        ...
        'startingProficiencies': {
          ...
          'skills': [
            {
              'choose': {
                'from': ['athletics', 'history', 'insight', 'intimidation', 'persuasion'],
                'count': 2,
              },
            },
          ],
      ...
      final dto = HomebrewEntityDto.fromJson(json, ruleset: RulesetVersion.srd2014);
      ...
      expect(flex['count'], 2);
      expect(flex['from'], containsAll(['athletics', 'history', 'insight', 'intimidation', 'persuasion']));
      expect(dto.normalizedData['allowedSkills'], containsAll(['athletics', 'history', 'insight', 'intimidation', 'persuasion']));
    });

    test('Prevents startingProficiencies and class skill keys from leaking into unparsedPayload', () {
      final json = <String, dynamic>{
        'name': 'Scholar',
        'hd': 6,
        'startingProficiencies': {
      ...
      final dto = HomebrewEntityDto.fromJson(json, ruleset: RulesetVersion.srd2024);
      expect(dto.entityType, 'class');
    ...
    test('Extracts fixed skills and choice pools simultaneously', () {
      ...
      final dto = HomebrewEntityDto.fromJson(json, ruleset: RulesetVersion.srd2014);
      expect(dto.normalizedData['skillProficiencies'], containsAll(['investigation', 'insight']));
      expect(dto.normalizedData['flexibleSkills']?['count'], 1);
      expect(dto.normalizedData['flexibleSkills']?['from'], containsAll(['perception', 'religion', 'stealth']));
    });
    ...
    test('Parses natural language skill selection string', () {
      final json = <String, dynamic>{
        ...
        'hitDie': 10,
        'startingProficiencies': {
          'skills': 'Choose two from Acrobatics, Athletics, Deception, Insight, Perception, and Sleight of Hand',
        },
      };
      ...
      expect(flex!['count'], 2);
      expect(flex['from'], containsAll(['acrobatics', 'athletics', 'deception', 'insight', 'perception', 'sleight of hand']));
    });
    ...
    test('Maps HomebrewEntityDto into domain ClassDefinition / CharacterClass', () {
      ...
      expect(domainClass.allowedSkills, containsAll([
        SkillType.athletics,
