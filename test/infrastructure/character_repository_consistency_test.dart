...
void main() {
  ...
  group('LocalCharacterRepository & CharacterPersistenceService Consistency',
      () {
    test(
        'Saving via CharacterPersistenceService is immediately visible to LocalCharacterRepository',
        () async {
    ...
    test(
        'Saving via LocalCharacterRepository is immediately visible to CharacterPersistenceService',
        () async {
      final persistence = CharacterPersistenceService();
      final repository = LocalCharacterRepository();
    ...
    test('Zero stale cache split-brain across multiple interleaved writes',
        () async {
      final persistence = CharacterPersistenceService();
      final repository = LocalCharacterRepository();
      ...
      expect(char!.resources.currentHp, 19,
          reason: 'Repository must not cache stale character objects');
    });
  });
}
