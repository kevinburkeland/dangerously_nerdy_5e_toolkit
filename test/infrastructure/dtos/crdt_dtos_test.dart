...
void main() {
  ...
  group('CrdtLwwRegisterDto Tests', () {
    test(
        'Clamping Enforcement Test: parses and clamps numeric values strictly within bounds',
        () {
      ...
      final regNegative =
          CrdtLwwRegisterDto.fromMap<int>(jsonPayloadNegative, hpDecoder);
      ...
      final regExorbitant =
          CrdtLwwRegisterDto.fromMap<int>(jsonPayloadExorbitant, hpDecoder);
    ...
    test('fromMap returns null on malformed timestamp or decoder error', () {
      ...
      final reg =
          CrdtLwwRegisterDto.fromMap<int>(malformedPayload, (v) => v as int);
      expect(reg, isNull);
    });
  });
  ...
  group('CrdtOrSetDto Tests', () {
    test(
        'Fault Isolation Test: corrupted item 2 does not crash set and preserves items 1 and 3',
        () {
    ...
    test(
        'Round-Trip Fidelity: CrdtOrSet<AnimatedObjectInstance> survives full serialization cycle',
        () {
      ...
      const ts1 = HybridLogicalClock(
          physicalTime: 1700000001000, logicalCounter: 1, nodeId: 'nodeA');
      const ts2 = HybridLogicalClock(
          physicalTime: 1700000002000, logicalCounter: 0, nodeId: 'nodeB');
      const tombstoneTs = HybridLogicalClock(
          physicalTime: 1700000000500, logicalCounter: 0, nodeId: 'nodeA');
      ...
      final restoredSet = CrdtOrSetDto.fromMap<AnimatedObjectInstance>(
        decodedMap,
        (raw) =>
            AnimatedObjectDto.fromMap(raw as Map<String, dynamic>).toDomain(),
      );
    ...
    test(
        'Strict Type Casting Test: handles <dynamic, dynamic> maps without throwing type errors',
        () {
