...
void main() {
  group('CharacterDto Serialization & ACL Integrity', () {
    test('round-trips arbitrary unknown homebrew fields with 100% fidelity',
      ...
      expect(dto.unparsedPayload['thirdPartyTags'],
          equals(['veteran', 'battle-forged']));
      ...
      expect(
          outputJson['thirdPartyTags'], equals(['veteran', 'battle-forged']));
      expect(outputJson['vtt_meta']['token_url'],
          equals('https://vtt.example/valeros.png'));
    });
    ...
    test(
        'safely defaults missing or erroneous keys without throwing TypeErrors',
        () {
