...
void main() {
  ...
  group('Single-Source Persistence Verification Tests', () {
    test(
        'saveCharacter writes to AppDatabaseService box and bypasses SharedPreferences',
        () async {
      ...
      expect(mockDb.writtenData.containsKey(AppDatabaseService.boxCharacters),
          isTrue);
      ...
      expect(rawList.first['name'], equals('Thorek Ironbreaker'));

      // Verify SharedPreferences received zero writes
      final prefs = await SharedPreferences.getInstance();
      expect(prefs.getKeys(), isEmpty,
          reason:
              'No SharedPreferences setString/put calls should occur during standard saveCharacter');
    });

    test(
        'saveProfile writes to AppDatabaseService box and bypasses SharedPreferences',
        () async {
      final initialPrefs = await SharedPreferences.getInstance();
      expect(initialPrefs.getKeys(), isEmpty);
      ...
      expect(
          mockDb.writtenData
              .containsKey(AppDatabaseService.boxCampaignProfiles),
          isTrue);
      final campBox =
          mockDb.writtenData[AppDatabaseService.boxCampaignProfiles]!;
      expect(
          campBox.containsKey('dn5e_campaign_profile_camp_iron_peaks'), isTrue);

      // Verify SharedPreferences received zero writes
      final prefs = await SharedPreferences.getInstance();
      expect(prefs.getKeys(), isEmpty,
          reason:
              'No SharedPreferences setString/setStringList calls should occur during standard saveProfile');
    });

    test(
        'retains SharedPreferences read strictly inside load methods as one-time legacy fallback',
        () async {
      final legacyJson =
          json.encode([CharacterDto.fromDomain(legacyHero).toMap()]);
      ...
      expect(loaded.first.name, equals('Alden the Grey'));

      // Flushed to database box during migration
      expect(
          mockDb.writtenData[AppDatabaseService.boxCharacters]
