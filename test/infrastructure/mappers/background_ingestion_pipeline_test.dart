...
void main() {
  group('Background Ingestion & Character Compilation Pipeline', () {
    test(
        'Ingests custom background JSON and parses skills, equipment, and 2024 ASIs',
        () {
      ...
      expect(
          bg.customProperties['startingEquipment'],
          containsAll(
              ['Herbalism kit', 'Poultice pouch', 'Traveler\'s clothes']));
    ...
    test(
        'CharacterFactory.buildFromDraft grants background skills, equipment, and 2024 ASIs onto Character',
        () {
      final rawBackgroundJson = {
        'name': 'Wandering Herbalist',
        'entityType': 'background',
      ...
      expect(character.skillProficiencies[SkillType.survival],
          equals(SkillProficiencyLevel.proficient));
      expect(character.skillProficiencies[SkillType.medicine],
          equals(SkillProficiencyLevel.proficient));
      expect(character.skillProficiencies[SkillType.nature],
          equals(SkillProficiencyLevel.proficient));

      // 2. Starting Equipment: background items instantiated in inventory
      expect(
          character.inventory
              .any((i) => i.itemRef.displayName.contains('Herbalism kit')),
          isTrue);
      expect(
          character.inventory.any(
              (i) => i.itemRef.displayName.contains('Traveler\'s clothes')),
          isTrue);
    ...
    test(
        'Ingests 2014 custom background JSON without ASIs without throwing validation error',
        () {
      final raw2014BackgroundJson = {
        ...
        'entries': ['You spent years walking the watch on city ramparts.'],
      };
