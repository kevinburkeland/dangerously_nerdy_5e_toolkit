...
void main() {
  group('SpellDto & Unparsed Payload Scooping', () {
    test(
        'fromJson preserves standard fields and scoops unknown keys into unparsedPayload',
        () {
      final json = {
      ...
      expect(dto.unparsedPayload['nestedVendorMetadata'],
          equals({'price': 500, 'rarity': 'uncommon'}));
      ...
      expect(serialized['nestedVendorMetadata'],
          equals({'price': 500, 'rarity': 'uncommon'}));
      ...
      expect(domainSpell.customProperties['homebrewSource'],
          equals('Custom Campaign'));
    });

    test(
        'SpellDto clamps level between 0 and 9 and defaults missing fields safely',
        () {
      final json = {
        'id': 'epic-spell',
  ...
  group('AnimatedObjectDto Unparsed Payload Tests', () {
    test(
        'fromMap scoops unknown fields into unparsedPayload and retains them in toMap',
        () {
      final map = {
        'id': 'minion-123',
        'name': 'Animated Sword',
  ...
  group('HomebrewIngestor ACL Tests', () {
    test(
        'parseCustomSpells isolates malformed records and preserves valid spells',
        () {
      final rawBundle = [
        // Valid spell 1
        {
    ...
    test('parseCustomCharacters isolates corrupt entries', () {
      ...
      expect(characters[0].unparsedPayload['customBackstory'],
          equals('Knight of the Realm'));
  ...
  group('Homebrew Ingestor Fortification & Parsing Edge Cases', () {
    test(
        'Elemental Absorption and Frost Shard map higherLevelsMarkdown dice step into damageMath.scalingFormula',
        () {
      final absorbJson = {
      ...
      final coldMath = frostShardDto.damageMath
          .firstWhere((dm) => dm.damageType == DamageType.cold);
      expect(coldMath.scalingFormula, equals('1d6'));
    });

    test(
        'Chaotic Blast and Prismatic Orb map damageType as variable rather than acid',
        () {
      ...
      expect(chaosBlastDto.damageMath.first.damageType,
          equals(DamageType.variable));
      ...
      expect(prismaticOrbDto.damageMath.first.damageType,
          equals(DamageType.variable));
    });

    test(
        'Scorching Line range normalizes to rangeDistanceFeet: 30 and rangeType: "line"',
        () {
    ...
    test(
        'Frost Shard damageMath array flags 1d10 piercing as attack roll and 2d6 cold as saving throw',
        () {
      final frostShardJson = {
        'id': 'frost-shard',
        'name': 'Frost Shard',
      ...
      final piercing =
          dto.damageMath.firstWhere((dm) => dm.diceFormula == '1d10');
    ...
    test(
        'reconciles narrative spatial geometry and repairs mislabeled damage types from description',
        () {
      ...
      final piercingDm =
          frostShardDto.damageMath.firstWhere((dm) => dm.diceFormula == '1d10');
      final coldDm =
          frostShardDto.damageMath.firstWhere((dm) => dm.diceFormula == '2d6');
    ...
    test(
        'deserialization priority correctly overrides stale raw fields and persists boolean flags in exported JSON',
        () {
      ...
      final exportedMath =
          (exportedFrost['damageMath'] as List).cast<Map<String, dynamic>>();

      final piercingMap =
          exportedMath.firstWhere((m) => m['diceFormula'] == '1d10');
    ...
    test(
        're-ingesting homebrew JSON bundle sets Chaotic Blast to variable and retains Frost Shard attack roll in exported JSON',
        () {
      final rawBundle = [
        {
          'id': 'chaotic-blast',
      ...
      expect(chaoticBlast.damageMath.first.damageType,
          equals(DamageType.variable));
      ...
      final exportedMath = (exportedFrostShard['damageMath'] as List)
      ...
      final coldMath =
          exportedMath.firstWhere((m) => m['diceFormula'] == '2d6');
