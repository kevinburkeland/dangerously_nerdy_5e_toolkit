...
void main() {
  group('Comprehensive Accessibility (a11y) Verification Tests', () {
    testWidgets(
        'PressableCard supports keyboard focus and visible focus border',
        (tester) async {
      bool tapped = false;
    ...
    testWidgets(
        'ObjectCard provides minimum 48x48dp HP button touch targets & semantic stats',
        (tester) async {
      final handle = tester.ensureSemantics();
      final object = MinionInstance(
        id: 'obj_1',
      ...
      expect(
          find.bySemanticsLabel(RegExp(r'Armor Class: \d+')), findsOneWidget);
      expect(find.bySemanticsLabel(RegExp(r'Attack Bonus: \+\d+')),
          findsOneWidget);
    ...
    testWidgets(
        'CreatureStatBlockDialog expands ability score acronyms for screen readers',
        (tester) async {
      final handle = tester.ensureSemantics();
      ...
      expect(find.bySemanticsLabel(RegExp(r'Strength: \d+, modifier .*')),
          findsOneWidget);
      expect(find.bySemanticsLabel(RegExp(r'Dexterity: \d+, modifier .*')),
          findsOneWidget);
      expect(find.bySemanticsLabel(RegExp(r'Constitution: \d+, modifier .*')),
          findsOneWidget);
    ...
    testWidgets(
        'BatchAttackResultsCard exposes comprehensive a11y labels and high-contrast badges',
        (tester) async {
      final handle = tester.ensureSemantics();
      final obj = MinionInstance(
        id: '1',
      ...
      expect(
        find.bySemanticsLabel(RegExp(
            r'Batch Attack Summary: 8 total damage, 1 of 1 attacks hit, 0 critical hits.')),
        findsOneWidget,
      );
      ...
      expect(
        find.bySemanticsLabel(RegExp(
            r'Silver Coin #1 \(Tiny\): rolled 18 with bonus \d+ equals 26 against Armor Class 15. Hit! 8 damage dealt.')),
        findsOneWidget,
      );
    ...
    testWidgets(
        'LatestRollCard exposes detailed dice breakdown in semantic summary and liveRegion',
        (tester) async {
      final handle = tester.ensureSemantics();
      ...
      expect(
        find.bySemanticsLabel(RegExp(
            r'Latest Roll Result: 23\. Formula: 1d20 \+ 3\. Dice rolled: d20: 20\. Modifier: \+3\. 🔥 NATURAL 20! CRITICAL HIT')),
        findsOneWidget,
      );
    ...
    testWidgets('AnimatedResourceMeter respects MediaQuery disableAnimations',
        (tester) async {
      await tester.pumpWidget(
        const MaterialApp(
          home: MediaQuery(
    ...
    testWidgets(
        'CriticalEffectOverlay silences screen shake when disableAnimations is active',
        (tester) async {
    ...
    test('A11yService methods execute cleanly without throwing', () {
      ...
      expect(
        () => A11yService.announceHpChange('Goblin',
            currentHp: 5, maxHp: 10, delta: -2),
        returnsNormally,
      );
    });
  });
