...
void main() {
  group('DiceFormatters Tests', () {
    ...
    test('formatFormula formats complete dice expressions with damage types',
        () {
      expect(
          DiceFormatters.formatFormula(
              count: 1, sides: 4, bonus: 4, damageType: 'Bludgeoning'),
          '1d4+4 Bludgeoning');
      expect(
          DiceFormatters.formatFormula(
              count: 2, sides: 6, bonus: -1, damageType: 'Slashing'),
          '2d6-1 Slashing');
      expect(DiceFormatters.formatFormula(count: 8, sides: 6), '8d6');
      expect(DiceFormatters.formatFormula(count: 1, sides: 20, bonus: 5),
          '1d20+5');
    });

    test(
        'formatCompositeFormula formats composite and single damage formulas accurately',
        () {
      expect(
