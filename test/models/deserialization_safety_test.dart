...
void main() {
  group('AnimatedObjectInstance Equality Contract', () {
    test(
        'Instances with identical IDs but different combat state are NOT equal',
        () {
    ...
    test(
        'Instances with identical fields evaluate as equal and share hashCodes',
        () {
      final obj1 = AnimatedObjectInstance(
        id: 'token_1',
        name: 'Animated Table',
        ...
        customProperties: const {'isSilvered': true},
        customAc: 15,
        customAttackBonus: 6,
      );
  ...
  group('CampaignProfile Deserialization Data Safety & Roster Preservation',
      () {
    test(
        'Preserves malformed or unparsed character payloads without dropping them on save',
        () {
      final malformedCharacterPayload = {
        'id': 'corrupt_char_99',
        'name': 'Corrupted Hero',
        'abilityScores':
            'INVALID_STRING_INSTEAD_OF_MAP', // Causes Character.fromMap to throw
      };
    ...
    test('Preserves malformed active minions payloads on round-trip', () {
      ...
      expect(profile.roomState.activeMinions.length,
          equals(1)); // AnimatedObjectDto.fromMap falls back safely
    });
  });

  group('MonsterCombatProfile & Arena Simulation Pre-Computation', () {
    test(
        'Parses spell slots, DCs, reach, and saving throws once into typed primitives',
        () {
  ...
  group('Race Normalized Typed Fields', () {
    test(
        'Correctly exposes typed bonus feats and flexible ability score increases',
        () {
      final variantHuman = Race(
        id: const EntityId(
            slug: 'human-variant', ruleset: RulesetVersion.v2014),
        name: 'Human (Variant)',
