...
void main() {
  group('ArenaCondition & ActiveCondition Model Tests', () {
    test(
        'ArenaCondition properties and metadata are defined for all 5e conditions',
        () {
    ...
    test('ActiveCondition duration tracking, badges, and turn ticking', () {
      const infiniteCondition =
          ActiveCondition(condition: ArenaCondition.poisoned);
      expect(infiniteCondition.hasFiniteDuration, false);
      expect(infiniteCondition.durationBadge, '∞');
      expect(
          infiniteCondition.durationDisplay, contains('Until End of Combat'));
    ...
    test(
        'ArenaCombatant clone and reset preserve and isolate condition collections',
        () {
      final wolfMonster = MonsterCodexLibrary.allMonsters
          .firstWhere((m) => m.name.toLowerCase() == 'wolf');
      ...
      original.applyActiveCondition(
        const ActiveCondition(
            condition: ArenaCondition.frightened, durationRounds: 2),
      );
      ...
      expect(original.hasCondition(ArenaCondition.frightened),
          true); // Original unaffected
    ...
    test(
        'ArenaCombatant clone and reset iterations complete rapidly via structural sharing',
        () {
      final wolfMonster = MonsterCodexLibrary.allMonsters
          .firstWhere((m) => m.name.toLowerCase() == 'wolf');
      final combatant = ArenaCombatant.fromMonster(
      ...
      combatant.applyActiveCondition(
        const ActiveCondition(
            condition: ArenaCondition.poisoned, durationRounds: 3),
      );
