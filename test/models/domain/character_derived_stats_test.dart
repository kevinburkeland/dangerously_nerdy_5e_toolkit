...
void main() {
  ...
  group('Character Derived Stats Engine - Armor Class', () {
    ...
    test(
      ...
      // With mediumArmorDexCapBonus capability flag
      final charWithMam = _createTestCharacter(
        baseScores: const AbilityScores(dexterity: 16), // +3 DEX
        inventory: [scaleMail],
        customProperties: {
          'flags': {
            'mediumArmorDexCapBonus': true,
          },
        },
      );

      // 14 (Scale mail) + 3 (DEX cap is 3 with mediumArmorDexCapBonus) = 17
    ...
    test('Barbarian Unarmored Defense adds CON modifier and works with shields',
      ...
      final char = _createTestCharacter(
        baseScores: const AbilityScores(
            dexterity: 14, constitution: 16), // +2 DEX, +3 CON
    ...
    test('Monk Unarmored Defense adds WIS modifier but is negated by shields',
      ...
      final char = _createTestCharacter(
        baseScores:
            const AbilityScores(dexterity: 16, wisdom: 16), // +3 DEX, +3 WIS
        classes: [monkClass],
      );
  ...
  group('Character Derived Stats Engine - Initiative', () {
    ...
    test('Initiative bonus mode capability flag 2014 adds flat +5 bonus', () {
      final char = _createTestCharacter(
        baseScores: const AbilityScores(dexterity: 14), // +2
        customProperties: {
          'flags': {
            'initiativeBonusMode': true,
          },
        },
    ...
    test('Initiative bonus mode capability flag 2024 adds proficiency bonus',
        () {
      final char = _createTestCharacter(
        baseScores: const AbilityScores(dexterity: 14), // +2
        customProperties: {
    ...
    test(
      ...
      final char = _createTestCharacter(
        baseScores:
            const AbilityScores(dexterity: 14), // +2 DEX, prof bonus is 2
        classes: [bardClass],
      );
  ...
  group('Character Derived Stats Engine - Skills & Saves', () {
    ...
    test(
      ...
      final char = _createTestCharacter(
        baseScores:
            const AbilityScores(strength: 14, charisma: 16), // +2 STR, +3 CHA
        classes: [bardClass],
        skillProficiencies: {
    ...
    test('Saving throw modifier factors in ability modifier and proficiency',
      ...
      final char = _createTestCharacter(
        baseScores:
            const AbilityScores(constitution: 16, wisdom: 10), // +3 CON, +0 WIS
  ...
  group('Character Derived Stats Engine - Passive Senses', () {
    test(
        'Passive perception reflects skill modifier and capability flag / grant bonus',
        () {
      final char = _createTestCharacter(
      ...
      final observantChar = char.copyWith(
        customProperties: {
          'flags': {
            'passivePerceptionBonus': true,
            'passiveInvestigationBonus': true,
          },
        },
      );
    ...
    test(
      ...
      const eldritchBlast = Spell(
        ...
        castingTime: CastingTime(
            ...
            triggerCondition: '1 Action'),
        duration: SpellDuration(
            type: DurationType.instantaneous, rawText: 'Instantaneous'),
        range: '120 ft',
  ...
  group('Character Builder Controller - Skill Overlap & Refund State Tracking',
    ...
    test('pendingReplacementSkills reflects resolved refunds', () {
      ...
      controller.setBackgroundSlug(
          'acolyte'); // grants Insight & Religion -> 1 collision (insight)
