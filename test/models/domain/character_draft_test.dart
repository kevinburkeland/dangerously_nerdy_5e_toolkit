...
void main() {
  group('CharacterDraft Model Tests', () {
    ...
    test(
        'Out-of-order population correctly updates granular validation getters',
        () {
      final draft = CharacterDraft();
  ...
  group('CharacterFactory.buildFromDraft Tests', () {
    ...
    test('Compiles complete draft into valid Character domain entity', () {
      ...
      expect(
          character.progression.classes.first.classRef.slug, equals('wizard'));
      expect(character.progression.classes.first.level, equals(1));
      expect(character.progression.classes.first.hitDie, equals('d6'));
      expect(character.skillProficiencies[SkillType.arcana],
          equals(SkillProficiencyLevel.proficient));
      expect(character.savingThrowProficiencies,
          contains(AbilityType.intelligence));
