...
void main() {
  group('SpellbookLibrary & SpellItem Data Model Tests', () {
    ...
    test(
        'True Strike reflects 2014 advantage vs 2024 weapon attack radiant redesign',
        () {
      ...
      expect(
          r2014.description
              .any((d) => d.contains('advantage on your first attack roll')),
          isTrue);

      expect(r2024.concentration, isFalse);
      expect(r2024.damageOrHealType, 'Radiant');
      expect(
          r2024.description.any((d) => d.contains('Radiant damage')), isTrue);
      expect(
          r2024.description.any((d) =>
              d.contains('spellcasting ability modifier instead of Strength')),
          isTrue);
    });
    ...
    test('Cure Wounds and Healing Word reflect 2024 dice doubling buff', () {
      ...
      expect(
          cureWounds.getRules(DmRulesEdition.v2014).rollFormula, '1d8 + mod');
      expect(
          cureWounds.getRules(DmRulesEdition.v2024).rollFormula, '2d8 + mod');

      final healingWord = SpellbookLibrary.getSpellById('spell_healing_word')!;
      expect(
          healingWord.getRules(DmRulesEdition.v2014).rollFormula, '1d4 + mod');
      expect(
          healingWord.getRules(DmRulesEdition.v2024).rollFormula, '2d4 + mod');
    });
    ...
    test('Counterspell reflects 2014 ability check vs 2024 Constitution save',
      ...
      expect(
          r2014.description
              .any((d) => d.contains('DC equals 10 + the spell’s level')),
          isTrue);
      expect(r2024.savingThrow, 'Constitution');
      expect(
          r2024.description.any((d) => d.contains('Constitution saving throw')),
          isTrue);
    });
    ...
    test(
        'Spiritual Weapon reflects 2014 non-concentration vs 2024 concentration',
        () {
    ...
    test(
        'SpellItem.matches filters correctly by query, level, school, and tags',
        () {
      ...
      expect(fireball.matches('fire', schoolFilter: SpellSchool.evocation),
          isTrue);
      expect(fireball.matches('fire', schoolFilter: SpellSchool.illusion),
          isFalse);
    });
    ...
    test('Summon presets link directly to SpellbookLibrary source spells', () {
      final animateObjectsSpell =
          SpellbookLibrary.getSpellById('spell_animate_objects');
      ...
      final conjureAnimalsSpell =
          SpellbookLibrary.getSpellById('spell_conjure_animals');
      expect(conjureAnimalsSpell, isNotNull);
      expect(conjureAnimalsSpell!.isChangedIn2024, isTrue);

      final animateDeadSpell =
          SpellbookLibrary.getSpellById('spell_animate_dead');
    ...
    test('Glyph rings only appear for explicit AoE or recharge mechanics', () {
      ...
      final healingWord = SpellbookLibrary.getSpellById('spell_healing_word')!;
      final magicMissile =
          SpellbookLibrary.getSpellById('spell_magic_missile')!;

      final fireballRings = fireball.getGlyphActionRings(DmRulesEdition.v2024);
      final healingWordRings =
          healingWord.getGlyphActionRings(DmRulesEdition.v2024);
      final magicMissileRings =
          magicMissile.getGlyphActionRings(DmRulesEdition.v2024);

      expect(
          fireballRings.any((ring) => ring.ringType == ActionRingType.recharge),
          isTrue,
          reason:
              'Fireball is an explicit AoE spell and should keep an AoE/recharge marker');
      expect(
          healingWordRings
              .any((ring) => ring.ringType == ActionRingType.recharge),
          isFalse,
          reason:
              'Healing Word is a targeted heal and should not show a recharge/AoE glyph ring');
      expect(
          magicMissileRings
              .any((ring) => ring.ringType == ActionRingType.recharge),
          isFalse,
          reason:
              'Magic Missile is not an AoE or recharge spell and should not show a recharge glyph ring');
    });

    test(
        'Glyph rings derive control and sustain semantics from spell mechanics',
        () {
      final holdPerson = SpellbookLibrary.getSpellById('spell_hold_person')!;
      final massHealingWord =
          SpellbookLibrary.getSpellById('spell_mass_healing_word')!;

      final holdPersonRings =
          holdPerson.getGlyphActionRings(DmRulesEdition.v2024);
      final massHealingWordRings =
          massHealingWord.getGlyphActionRings(DmRulesEdition.v2024);

      expect(
          holdPersonRings
              .any((ring) => ring.ringType == ActionRingType.control),
          isTrue,
          reason:
              'Hold Person applies the paralyzed condition and should show a control ring');
      expect(
          massHealingWordRings
              .any((ring) => ring.ringType == ActionRingType.sustain),
          isTrue,
          reason:
              'Mass Healing Word is group healing and should show a sustain ring');
      expect(
          massHealingWordRings
              .any((ring) => ring.ringType == ActionRingType.control),
          isFalse,
    ...
    test('Glyph rings preserve multi-damage palettes for color cycling', () {
      final spiritGuardians =
          SpellbookLibrary.getSpellById('spell_spirit_guardians')!;
      final rings = spiritGuardians.getGlyphActionRings(DmRulesEdition.v2024);
      final aoeRing =
          rings.firstWhere((ring) => ring.ringType == ActionRingType.recharge);

      expect(aoeRing.damageType, DamageAccent.radiant,
          reason:
              'Primary color should begin with the first parsed damage accent');
      expect(aoeRing.damageTypes, contains(DamageAccent.necrotic),
          reason:
              'Secondary damage accents should be retained for animated palette cycling');
    ...
    test(
        'Cleric spell list includes spells across all spell levels 0 through 9',
        () {
      final clericSpells2014 = SpellbookLibrary.getSpellsByClass(
          SpellClass.cleric,
          edition: DmRulesEdition.v2014);
      final clericSpells2024 = SpellbookLibrary.getSpellsByClass(
          SpellClass.cleric,
      ...
      for (int lvl = 0; lvl <= 9; lvl++) {
        expect(clericSpells2014.any((s) => s.level == lvl), isTrue,
            reason: 'Missing 2014 Cleric spell at level $lvl');
        expect(clericSpells2024.any((s) => s.level == lvl), isTrue,
            reason: 'Missing 2024 Cleric spell at level $lvl');
      }
      ...
      expect(
          SpellbookLibrary.getSpellById('spell_true_resurrection'), isNotNull);
      expect(SpellbookLibrary.getSpellById('spell_gate'), isNotNull);
      expect(
          SpellbookLibrary.getSpellById('spell_astral_projection'), isNotNull);
    });

    test(
        'Every spellcasting class has rich spell coverage in both 2014 and 2024 editions',
        () {
      for (final cls in SpellClass.values) {
        final spells2014 = SpellbookLibrary.getSpellsByClass(cls,
            edition: DmRulesEdition.v2014);
        final spells2024 = SpellbookLibrary.getSpellsByClass(cls,
            edition: DmRulesEdition.v2024);

        expect(spells2014.isNotEmpty, isTrue,
            reason: 'Class ${cls.label} has no 2014 spells');
        expect(spells2024.isNotEmpty, isTrue,
            reason: 'Class ${cls.label} has no 2024 spells');

        // Full casters have spells across levels 0 to 9
        if (cls == SpellClass.cleric ||
            cls == SpellClass.wizard ||
            cls == SpellClass.sorcerer ||
            cls == SpellClass.druid ||
            cls == SpellClass.bard ||
            cls == SpellClass.warlock) {
          final maxLevel =
              spells2024.map((s) => s.level).reduce((a, b) => a > b ? a : b);
          expect(maxLevel, 9,
              reason: 'Full caster ${cls.label} should reach 9th level spells');
        }
      }
    });
    ...
    test('All classes match canonical SRD 5.1 class spell assignments', () {
      ...
      for (final name in paladinSrd) {
        final spell = SpellbookLibrary.allSpells.firstWhere(
          ...
          orElse: () =>
              throw Exception('Paladin spell $name not found in library'),
        );
        expect(
      ...
      for (final name in rangerSrd) {
        final spell = SpellbookLibrary.allSpells.firstWhere(
          (s) => s.name.toLowerCase() == name.toLowerCase(),
          orElse: () =>
              throw Exception('Ranger spell $name not found in library'),
        );
        expect(
      ...
      for (final name in warlockSrd) {
        final spell = SpellbookLibrary.allSpells.firstWhere(
          (s) => s.name.toLowerCase() == name.toLowerCase(),
          orElse: () =>
              throw Exception('Warlock spell $name not found in library'),
        );
        expect(
      ...
      for (final name in clericSrd) {
        final spell = SpellbookLibrary.allSpells.firstWhere(
          (s) => s.name.toLowerCase() == name.toLowerCase(),
          orElse: () =>
              throw Exception('Cleric spell $name not found in library'),
        );
        expect(
      ...
      for (final name in druidSrd) {
        final spell = SpellbookLibrary.allSpells.firstWhere(
          (s) => s.name.toLowerCase() == name.toLowerCase(),
          orElse: () =>
              throw Exception('Druid spell $name not found in library'),
        );
        expect(
      ...
      for (final name in bardSrd) {
        final spell = SpellbookLibrary.allSpells.firstWhere(
          (s) => s.name.toLowerCase() == name.toLowerCase(),
          orElse: () =>
              throw Exception('Bard spell $name not found in library'),
        );
        expect(
      ...
      for (final name in sorcererSrd) {
        final spell = SpellbookLibrary.allSpells.firstWhere(
          (s) => s.name.toLowerCase() == name.toLowerCase(),
          orElse: () =>
              throw Exception('Sorcerer spell $name not found in library'),
        );
        expect(
      ...
      for (final name in wizardSrd) {
        final spell = SpellbookLibrary.allSpells.firstWhere(
          (s) => s.name.toLowerCase() == name.toLowerCase(),
          orElse: () =>
              throw Exception('Wizard spell $name not found in library'),
        );
        expect(
    ...
    test(
        'All changed spells have diffSummary and unchanged spells have uniform rules properties',
        () {
    ...
    test('Identifies missing SRD 5.1 spells', () {
      const srd51SpellNames = {
        ...
        'Hellish Rebuke', 'Heroism', 'Hideous Laughter', 'Hunter’s Mark',
        'Identify',
        'Illusory Script', 'Inflict Wounds', 'Jump', 'Longstrider',
        'Mage Armor',
        ...
        'See Invisibility',
        'Shatter', 'Silence', 'Spider Climb', 'Spike Growth',
        'Spiritual Weapon',
        ...
        'Call Lightning',
        'Clairvoyance', 'Conjure Animals', 'Counterspell',
        'Create Food and Water',
        ...
        'Guardian of Faith', 'Hallucinatory Terrain', 'Ice Storm',
        'Locate Creature',
        ...
        'Dominate Person', 'Dream', 'Flame Strike', 'Geas',
        'Greater Restoration',
        ...
        'Harm', 'Heal', 'Heroes\' Feast', 'Instant Summons',
        'Irresistible Dance',
        ...
        // 7th Level
        'Arcane Sword', 'Conjure Celestial', 'Delayed Blast Fireball',
        'Divine Word',
  ...
  group('SpellItem.getName() — null-branch fallback (mutation guards)', () {
    ...
    test(
        'getName always returns a non-empty string for every spell and edition',
        () {
      for (final spell in SpellbookLibrary.allSpells) {
