...
void main() {
  group('SRD & Open Gaming License / CC-BY-4.0 Legality Audit', () {
    ...
    test(
        'SRD summons library contains no WotC Product Identity terms in names or descriptions',
        () {
      ...
      for (final preset in allPresets) {
        ...
        for (final forbidden in forbiddenProductIdentity) {
          expect(
            ...
            isFalse,
            reason:
                'Preset "${preset.name}" contains Product Identity term "$forbidden"',
          );
          expect(
            lowerDesc.contains(forbidden),
            isFalse,
            reason:
                'Preset description in "${preset.name}" contains Product Identity term "$forbidden"',
          );
        }
        ...
        for (final statBlock in preset.statBlocks) {
          ...
          for (final forbidden in forbiddenProductIdentity) {
            expect(
              ...
              isFalse,
              reason:
                  'Minion stat block "${statBlock.name}" contains Product Identity term "$forbidden"',
            );
          }
        }
      }
      ...
      for (final item in items) {
        ...
        for (final forbidden in forbiddenProductIdentity) {
          expect(
            ...
            isFalse,
            reason:
                'DM Reference item "${item.title}" contains Product Identity term "$forbidden"',
          );
          expect(
            summary.contains(forbidden),
            isFalse,
            reason:
                'DM Reference summary for "${item.title}" contains Product Identity term "$forbidden"',
          );
          expect(
            rules14.contains(forbidden),
            isFalse,
            reason:
                'DM Reference rules2014 in "${item.title}" contains Product Identity term "$forbidden"',
          );
          expect(
            rules24.contains(forbidden),
            isFalse,
            reason:
                'DM Reference rules2024 in "${item.title}" contains Product Identity term "$forbidden"',
          );
          expect(
            tags.contains(forbidden),
            isFalse,
            reason:
                'DM Reference tags in "${item.title}" contains Product Identity term "$forbidden"',
          );
        }
      }
    });
        ...
        for (final entry in table.entries) {
          ...
          for (final forbidden in forbiddenProductIdentity) {
            expect(
              ...
              isFalse,
              reason:
                  'Loot table "${table.name}" entry "${entry.label}" contains Product Identity term "$forbidden"',
            );
            expect(
              desc.contains(forbidden),
              isFalse,
              reason:
                  'Loot table "${table.name}" entry description in "${entry.label}" contains Product Identity term "$forbidden"',
            );
          }
        }
      }
    ...
    test('Subclass Spells Library contains no WotC Product Identity terms', () {
      final fiendSpells =
          SubclassSpellsLibrary.getExpandedSpells('warlock', 'fiend');

      expect(
          SubclassSpellsLibrary.getExpandedSpells('warlock', 'great_old_one'),
          isEmpty);
      expect(SubclassSpellsLibrary.getExpandedSpells('warlock', 'archfey'),
          isEmpty);
      expect(SubclassSpellsLibrary.getExpandedSpells('warlock', 'celestial'),
          isEmpty);
      expect(SubclassSpellsLibrary.getExpandedSpells('warlock', 'fathomless'),
          isEmpty);
      expect(SubclassSpellsLibrary.getExpandedSpells('warlock', 'hexblade'),
          isEmpty);
      ...
      for (final spell in allSpellNames) {
        ...
        for (final forbidden in forbiddenProductIdentity) {
          expect(
            ...
            isFalse,
            reason:
                'Subclass expanded spell "$spell" contains Product Identity term "$forbidden"',
          );
        }
      }
    });
      ...
      for (final cls in classes) {
        ...
        for (final forbidden in forbiddenProductIdentity) {
          expect(
            ...
            isFalse,
            reason:
                'Class "${cls.name}" contains Product Identity term "$forbidden"',
          );
          expect(
            lowerSlug.contains(forbidden),
            isFalse,
            reason:
                'Class slug "${cls.id.slug}" contains Product Identity term "$forbidden"',
          );
        }
        ...
        for (final sub in cls.subclasses) {
          ...
          for (final forbidden in forbiddenProductIdentity) {
            expect(
              ...
              isFalse,
              reason:
                  'Subclass "${sub.name}" contains Product Identity term "$forbidden"',
            );
            expect(
              subSlug.contains(forbidden),
              isFalse,
              reason:
                  'Subclass slug "${sub.id.slug}" contains Product Identity term "$forbidden"',
            );
          }
        }
      }
      ...
      for (final sp in species) {
        ...
        for (final forbidden in forbiddenProductIdentity) {
          expect(
            ...
            isFalse,
            reason:
                'Species "${sp.name}" contains Product Identity term "$forbidden"',
          );
          expect(
            lowerTraits.contains(forbidden),
            isFalse,
            reason:
                'Species traits in "${sp.name}" contains Product Identity term "$forbidden"',
          );
        }
      }
    });
      ...
      for (final feat in feats) {
        ...
        for (final forbidden in forbiddenProductIdentity) {
          expect(
            ...
            isFalse,
            reason:
                'Feat "${feat.name}" contains Product Identity term "$forbidden"',
          );
          expect(
            lowerDesc.contains(forbidden),
            isFalse,
            reason:
                'Feat description in "${feat.name}" contains Product Identity term "$forbidden"',
          );
        }
      }
    });
      ...
      for (final bg in backgrounds) {
        ...
        for (final forbidden in forbiddenProductIdentity) {
          expect(
            ...
            isFalse,
            reason:
                'Background "${bg.name}" contains Product Identity term "$forbidden"',
          );
          expect(
            lowerDesc.contains(forbidden),
            isFalse,
            reason:
                'Background description in "${bg.name}" contains Product Identity term "$forbidden"',
          );
        }
      }
    });
      ...
      for (final pkg in packages) {
        ...
        for (final forbidden in forbiddenProductIdentity) {
          expect(
            ...
            isFalse,
            reason:
                'Package "${pkg.name}" contains Product Identity term "$forbidden"',
          );
          expect(
            lowerSub.contains(forbidden),
            isFalse,
            reason:
                'Package subtitle in "${pkg.name}" contains Product Identity term "$forbidden"',
          );
        }
        ...
        for (final item in pkg.items) {
          ...
          for (final forbidden in forbiddenProductIdentity) {
            expect(
              ...
              isFalse,
              reason:
                  'Equipment item "$itemName" in "${pkg.name}" contains Product Identity term "$forbidden"',
            );
          }
        }
      }
      ...
      for (final monster in monsters) {
        ...
        for (final forbidden in forbiddenProductIdentity) {
          expect(
            ...
            isFalse,
            reason:
                'Monster "${monster.name}" contains Product Identity term "$forbidden"',
          );
          expect(
            lowerId.contains(forbidden),
            isFalse,
            reason:
                'Monster ID "${monster.id}" contains Product Identity term "$forbidden"',
          );
          expect(
            lowerDiff.contains(forbidden),
            isFalse,
            reason:
                'Monster diff in "${monster.name}" contains Product Identity term "$forbidden"',
          );
        }
      }
    });
    ...
    test(
        'Spellbook Library contains no WotC Product Identity terms in spell names or IDs',
        () {
      ...
      for (final spell in spells) {
        ...
        for (final forbidden in forbiddenProductIdentity) {
          expect(
            ...
            isFalse,
            reason:
                'Spell "${spell.name}" contains Product Identity term "$forbidden"',
          );
          expect(
            lowerId.contains(forbidden),
            isFalse,
            reason:
                'Spell ID "${spell.id}" contains Product Identity term "$forbidden"',
          );
          expect(
            lowerDiff.contains(forbidden),
            isFalse,
            reason:
                'Spell diff summary in "${spell.name}" contains Product Identity term "$forbidden"',
          );
        }
      }
    });
    ...
    test(
        'Magic Item Library and Crafting Rules contain no WotC Product Identity terms',
        () {
      ...
      for (final item in items) {
        ...
        for (final forbidden in forbiddenProductIdentity) {
          expect(
            ...
            isFalse,
            reason:
                'Item "${item.name}" contains Product Identity term "$forbidden"',
          );
          expect(
            lowerId.contains(forbidden),
            isFalse,
            reason:
                'Item ID "${item.id}" contains Product Identity term "$forbidden"',
          );
          expect(
            lowerDiff.contains(forbidden),
            isFalse,
            reason:
                'Item diff in "${item.name}" contains Product Identity term "$forbidden"',
          );
        }
          ...
          for (final blob in textBlobs) {
            for (final forbidden in forbiddenProductIdentity) {
              expect(
                ...
                isFalse,
                reason:
                    'Crafting details for "${item.name}" ($edition) contains Product Identity term "$forbidden" in text: "$blob"',
              );
            }
          }
        }
    ...
    test(
        'All presets have non-empty attribution identifiers and valid minion configurations',
        () {
