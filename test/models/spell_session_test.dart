...
void main() {
  group('SpellSession Budget & Point Tests', () {
    ...
    test('canAddObject checks remaining point budget', () {
      ...
      expect(
          session.canAddObject(ObjectSize.huge), false); // Needs 8, only 2 left
    ...
    test('Dead objects do not attack in batch attack', () {
      ...
      session.updateObject(
          session.activeObjects.first.id, (m) => m.applyDamage(100));
    ...
    test('performBatchAttack supports useMaximizedCrits toggle', () {
      ...
      final summaryNormal =
          session.performBatchAttack(targetAc: 15, useMaximizedCrits: false);
      expect(summaryNormal.useMaximizedCrits, false);

      final summaryMax =
          session.performBatchAttack(targetAc: 15, useMaximizedCrits: true);
    ...
    test('SRD Summons support: addMinionFromStatBlock & rollBagOfTricks', () {
      ...
      final bagSession =
          SpellSession(activePreset: BagOfTricksSummons.grayBagPreset);
    ...
    test('rollHornOfValhalla generates correct Berserker counts per variant',
        () {
      final session =
          SpellSession(activePreset: ValhallaSummons.hornOfValhallaPreset);
