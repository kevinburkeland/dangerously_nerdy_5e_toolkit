  ...
  group('MagicItemLibrary Tests', () {
    test('allItems contains a large comprehensive catalog of items', () {
      ...
      expect(duplicates, isEmpty,
          reason: 'Duplicate item IDs found: $duplicates');
    });
    ...
    test('verifies all 13 item categories are populated with substantial depth',
      ...
      for (final cat in ItemCategory.values) {
        ...
        expect(itemsInCat, isNotEmpty,
            reason: 'Category ${cat.name} should have items');
    ...
    test(
        'verifies SRD loot items (gemstones, art objects, trinkets) are in the Item Codex',
        () {
      ...
      final goldChain = MagicItemLibrary.findById(
          'art_fine_gold_chain_with_fire_opal_pendant');
    ...
    test(
        'all 9 official SRD Figurines of Wondrous Power exist with correct data',
        () {
      const figurineIds = [
      ...
      for (final id in figurineIds) {
        ...
        expect(item, isNotNull,
            reason: 'Figurine $id should exist in MagicItemLibrary');
      ...
      final statBlockIds =
          FigurinesSummons.figurinesPreset.statBlocks.map((s) => s.id).toList();
      expect(
          statBlockIds,
          containsAll([
            'item_griffon',
    ...
    test(
        'explicit item color resolution applies to colored armor and Ioun stones',
        () {
      final redDragonArmor =
          MagicItemLibrary.findById('item_red_dragon_scale_mail');
      expect(redDragonArmor, isNotNull);
      expect(
          redDragonArmor!.effectiveGlyphColor, equals(const Color(0xFFEF4444)));

      final blueDragonArmor =
          MagicItemLibrary.findById('item_blue_dragon_scale_mail');
      expect(blueDragonArmor, isNotNull);
      expect(blueDragonArmor!.effectiveGlyphColor,
      ...
      final iounPaleGreen =
          MagicItemLibrary.findById('item_ioun_stone_pale_green');
      expect(iounPaleGreen, isNotNull);
      expect(
          iounPaleGreen!.effectiveGlyphColor, equals(const Color(0xFF10B981)));

      final iounDustyRose =
          MagicItemLibrary.findById('item_ioun_stone_dusty_rose');
      expect(iounDustyRose, isNotNull);
      expect(
          iounDustyRose!.effectiveGlyphColor, equals(const Color(0xFFFB7185)));
    });
    ...
    test(
        'verifies effective price resolution for nonmagical, magic items, and consumables',
        () {
    ...
    test(
        'CurrencyCoinType accurately resolves and color-codes cp, sp, ep, gp, pp',
        () {
    ...
    test(
        'verifies all 59 newly added SRD wondrous items exist and resolve crafting correctly',
        () {
      ...
      for (final name in keyWondrousItems) {
        ...
        expect(item, isNotNull,
            reason: 'Expected $name to exist in MagicItemLibrary');
        final crafting = item!.getCraftingDetails();
        expect(crafting.goldCost, greaterThan(0),
            reason: 'Expected $name to have gold cost');
        expect(crafting.primaryTool, isNotEmpty,
            reason: 'Expected $name to have primary crafting tool');
      }
    });
  });
}
