...
void main() {
  ...
  group('AppSettings — Derived Boolean Getters (mutation guards)', () {
    ...
    test(
        'areParticlesAllowed is true when particles enabled and performanceMode off',
        () {
    ...
    test(
        'areParticlesAllowed is false when performanceMode is on, even if particles enabled',
        () {
    ...
    test(
        'areParticlesAllowed is false when particles disabled, even if performanceMode off',
        () {
      const s =
          AppSettings(enableSpellParticles: false, performanceMode: false);
      expect(s.areParticlesAllowed, isFalse);
    });
    ...
    test(
        'areGlyphAnimationsAllowed is true when animations enabled and performanceMode off',
        () {
      const s =
          AppSettings(enableGlyphAnimations: true, performanceMode: false);
    ...
    test('areGlyphAnimationsAllowed is false when animations disabled', () {
      const s =
          AppSettings(enableGlyphAnimations: false, performanceMode: false);
      expect(s.areGlyphAnimationsAllowed, isFalse);
    });
  });
  ...
  group('AppSettings — Equality & HashCode', () {
    ...
    test('instances with the same pinned sets are equal', () {
      final a =
          const AppSettings().copyWith(pinnedRuleIds: {'rule_x', 'rule_y'});
      final b =
          const AppSettings().copyWith(pinnedRuleIds: {'rule_y', 'rule_x'});
