...
void main() {
  ...
  group('LandingToolItem — Field Completeness', () {
    test(
        'every tool has a non-empty id, title, category, badgeText, and description',
        () {
      for (final t in tools) {
        expect(t.id.isNotEmpty, isTrue, reason: 'Tool has empty id');
  ...
  group('LandingToolItem.matches() — Boolean Logic', () {
    ...
    test(
        'matches returns true when query matches a keyword in the keywords list',
        () {
      ...
      for (final t in tools) {
        for (final kw in t.keywords) {
          ...
          if (kw.length > 3 &&
              !t.title.toLowerCase().contains(kw.toLowerCase())) {
    ...
    test(
        'matches returns true when query matches a keyword but not title or description',
        () {
  ...
  group('LandingToolItem.getLegibleAccent() — Legibility Variants', () {
    test(
        'getLegibleAccent returns a non-null color in both dark and light mode',
        () {
      for (final t in tools) {
