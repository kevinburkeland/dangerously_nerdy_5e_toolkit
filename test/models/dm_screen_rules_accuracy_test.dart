...
void main() {
  group('5e Rules Accuracy & 2014 vs 2024 Diffs', () {
    test(
        'Exhaustion rules accurately capture 2014 tiers vs 2024 numeric penalties',
        () {
      final exhaustion =
      ...
      expect(
          exhaustion.rules2014.any(
              (r) => r.contains('Level 1: Disadvantage on ability checks')),
          isTrue);
      expect(exhaustion.rules2014.any((r) => r.contains('Level 6: Death')),
          isTrue);

      // Verify 2024 has the 2x exhaustion level D20 test penalty
      expect(
          exhaustion.rules2024.any((r) => r.contains(
              'Subtract 2 × your exhaustion level from all D20 Tests')),
          isTrue);
      expect(
          exhaustion.rules2024.any((r) =>
              r.contains('Reduce speed by 5 feet × your exhaustion level')),
          isTrue);
      expect(exhaustion.isChangedIn2024, isTrue);
    });

    test(
        'Grappled condition captures 2014 vs 2024 save DC and escape mechanics',
        () {
      final grappled =
          DmScreenLibrary.allItems.firstWhere((i) => i.id == 'cond_grappled');
      expect(grappled, isNotNull);

      // Verify 2024 specifies end of turn save DC
      expect(
          grappled.rules2024.any((r) =>
              r.contains('saving throw at the end of each of your turns')),
          isTrue);
    ...
    test('Surprise rule reflects disadvantage on initiative in 2024', () {
      final surprise =
          DmScreenLibrary.allItems.firstWhere((i) => i.id == 'cond_surprise');
      expect(surprise, isNotNull);

      expect(
          surprise.rules2014
              .any((r) => r.contains('cannot move or take an Action')),
          isTrue);
      expect(
          surprise.rules2024
              .any((r) => r.contains('Disadvantage on your Initiative roll')),
          isTrue);
      expect(surprise.isChangedIn2024, isTrue);
    });

    test(
        'Drinking a potion changes from 1 Action (2014) to 1 Bonus Action (2024)',
        () {
      final potion =
    ...
    test(
        'Dnd5eScoreMath accurately computes ability modifiers across the entire standard range',
        () {
    ...
    test(
        'Dnd5eScoreMath accurately scales proficiency bonus by character/minion level',
        () {
      // 1-4 => +2
