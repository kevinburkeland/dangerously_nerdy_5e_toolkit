...
void main() {
  group('Summoning Spells Upcasting & Roll Rules Audit', () {
    ...
    test(
        'Creature statblocks include appropriate roll formulas and special traits',
        () {
      ...
      expect(
          BeastSummons.giantSpider.fullDamageFormula, contains('+ 2d8 Poison'));
      ...
      expect(ElementalSummons.airElemental.fullDamageFormula,
          equals('2d8+5 Bludgeoning'));
    });

    test(
        'Batch attack roller calculates primary and secondary damage rolls correctly',
        () {
      final session = SpellSession(
        activePreset: BeastSummons.conjureAnimalsPreset,
        spellLevel: 3,
      ...
      final spiderResult = summary.results
          .firstWhere((r) => r.object.name.startsWith('Giant Spider'));
    ...
    test(
        'Bag of Tricks variants (Gray, Rust, Tan) have distinct 8-creature tables',
        () {
      ...
      expect(BagOfTricksSummons.grayBagPreset.statBlocks.map((s) => s.name),
          containsAll(['Dire Wolf', 'Giant Elk', 'Weasel']));

      // Rust bag contains Lion & Brown Bear
      expect(BagOfTricksSummons.rustBagPreset.statBlocks.map((s) => s.name),
          containsAll(['Lion', 'Brown Bear', 'Owl']));

      // Tan bag contains Tiger & Giant Hyena
      expect(BagOfTricksSummons.tanBagPreset.statBlocks.map((s) => s.name),
          containsAll(['Tiger', 'Giant Hyena', 'Jackal']));
    });

    test(
        'Conjure Animals contains full CR 2, CR 1, CR 1/2, and CR 1/4 beast options',
        () {
      final names = BeastSummons.conjureAnimalsPreset.statBlocks
          .map((s) => s.name)
          .toList();
      // CR 2 beasts
      expect(
    ...
    test('Giant Insect contains Giant Scorpion (CR 3)', () {
      final names = InsectSummons.giantInsectPreset.statBlocks
          .map((s) => s.name)
          .toList();
      expect(
          names,
    ...
    test(
        'Conjure Elemental & Minor Elementals contain Salamander, Xorn, Fire Snake, and Steam Mephit',
        () {
      final elemNames = ElementalSummons.conjureElementalPreset.statBlocks
          .map((s) => s.name)
          .toList();
      expect(
          elemNames,
      ...
      final minorNames = ElementalSummons
          .conjureMinorElementalsPreset.statBlocks
          .map((s) => s.name)
          .toList();
      expect(
          minorNames,
