...
void main() {
  ...
  group('DmReferenceItem.getRules() — Edition Routing', () {
    ...
    test('potion drinking reflects Action (2014) vs Bonus Action (2024)', () {
      ...
      expect(
          potion
              .getRules(DmRulesEdition.v2014)
              .any((r) => r.contains('1 Action')),
          isTrue);
      expect(
          potion
  ...
  group('DmReferenceItem.getTitle() — Null-Branch Fallback (mutation guards)',
    ...
    test(
        'getTitle returns base title for all items where title2014 is null and edition is v2014',
        () {
      for (final item in DmScreenLibrary.allItems) {
    ...
    test(
        'getTitle returns base title for all items where title2024 is null and edition is v2024',
        () {
      for (final item in DmScreenLibrary.allItems) {
    ...
    test(
        'getTitle returns the correct override when both title2014 and title2024 exist',
        () {
      ...
      expect(
          attack.getTitle(DmRulesEdition.v2014), isNot(equals(attack.title)));
    });
  ...
  group('DmScreenLibrary — Filter Helpers', () {
    ...
    test('standardActions(v2014) includes attack item', () {
      final actions2014 = DmScreenLibrary.standardActions(DmRulesEdition.v2014);
    ...
    test('standardActions(v2024) includes attack item', () {
      final actions2024 = DmScreenLibrary.standardActions(DmRulesEdition.v2024);
