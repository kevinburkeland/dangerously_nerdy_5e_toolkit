...
void main() {
  ...
  group('MonsterCodexLibrary', () {
    ...
    test('provides source-category and preset-specific lists', () {
      ...
      expect(animateDead.map((m) => m.name).toSet(),
          containsAll(['Skeleton', 'Zombie']));
    });
    ...
    test('builds CR-band source lists with complete coverage', () {
      ...
      final totalBandEntries = SrdMonsterCrBands.cr0ToQuarter.length +
    ...
    test('provides codex-level CR band helper lists', () {
      ...
      expect(
          low.every((m) => m.challengeRating >= 0 && m.challengeRating <= 0.25),
          isTrue);
      expect(
          lowMid
              .every((m) => m.challengeRating >= 0.5 && m.challengeRating <= 1),
          isTrue);
      expect(mid.every((m) => m.challengeRating >= 2 && m.challengeRating <= 4),
          isTrue);
      expect(
          high.every((m) => m.challengeRating >= 5 && m.challengeRating <= 8),
          isTrue);
    });
    ...
    test('supports type, size, and free-text search filters', () {
      ...
      expect(
        packTactics.any(
          (m) =>
              m.name.toLowerCase().contains('wolf') ||
    ...
    test('preserves action and trait details for codex entries', () {
      final giantScorpion =
          MonsterCodexLibrary.getMonsterByName('Giant Scorpion');
    ...
    test(
        'calculates baseline DPR and sorts monsters logically by offensive output',
        () {
      ...
      expect(bearDpr > wolfDpr, isTrue,
          reason:
              'Brown Bear multiattack should deal more DPR than single wolf bite');
      expect(fireDpr > bearDpr, isTrue,
          reason: 'Fire Elemental multiattack should out-damage Brown Bear');
    ...
    test(
        'extractDprAttacks does not double up attacks for creatures with alternative weapons or single actions',
        () {
      final skeleton = MonsterCodexLibrary.getMonsterByName('Skeleton');
      if (skeleton != null) {
        final attacks = skeleton.statBlock2014.extractDprAttacks();
        final activeAttacks =
            attacks.fold<int>(0, (sum, a) => sum + a.attacksPerRound);
        expect(activeAttacks, 1,
            reason:
                'Skeleton without multiattack should have exactly 1 active attack per round');
      }
      ...
      if (knight != null) {
        ...
        final activeAttacks =
            attacks.fold<int>(0, (sum, a) => sum + a.attacksPerRound);
        expect(activeAttacks, 2,
            reason:
                'Knight with two melee attacks should have exactly 2 active attacks per round');
      }
      ...
      if (brownBear != null) {
        ...
        expect(activeAttacks, 2,
            reason:
                'Brown Bear with 1 bite + 1 claws should have 2 active attacks per round');
      }
      ...
      if (roper != null) {
        ...
        final biteAttack =
            attacks.firstWhere((a) => a.name.toLowerCase() == 'bite');
        final tendrilAttack =
            attacks.firstWhere((a) => a.name.toLowerCase() == 'tendril');

        expect(biteAttack.attacksPerRound, 1,
            reason:
                'Roper makes exactly 1 bite attack in its multiattack routine');
        expect(tendrilAttack.attacksPerRound, 4,
            reason: 'Roper makes 4 tendril attacks in its multiattack routine');

        final roperDpr = roper.calculateBaselineDpr();
        expect(roperDpr > 10 && roperDpr < 25, isTrue,
            reason:
                'Roper baseline DPR: $roperDpr should reflect 1 bite (4d8+4 = 22 avg dmg), not 4 bites!');
      }
      ...
      if (medusa != null) {
        ...
        final snakeHair = attacks
            .firstWhere((a) => a.name.toLowerCase().contains('snake hair'));
        final shortsword = attacks
            .firstWhere((a) => a.name.toLowerCase().contains('shortsword'));
        final longbow =
            attacks.firstWhere((a) => a.name.toLowerCase().contains('longbow'));

        final totalActiveAttacks =
            attacks.fold<int>(0, (sum, a) => sum + a.attacksPerRound);
        expect(totalActiveAttacks, 3,
            reason:
                'Medusa makes exactly 3 attacks in any chosen turn routine, not 7 attacks!');
        ...
        expect(shortsword.attacksPerRound, 0,
            reason:
                'Medusa alternative shortsword routine should be 0 by default to avoid doubling');
        expect(longbow.attacksPerRound, 0,
            reason:
                'Medusa alternative longbow routine should be 0 by default to avoid doubling');
      }
      ...
      if (oni != null) {
        ...
        expect(totalActiveAttacks, 2,
            reason:
                'Oni makes exactly 2 attacks (either 2 claws or 2 glaive), not 4 attacks!');
      }
      ...
      if (youngRed != null) {
        ...
        final breath = attacks
            .firstWhere((a) => a.name.toLowerCase().contains('fire breath'));
        ...
        expect(baselineDpr > 40 && baselineDpr < 90, isTrue,
            reason:
                'Young Red Dragon baseline DPR ($baselineDpr) amortizes 16d6 breath (55.6%) + Multiattack (44.4%)');
      }
      ...
      if (adultRed != null) {
        ...
        final legTail =
            legAttacks.firstWhere((a) => a.name.toLowerCase().contains('tail'));
        ...
        expect(legTail.attackBonus, equals(14),
            reason:
                'Tail Attack legendary action should inherit +14 to hit, 2d8 + 8 from regular Tail attack');

        final totalDpr = adultRed.calculateBaselineDpr();
        expect(totalDpr > 80 && totalDpr < 160, isTrue,
            reason:
                'Adult Red Dragon total DPR ($totalDpr) includes turn routine + 3 legendary actions');
      }
      ...
      if (vampire != null) {
        ...
        final legUnarmed = attacks.firstWhere((a) =>
            a.isLegendaryAction &&
            a.name.toLowerCase().contains('unarmed strike'));
        ...
        expect(legUnarmed.attackBonus, equals(9),
            reason:
                'Vampire Unarmed Strike legendary action should inherit 1d8 + 4 damage from regular Unarmed Strike');
      }
      ...
      if (aboleth != null) {
        ...
        final legTailSwipe = attacks.firstWhere((a) =>
            a.isLegendaryAction && a.name.toLowerCase().contains('tail'));
        ...
        expect(legTailSwipe.attackBonus, equals(9),
            reason:
                'Aboleth Tail Swipe legendary action should inherit 3d6 + 5 damage from regular Tail attack');
      }
      ...
      if (tarrasque != null) {
        ...
        final bite =
            turnAttacks.firstWhere((a) => a.name.toLowerCase() == 'bite');
        final claw =
            turnAttacks.firstWhere((a) => a.name.toLowerCase() == 'claw');
        final horns =
            turnAttacks.firstWhere((a) => a.name.toLowerCase() == 'horns');
        final tail =
            turnAttacks.firstWhere((a) => a.name.toLowerCase() == 'tail');

        expect(bite.attacksPerRound, equals(1),
            reason:
                'Tarrasque makes exactly 1 Bite attack in multiattack, not 5!');
    ...
    test('getMonsterByName query for "Dragon" does not return "Dragon Turtle"',
      ...
      final dragonTurtle =
          MonsterCodexLibrary.getMonsterByName('Dragon Turtle');
