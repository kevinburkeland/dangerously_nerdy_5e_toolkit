...
void main() {
  ...
  group('Rules Compendium Dataset & Search Engine Tests', () {
    test(
        'comprehensive rules corpus indexes all SRD categories and required rules',
        () {
      ...
      for (final item in items) {
        ...
        expect(item.tags.isNotEmpty, isTrue,
            reason: 'Item ${item.id} has empty tags');
        expect(item.rules2014.isNotEmpty, isTrue,
            reason: 'Item ${item.id} has empty 2014 rules');
        expect(item.rules2024.isNotEmpty, isTrue,
            reason: 'Item ${item.id} has empty 2024 rules');
      }
    });
    ...
    test('verifies all 15 core conditions and statuses are indexed', () {
      ...
      for (final cond in expectedConditions) {
        expect(conditionIds.contains(cond), isTrue,
            reason: 'Missing condition $cond');
      }
    });
    ...
    test(
        'tokenized multi-word search and search operators (tag:, category:, edition:) work accurately',
        () {
      final items = DmScreenLibrary.allItems;

      // Operator tag:
      final actionMatches =
          items.where((i) => i.matches('tag:standard_action')).toList();
      expect(actionMatches.isNotEmpty, isTrue);
      expect(actionMatches.every((i) => i.tags.contains('standard_action')),
          isTrue);

      // Operator category:
      final envMatches =
          items.where((i) => i.matches('category:environment')).toList();
      expect(envMatches.isNotEmpty, isTrue);
      expect(envMatches.every((i) => i.category == DmCategory.environment),
          isTrue);

      // Operator edition:diff
      final diffMatches =
          items.where((i) => i.matches('edition:diff')).toList();
      expect(diffMatches.isNotEmpty, isTrue);
      expect(diffMatches.every((i) => i.isChangedIn2024), isTrue);

      // Multi-word search
      final potionBonus =
          items.where((i) => i.matches('potion bonus action')).toList();
      expect(potionBonus.any((i) => i.id == 'action_potions'), isTrue);

      // Weapon masteries search
      final nickMastery =
          items.where((i) => i.matches('nick mastery')).toList();
      expect(
          nickMastery.any((i) =>
              i.id == 'table_weapon_properties_masteries' ||
              i.id == 'action_two_weapon_fighting'),
          isTrue);
    });
  });
  ...
  group('Interactive Rules Calculators Unit & Widget Tests', () {
    ...
    testWidgets(
        'ConcentrationCalculatorWidget computes DC and performs roll test',
        (WidgetTester tester) async {
      await tester.pumpWidget(createTestableWidget(
          const Scaffold(body: ConcentrationCalculatorWidget())));
    ...
    testWidgets(
        'FallingDamageCalculatorWidget computes dice pool and rolls damage',
        (WidgetTester tester) async {
      await tester.pumpWidget(createTestableWidget(
          const Scaffold(body: FallingDamageCalculatorWidget())));
    ...
    testWidgets('GrappleShoveCalculatorWidget computes 2024 Save DC',
        (WidgetTester tester) async {
      await tester.pumpWidget(createTestableWidget(
          const Scaffold(body: GrappleShoveCalculatorWidget())));
    ...
    testWidgets('DcBenchmarkSelectorWidget renders DC scale from 5 to 30',
        (WidgetTester tester) async {
      await tester.pumpWidget(createTestableWidget(
          const Scaffold(body: DcBenchmarkSelectorWidget())));
  ...
  group('RulesCompendiumScreen Integrated UI Tests', () {
    ...
    testWidgets(
        'renders Rules Compendium with all category chips and quick dice',
        (WidgetTester tester) async {
      ...
      await tester
          .pumpWidget(createTestableWidget(const RulesCompendiumScreen()));
      ...
      expect(
          find.widgetWithText(ChoiceChip, 'Actions & Combat'), findsOneWidget);
      expect(find.widgetWithText(ChoiceChip, 'Conditions & Statuses'),
          findsOneWidget);
      expect(find.widgetWithText(ChoiceChip, 'Environment & Hazards'),
          findsOneWidget);
      expect(
          find.widgetWithText(ChoiceChip, 'Exploration & DCs'), findsOneWidget);
      expect(
          find.widgetWithText(ChoiceChip, 'Magic & Resting'), findsOneWidget);
      expect(find.widgetWithText(ChoiceChip, 'Quick Reference Tables'),
          findsOneWidget);
    });

    testWidgets('search highlighting and operator filtering work in UI',
        (WidgetTester tester) async {
      tester.view.physicalSize = const Size(1200, 1600);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() {
    ...
    testWidgets(
        'opening interactive calculator inside rule card expands calculator widget',
        (WidgetTester tester) async {
      tester.view.physicalSize = const Size(1200, 1600);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() {
    ...
    testWidgets(
        'Quick Reference Tables category displays Table Roller banner and launches TableIndexScreen',
        (WidgetTester tester) async {
      tester.view.physicalSize = const Size(1200, 1600);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() {
      ...
      final tablesChip =
          find.widgetWithText(ChoiceChip, 'Quick Reference Tables');
      ...
      expect(
          find.text('Interactive Table Roller & Generators'), findsOneWidget);
