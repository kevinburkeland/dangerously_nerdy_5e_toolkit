...
void main() {
  ...
  group('SRD Equipment & Inventory Character Builder Tests', () {
    test(
        'SrdEquipmentLibrary returns class-tailored packages for all core classes',
        () {
      ...
      expect(wizardPackages.any((p) => p.name.contains('Starting Wealth')),
          isTrue);
      ...
      final barbarianPackages =
          SrdEquipmentLibrary.getPackagesForClass('barbarian');
      expect(barbarianPackages.any((p) => p.name.contains('Greataxe')), isTrue);
      expect(barbarianPackages.last.startingGold, equals(50));
    });

    testWidgets(
        'Wizard class displays Wizard SRD starting equipment packages and starts with spellbook',
        (tester) async {
      ...
      expect(find.textContaining('Starting Equipment & Inventory (SRD)'),
          findsOneWidget);
      expect(find.text('Spellweaver: Quarterstaff, Arcane Focus & Spellbook'),
          findsOneWidget);
      expect(find.text('Arcane Explorer: Dagger, Component Pouch & Spellbook'),
          findsOneWidget);
      expect(find.text('Starting Wealth (Gold Only Option)'), findsOneWidget);

      // Select Arcane Explorer package
      await tester.tap(
          find.text('Arcane Explorer: Dagger, Component Pouch & Spellbook'));
      await tester.pumpAndSettle();
      ...
      expect(
          find.textContaining(
              'Arcane Explorer: Dagger, Component Pouch & Spellbook'),
          findsOneWidget);
      ...
      expect(wizard.progression.startingClass?.classRef.displayName,
          equals('Wizard'));
      expect(wizard.inventory.any((i) => i.displayName == 'Spellbook'), isTrue);
      expect(wizard.inventory.any((i) => i.displayName == 'Dagger'), isTrue);
      expect(wizard.inventory.any((i) => i.displayName == 'Potion of Healing'),
          isTrue);
    });

    testWidgets(
        'Starting Wealth (Gold Only) option assigns class gold to purse and zero initial gear',
        (tester) async {
      tester.view.physicalSize = const Size(1200, 2600);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      if (find
          .textContaining('Skill Proficiency Overlap Detected')
          .evaluate()
          .isNotEmpty) {
