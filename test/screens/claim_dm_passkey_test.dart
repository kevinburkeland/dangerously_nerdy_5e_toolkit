...
''';
      expect(CryptoUtils.extractHostKey(snippet),
          equals('c4974958-3d12-4217-bf45-ee77e9b0ab83'));
    });

    test(
        'encodeHostKeyToMnemonic produces 6 distinct non-repeating words for any input',
        () {
      final inputs = [
      ...
      for (final input in inputs) {
        ...
        expect(words.toSet().length, equals(6),
            reason: 'Words in $phrase must be distinct without repeating');
  ...
  group('ClaimDmPasskeyDialog Widget Tests', () {
    ...
    testWidgets(
        'Submitting valid passkey promotes membership to DM and closes dialog',
        (tester) async {
      ...
      // Enter passkey
      final passkeyField =
          find.widgetWithText(TextField, 'DM Passkey / Host Key');
    ...
    testWidgets('JoinCampaignDialog renders DM passkey toggle and inputs',
        (tester) async {
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
