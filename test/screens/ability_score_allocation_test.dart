...
void main() {
  ...
  Future<void> advanceToAbilityScores(WidgetTester tester) async {
    for (int i = 0; i < 10; i++) {
      if (find
          .textContaining('Ability Score Allocation')
          .evaluate()
          .isNotEmpty) {
        break;
      }
      if (find.text('Step 2: Choose Species / Race').evaluate().isNotEmpty) {
        ...
        if (defense.evaluate().isNotEmpty) {
          ...
          .textContaining('Choose Background')
          .evaluate()
          .isNotEmpty) {
  ...
  group('Character Builder Ability Score Generation & Lineage Bonus Tests', () {
    testWidgets('supports switching to Dice Roll mode and rolling stats',
        (tester) async {
      ...
      expect(find.textContaining('FINAL STARTING ATTRIBUTES PREVIEW'),
          findsOneWidget);
    });
    ...
    testWidgets('supports Enter Own manual score input', (tester) async {
      ...
      expect(find.text('Enter your own custom attributes (3 to 30):'),
          findsOneWidget);
    ...
    testWidgets(
        'Variant Human in 2014 mode shows +1 to two scores selection chips',
        (tester) async {
      tester.view.physicalSize = const Size(1200, 2400);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      expect(find.text('Human (Variant) Lineage Bonus (+1 to 2 Scores)'),
          findsOneWidget);
    ...
    testWidgets(
        'Custom Lineage in 2014 mode grants +2 to chosen score and includes bonus feat step',
        (tester) async {
      tester.view.physicalSize = const Size(1200, 2400);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      expect(find.text('Custom Lineage Lineage Bonus (+2 to 1 Score)'),
          findsOneWidget);
      ...
      expect(
          find.textContaining(
              'As a Custom Lineage, choose your 1st-level bonus feat'),
          findsOneWidget);
    });

    testWidgets(
        'Dynamic Homebrew race with flexible ability bonus choices and bonus feat pipeline',
        (tester) async {
      tester.view.physicalSize = const Size(1200, 2400);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      final homebrewRace = Race(
        id: const EntityId(
            slug: 'homebrew-astral-elf', ruleset: RulesetVersion.v2014),
      ...
      addTearDown(() =>
          HomebrewPersistenceService().deleteCustomRace('homebrew-astral-elf'));
      ...
      expect(find.text('Astral Elf (Homebrew) Lineage Bonus (+1 to 3 Scores)'),
          findsOneWidget);
      expect(
          find.text('Select 3 different ability scores to receive a +1 bonus:'),
          findsOneWidget);
      ...
      expect(find.textContaining('Astral Elf (Homebrew) Bonus Feat'),
          findsOneWidget);
      expect(
          find.textContaining(
              'As a Astral Elf (Homebrew), choose your 1st-level bonus feat'),
          findsOneWidget);
    });
  });
}
