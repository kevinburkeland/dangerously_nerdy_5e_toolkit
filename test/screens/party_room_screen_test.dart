...
void main() {
  ...
  group('PartyRoomScreen Widget Tests', () {
    ...
    testWidgets('Renders header, coin purse card, and tabs', (tester) async {
      ...
      await tester
          .pumpWidget(createWidgetUnderTest(roomCode, partyService, registry));
    ...
    testWidgets('Tapping Deposit Coins opens CoinTransactionDialog',
        (tester) async {
    ...
    testWidgets('Displays vault loot items with Claim and Attunement actions',
        (tester) async {
      ...
      await partyService.addLootItem(
          roomCode: roomCode, playerName: 'DM', item: ring);
    ...
    testWidgets('Active Character Banner, Switch Dialog, and Roster Manager',
        (tester) async {
      tester.view.physicalSize = const Size(800, 1200);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() {
      ...
      await partyService.addCharacterToRoster(
        roomCode: roomCode,
        characterName: 'Corin (Rogue)',
        playerName: 'DM',
      );

      await tester.pumpWidget(createWidgetUnderTest(
          roomCode, partyService, registry,
          initialPlayerName: 'Alden'));
      await tester.pumpAndSettle();

      // Check Active Character Banner
      expect(find.text('ACTIVE CHARACTER / SESSION IDENTITY'), findsOneWidget);
      expect(find.text('Alden'), findsWidgets);
      expect(find.text('Corin (Rogue)'), findsWidgets);

      // Tap Quick Roster Select chip for 'Corin (Rogue)'
      await tester.tap(find.text('Corin (Rogue)').first);
      await tester.pumpAndSettle();
    ...
    testWidgets(
        'PartyRoomScreen embeds RoomConnectionBadge and receives telemetry',
        (tester) async {
    ...
    testWidgets(
        'Live dice feed immediately renders cached rolls and persists across tab switches',
        (tester) async {
      ...
      expect(find.text('Sylas'), findsOneWidget);
      ...
      expect(find.text('Dain'), findsOneWidget);
    ...
    testWidgets(
        'Active character banner renders horizontally without squishing on mobile view (360x640)',
        (tester) async {
