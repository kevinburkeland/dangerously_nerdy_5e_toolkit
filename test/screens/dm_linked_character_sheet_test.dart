...
Character _createSampleHero(
    {required String id, required String name, int currentHp = 25}) {
  return Character(
...
void main() {
  ...
  group('DM Linked Character Sheet & Modification Tests', () {
    testWidgets(
        'DM Dashboard renders linked character vitality tile with open sheet button',
        (tester) async {
      tester.view.physicalSize = const Size(1400, 1000);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());

      final hero = _createSampleHero(
          id: 'hero_alden', name: 'Alden the Gray', currentHp: 32);
      ...
      expect(find.text('Alden the Gray'), findsOneWidget);
      expect(find.text('32 HP'), findsOneWidget);
      expect(find.byTooltip('Open Full Sheet (DM Mode)'), findsOneWidget);
    });

    testWidgets(
        'Tapping sheet icon opens CharacterSheetView in DM Mode and reflects back on return',
        (tester) async {
      tester.view.physicalSize = const Size(1400, 1000);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      final hero =
          _createSampleHero(id: 'hero_valen', name: 'Valen', currentHp: 40);
      await CharacterPersistenceService().saveCharacter(hero);
      ...
      expect(find.text('Valen'), findsOneWidget);
      ...
      expect(find.text('Valen'), findsAtLeast(1));
    ...
    testWidgets(
        'DM Dashboard allows linking existing saved character from library',
        (tester) async {
      tester.view.physicalSize = const Size(1400, 1000);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      final hero = _createSampleHero(
          id: 'hero_sylas', name: 'Sylas Swiftbow', currentHp: 28);
      await CharacterPersistenceService().saveCharacter(hero);
      ...
      expect(find.text('Sylas Swiftbow'), findsOneWidget);
      expect(find.text('28 HP'), findsOneWidget);

      final updatedProfile = await CampaignProfileService().getActiveProfile();
      expect(updatedProfile.partyCharacterIds, contains('hero_sylas'));
    });

    testWidgets(
        'PartyRoomScreen shows Sheet button and opens CharacterSheetView for active character',
        (tester) async {
      tester.view.physicalSize = const Size(1400, 1000);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      final hero =
          _createSampleHero(id: 'hero_gimli', name: 'Dain', currentHp: 50);
      await CharacterPersistenceService().saveCharacter(hero);

      final partyService = PartyRoomService();
      await partyService.createCampaign(
        campaignName: 'Deep Mine Expedition',
        playerName: 'Dain',
      ...
      await partyService.linkCharacterToCampaign(
        roomCode: 'DWARF1',
        character: hero,
        existingRosterName: 'Dain',
      );

      await tester.pumpWidget(_buildTestApp(
        home: PartyRoomScreen(
          roomCode: 'DWARF1',
          initialPlayerName: 'Dain',
          partyService: partyService,
        ),
      ));
      await tester.pumpAndSettle();

      expect(find.text('Dain'), findsAtLeast(1));
