...
void main() {
  ...
  group('CharacterBuilderScreen Live Sheet & Wizard UI Tests', () {
    testWidgets('Renders empty roster state when no characters are created',
        (tester) async {
    ...
    testWidgets(
        'Tapping Open Sheet on character opens Live Sheet with vitals and skills',
        (tester) async {
      ...
      final openSheetBtn =
          find.byKey(const ValueKey('open_sheet_valeros-ironclad'));
      ...
      expect(find.byKey(const Key('character_sheet_roster_button')),
          findsOneWidget);
    });

    testWidgets(
        'Tapping Switch Hero in Live Sheet returns to Character Selector',
        (tester) async {
      tester.view.physicalSize = const Size(1200, 2400);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
    ...
    testWidgets(
        'Deleting a character prompts confirmation and removes from roster',
        (tester) async {
      tester.view.physicalSize = const Size(1200, 2400);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      final deleteBtn =
          find.byKey(const ValueKey('delete_character_lyra-sunseeker'));
    ...
    testWidgets('Tapping an ability score triggers roll feedback',
        (tester) async {
      tester.view.physicalSize = const Size(1200, 1600);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
    ...
    testWidgets('Switching to Guided Builder tab displays 8-step wizard',
        (tester) async {
      tester.view.physicalSize = const Size(1200, 2400);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
    ...
    testWidgets(
        'Toggling RulesEditionToggle updates edition state and SettingsProvider',
        (tester) async {
      tester.view.physicalSize = const Size(1200, 1600);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      await tester
          .tap(find.descendant(of: toggleFinder, matching: find.text('2014')));
      await tester.pumpAndSettle();
      ...
      // Switch back to 2024
      await tester
          .tap(find.descendant(of: toggleFinder, matching: find.text('2024')));
      await tester.pumpAndSettle();
    ...
    testWidgets(
        'Guided Builder Step 1 ruleset switch updates global SettingsProvider and species speed',
        (tester) async {
      tester.view.physicalSize = const Size(1200, 1600);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
    ...
    testWidgets(
        '2014 character creation skips Feat step and creates character with no origin feats',
        (tester) async {
      tester.view.physicalSize = const Size(1200, 1600);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      if (defenseFinder.evaluate().isNotEmpty) {
        await tester.tap(defenseFinder.first);
        await tester.pumpAndSettle();
      }
      await tester.tap(find.text('Next Step'));
      await tester.pumpAndSettle();
    ...
    testWidgets(
        '2014 Variant Human character creation includes Feat step and saves starting feat',
        (tester) async {
      ...
      final defenseFinder2 = find.widgetWithText(ListTile, 'Defense');
      if (defenseFinder2.evaluate().isNotEmpty) {
        await tester.tap(defenseFinder2.first);
        await tester.pumpAndSettle();
      }
      await tester.tap(find.text('Next Step'));
      await tester.pumpAndSettle();
