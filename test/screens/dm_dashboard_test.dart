...
void main() {
  ...
  group('DmDashboardScreen Widget Tests', () {
    testWidgets(
        'Renders all 5 tactical HUD sections and default campaign header',
        (tester) async {
    ...
    testWidgets(
        'Initiative turn cycling advances active turn and round counter',
        (tester) async {
      tester.view.physicalSize = const Size(1400, 1000);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
    ...
    testWidgets(
        'Applying damage and heal chips updates participant HP with Temp HP absorption',
        (tester) async {
      tester.view.physicalSize = const Size(1400, 1000);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
    ...
    testWidgets('Adds new combatant via Add Combatant dialog', (tester) async {
      ...
      final p = CampaignProfile.defaultProfile(
          id: 'camp_add_c', name: 'Add Combatant Table', nodeId: 'test_node');
      await CampaignProfileService().saveProfileImmediate(p);
      await CampaignProfileService().switchProfile(p.id);
    ...
    testWidgets('Adds animated minion and updates minion HP', (tester) async {
      ...
      final p = CampaignProfile.defaultProfile(
          id: 'camp_minion_t', name: 'Minions Table', nodeId: 'test_node');
      await CampaignProfileService().saveProfileImmediate(p);
      await CampaignProfileService().switchProfile(p.id);
    ...
    testWidgets('Session notes scratchpad edits persist', (tester) async {
      ...
      final p = CampaignProfile.defaultProfile(
          id: 'camp_notes_t', name: 'Notes Table', nodeId: 'test_node');
      await CampaignProfileService().saveProfileImmediate(p);
      await CampaignProfileService().switchProfile(p.id);
      ...
      final textFieldFinder =
          find.widgetWithText(TextField, 'DM Scratchpad & Session Notes');
      expect(textFieldFinder, findsOneWidget);

      await tester.enterText(
          textFieldFinder, 'Secret trap door behind the painting.');
      await tester.pumpAndSettle();

      final current = await CampaignProfileService().getActiveProfile();
      expect(current.notesMarkdown,
          equals('Secret trap door behind the painting.'));
    });

    testWidgets('Campaign picker bottom sheet opens and switches campaign',
        (tester) async {
      tester.view.physicalSize = const Size(1400, 1000);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      final p1 = CampaignProfile.defaultProfile(
          id: 'c1', name: 'First Campaign', nodeId: 'test_node');
      final p2 = CampaignProfile.defaultProfile(
          id: 'c2', name: 'Second Campaign', nodeId: 'test_node');
    ...
    testWidgets('Opens Snapshot Export dialog and copies JSON', (tester) async {
      ...
      final p = CampaignProfile.defaultProfile(
          id: 'c_exp', name: 'Export Test Campaign', nodeId: 'test_node');
      await CampaignProfileService().saveProfileImmediate(p);
      await CampaignProfileService().switchProfile(p.id);
    ...
    testWidgets(
        'Handles invalid snapshot JSON import gracefully with error snackbar',
        (tester) async {
      tester.view.physicalSize = const Size(1400, 1000);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      final p = CampaignProfile.defaultProfile(
          id: 'c_imp', name: 'Import Test Table', nodeId: 'test_node');
      await CampaignProfileService().saveProfileImmediate(p);
      await CampaignProfileService().switchProfile(p.id);
