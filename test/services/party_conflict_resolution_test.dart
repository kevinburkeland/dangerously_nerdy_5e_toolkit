...
void main() {
  ...
  group('Multi-Tier Conflict & Race Resolution Tests', () {
    ...
    test('Tier 2: Deterministic Claim Race Fallback & Stream Event', () async {
      ...
      expect(loot.firstWhere((i) => i.id == sword.id).claimedByPlayer,
          equals('Valen'));
      ...
      final sub =
          partyService.claimConflictStream.listen((e) => conflictEvents.add(e));

      // Re-claiming with Valen already set
      await partyService.claimLootItem(
        roomCode: session.roomCode,
        lootId: sword.id,
        playerName: 'Kaelen',
    ...
    test('Purse Overdraft Clamping and Audit Event Logging', () async {
      ...
      final sub =
          partyService.overdraftStream.listen((e) => overdraftEvents.add(e));
      ...
      expect(
          events.firstWhere((e) => e.type == 'purseOverdraftWarning').details,
          contains('clamped to 0'));
    });

    test(
        'Tier 3: Structural Fork & Host Diff Resolution 1 (Accept Cloud Version)',
        () async {
      final session = await partyService.createCampaign(
      ...
      partyService.flagItemConflict(
          session.roomCode, localItem.id, cloudPayload);
      ...
      expect(
          resolved.name, equals('Ring of Protection +2 (Cloud Authoritative)'));
      expect(resolved.gpValue, equals(3500.0));
      expect(resolved.claimedByPlayer, equals('Wizard'));
    });

    test(
        'Tier 3: Structural Fork & Host Diff Resolution 2 (Overwrite with Local)',
        () async {
      final session = await partyService.createCampaign(
    ...
    test(
        'Tier 3: Structural Fork & Host Diff Resolution 3 (Keep Both / Duplicate)',
        () async {
      final session = await partyService.createCampaign(
      ...
      expect(
          cloudRetained.name, equals('Staff of Power (Attuned to Archmage)'));
    ...
    test(
        'Unauthorized user cannot execute Host Diff resolutions without valid hostKey',
        () async {
      final session = await partyService.createCampaign(
