...
void main() {
  group('CompendiumTagParser', () {
    ...
    test('detects custom / homebrew source', () {
      expect(
          parser.detectRuleset('KoboldPress'), equals(RulesetVersion.homebrew));
    ...
    test('strips damage tags and extracts evaluation math', () {
      ...
      expect(
          result.cleanMarkdown, equals('Deals 8d6 damage on a failed save.'));
    ...
    test('parses spell references with source scoping', () {
      const input =
          'Can cast {@spell Fireball|PHB} and {@spell Counterspell|XPHB}.';
      final result = parser.parseEntries(input);

      expect(
          result.cleanMarkdown, equals('Can cast Fireball and Counterspell.'));
      expect(result.extractedReferences.length, equals(2));
      expect(result.extractedReferences[0].slug, equals('fireball'));
      expect(result.extractedReferences[0].rulesetPreferred,
          equals(RulesetVersion.v2014));
      expect(result.extractedReferences[1].slug, equals('counterspell'));
      expect(result.extractedReferences[1].rulesetPreferred,
          equals(RulesetVersion.v2024));
    });

    test('parses monster and item references', () {
      const input =
          'Summons a {@creature Goblin|MM} wielding a {@item Longsword|PHB}.';
      final result = parser.parseEntries(input);

      expect(result.cleanMarkdown,
          equals('Summons a Goblin wielding a Longsword.'));
      expect(result.extractedReferences.length, equals(2));
      expect(result.extractedReferences[0].refType, equals(EntityType.monster));
      expect(
          result.extractedReferences[1].refType, equals(EntityType.equipment));
    });
