...
void main() {
  ...
  group('CommunityCompendiumImporterService', () {
    ...
    setUp(() async {
      ...
      importer =
          CommunityCompendiumImporterService(homebrewService: homebrewService);
    });
    ...
    test('detects schema types accurately', () {
      ...
      expect(
        importer.detectType(
            {'name': 'Cloak of Protection', 'rarity': 'Uncommon', 'type': 'W'}),
    ...
    test(
        'imports bundle JSON, preserves dual rulesets, and synchronizes to compendium libraries',
        () async {
        ...
        'spell': [
          {
            ...
            'entries': [
              'A creature of your choice that you can see within range regains hit points equal to {@damage 1d4} + your spellcasting ability modifier.'
            ],
          },
          {
            'name': 'Healing Word',
      ...
      final result =
          await importer.importJsonString(bundleJson, persistAndSync: true);
      ...
      expect(
          spells.any((s) =>
              s.id.slug == 'healing-word' &&
              s.id.ruleset == RulesetVersion.v2014),
          isTrue);
      expect(
          spells.any((s) =>
              s.id.slug == 'healing-word' &&
              s.id.ruleset == RulesetVersion.v2024),
          isTrue);
    ...
    test('handles empty or malformed JSON payloads safely', () async {
      ...
      final invalidResult =
          await importer.importJsonString('not a valid json string');
