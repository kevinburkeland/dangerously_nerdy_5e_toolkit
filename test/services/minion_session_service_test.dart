...
void main() {
  ...
  group('MinionSessionService Unit Tests', () {
    test(
        'getOrCreateSession creates and initializes default minions for Animate Objects',
        () {
    ...
    test(
        'getOrCreateSession returns existing session without overwriting modifications',
        () {
    ...
    test('Multiple presets maintain distinct and independent session states',
      ...
      final animalsSession =
          service.getOrCreateSession(animalsPreset, defaultSpellLevel: 3);
      final undeadSession =
          service.getOrCreateSession(undeadPreset, defaultSpellLevel: 3);
    ...
    test(
        'Bag of Tricks variants auto-roll upon initial session creation and persist across visits',
        () {
      final graySession =
          service.getOrCreateSession(BagOfTricksSummons.grayBagPreset);
      ...
      final rustSession =
          service.getOrCreateSession(BagOfTricksSummons.rustBagPreset);
      expect(rustSession.activeObjects.length, equals(1));

      final tanSession =
          service.getOrCreateSession(BagOfTricksSummons.tanBagPreset);
      expect(tanSession.activeObjects.length, equals(1));
    });

    test(
        'Each Horn of Valhalla variant rolls appropriate Berserker squad and persists through session',
        () {
      final silverSession =
          service.getOrCreateSession(ValhallaSummons.silverHornPreset);
