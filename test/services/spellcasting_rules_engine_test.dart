...
void main() {
  ...
  group('SRD Spell Catalog Model Integrity Tests', () {
    test('Cure Wounds and Healing Word have edition-accurate schools', () {
      ...
      expect(
          cureWounds.getSchool(DmRulesEdition.v2024), SpellSchool.abjuration);

      final healingWord = SpellbookLibrary.getSpellById('spell_healing_word')!;
      expect(
          healingWord.getSchool(DmRulesEdition.v2014), SpellSchool.evocation);
      expect(
          healingWord.getSchool(DmRulesEdition.v2024), SpellSchool.abjuration);
    });

    test('Create Undead has costly material component marked', () {
      final createUndead =
          SpellbookLibrary.getSpellById('spell_create_undead')!;
    ...
    test('All spell levels 0 through 9 are present in library', () {
      for (int lvl = 0; lvl <= 9; lvl++) {
        ...
        expect(spellsAtLevel.isNotEmpty, isTrue,
            reason: 'Level $lvl spells should not be empty');
      }
    });
    ...
    test('Costly material components consume flags and costs are accurate', () {
      ...
      final greaterRestoration =
          SpellbookLibrary.getSpellById('spell_greater_restoration')!;
