...
void main() {
  ...
  group('Compendium Extended Ingestion & Tools for Nerds Tests', () {
    ...
    test(
        'ingests multi-entity bundle with classes, subclasses, races, feats, and backgrounds',
        () {
...
''';
      ...
      expect(
          result.classes.first.featuresMarkdown, contains('Tactical Command'));
      ...
      expect(
          result.subclasses.first.featuresMarkdown, contains('Battle Clarity'));
      ...
      expect(result.feats.first.descriptionMarkdown,
          contains('You gain a +1 bonus'));

      // Backgrounds
      expect(result.backgrounds.length, equals(1));
      expect(result.backgrounds.first.name, equals('Cartographer'));
      expect(result.backgrounds.first.skillProficiencies,
      ...
      expect(result.otherEntries.first.name,
          equals('Wild Magic Surges (Expanded)'));
      expect(result.otherEntries.first.category, equals('Table'));
    });

    test(
        'persists and hydrates all extended homebrew types across repository and backups',
        () async {
      ...
      const customFeat = Feat(
        id: EntityId(
            slug: 'spell-sniper-plus', ruleset: RulesetVersion.homebrew),
      ...
      final homebrewLayer =
          repo.layers.firstWhere((l) => l.layerId == 'homebrew-packs');
      expect(homebrewLayer.get('shaman', type: EntityType.classDefinition),
          isNotNull);
      expect(
          homebrewLayer.get('gorgonkin', type: EntityType.species), isNotNull);
      expect(homebrewLayer.get('spell-sniper-plus', type: EntityType.feat),
          isNotNull);
      expect(homebrewLayer.get('astronomer', type: EntityType.background),
          isNotNull);
    });
  });
}
