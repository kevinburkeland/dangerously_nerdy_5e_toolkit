...
void main() {
  group('ArenaCombatEngine Tests', () {
    ...
    setUp(() {
      ...
      wolfMonster = MonsterCodexLibrary.getMonsterByName('Wolf') ??
          ...
          MonsterCodexLibrary.allMonsters
              .firstWhere((m) => m.name.toLowerCase() == 'wolf');
      trexMonster = MonsterCodexLibrary.getMonsterByName('Tyrannosaurus Rex') ??
          ...
          MonsterCodexLibrary.allMonsters.firstWhere(
              (m) => m.name.toLowerCase().contains('tyrannosaurus'));
      dragonMonster =
          MonsterCodexLibrary.getMonsterByName('Young Red Dragon') ??
              MonsterCodexLibrary.getMonsterById('srd_mon_young_red_dragon') ??
              MonsterCodexLibrary.allMonsters.firstWhere(
                  (m) => m.name.toLowerCase() == 'young red dragon');
    });
    ...
    test('rollInitiatives correctly assigns and sorts combatants by initiative',
      ...
      expect(sorted.first.initiative,
          greaterThanOrEqualTo(sorted.last.initiative));
    });
    ...
    test('selectTarget chooses target based on strategy', () {
      ...
      final targetLowest = engine.selectTarget(
          attacker, all, ArenaTargetingStrategy.focusLowestHp);
      expect(targetLowest?.id, 'weak');

      // Highest threat should select toughEnemy (Dragon has higher CR)
      final targetThreat = engine.selectTarget(
          attacker, all, ArenaTargetingStrategy.highestThreat);
    ...
    test(
        'runMonteCarlo runs high-speed simulations and aggregates statistical win rates',
        () {
      final teamA = [
        ArenaCombatant.fromMonster(
          id: 'trex',
      ...
      expect(
          monteCarlo.teamAWins + monteCarlo.teamBWins + monteCarlo.draws, 50);
      expect(monteCarlo.teamAWinRate,
          greaterThan(80.0)); // T-Rex should overwhelmingly beat 1 regular wolf
    ...
    test(
        'Flooded abyss grants swimmers advantage and gives non-swimmers disadvantage',
        () {
      final sharkMonster =
          MonsterCodexLibrary.getMonsterByName('Giant Shark') ??
    ...
    test(
        'Monsters with both melee and ranged attacks do not execute both in one turn',
        () {
      final goblinMonster = MonsterCodexLibrary.getMonsterByName('Goblin') ??
          MonsterCodexLibrary.getMonsterById('srd_mon_goblin') ??
          MonsterCodexLibrary.allMonsters
              .firstWhere((m) => m.name.toLowerCase() == 'goblin');
      ...
      expect(step.attackEvents.length, 1,
          reason:
              'Goblin has 1 action per round and must make only 1 attack, not both melee and ranged');
    });

    test(
        'Monsters with multiattack options respect action economy and do not execute all branches',
        () {
      final medusaMonster = MonsterCodexLibrary.getMonsterByName('Medusa') ??
          MonsterCodexLibrary.getMonsterById('srd_mon_medusa') ??
          MonsterCodexLibrary.allMonsters
              .firstWhere((m) => m.name.toLowerCase() == 'medusa');
      ...
      expect(step.attackEvents.length, 3,
          reason:
              'Medusa must make exactly 3 attacks in her multiattack routine, not execute every weapon');
    });
    ...
    test('Preset matchups correctly resolve into combatants', () {
      for (final preset in ArenaPresetMatchup.defaultPresets) {
        ...
        expect(resolved.teamA.isNotEmpty, true,
            reason: 'Preset ${preset.title} team A should not be empty');
        expect(resolved.teamB.isNotEmpty, true,
            reason: 'Preset ${preset.title} team B should not be empty');
    ...
    test(
        'Spellcaster parsing pre-caches slots, DC, attack bonus, and known spells',
        () {
      final lichMonster = MonsterCodexLibrary.getMonsterByName('Lich') ??
          MonsterCodexLibrary.allMonsters
              .firstWhere((m) => m.name.toLowerCase().contains('lich'));
    ...
    test(
        'Defensive Reaction Hook: Shield negates incoming attack and expends spell slot',
        () {
    ...
    test(
        'Concentration Check Pipeline: Breaks concentration on failed Con save',
        () {
      ...
      final res = combatant.checkConcentration(
          40, (_) => 1); // DC max(10, 20) = 20, roll 1+conMod < 20
      expect(res.broken, true);
      expect(combatant.activeConcentrationSpellId, isNull);
    });

    test(
        'Elemental damage routing respects monster immunities (e.g. Fire Elemental)',
        () {
      final fireElementalMonster =
          MonsterCodexLibrary.getMonsterByName('Fire Elemental') ??
              MonsterCodexLibrary.allMonsters.firstWhere(
                  (m) => m.name.toLowerCase().contains('fire elemental'));
    ...
    test(
        'DMG p.249 AoeResolver: deterministic base target caps and shape parsing',
        () {
      ...
      expect(AoeResolver.getBaseTargetCap(AoeShape.line, 100),
          closeTo(3.33, 0.05));

      // Parsing
      final fireballParsed =
          AoeResolver.parseShapeAndSize('Fireball (20-foot radius)');
      expect(fireballParsed.shape, AoeShape.sphere);
      expect(fireballParsed.sizeInFeet, 20.0);

      final breathParsed =
          AoeResolver.parseShapeAndSize('Fire Breath (60-foot cone)');
      expect(breathParsed.shape, AoeShape.cone);
      expect(breathParsed.sizeInFeet, 60.0);

      final lightningParsed =
          AoeResolver.parseShapeAndSize('Lightning Bolt (100-foot line)');
      expect(lightningParsed.shape, AoeShape.line);
      expect(lightningParsed.sizeInFeet, 100.0);
    });

    test(
        'DMG p.249 AoeResolver: Box-Muller Gaussian clustering and target selection',
        () {
      ...
      for (int i = 0; i < selected.length - 1; i++) {
        expect(selected[i].currentHp,
            lessThanOrEqualTo(selected[i + 1].currentHp));
      }
    });

    test(
        'Aerial Combat & Reach Validation: Grounded meleeStandard cannot hit airborne enemy',
        () {
    ...
    test(
        'Fall Damage & Condition Trigger: Non-hovering airborne entity falls on prone/stun/paralyze',
        () {
      final flyer = ArenaCombatant.fromMonster(
        id: 'dragon',
        monster: dragonMonster,
    ...
    test(
        'Hover capability prevents falling when conditions like Prone or Stunned are inflicted',
        () {
    ...
    test(
        'High-Tier Optimization: Lich vs 4 Gladiators completes in under 5 rounds on average with active 1st-5th slot utilization and legendary actions',
        () {
      final lichMonster = MonsterCodexLibrary.getMonsterByName('Lich') ??
          MonsterCodexLibrary.allMonsters
              .firstWhere((m) => m.name.toLowerCase().contains('lich'));
      final gladiatorMonster =
          MonsterCodexLibrary.getMonsterByName('Gladiator') ??
              MonsterCodexLibrary.allMonsters.firstWhere(
                  (m) => m.name.toLowerCase().contains('gladiator'));
      ...
      expect(mcResult.averageRounds, lessThanOrEqualTo(5.0),
          reason: 'Average battle duration should be <= 5 rounds');
      expect(mcResult.teamAWinRate, greaterThan(75.0),
          reason: 'Lich (CR 21) should decisively defeat 4 Gladiators (CR 5)');
      ...
      final allEvents =
          singleMatch.steps.expand((s) => s.attackEvents).toList();
      final hasLegendaryEvent = allEvents
          .any((e) => e.attackName.toLowerCase().contains('legendary'));
      expect(hasLegendaryEvent, true,
          reason: 'Lich should execute off-turn legendary actions');

      final spellEvents = allEvents
          .where((e) =>
              ...
              e.attackName.contains('Ray of Frost'))
          .toList();

      expect(spellEvents.isNotEmpty, true,
          reason:
              'Lich should actively cast leveled offensive spells and cantrips');
    });

    test(
        'Off-turn Legendary Actions trigger and consume legendary actions pool',
        () {
      final lichMonster = MonsterCodexLibrary.getMonsterByName('Lich') ??
          MonsterCodexLibrary.allMonsters
              .firstWhere((m) => m.name.toLowerCase().contains('lich'));
      ...
      final legEvents = step.attackEvents
          .where((e) => e.attackName.contains('Legendary'))
          .toList();
    ...
    test(
        'Spellcaster upcasts Fireball and Blight into higher slots when base slot is empty',
        () {
      final caster = ArenaCombatant.fromMonster(
        id: 'mage',
        monster: wolfMonster,
      ...
      expect(
          step.attackEvents
              .any((e) => e.attackName.contains('Fireball (Slot 5)')),
          true);
    });

    test(
        'Concentration on defensive spell does not block instantaneous leveled damage spells',
        () {
      final caster = ArenaCombatant.fromMonster(
        id: 'caster',
        monster: wolfMonster,
    ...
    test(
        'Audit all Monster Codex spellcasters: verify spell slots, known spells, and DC/attack bonuses',
        () {
      ...
      for (final monster in MonsterCodexLibrary.allMonsters) {
        ...
        if (mentionsSpellcasting) {
          ...
          spellcasterMonsters.add(
              '${monster.name} (CR ${monster.challengeRating}) -> Slots: ${combatant.maxSpellSlots}, Known Spells: ${combatant.knownSpellIds.length}');

          if (!combatant.isSpellcaster ||
              (combatant.maxSpellSlots.isEmpty &&
                  combatant.knownSpellIds.isEmpty)) {
            failedSpellcasters.add(
                '${monster.name} (${monster.id}) failed to parse spell slots or known spells');
          }
        }
      }
      ...
      // ignore: avoid_print
      print(
          '=== AUDITED ${spellcasterMonsters.length} SPELLCASTERS IN MONSTER CODEX ===');
      for (final sc in spellcasterMonsters) {
        // ignore: avoid_print
        print(sc);
      }

      expect(failedSpellcasters, isEmpty,
          reason:
              'All monsters with spellcasting traits should have non-empty parsed spells or slots');
    ...
    test('Legendary Resistance parses correctly on high-CR boss monsters', () {
      final redDragon = MonsterCodexLibrary.allMonsters
          .firstWhere((m) => m.name.toLowerCase() == 'ancient red dragon');
      ...
      final lichMonster = MonsterCodexLibrary.allMonsters
          .firstWhere((m) => m.name.toLowerCase() == 'lich');
    ...
    test(
        'Legendary Resistance automatically converts failed saves to successes and decrements remaining charges',
        () {
      final engine = ArenaCombatEngine(rng: Random(42));
      final lichMonster = MonsterCodexLibrary.allMonsters
          .firstWhere((m) => m.name.toLowerCase() == 'lich');
      final wolfMonster = MonsterCodexLibrary.allMonsters
          .firstWhere((m) => m.name.toLowerCase() == 'wolf');
      ...
      final attackEvent = turnResult.attackEvents
          .firstWhere((e) => e.attackName.contains('Disintegrate'));
      expect(attackEvent.isSavingThrow, true);
      if (attackEvent.saved &&
          attackEvent.summaryText.contains('LEGENDARY RESISTANCE')) {
    ...
    test(
        'Depleted Legendary Resistances (0 remaining) do not prevent failed saves',
        () {
      final engine = ArenaCombatEngine(rng: Random(42));
      final lichMonster = MonsterCodexLibrary.allMonsters
          .firstWhere((m) => m.name.toLowerCase() == 'lich');
    ...
    test(
        'Cage Match Arena: Goblin prioritizes Scimitar (melee) over Shortbow (ranged)',
        () {
      final engine = ArenaCombatEngine(rng: Random(42));
      final goblinMonster = MonsterCodexLibrary.getMonsterByName('Goblin') ??
          MonsterCodexLibrary.allMonsters
      ...
      expect(attack.attackName.toLowerCase().contains('scimitar'), true,
          reason:
              'In Cage Match, Goblin should choose melee Scimitar instead of ranged Shortbow');
      expect(attack.attackName.toLowerCase().contains('shortbow'), false);
    });

    test(
        'runMonteCarloAsync executes simulation across background isolate cleanly',
        () async {
      final engine = ArenaCombatEngine(rng: Random(42));
      final goblinMonster = MonsterCodexLibrary.allMonsters
          .firstWhere((m) => m.name.toLowerCase() == 'goblin');
      final wolfMonster = MonsterCodexLibrary.allMonsters
          .firstWhere((m) => m.name.toLowerCase() == 'wolf');
    ...
    test('simulateMatchAsync executes complete match in background isolate',
        () async {
      final engine = ArenaCombatEngine(rng: Random(42));
      final goblinMonster = MonsterCodexLibrary.allMonsters
          .firstWhere((m) => m.name.toLowerCase() == 'goblin');
