...
void main() {
  ...
  group('Minion Session Persistence Tests', () {
    ...
    test(
        'SpellSession serialization round-trip retains minion state and damage',
        () {
    ...
    test('MinionSessionService persists and restores session state', () async {
      ...
      session.updateObject(
          session.activeObjects.first.id, (m) => m.applyDamage(5));
      expect(
          session.activeObjects.first.currentHp <
              session.activeObjects.first.maxHp,
          isTrue);
      ...
      expect(reloadedSession.activeObjects.first.currentHp,
          session.activeObjects.first.currentHp);
    });
  });
}
