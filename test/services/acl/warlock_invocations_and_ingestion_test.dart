...
void main() {
  group('Warlock 2nd Level Invocations & Homebrew Ingestion Tests', () {
    ...
    test(
        'Warlock has Eldritch Invocations decision at 2nd level requiring 2 choices',
        () {
      ...
      expect(
          invocationDecision.availableOptions.length, greaterThanOrEqualTo(25));
    });
    ...
    test(
        'Ingests custom homebrew Eldritch Invocations and links to warlock feature choices',
        () async {
...
''';
      ...
      final agonizing = analysis.otherEntries
          .firstWhere((e) => e.displayName == 'Agonizing Blast');
      expect(agonizing.isSrdCanon, isTrue);
      expect(agonizing.isSelected, isFalse);

      // Grasp of the Void and Tomb of Winter should be Novel homebrew (selected)
      final grasp = analysis.otherEntries
          .firstWhere((e) => e.displayName == 'Grasp of the Void');
      expect(grasp.isSrdCanon, isFalse);
      expect(grasp.disposition, equals(ImportDisposition.novel));
      expect(grasp.isSelected, isTrue);

      final tomb = analysis.otherEntries
          .firstWhere((e) => e.displayName == 'Tomb of Winter');
      ...
      expect(updatedOptions.any((o) => o.name == 'Grasp of the Void'), isTrue);
      expect(updatedOptions.any((o) => o.name == 'Tomb of Winter'), isTrue);

      // Level 2 Warlock decisions should now offer these newly imported invocations
      final warlockLvl2 =
          SrdClassesLibrary.warlock.getDecisionsForLevel(2).first;
      expect(
          warlockLvl2.availableOptions
              .any((o) => o.name == 'Grasp of the Void'),
          isTrue);
      expect(
          warlockLvl2.availableOptions.any((o) => o.name == 'Tomb of Winter'),
          isTrue);

      // Deleting custom invocation removes it from options
      await persistence.deleteCustomOtherEntry('grasp-of-the-void');
      expect(
          SrdFeatureOptions.warlockInvocationsAndBoons
              .any((o) => o.name == 'Grasp of the Void'),
          isFalse);
      expect(
          SrdFeatureOptions.warlockInvocationsAndBoons
              .any((o) => o.name == 'Tomb of Winter'),
          isTrue);
    });

    test(
        'Pact of the Shroud is classified as Pact Boon, not Invocation, and Bond of the Shroud requires it',
        () async {
      const jsonCompendium = '''
{
  "optionalfeature": [
    {
      ...
      "featureType": ["PB"],
      "entries": [
        "Your patron gives you an amulet, a shroud of mist that can aid the wearer when the need is great."
      ]
    },
    {
      "name": "Bond of the Shroud",
      "source": "HOMEBREW",
      "featureType": ["EI"],
      "prerequisite": [
        {
      ...
      "entries": [
        "While someone else is wearing your shroud, you can use your action to teleport to them."
      ]
    }
  ]
}
''';
      ...
      final pactEntry = ingestion.otherEntries
          .firstWhere((e) => e.name == 'Pact of the Shroud');
      expect(pactEntry.category, equals('Pact Boon'));

      final bondEntry = ingestion.otherEntries
          .firstWhere((e) => e.name == 'Bond of the Shroud');
      ...
      expect(
          SrdFeatureOptions.warlockPactBoons
              .any((b) => b.name == 'Pact of the Shroud'),
          isTrue);

      // Verify Bond of the Shroud is registered in invocations
      final bondOpt = SrdFeatureOptions.warlockInvocations
          .firstWhere((i) => i.name == 'Bond of the Shroud');
      expect(bondOpt.prerequisite.minLevel, equals(12));
      expect(bondOpt.prerequisite.requiredPact, equals('shroud'));

      // Level 5 character with blade pact is gated
      final evalLocked = bondOpt.prerequisite
          .evaluate(classLevel: 5, selectedPacts: {'blade'});
      expect(evalLocked.isMet, isFalse);
      expect(
          evalLocked.unmetReasons, contains('Requires Level 12 (Current: 5)'));
      expect(evalLocked.unmetReasons, contains('Requires Pact of the Shroud'));

      // Level 12 character with shroud pact is met
      final evalUnlocked = bondOpt.prerequisite
          .evaluate(classLevel: 12, selectedPacts: {'shroud'});
