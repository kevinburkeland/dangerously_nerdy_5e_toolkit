...
void main() {
  group('CompendiumGenericEntryParser Tests', () {
    ...
    test('parses rollable table into formatted markdown table', () {
      ...
      expect(entry.descriptionMarkdown,
          contains('| 1 | A shower of falling starlight |'));
      expect(entry.descriptionMarkdown,
          contains('| 4 | A planar boundary shimmering faintly |'));
    });

    test(
        'parses deity entry with structured pantheon and domain metadata header',
        () {
      final json = {
      ...
      expect(
          entry.descriptionMarkdown, contains('**Pantheon:** Solar Expanse'));
      expect(entry.descriptionMarkdown, contains('**Alignment:** LG'));
      expect(entry.descriptionMarkdown, contains('**Domains:** Light, Life'));
      expect(entry.descriptionMarkdown,
          contains('**Symbol:** A blazing sun rising above twin mountains'));
      expect(entry.descriptionMarkdown,
          contains('Solas embodies the unyielding light'));
    });
    ...
    test('normalizes categorized entries for vehicles, traps, and hazards', () {
      final vehicleJson = {
        'name': 'Dune Skiff',
        'entries': [
          'A light wooden skiff equipped with silk sails for gliding over sand dunes.'
        ],
      };
      final vehicleEntry =
          parser.parseGenericEntry(vehicleJson, defaultCategory: 'vehicle');
      expect(vehicleEntry.category, equals('Vehicle'));

      final trapJson = {
        'name': 'Pressure Plate Dart Trap',
        'entries': [
          'Stepping on this stone plate triggers a volley of poisoned needles.'
        ],
      };
      final trapEntry =
          parser.parseGenericEntry(trapJson, defaultCategory: 'trap');
      expect(trapEntry.category, equals('Trap'));

      final hazardJson = {
        'name': 'Spore Blossom Cloud',
        'entries': [
          'Disturbing the fungal blooms creates a choking yellow mist.'
        ],
      };
      final hazardEntry =
          parser.parseGenericEntry(hazardJson, defaultCategory: 'hazard');
      expect(hazardEntry.category, equals('Hazard'));
    });

    test(
        'preserves extra properties in customProperties for lossless serialization',
        () {
      final json = {
    ...
    test(
        'infers categories from entityType, customProperties, or structural markers',
        () {
      final tableJson = {
      ...
      expect(parser.parseGenericEntry(vehicleUpgradeJson).category,
          equals('Vehicle'));
      ...
      expect(
          parser.parseGenericEntry(infusionJson).category, equals('Infusion'));
      ...
      expect(parser.parseGenericEntry(charOptionJson).category,
          equals('Character Option'));
