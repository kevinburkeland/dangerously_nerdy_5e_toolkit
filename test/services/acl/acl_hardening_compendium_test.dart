...
void main() {
  group('Compendium ACL Hardening & Boot Sequence Verification', () {
    test('1. Homebrew cantrip correctly routes to cantrips list on Character',
      ...
      final spellItem =
          HomebrewPersistenceService.spellToSpellItem(homebrewCantrip);
      ...
      expect(character.cantrips.map((c) => c.slug),
          isNot(contains('magic-missile')));
      expect(
          character.spellsKnown.map((s) => s.slug), contains('magic-missile'));
      expect(character.spellsKnown.map((s) => s.slug),
          isNot(contains('mystic-spark')));
    });

    test(
        '2. Ingesting compendium Fighter correctly stitches classFeature pointers (Second Wind)',
        () {
      ...
      expect(fighter.featuresMarkdown,
          isNot(contains('Second Wind|Fighter|PHB|1')));
    });

    test(
        '3. Monster with polymorphic ac: [{"ac": 16, "from": ["chain mail"]}] parses to ac = 16',
        () {
    ...
    test(
        '5. Item with polymorphic reqAttune: {"optional": true} and string reqAttune parses attunement',
        () {
    ...
    test('6. Spell with string components.m parses cleanly with 0 cost', () {
      ...
      expect(spell.components.materialDescription,
          'a tiny ball of bat guano and pitch');
      expect(spell.components.materialCostGp, 0);
      expect(spell.components.consumesMaterial, isFalse);
    });

    test(
        '7. Monster spellcasting normalization handles daily suffixes (1e, 3e) and slot objects',
        () {
      final parser = CompendiumMonsterParser();
      final raw = {
        'name': 'Archmage Lich',
        ...
        'spellcasting': [
          {
            ...
            'daily': {
              '1e': [
                '{@spell dominate monster|phb}',
                '{@spell plane shift|phb}'
              ],
      ...
      final monster = parser.parseMonster(raw);
      expect(monster.actionsMarkdown,
          contains('- **1/day each:** *dominate monster*, *plane shift*'));
      expect(monster.actionsMarkdown,
          contains('- **3/day each:** *darkness*, *misty step*'));
      expect(monster.actionsMarkdown,
          contains('- **At will:** *mage hand*, *prestidigitation*'));
      expect(monster.actionsMarkdown,
          contains('- **Cantrips (at will):** *ray of frost*'));
      expect(monster.actionsMarkdown,
          contains('- **Level 1 (4 slots):** *magic missile*, *shield*'));
    ...
    test(
        '8. {@atk mw} renders as *Melee Weapon Attack:* and {@spell fireball|phb} strips source',
        () {
      final transformer = EntryNodeTransformer();

      final res1 = transformer.transformEntries(
      ...
      final res2 = transformer
          .transformEntries('{@atk rw} {@hit 7} to hit, range 150/600 ft.');
      expect(
          res2.markdown,
          contains(
              '*Ranged Weapon Attack:* **`+7`** to hit, range 150/600 ft.'));

      final res3 = transformer.transformEntries('{@atk rs} {@hit 6} to hit.');
      expect(
          res3.markdown, contains('*Ranged Spell Attack:* **`+6`** to hit.'));

      final res4 = transformer.transformEntries(
          'The wizard casts {@spell fireball|phb} on the goblins.');
    ...
    test('9. Deep AST recursion handles type: "options" and nested entry nodes',
              ...
              'entries': [
                {
                  ...
                  'entries': [
                    'When you cast {@spell eldritch blast|phb}, add your Charisma modifier to the damage.'
                  ]
                },
                {
                  'type': 'entries',
                  ...
                  'entries': [
                    'You can cast {@spell mage armor|phb} on yourself at will.'
                  ]
                }
              ]
            }
      ...
      expect(res.markdown,
          contains('[eldritch blast](ref://spell/eldritch-blast)'));
      expect(res.markdown, contains('[mage armor](ref://spell/mage-armor)'));
    });

    test(
        '10. Explicit 2014 / 2024 ruleset toggle forces target edition on parsed homebrew',
        () {
      final pipeline = CompendiumJsonIngestionPipeline();
      const rawSpellJson = '''
      {
      ...
      final res2024 = pipeline.ingestJsonString(rawSpellJson,
          forceRuleset: RulesetVersion.v2024);
      expect(res2024.spells.first.id.ruleset, RulesetVersion.v2024);

      final res2014 = pipeline.ingestJsonString(rawSpellJson,
          forceRuleset: RulesetVersion.v2014);
      expect(res2014.spells.first.id.ruleset, RulesetVersion.v2014);

      final resAuto =
          pipeline.ingestJsonString(rawSpellJson, forceRuleset: null);
