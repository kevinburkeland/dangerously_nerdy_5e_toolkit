...
void main() {
  group('CompendiumClassParser Spell & Grant Extraction Tests', () {
    ...
    test(
        '1. Extracts bonus spells from prepared spell map (Cleric/Druid archetype style)',
        () {
      final rawSubclass = {
        'name': 'Astral Domain',
        'className': 'Cleric',
      ...
      final commandGrant =
          sub.grants.firstWhere((g) => g.payload['slug'] == 'command');
      expect(commandGrant.type, GrantType.bonusSpell);
      expect(commandGrant.payload['displayName'], 'command');
    });

    test(
        '2. Extracts bonus spells from expanded & known maps with cantrips (#c) and pipes',
        () {
      final rawSubclass = {
        'name': 'Star Patron',
        'className': 'Warlock',
      ...
      expect(sub.grants.length,
          5); // guidance, burning hands, shield, blur, misty step
      final grantNames =
          sub.grants.map((g) => g.payload['displayName']).toList();
      expect(
          grantNames,
          containsAll(
              ['guidance', 'burning hands', 'shield', 'blur', 'misty step']));

      final guidance =
          sub.grants.firstWhere((g) => g.payload['slug'] == 'guidance');
    ...
    test('3. Extracts bonus spells from nested innate ritual structures', () {
      ...
      expect(
          grantSlugs,
          containsAll(
              ['beast-sense', 'speak-with-animals', 'commune-with-nature']));
    });
  ...
  group('SubclassSpellsLibrary Dynamic Homebrew Tests', () {
    setUp(() {
      SrdClassesLibrary.setCustomSubclasses([
        Subclass(
          id: const EntityId(
              slug: 'custom-sun-domain', ruleset: RulesetVersion.homebrew),
          name: 'Sun Domain',
          classSlug: 'cleric',
    ...
    test(
        '5. SubclassSpellsLibrary resolves expanded spells from dynamic custom subclasses',
        () {
      final spells = SubclassSpellsLibrary.getExpandedSpells(
          'cleric', 'custom-sun-domain');
      expect(spells, contains('daylight'));
      expect(spells, contains('fireball'));
    });

    test(
        '6. SubclassSpellsLibrary detects always-prepared subclasses including custom prepared subclasses',
        () {
      expect(
          SubclassSpellsLibrary.isAlwaysPreparedSubclass(
              'cleric', 'custom-sun-domain'),
          isTrue);
    });
  ...
  group('Compendium Tag Processing Tests', () {
    test(
        '8. EntryNodeTransformer converts {@book} and {@variantrule} into clean text',
        () {
      final transformer = EntryNodeTransformer();
      final result1 = transformer.transformEntries([
        'Access to {@book Cosmic Magic Spells|CUSTOM_BOOK|Cosmic Magic Spells}.',
      ]);
      expect(result1.markdown, contains('Access to Cosmic Magic Spells.'));
      ...
      expect(
          result2.markdown,
          contains(
              'optional class features*, which replaces the Potent Spellcasting feature'));
      expect(result2.markdown, isNot(contains('{@variantrule')));
    });

    test(
        '9. CompendiumJsonIngestionPipeline.cleanRawTags cleans tags from markdown',
        () {
      const rawText =
          '> Note: This class has access to {@book Cosmic Magic Spells}. See {@variantrule optional features}.';
      ...
      expect(cleaned, contains('Cosmic Magic Spells'));
  ...
  group('Subclass Spells Ingestion & Storage Revitalization Tests', () {
    test(
        '10. The Deathless patron subclass extracts all spells from homebrew structure',
        () {
      final rawSubclass = {
        'name': 'The Deathless',
        'className': 'Warlock',
    ...
    test(
        '11. Subclass.fromMap auto-heals empty grants from customProperties additionalSpells',
        () {
      final storedMap = {
        'id': {
          'slug': 'the-deathless',
      ...
      addTearDown(
          () => SrdClassesLibrary.removeCustomSubclass('the-deathless'));
      final expanded =
          SubclassSpellsLibrary.getExpandedSpells('warlock', 'the-deathless');
      expect(expanded, contains('false life'));
      expect(expanded, contains('blindness/deafness'));
    });

    test(
        '12. SpellbookLibrary.findSpell matches spells with punctuation and special formatting',
        () {
    ...
    test(
        '13. Ingesting exported homebrew pack parses customProperties.classes cleanly into SpellItem',
        () {
      const exportedJson = '''
      {
        "schemaVersion": 1,
      ...
      expect(spell.customProperties['classes'],
          containsAll(['Sorcerer', 'Wizard']));

      final item = HomebrewPersistenceService.spellToSpellItem(spell);
      expect(item.rules2014.classes,
          containsAll([SpellClass.sorcerer, SpellClass.wizard]));
      expect(item.rules2024.classes,
          containsAll([SpellClass.sorcerer, SpellClass.wizard]));
    });

    test(
        '14. CompendiumSpellParser extracts classes from fromClassList, fromClassListVariant, and fromSubclass',
        () {
      final rawSpell = {
        'name': 'Astral Cascade',
        'source': 'Homebrew',
      ...
      expect(
          item.rules2014.classes,
          containsAll(
              [SpellClass.sorcerer, SpellClass.ranger, SpellClass.cleric]));
    });

    test(
        '15. Spell.fromMap un-nests nested customProperties and preserves root classes',
        () {
      ...
      expect(
          spell.customProperties['classes'], containsAll(['Druid', 'Cleric']));
      expect(spell.customProperties['page'], 100);

      final item = HomebrewPersistenceService.spellToSpellItem(spell);
      expect(item.rules2014.classes,
          containsAll([SpellClass.druid, SpellClass.cleric]));
      expect(item.rules2014.ritual, isTrue);
    });

    test(
        '16. HomebrewMergeResolver applyResolutionToAllCollisions marks isSelected true for overwrite',
        () {
      ...
      final localSpell =
          incSpell.copyWith(descriptionMarkdown: 'Old description.');
    ...
    test(
        '17. SubclassSpellsLibrary.resolveFilterSpells handles source filters and guards against empty filters',
        () {
      final clericFilter =
          SubclassSpellsLibrary.resolveFilterSpells('level=1|class=Cleric');
      expect(clericFilter, isNotEmpty);
      expect(clericFilter.every((s) => s.level == 1), isTrue);

      final unhandledFilter =
          SubclassSpellsLibrary.resolveFilterSpells('invalid_clause');
