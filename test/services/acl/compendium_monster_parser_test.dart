...
void main() {
  group('Community Compendium & Homebrew Monster Parser Tests', () {
    ...
    test(
        'parses complex monster with abbreviated size, alignment, action economy, and spellcasting',
        () {
      final raw = {
        ...
        'trait': [
          {
            'name': 'Void Aura',
            'entries': [
              'Bright light within 20 feet of the dragon becomes dim light.'
            ]
          }
        ],
        'action': [
        ...
        'bonus': [
          {
            'name': 'Shadow Slip',
            'entries': [
              'The dragon teleports up to 30 feet to an unoccupied space in dim light or darkness.'
            ]
          }
        ],
        'reaction': [
          {
            'name': 'Gravity Bend',
            'entries': [
              'When targeted by a ranged attack, the dragon imposes disadvantage on the attack roll.'
            ]
          }
        ],
        'spellcasting': [
          {
            ...
            'headerEntries': [
              'The dragon\'s innate spellcasting ability is Charisma (spell save {@dc 12}).'
            ],
            'daily': {
      ...
      expect(monster.actionsMarkdown,
          contains('**Speed:** walk 30ft., fly 60ft., (hover)'));
      expect(monster.actionsMarkdown,
          contains('| STR | DEX | CON | INT | WIS | CHA |'));
      expect(monster.actionsMarkdown,
          contains('**Saving Throws:** DEX +3, CON +5, WIS +2, CHA +4'));
      expect(monster.actionsMarkdown,
          contains('**Damage Immunities:** cold, radiant'));
      ...
      expect(monster.attackMath.any((m) => m.diceFormula.contains('1d10')),
          isTrue);
      expect(monster.attackMath.any((m) => m.damageType == DamageType.radiant),
          isTrue);
      ...
      expect(monster.customProperties['environment'],
          equals(['Underdark', 'Space']));
