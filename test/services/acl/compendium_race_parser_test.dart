...
void main() {
  group('Community Compendium & Homebrew Race/Subrace Parser Tests', () {
    ...
    test('parses lineage with multi-attribute bonuses and subraces', () {
      final raw = {
        ...
        'trait': [
          ...
          },
          {
            'name': 'Astral Fire',
            'entries': [
              'You know one cantrip of your choice from the cleric or wizard spell list.'
            ]
          }
        ],
        'subraces': [
            ...
            'trait': [
              {
                'name': 'Solar Wrath',
                'entries': [
                  'Deal extra radiant damage on your first hit each turn.'
                ]
              }
            ]
          }
      ...
      // 0% data loss
      expect(race.customProperties['lineageNotes'],
          equals('Inhabitants of the Astral Plane'));
      expect(race.customProperties['darkvision'], equals(60));
      expect(race.customProperties['additionalSpells'], isNotNull);
    });

    test(
        'parseSubrace extracts abilities, speed, darkvision, grants and markdown cleanly',
        () {
      final subRaw = {
      ...
      expect(
          sub.customProperties['customLore'], equals('Subterranean dwellers'));
    });
