...
void main() {
  group('Monster Feature ACL Parser & Discrepancy Resolution Tests', () {
    test('StatBlockAclParser.parseMonsterFeature extracts mobility, combat flags, reach, and riders', () {
    ...
    test('MonsterCombatProfile.fromStatBlock checks explicit declarations against ACL parser and favors ACL on discrepancies', () {
      ...
      const conflictedStatBlock = MinionStatBlock(
        ...
        traits: [
          ...
          CreatureTrait(
            name: 'Pack Tactics',
            description: 'The creature has advantage on attack rolls against a creature if at least one ally is within 5 feet.',
          ),
          CreatureTrait(
            name: 'Spellcasting',
            description: 'The creature is a 5th-level spellcaster. Its spellcasting ability is Wisdom (spell save DC 16, +8 to hit with spell attacks).',
          ),
        ],
        actions: [
          CreatureAction(
      ...
      final profile = MonsterCombatProfile.fromStatBlock(conflictedStatBlock, challengeRating: 5.0);

      // Verify that the internal ACL parser was favored on every discrepancy:
      expect(profile.meleeReachInFeet, equals(10), reason: 'ACL parsed reach 10 ft must take precedence over explicit 5 ft');
      expect(profile.canFly, isTrue, reason: 'ACL parsed fly speed must take precedence over explicit canFly: false');
      expect(profile.canSwim, isTrue, reason: 'ACL parsed Amphibious trait must grant canSwim');
      expect(profile.hasFlyby, isTrue, reason: 'ACL parsed Flyby trait must be active');
      expect(profile.spellSaveDc, equals(16), reason: 'ACL parsed spell save DC 16 must take precedence over explicit 10');
      expect(profile.spellAttackBonus, equals(8), reason: 'ACL parsed spell attack bonus +8 must take precedence');
      expect(profile.maxSpellSlots[1], equals(4), reason: 'ACL parsed 5th-level spellcaster slots must be populated');
      expect(profile.maxSpellSlots[3], equals(2), reason: 'ACL parsed 3rd-level spell slots must be populated');
    });

    test('CompendiumGenericEntryParser checks Monster Feature against ACL parser and favors ACL over conflicting raw properties', () {
      ...
      final entry = parser.parseGenericEntry(raw, defaultCategory: 'Monster Feature');
      ...
      expect(cp['canSwim'], isTrue, reason: 'Internal ACL parser identifies breathing air and water');
      expect(cp['maxReachFt'], equals(10), reason: 'Internal ACL parser identifies reach 10 ft');
      expect(cp['riders'], isNotNull);
      final riders = cp['riders'] as List;
      expect(riders.length, equals(2), reason: 'Internal ACL parser extracts grappled and restrained riders');
    });
  });
}
