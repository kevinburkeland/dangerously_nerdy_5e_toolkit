...
void main() {
  group('EntryNodeTransformer AST & Tag Processing Tests', () {
    ...
    test(
        'transforms basic string entries with inline tags to markdown and math',
        () {
      const input =
          'Deals {@damage 2d6|fire} damage and forces a {@dc 15} saving throw.';
      final result = transformer.transformEntries(input);

      expect(
          result.markdown,
          equals(
              'Deals **`2d6 fire`** damage and forces a DC 15 saving throw.'));
    ...
    test('extracts entity references for spells, items, monsters, and feats',
        () {
      const input =
          'Cast {@spell Fireball} or equip {@item Longsword} against a {@creature Goblin} with {@feat Alert}.';
      final result = transformer.transformEntries(input);

      expect(result.markdown, contains('[Fireball](ref://spell/fireball)'));
      expect(
          result.markdown, contains('[Longsword](ref://equipment/longsword)'));
      expect(result.markdown, contains('[Goblin](ref://monster/goblin)'));
      expect(result.markdown, contains('[Alert](ref://feat/alert)'));

      expect(
          result.extractedRefs.any(
              (r) => r.slug == 'fireball' && r.refType == EntityType.spell),
          isTrue);
      expect(
          result.extractedRefs.any((r) =>
              r.slug == 'longsword' && r.refType == EntityType.equipment),
          isTrue);
      expect(
          result.extractedRefs.any(
              (r) => r.slug == 'goblin' && r.refType == EntityType.monster),
          isTrue);
      expect(
          result.extractedRefs
              .any((r) => r.slug == 'alert' && r.refType == EntityType.feat),
          isTrue);
    });
    ...
    test('renders abilityGeneric / abilityScore node cleanly', () {
      ...
      expect(result.markdown,
          contains('**Ability Score Improvement.** STR +2, CON +1'));
    });
  });
}
