...
void main() {
  ...
  group('Homebrew Bundle Import/Export & Deduplication Tests', () {
    test('round-trip serialization of HomebrewBundle with all categories', () {
      ...
      expect(reloaded.spells.first.customProperties['flavor'],
          equals('Chilling effect'));
      expect(reloaded.monsters.length, equals(1));
      expect(reloaded.monsters.first.name, equals('Void Crawler'));
      expect(reloaded.monsters.first.customProperties['habitat'],
          equals('The Void'));
    });

    test(
        'HomebrewMergeResolver correctly classifies Novel, Identical, and Collision entities',
        () {
      ...
      const incomingConflictingSpell = Spell(
        ...
        castingTime: CastingTime(
            cost: 1, actionType: ActionType.bonusAction), // Changed action
        duration: SpellDuration(type: DurationType.timed, durationSeconds: 60),
        range: '30 ft.',
        components: SpellComponents(v: true, s: true),
      ...
      expect(
          result1.spells[1].disposition, equals(ImportDisposition.identical));
      ...
      expect(
          result2.spells[0].disposition, equals(ImportDisposition.collision));
      expect(result2.spells[0].diffSummary, contains('level'));
      expect(result2.spells[0].isSelected, isTrue);
    });

    test('importResolvedBundle correctly handles Overwrite vs Duplicate Rename',
        () async {
      final persistence = HomebrewPersistenceService();

      const existingSpell = Spell(
        id: EntityId(
            slug: 'shadow-blade-custom', ruleset: RulesetVersion.homebrew),
      ...
      expect(
          allSpells
              .any((s) => s.id.slug == 'shadow-blade-custom' && s.level == 2),
          isTrue);
      expect(
          allSpells.any(
              (s) => s.id.slug == 'shadow-blade-custom-copy' && s.level == 3),
          isTrue);
    });

    test(
        'applyResolutionToAllCollisions updates all colliding entities across categories',
        () {
      const existingSpell = Spell(
      ...
      expect(analysis.spells.first.resolution,
          equals(CollisionResolution.overwrite));
      expect(analysis.monsters.first.resolution,
          equals(CollisionResolution.overwrite));

      // Apply bulk resolution
      analysis.applyResolutionToAllCollisions(CollisionResolution.keepLocal);
      expect(analysis.spells.first.resolution,
          equals(CollisionResolution.keepLocal));
      expect(analysis.monsters.first.resolution,
          equals(CollisionResolution.keepLocal));

      analysis
          .applyResolutionToAllCollisions(CollisionResolution.duplicateRename);
      expect(analysis.spells.first.resolution,
          equals(CollisionResolution.duplicateRename));
      expect(analysis.monsters.first.resolution,
          equals(CollisionResolution.duplicateRename));
    });

    test('importResolvedBundle reports onProgress for each imported entity',
        () async {
      final persistence = HomebrewPersistenceService();
      ...
      const spell1 = Spell(
        id: EntityId(
            slug: 'progress-spell-1', ruleset: RulesetVersion.homebrew),
      ...
      const spell2 = Spell(
        id: EntityId(
            slug: 'progress-spell-2', ruleset: RulesetVersion.homebrew),
      ...
      const monster1 = Monster(
        id: EntityId(
            slug: 'progress-monster-1', ruleset: RulesetVersion.homebrew),
