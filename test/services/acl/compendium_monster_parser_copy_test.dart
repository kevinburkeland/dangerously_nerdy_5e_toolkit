...
void main() {
  group('CompendiumMonsterParser Copy Resolution & Stats Tests', () {
    ...
    test(
        'preserves ability scores, speed, defenses, and senses in customProperties',
        () {
      final raw = {
        'name': 'Ironfang Beast',
        'source': 'HOMEBREW',
        ...
        'action': [
          {
            ...
            'entries': [
              '{@atk mw} {@hit 7} to hit. {@h}15 ({@damage 2d10 + 4|piercing}) piercing damage.'
            ],
          }
        ],
      };
      ...
      expect(
          monster.customProperties['speed'], equals('walk 40ft., swim 30ft.'));
      ...
      expect(
          monster.customProperties['savingThrows'], equals('CON +6, WIS +4'));
      expect(monster.customProperties['skills'],
          equals('perception +4, stealth +5'));
      expect(
          monster.customProperties['damageResistances'], equals('cold, fire'));
      expect(monster.customProperties['damageImmunities'], equals('poison'));
      expect(
          monster.customProperties['conditionImmunities'], equals('poisoned'));
      expect(monster.customProperties['senses'],
          equals('darkvision 60 ft., passive Perception 14'));
      expect(monster.customProperties['languages'],
          equals('understands Draconic but cannot speak'));
      ...
      expect(
          statBlock.senses, equals('darkvision 60 ft., passive Perception 14'));
    });
    ...
    test('resolves monster using _copy against SRD base monster with _mod', () {
          ...
          '_mod': {
            ...
            'trait': {
              ...
              'items': {
                'name': 'Spider Climb',
                'entries': [
                  'The statue can climb difficult surfaces without needing an ability check.'
                ],
              },
            },
          },
      ...
      expect(monster.customProperties['languages'],
          equals('understands Abyssal but cannot speak'));
      ...
      expect(
          sb.traits.any((t) =>
              t.name.contains('Spider Climb') ||
              t.description.contains('Spider Climb')),
          isTrue);
    });

    test(
        'revitalizes _copy creature from backup bundle where actions were empty',
        () {
      // Simulates creature from ~/homebrew.json where actionsMarkdown was only table and stats were 10
      const backupMonster = Monster(
        id: EntityId(
            slug: 'animated-spider-idol', ruleset: RulesetVersion.v2014),
        name: 'Animated Spider Idol',
        size: 'Large',
