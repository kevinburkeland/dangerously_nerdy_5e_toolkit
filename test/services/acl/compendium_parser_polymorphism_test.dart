...
void main() {
  group('Compendium Ingestion & SRD Deduplication Polymorphism Tests', () {
    ...
    test(
        'ingests mixed bundle and accurately quarantines all canonical SRD entries',
        () {
      const mixedJson = '''
{
  "spell": [
...
''';
      ...
      // Check Spells
      final fireball =
          analysis.spells.firstWhere((s) => s.displayName == 'Fireball');
      expect(fireball.isSrdCanon, isTrue);
      expect(fireball.isSelected, isFalse);

      final voidLance =
          analysis.spells.firstWhere((s) => s.displayName == 'Void Lance');
      expect(voidLance.isSrdCanon, isFalse);
      expect(voidLance.isSelected, isTrue);
      expect(
          voidLance.incomingEntity.damageMath.first.diceFormula, equals('6d8'));

      // Check Monsters
      final goblin =
          analysis.monsters.firstWhere((m) => m.displayName == 'Goblin');
      expect(goblin.isSrdCanon, isTrue);
      expect(goblin.isSelected, isFalse);

      final voidAbom = analysis.monsters
          .firstWhere((m) => m.displayName == 'Void Abomination');
      ...
      // Check Items
      final potion = analysis.items
          .firstWhere((i) => i.displayName == 'Potion of Healing');
      expect(potion.isSrdCanon, isTrue);
      expect(potion.isSelected, isFalse);

      final blade = analysis.items
          .firstWhere((i) => i.displayName == 'Blade of the Nether');
      expect(blade.isSrdCanon, isFalse);
      expect(blade.isSelected, isTrue);
      expect(blade.incomingEntity.rarity, equals('Very Rare'));
      expect(blade.incomingEntity.descriptionMarkdown,
          contains('necrotic whispers'));
      ...
      final bloodMage =
          analysis.classes.firstWhere((c) => c.displayName == 'Blood Mage');
      expect(bloodMage.isSrdCanon, isFalse);
      expect(bloodMage.isSelected, isTrue);
      expect(
          bloodMage.incomingEntity.featuresMarkdown, contains('Blood Siphon'));
    });
  });
}
