...
void main() {
  ...
  group('Imported Character & Monster Optimal Attack Pattern Tests', () {
    test(
        'extracts optimal multiattack branch when creature has multiple weapon options',
        () {
      ...
      const statBlock = MinionStatBlock(
        ...
        actions: [
          ...
          CreatureAction(
            name: 'Shield Bash',
            description:
                'Melee Weapon Attack: +7 to hit, reach 5 ft., one creature. Hit: 9 (2d4 + 4) bludgeoning damage.',
            attackBonus: 7,
            hitDamage: '2d4 + 4',
          ),
          CreatureAction(
      ...
      final scimitar =
          attacks.firstWhere((a) => a.name.toLowerCase().contains('scimitar'));
      final shieldBash = attacks
          .firstWhere((a) => a.name.toLowerCase().contains('shield bash'));
      final javelin =
          attacks.firstWhere((a) => a.name.toLowerCase().contains('javelin'));
    ...
    test(
        'picks the highest DPR attack when an imported character has multiple single-attack weapons',
        () {
      ...
      final attacks = statBlock.extractDprAttacks();
      final dagger =
          attacks.firstWhere((a) => a.name.toLowerCase().contains('dagger'));
      final greatsword = attacks
          .firstWhere((a) => a.name.toLowerCase().contains('greatsword'));
    ...
    test(
        'DprCombatantProfile.fromMonsterItem accurately extracts multiattacks and riders',
        () {
        ...
        statBlock: const MinionStatBlock(
          ...
          actions: [
            CreatureAction(
              name: 'Multiattack',
              description:
                  'The wyvern makes two attacks: one with its bite and one with its stinger. While flying, it can use its claws in place of its bite.',
            ),
            CreatureAction(
              name: 'Bite',
              description:
            ...
            CreatureAction(
              name: 'Stinger',
              description:
                  'Melee Weapon Attack: +7 to hit, reach 10 ft., one creature. Hit: 11 (2d6 + 4) piercing damage plus 24 (7d6) poison damage.',
              attackBonus: 7,
              hitDamage: '2d6 + 4',
            ),
          ],
      ...
      final bite = profile.attacks
          .firstWhere((a) => a.name.toLowerCase().contains('bite'));
      final stinger = profile.attacks
          .firstWhere((a) => a.name.toLowerCase().contains('stinger'));
    ...
    test(
        'compendium JSON ingestion parses actions, bonus actions, and multiattacks',
        () {
      const monsterJson = '''
{
  "monster": [
...
''';
      ...
      final result =
          CompendiumJsonIngestionPipeline().ingestJsonString(monsterJson);
      ...
      final attacks =
          item.getStatBlock(DmRulesEdition.v2024).extractDprAttacks();
      final longsword =
          attacks.firstWhere((a) => a.name.toLowerCase().contains('longsword'));
