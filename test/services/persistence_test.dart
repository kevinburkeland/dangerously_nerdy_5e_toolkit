...
void main() {
  ...
  group('StorageMigrationService Tests', () {
    ...
    test('Migrates legacy v1 integer enum indices to v2 string names',
        () async {
      SharedPreferences.setMockInitialValues({
  ...
  group('SettingsProvider Pre-Hydration Tests', () {
    test('hydrateFromPrefs restores all settings correctly from v2 string keys',
        () async {
      SharedPreferences.setMockInitialValues({
      ...
      expect(
          settings.pinnedRuleIds, containsAll(['rule_grapple', 'rule_shove']));
    });

    test('hydrateFromPrefs gracefully falls back on corrupt or missing values',
        () async {
      SharedPreferences.setMockInitialValues({
  ...
  group('DebouncedStorageService Tests', () {
    ...
    test('Batches multiple rapid writes and executes only the last one',
        () async {
  ...
  group('AppBackupService Tests', () {
    ...
    test('Full export and restore preserves data integrity', () async {
      ...
      final backupJson =
          await AppBackupService().exportFullBackupJson(initialSettings);
      expect(
          backupJson,
          contains(
              '"schemaVersion": ${AppBackupService.currentSchemaVersion}'));
      expect(backupJson, contains('"arcaneSapphire"'));

      final restoreResult =
          await AppBackupService().importFullBackupJson(backupJson);
      expect(restoreResult.success, isTrue);
    });

    test('Gracefully rejects corrupted JSON in backup import', () async {
      final restoreResult =
          await AppBackupService().importFullBackupJson('invalid { [ json');
  ...
  group('AppServices Dependency Injection Container Tests', () {
    ...
    test('AppServices provides default singletons and supports overrides', () {
      ...
      final customDebouncer =
          DebouncedStorageService.custom(logger: customLogger);
