...
void main() {
  ...
  group('Homebrew Creature Cross-Toolkit Integration Tests', () {
    ...
    test(
        'converts domain Monster into MinionStatBlock and MonsterItem accurately',
        () {
    ...
    test(
        'parses ### markdown headers without colon into named traits and actions',
        () {
...
''',
      ...
      expect(statBlock.traits[0].description,
          contains('advantage on an attack roll'));
      expect(statBlock.traits[1].name, equals('Keen Smell'));
      expect(statBlock.traits[1].description,
          contains('Wisdom (Perception) checks'));
      ...
      expect(statBlock.actions[0].hitDamage,
          contains('8 (1d8 + 3) piercing damage'));
    ...
    test('HomebrewPersistenceService syncs to MonsterCodexLibrary dynamically',
        () async {
      ...
      expect(
          MonsterCodexLibrary.getMonsterByName('Dire Timber Wolf'), isNotNull);
    ...
    test('Conjure Animals dynamically includes homebrew beasts with CR <= 2',
        () async {
      final service = HomebrewPersistenceService();
      await service.saveCustomMonster(customBeast);
      await service.saveCustomMonster(customDragon);
      ...
      expect(
        effectiveCreatures.any((sb) =>
            sb.id == 'dire-timber-wolf' && sb.name == 'Dire Timber Wolf'),
        isTrue,
      );
    ...
    testWidgets(
        'Monster Codex Screen displays Homebrew creatures with HOMEBREW badge and filter',
        (tester) async {
      ...
      expect(
          find.widgetWithText(MonsterCard, 'Dire Timber Wolf'), findsOneWidget);
    ...
    testWidgets(
        'Arena Monster Picker Sheet lists Homebrew creatures for simulation',
        (tester) async {
