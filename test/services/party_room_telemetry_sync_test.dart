...
void main() {
  ...
  group('Party Room Telemetry Sync & Payload Compression', () {
    test(
        'telemetry packet size is < 1.5 KB (down from ~30 KB legacy sheet) with zero markdown',
        () {
      ...
      expect(legacyByteSize, greaterThan(20000),
          reason: 'Legacy character sheet should be heavy');
      ...
      expect(telemetryByteSize, lessThan(1500),
          reason:
              'Telemetry packet must be compact (< 1.5 KB). Actual: $telemetryByteSize bytes');
    ...
    test(
        'PartySessionState serializes and deserializes partyTelemetry seamlessly',
        () {
      ...
      final session = PartySessionState(
        ...
        hostKeyHash: 'hash123',
        characterRoster: const ['Thorek', 'Lyra'],
        partyTelemetry: {
          'Thorek': telemetry1,
          'Lyra': telemetry2,
        },
      ...
      expect(restored.partyTelemetry['Thorek']?.speciesSlug, 'dwarf');
      expect(restored.partyTelemetry['Thorek']?.currentHp, 28);
      expect(restored.partyTelemetry['Lyra']?.speciesSlug, 'elf');
      expect(restored.partyTelemetry['Lyra']?.currentHp, 18);
    });

    test(
        'PartySessionState hydrates partyTelemetry from legacy sharedCharacters if incoming from older client',
        () {
