...
void main() {
  ...
  group('Homebrew Complete Bundle Exporter Tests', () {
    ...
    test(
        'exportBundle serializes all active registries into comprehensive sibling arrays',
        () async {
      // 1. Seed Custom Spell
      const spell = Spell(
      ...
      final race = Race(
        id: const EntityId(
            slug: 'crystallid', ruleset: RulesetVersion.homebrew),
        name: 'Crystallid',
        size: 'Medium',
        speed: '30 ft.',
        ...
        subraces: const [
          Subrace(
            id: EntityId(
                slug: 'prismatic-crystallid', ruleset: RulesetVersion.homebrew),
      ...
      const background = Background(
        id: EntityId(
            slug: 'planar-cartographer', ruleset: RulesetVersion.homebrew),
        name: 'Planar Cartographer',
        descriptionMarkdown:
            'Planar Navigation: You can intuitively navigate the outer planes.',
      );
      ...
      expect(exported['description'],
          equals('Complete homebrew collection of planar entities.'));
      ...
      expect(
          (exported['spells'] as List).first['name'], equals('Astral Pulse'));

      expect(exported['monsters'], isA<List>());
      expect((exported['monsters'] as List).length, equals(1));
      expect(
          (exported['monsters'] as List).first['name'], equals('Void Lurker'));

      expect(exported['items'], isA<List>());
      expect((exported['items'] as List).length, equals(1));
      expect(
          (exported['items'] as List).first['name'], equals('Blade of Echoes'));
      ...
      expect((exported['subclasses'] as List).first['name'],
          equals('Iron Warden'));
      ...
      expect((exported['subraces'] as List).first['name'],
          equals('Prismatic Crystallid'));
      ...
      expect((exported['backgrounds'] as List).first['name'],
          equals('Planar Cartographer'));
    });
    ...
    test('exportHomebrewBundle populates subraces in portable bundle object',
        () async {
      final race = Race(
        ...
        subraces: const [
          Subrace(
            id: EntityId(
                slug: 'solar-starborn', ruleset: RulesetVersion.homebrew),
            name: 'Solar Starborn',
            raceSlug: 'starborn',
      ...
      final bundle =
          await persistence.exportHomebrewBundle(bundleName: 'Starborn Pack');
