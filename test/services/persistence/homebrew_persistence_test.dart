...
void main() {
  ...
  group('HomebrewPersistenceService Tests', () {
    ...
    setUp(() {
      ...
      persistence.invalidateAllCaches();
    });
    ...
    test('saves, loads, and deletes custom spells', () async {
      const spell = Spell(
        ...
        damageMath: [
          EvaluationMath(diceFormula: '6d8', damageType: DamageType.necrotic)
        ],
      );
    ...
    test('hydrates LayeredPriorityRepository with saved homebrew entities',
        () async {
      const spell = Spell(
        id: EntityId(
            slug: 'chaotic-blast-homebrew', ruleset: RulesetVersion.homebrew),
    ...
    test('batch deletes custom entities by slug list and cleans up storage',
        () async {
      const spell1 = Spell(
    ...
    test(
        'clearHomebrewCategory purges specific category from AppDatabaseService and updates runtime library',
        () async {
      const monster = Monster(
      ...
      expect(
          AppDatabaseService.instance
              .get(AppDatabaseService.boxHomebrew, 'dn_homebrew_monsters_v1'),
          isNull);
      expect(MonsterCodexLibrary.homebrewMonsters, isEmpty);
    });

    test(
        'reparseAllHomebrew upgrades entities from raw JSON and removes exact SRD matches',
        () async {
      ...
      await persistence.saveCustomFeat(
        ...
        rawPayload: {
          ...
          'category': 'General',
          'entries': [
            'Advantage on attack rolls against creatures you grapple.'
          ],
        },
      );
    ...
    test(
      ...
      expect(
          MonsterCodexLibrary.allMonsters
              .any((m) => m.name == 'Abyssal Stalker'),
          isTrue);
      expect(SpellbookLibrary.allSpells.any((s) => s.name == 'Abyssal Chains'),
          isTrue);
    });
    ...
    test(
      ...
      await persistence
          .saveHomebrewEntitiesBatch([spellEntity], syncLibraries: false);
      ...
      expect(SpellbookLibrary.allSpells.any((s) => s.name == 'Cave Curse'),
          isFalse);
    ...
    test(
      ...
      await persistence
          .saveHomebrewEntitiesBatch([humanoidMonster, nestedTypeMonster]);
    ...
    test(
          ...
          'trait': [
            {
              'name': 'Nimble',
              'entries': [
                'Onyx can move through the space of any hostile creature.'
              ],
            }
          ],
          '_copy': {
      ...
      await persistence.saveHomebrewEntitiesBatch([bepisEntity, onyxEntity]);

      final monsters = await persistence.loadCustomMonsters();
      expect(monsters.any((m) => m.name == 'Bepis Honeymaker'), isTrue);
      expect(monsters.any((m) => m.name == 'Onyx'), isTrue);

      final races = await persistence.loadCustomRaces();
      expect(races.any((r) => r.name == 'Bepis Honeymaker'), isFalse);
      expect(races.any((r) => r.name == 'Onyx'), isFalse);
    });

    test('saves, loads, and exports custom subraces and fluff', () async {
      const subrace = Subrace(
        id: EntityId(
            slug: 'astral-elf-variant', ruleset: RulesetVersion.homebrew),
      ...
      expect(
          SrdSpeciesLibrary.customSubraces
              .any((s) => s.id.slug == 'astral-elf-variant'),
          isTrue);
      ...
      expect(bundle.subraces.any((s) => s.id.slug == 'astral-elf-variant'),
          isTrue);
      ...
      expect(
          subList.any((s) => s['id']['slug'] == 'astral-elf-variant'), isTrue);
      expect(fluffList.any((f) => f['slug'] == 'abyssal-stalker'), isTrue);
    });

    test(
        'reparseAllHomebrew preserves custom items even when synced to MagicItemLibrary',
        () async {
      ...
      await persistence.syncToLibraries();
      expect(MagicItemLibrary.allItems.any((i) => i.name == 'Chrono Dagger'),
          isTrue);
    ...
    test(
        'reparse is idempotent: repeated reparse cycles produce identical entity hashes and payload parity',
        () async {
      ...
      final predefinedEntities = [
        ...
        const HomebrewEntity(
          ...
          rawPayload: {
            ...
            'raceName': 'human',
            'entries': [
              'Humans bearing the celestial mark of the astral sphere.'
            ],
          },
        ),
        const HomebrewEntity(
          ...
          rawPayload: {
            'name': 'Void Touched',
            'entries': [
              'You have stared into the void and gained resistance to psychic damage.'
            ],
          },
        ),
        const HomebrewEntity(
          ...
          rawPayload: {
            'name': 'Void Hermit',
            'entries': [
              'You spent years meditating at the edge of planar rifts.'
            ],
          },
        ),
        const HomebrewEntity(
      ...
      await persistence.saveHomebrewEntitiesBatch(predefinedEntities,
          syncLibraries: true);
