...
void main() {
  ...
  group('Homebrew Cross-Tool Propagation Tests', () {
    test('imported custom table propagates to SrdTablesLibrary and is rollable', () async {
      ...
      expect(allTables.any((t) => t.id == 'wandering-celestial-encounters'), isTrue);

      final foundTable = SrdTablesLibrary.getTableById('wandering-celestial-encounters');
      ...
      final customSearchResults = SrdTablesLibrary.search('', category: TableCategory.custom);
      expect(customSearchResults.any((t) => t.id == 'wandering-celestial-encounters'), isTrue);
    });

    test('imported deity, trap, hazard, and condition propagate to DmScreenLibrary', () async {
      final service = HomebrewPersistenceService();

      const deityEntry = HomebrewCompendiumEntry(
        id: EntityId(slug: 'solas-the-dawnbringer', ruleset: RulesetVersion.v2014),
        name: 'Solas the Dawnbringer',
        category: 'Deity',
        descriptionMarkdown: '**Pantheon:** Solar Expanse | **Domains:** Light, Life\n\nSolas embodies the unyielding light of truth.',
        customProperties: {'pantheon': 'Solar Expanse'},
      );

      const trapEntry = HomebrewCompendiumEntry(
        id: EntityId(slug: 'obsidian-scythe-pendulum', ruleset: RulesetVersion.v2014),
        name: 'Obsidian Scythe Pendulum',
        category: 'Trap',
        descriptionMarkdown: 'A razor-sharp obsidian scythe swings from the vaulted ceiling.',
      ...
      const conditionEntry = HomebrewCompendiumEntry(
        ...
        name: 'Sand Rot',
        category: 'Disease',
        descriptionMarkdown: 'A desiccating affliction that causes joints to calcify and crack.',
        customProperties: {},
      );
      ...
      expect(allRefItems.any((i) => i.id == 'obsidian-scythe-pendulum'), isTrue);
      expect(allRefItems.any((i) => i.id == 'sand-rot'), isTrue);

      final foundDeity = allRefItems.firstWhere((i) => i.id == 'solas-the-dawnbringer');
      expect(foundDeity.category, equals(DmCategory.exploration));
      expect(foundDeity.subCategory, equals('Pantheon & Deities'));

      final foundTrap = allRefItems.firstWhere((i) => i.id == 'obsidian-scythe-pendulum');
    ...
    test('imported character options propagate to SrdFeatureOptions', () async {
      ...
      const maneuver = HomebrewCompendiumEntry(
        ...
        category: 'Maneuver',
        descriptionMarkdown: 'When you hit with a melee weapon, spend a superiority die to strike all adjacent foes.',
        customProperties: {'featureType': 'MV'},
      );

      await service.saveCustomOtherEntry(maneuver);

      expect(SrdFeatureOptions.customCharacterOptions.any((o) => o.id == 'whirlwind-cleave'), isTrue);
      expect(SrdFeatureOptions.allOptions.any((o) => o.id == 'whirlwind-cleave'), isTrue);
    });

    test('granular clearing prunes only selected category from runtime libraries', () async {
      final service = HomebrewPersistenceService();

      const table = HomebrewCompendiumEntry(
        id: EntityId(slug: 'desert-hazards-table', ruleset: RulesetVersion.v2014),
        name: 'Desert Hazards Table',
        category: 'Table',
        descriptionMarkdown: '| d6 | Hazard |\n| :--- | :--- |\n| 1 | Quicksand |',
        customProperties: {'rows': [['1', 'Quicksand']]},
      );

      const deity = HomebrewCompendiumEntry(
        id: EntityId(slug: 'aethelgard-the-iron-watcher', ruleset: RulesetVersion.v2014),
      ...
      expect(SrdTablesLibrary.allTables.any((t) => t.id == 'desert-hazards-table'), isTrue);
      expect(DmScreenLibrary.allItems.any((i) => i.id == 'aethelgard-the-iron-watcher'), isTrue);

      // Prune only tables
      await service.clearOtherEntriesByCategories({HomebrewOtherCategory.tables});

      expect(SrdTablesLibrary.allTables.any((t) => t.id == 'desert-hazards-table'), isFalse);
      expect(DmScreenLibrary.allItems.any((i) => i.id == 'aethelgard-the-iron-watcher'), isTrue);
    });

    testWidgets('TableIndexScreen renders custom category chip and imported homebrew table', (tester) async {
      final service = HomebrewPersistenceService();

      const table = HomebrewCompendiumEntry(
        id: EntityId(slug: 'planar-anomalies-table', ruleset: RulesetVersion.v2014),
    ...
    testWidgets('RulesCompendiumScreen searches and displays custom imported DM reference item', (tester) async {
      final service = HomebrewPersistenceService();
