...
void main() {
  ...
  group('DmBackupService', () {
    ...
    test(
        'exports and restores full system master backup with all potential homebrew categories',
        () async {
      // 1. Seed data across all categories
      final profile = CampaignProfile.defaultProfile(
          name: 'Dragonfire', nodeId: 'test_node');
      ...
      final restoreSuccess =
          await backupService.restoreFullSystemSnapshot(fullBackupJson);
      ...
      expect(
          restoredItems.any((i) => i.id.slug == 'blade-of-eternity'), isTrue);
      ...
      final restoredRace =
          restoredRaces.firstWhere((r) => r.id.slug == 'human-custom');
      expect(restoredRace.subraces.any((s) => s.id.slug == 'astral-human'),
          isTrue);

      final restoredFeats = await homebrewService.loadCustomFeats();
      expect(
          restoredFeats.any((f) => f.id.slug == 'temporal-reflexes'), isTrue);
