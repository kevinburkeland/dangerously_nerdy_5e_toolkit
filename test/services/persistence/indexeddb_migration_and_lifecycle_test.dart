...
void main() {
  group('IndexedDB / Database Persistence & Web Lifecycle Tests', () {
    ...
    test(
        'AppDatabaseService stores, retrieves, and deletes values with in-memory fallback',
        () async {
      final db = AppDatabaseService.instance;
      await db
          .put(AppDatabaseService.boxCharacters, 'test_key', {'name': 'Grog'});
    ...
    test(
      ...
      expect(updatedRoster.map((c) => c.id.slug),
          containsAll(['legacy-hero', 'new-mage']));
    ...
    test(
        'HomebrewPersistenceService stores and loads custom spells seamlessly in database',
        () async {
      final homebrewService = HomebrewPersistenceService();
      const customSpell = Spell(
        id: EntityId(
            slug: 'hellish-rebuke-plus', ruleset: RulesetVersion.homebrew),
      ...
      final rawPayloads =
          await homebrewService.loadRawPayloads(EntityType.spell);
    ...
    test(
        'DebouncedStorageService.flushAllSync executes pending tasks synchronously before runtime death',
        () async {
