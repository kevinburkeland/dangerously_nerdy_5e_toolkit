...
void main() {
  ...
  group('Homebrew Ingestion & Batch Performance Tests', () {
    ...
    test(
        'saveCustomSpellsBatch merges into existing spells with O(1) lookup and batch raw payloads',
        () async {
      ...
      final existingSpells = List<Spell>.generate(
        ...
        (i) => Spell(
          id: EntityId(
              slug: 'existing-spell-$i', ruleset: RulesetVersion.homebrew),
      ...
      final incomingSpells = <Spell>[
        ...
        for (int i = 10; i < 25; i++)
          Spell(
            id: EntityId(
                slug: 'new-spell-$i', ruleset: RulesetVersion.homebrew),
            name: 'New Spell $i',
            level: 3,
      ...
      final rawPayloads = [
        for (final s in incomingSpells)
          {'name': s.name, 'slug': s.id.slug, 'source': 'HB'},
      ];

      await persistence.saveCustomSpellsBatch(incomingSpells,
          rawPayloads: rawPayloads);

      final result = await persistence.loadCustomSpells();
      expect(result.length,
          equals(25)); // 10 original - 5 updated + 15 new + 5 kept = 25 total

      final updatedZero =
          result.firstWhere((s) => s.id.slug == 'existing-spell-0');
      expect(updatedZero.name, equals('Updated Spell 0'));
      expect(updatedZero.level, equals(2));

      final keptNine =
          result.firstWhere((s) => s.id.slug == 'existing-spell-9');
    ...
    test(
        'saveCustomOtherEntriesBatch saves generic entries and updates invocations',
        () async {
      final entries = [
        const HomebrewCompendiumEntry(
          id: EntityId(
              slug: 'hb-eldritch-sight-extra',
    ...
    test(
        'importResolvedBundle processes categories in-memory with single disk transaction and progress ticks',
        () async {
      ...
      final renamedItem =
          savedItems.firstWhere((i) => i.id.slug == 'item-1-copy');
