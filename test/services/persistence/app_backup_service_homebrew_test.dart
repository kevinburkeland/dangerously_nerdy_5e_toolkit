...
void main() {
  ...
  group('AppBackupService Homebrew & Compendium Integration Tests', () {
    ...
    test(
        'exports full backup containing homebrew spells, monsters, items, and races with subraces',
        () async {
      ...
      const spell = Spell(
        ...
        damageMath: [
          EvaluationMath(diceFormula: '3d10', damageType: DamageType.fire)
        ],
      );
      ...
      const monster = Monster(
        id: EntityId(
            slug: 'hell-hound-alpha', ruleset: RulesetVersion.homebrew),
      ...
      const item = EquipmentItem(
        id: EntityId(
            slug: 'flame-tongue-greatsword', ruleset: RulesetVersion.homebrew),
      ...
      final race = Race(
        id: const EntityId(
            slug: 'dwarf-variant', ruleset: RulesetVersion.homebrew),
        name: 'Dwarf Variant',
      ...
      final backupJson =
          await backupService.exportFullBackupJson(const AppSettings());
      ...
      expect(
          decoded['customItems'][0]['name'], equals('Flame Tongue Greatsword'));
      expect(decoded['customRaces'][0]['name'], equals('Dwarf Variant'));
      expect(decoded['customSubraces'][0]['name'], equals('Volcanic Dwarf'));
    });

    test('imports full backup restoring all homebrew entities to storage',
        () async {
      final sampleBackup = {
        'schemaVersion': 3,
      ...
      final result =
          await backupService.importFullBackupJson(json.encode(sampleBackup));
