...
void main() {
  group('Homebrew Box Formatting & Cleanup Tests', () {
    test('CompendiumJsonIngestionPipeline.cleanRawTags cleans tags and nested tags', () {
      const input = 'A cult of {@creature Belashyrra|ERLW} serves a {@creature beholder} messiah. {@note Cast {@spell contagion|phb} now.}';
      final cleaned = CompendiumJsonIngestionPipeline.cleanRawTags(input);

      expect(cleaned, isNot(contains('{@')));
      expect(cleaned, contains('A cult of Belashyrra serves a beholder messiah.'));
      expect(cleaned, contains('> **Note:** Cast contagion now.'));
    });

    test('CompendiumGenericEntryParser cleans tags inside customProperties.rows and colLabels', () {
      ...
      final raw = {
        ...
        'rows': [
          [1, 'A cult of {@creature Belashyrra|ERLW} murders innocents.'],
          [2, 'A {@creature mind flayer} kidnaps commoners ({@creature commoner}).'],
        ],
      };
      ...
      expect(entry.descriptionMarkdown, contains('Belashyrra'));
      expect(entry.descriptionMarkdown, contains('mind flayer'));

      final rows = entry.customProperties['rows'] as List;
      final row0 = rows[0] as List;
      expect(row0[1], isNot(contains('{@')));
      expect(row0[1], contains('Belashyrra'));

      final row1 = rows[1] as List;
      expect(row1[1], isNot(contains('{@')));
      expect(row1[1], contains('mind flayer'));
      expect(row1[1], contains('commoner'));
    });

    test('compendiumEntryToRollableTable produces clean labels without raw tags or markdown links', () {
      const entry = HomebrewCompendiumEntry(
        ...
        customProperties: {
          ...
          'rows': [
            ['1-50', 'A cult of {@creature Belashyrra|ERLW} gathers.'],
            ['51-100', 'Meet a [Noble](ref://monster/noble) at night.'],
          ],
        },
      );

      final table = HomebrewPersistenceService.compendiumEntryToRollableTable(entry);
      expect(table.entries.length, 2);
      expect(table.entries[0].label, 'A cult of Belashyrra gathers.');
    ...
    test('CompendiumJsonIngestionPipeline rejects and filters corrupted 100k-character entries', () {
      final pipeline = CompendiumJsonIngestionPipeline();
      final giantCorruptedName = '[{name: Dragonborn, source: XGE, page: 175, tables: ' * 500;
      final malformedMap = {
    ...
    test('EntryNodeTransformer handles table cells that are Lists or AST Maps', () {
      ...
      final node = {
        ...
        'rows': [
          [
            {'type': 'cell', 'roll': {'min': 1, 'max': 50}},
            ['Found ', {'type': 'cell', 'entry': '{@item potion of healing|phb}'}]
          ]
        ]
      };
