...
void main() {
  group('SRD Spells Conformance & Baseline Ingestion Tests', () {
    ...
    SpellComponents parseComponents(
        String compStr, SpellMaterialComponent? mat) {
    ...
    test('ingests all SRD spells into 2014 & 2024 baseline repository layers',
      ...
      for (final item in SpellbookLibrary.allSpells) {
        layer2014
            .registerEntity(convertToDomainSpell(item, DmRulesEdition.v2014));
        layer2024
            .registerEntity(convertToDomainSpell(item, DmRulesEdition.v2024));
      }
      ...
      expect(
          layer2014.getAll().length, equals(SpellbookLibrary.allSpells.length));
      expect(
          layer2024.getAll().length, equals(SpellbookLibrary.allSpells.length));
    });

    test(
        'dynamically resolves 2014 vs 2024 ruleset differences for key SRD spells',
        () {
      final layer2014 = PriorityLayer(
        layerId: 'base-srd-2014',
        name: 'SRD 2014 Ruleset',
      ...
      for (final item in SpellbookLibrary.allSpells) {
        ...
        if (item.rules2024.materialDetails != null) {
          ...
          expect(spell2024.components.m, isTrue,
              reason:
                  '${item.name} has material details but component M is false');
          if (mat.costInGp > 0) {
