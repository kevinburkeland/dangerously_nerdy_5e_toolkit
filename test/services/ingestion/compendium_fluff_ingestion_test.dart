...
void main() {
  ...
  group('Compendium Fluff Ingestion Pipeline Tests', () {
    ...
    test(
        'ingests monsterFluff bundle and additively attaches lore to SRD creatures',
        () {
      ...
      expect(abolethFluff!.loreMarkdown,
          contains('Before the coming of the gods'));
      expect(abolethFluff.loreMarkdown, contains('eternal memories'));
      expect(abolethFluff.images, contains('bestiary/MM/Aboleth.webp'));

      final dragonFluff =
          EntityFluffService().getFluff('monster', 'adult-red-dragon');
      expect(dragonFluff, isNotNull);
      expect(dragonFluff!.loreMarkdown,
          contains('most covetous of the true dragons'));
    });
    ...
    test('ingests single-entity fluff payload with artificial flags', () {
      ...
      expect(elfFluff!.loreMarkdown,
          contains('magical people of otherworldly grace'));
    });

    test(
        'ingests authentic community compendium single fluff without artificial flags',
        () {
      final pipeline = CompendiumJsonIngestionPipeline();
      ...
      final aarakocraFluff =
          EntityFluffService().getFluff('monster', 'aarakocra');
    ...
    test('ingests subclassFluff alongside classFluff from class fluff bundles',
        ...
        'classFluff': [
          {
            ...
            'source': 'HOMEBREW',
            'entries': [
              'Masters of invention, apothecaries use ingenuity and herbal crafts to unlock extraordinary healing.',
            ]
          }
        ],
        'subclassFluff': [
          {
            ...
            'source': 'HOMEBREW',
            'entries': [
              'An Alchemical Savant is an expert at combining reagents to produce mystical effects.',
            ]
          }
        ]
      };
      ...
      final apothecaryFluff =
          EntityFluffService().getFluff('class', 'apothecary');
      expect(apothecaryFluff, isNotNull);
      expect(apothecaryFluff!.loreMarkdown, contains('Masters of invention'));

      final alchemistFluff =
          EntityFluffService().getFluff('subclass', 'alchemical-savant');
    ...
    test('resolves _copy inheritance across entities in fluff bundles', () {
        ...
        'itemFluff': [
          {
            ...
            'images': ['items/BMT/FateDealersDeck.webp']
          },
          {
            'name': "+1 Fate Dealer's Deck",
            'source': 'BMT',
            '_copy': {'name': "Fate Dealer's Deck", 'source': 'BMT'}
          }
        ]
      };
      ...
      final baseDeck =
          EntityFluffService().getFluff('item', 'fate-dealer-s-deck');
      expect(baseDeck, isNotNull);
      expect(baseDeck!.loreMarkdown, contains('astral divination power'));

      final copyDeck =
          EntityFluffService().getFluff('item', '1-fate-dealer-s-deck');
    ...
    test('ingests JSON list of fluff entries via ingestJsonString', () {
      ...
      [
        {
          ...
          "images": ["bestiary/CUSTOM/VoidBehemoth.webp"]
        },
        {
          "name": "Horned Titan",
          "source": "CUSTOM",
          "entries": ["The Horned King and Prince of Beasts."],
          "images": ["bestiary/CUSTOM/HornedTitan.webp"]
        }
      ]
      ''';
      ...
      final behemoth =
          EntityFluffService().getFluff('monster', 'void-behemoth');
      expect(behemoth, isNotNull);
      expect(behemoth!.loreMarkdown, contains('silvery void'));

      final titan = EntityFluffService().getFluff('monster', 'horned-titan');
      expect(titan, isNotNull);
      expect(titan!.loreMarkdown, contains('Prince of Beasts'));
    });
    ...
    test(
      ...
      final dragonFluff =
          EntityFluffService().getFluff('monster', 'ancient-brass-dragon');
      expect(dragonFluff, isNotNull);
      expect(dragonFluff!.loreMarkdown,
          contains('love conversation with mortals'));
      expect(
          dragonFluff.images, contains('bestiary/MM/AncientBrassDragon.webp'));
    });

    test(
        'HomebrewBundle serialization and deserialization retains attached fluff',
        () {
      const sampleFluff = EntityFluff(
        entityType: 'spell',
        slug: 'moonbeam',
        loreMarkdown:
            'A silvery beam of pale light shines down in a 5-foot-radius cylinder.',
      ...
      expect(deserialized.fluff.first.images,
          contains('spells/PHB/Moonbeam.webp'));
    });
    ...
    test('cleanRawTags sanitizes style and inline italic tags in loreMarkdown',
      ...
      expect(CompendiumJsonIngestionPipeline.cleanRawTags(styleTag),
          equals('The Far Realm: Real Yet Unreal'));

      const italicTag = '{@i The Binding Stone}';
      expect(CompendiumJsonIngestionPipeline.cleanRawTags(italicTag),
          equals('*The Binding Stone*'));
      ...
      expect(result.hasErrors, isFalse);
      final fluff =
          EntityFluffService().getFluff('monster', 'far-realm-observer');
