...
void main() {
  group('Comprehensive SRD Ecosystem Conformance & Repository Stack Tests', () {
    ...
    EquipmentItem convertItem(MagicItem item, DmRulesEdition edition) {
      ...
      final name = item.getName(edition);
      final rules =
          edition == DmRulesEdition.v2024 ? item.rules2024 : item.rules2014;
    ...
    Monster convertMonster(MonsterItem item, DmRulesEdition edition) {
      final ruleset = edition == DmRulesEdition.v2024
          ? RulesetVersion.v2024
          : RulesetVersion.v2014;
      final statBlock = edition == DmRulesEdition.v2024
          ? item.statBlock2024
          : item.statBlock2014;
      final name = item.getName(edition);
      ...
      return Monster(
        ...
        actionsMarkdown: statBlock.actions
    ...
    test(
        'ingests entire SRD corpus into 2014 & 2024 baseline repository layers',
        () {
      ...
      for (final monsterItem in monstersWithSpells) {
        ...
        for (final spellRef in domainMonster.innateSpells) {
          ...
          expect(
            ...
            isTrue,
            reason:
                'Monster "${domainMonster.name}" failed to resolve spell "${spellRef.slug}"',
          );
    ...
    test(
        'verifies live campaign overrides across Spells, Monsters, and Items simultaneously',
        () {
      final layer2024 = PriorityLayer(
        layerId: 'base-srd-2024',
        name: 'SRD 2024 Ruleset',
      ...
      final customFireball = baseFireball.copyWith(
        ...
        damageMath: [
          const EvaluationMath(diceFormula: '12d6', damageType: DamageType.fire)
        ],
      );
      ...
      expect(resolver.resolveTyped(fireballRef).entity!.name,
          equals('Dragon Flame Fireball'));
      expect(resolver.resolveTyped(potionRef).entity!.name,
          equals('Greater Potion of Quick Healing'));
      expect(resolver.resolveTyped(goblinRef).entity!.name,
          equals('Armored Goblin Chieftain'));

      // Disable Campaign Layer -> Fall through to base SRD
      repository.setLayerActive('campaign-overrides', false);

      expect(resolver.resolveTyped(fireballRef).entity!.name,
          equals(baseFireball.name));
      expect(resolver.resolveTyped(potionRef).entity!.name,
          equals(basePotion.name));
      expect(resolver.resolveTyped(goblinRef).entity!.name,
          equals(baseGoblin.name));
    });
    ...
    test('verifies DM Reference Screen rules and conditions integrity', () {
      ...
      final conditionTitles =
          conditions.map((c) => c.title.toLowerCase()).toList();
