...
void main() {
  group('Homebrew JSON Remediation & Pipeline Enhancement Tests', () {
    test(
        '1. Item Parser decodes shorthand codes, compound pipes, generates fallback MD, and extracts grants',
        () {
      ...
      expect(
          rod.grants.any((g) =>
              g.payload['stat'] == 'spell_attack' && g.payload['flat'] == 2),
          isTrue);
      expect(
          rod.grants.any((g) =>
              g.payload['stat'] == 'spell_save_dc' && g.payload['flat'] == 2),
          isTrue);
      ...
      final armor = parser.parseItem({
        ...
        'resist': ['cold'],
        'entries': [
          'While holding this shield, you have resistance to cold damage.'
        ],
      });
      expect(
          armor.grants.any(
              (g) => g.type == GrantType.acFormula && g.payload['amount'] == 1),
          isTrue);
      expect(
          GrantEvaluator.evaluateResistances(armor.grants), contains('cold'));
    });

    test(
        '2. Race Parser resolves boolean fly speed, modern lineage ASIs, grants, and subrace slug resolution',
        () {
      ...
      expect(GrantEvaluator.evaluateResistances(lineage.grants),
          contains('necrotic'));
    ...
    test(
        '3. Feat Parser parses compendium ability mapping for FeatAsiExtension and extracts grants',
        () {
      ...
      expect(tenaciousCon.customProperties['statIncreaseAbility'],
          equals('constitution'));
      expect(tenaciousCon.statIncreaseAmount, equals(1));
      expect(
          tenaciousCon.selectableAbilities, equals([AbilityType.constitution]));
      expect(
          tenaciousCon.grants.any((g) =>
              g.type == GrantType.abilityScoreBoost &&
              g.payload['ability'] == 'constitution' &&
              g.payload['amount'] == 1),
          isTrue);
      ...
      expect(
          tough.grants.any((g) =>
              g.type == GrantType.hpModifier && g.payload['perLevel'] == 2),
          isTrue);
    });

    test(
        '4. Background Parser resolves _copy against SRD base backgrounds and cleans origin feats',
        () {
      ...
      expect(
          bg.grants.any((g) =>
              g.type == GrantType.bonusFeat && g.payload['feat'] == 'Alert'),
          isTrue);
    });

    test(
        '5. Generic Entry Parser cleans bracketed shorthand tags and extracts featureType',
        () {
      ...
      final invocation = parser.parseGenericEntry({
        ...
        'category': '[EI]',
        'entries': [
          'Add your Charisma modifier to the damage of eldritch blast.'
        ],
      });
    ...
    test(
        '6. Spell Parser respects explicit class metadata and infers SRD spells without non-SRD expansion index',
        () {
      final parser = CompendiumSpellParser();

      final customSpell = parser.parseSpell({
        'name': 'Custom Blade',
        'level': 0,
        'school': 'V',
      ...
      expect(customSpell.customProperties['classes'],
          containsAll(['Sorcerer', 'Warlock', 'Wizard']));

      final fireball = parser.parseSpell({
        'name': 'Fireball',
        'level': 3,
        'school': 'EV',
        'entries': ['A bright streak flashes from your pointing finger.'],
      });
      expect(fireball.customProperties['classes'],
          containsAll(['Sorcerer', 'Wizard']));
    });

    test(
        '7. CompendiumJsonIngestionPipeline revitalizes HomebrewBundle and prevents empty subrace slugs',
        () {
      ...
      expect(
          compendium.races.any((r) =>
              r.id.slug.isEmpty || r.id.slug == 'ghost-subrace-without-base'),
          isFalse);

      // Verify feats revitalization
      expect(compendium.feats.first.selectableAbilities,
          equals([AbilityType.dexterity]));

      // Verify backgrounds revitalization
      expect(compendium.backgrounds.first.originFeat, equals('Skilled'));
      expect(
          compendium.backgrounds.first.skillProficiencies, contains('Insight'));

      // Verify generic entry revitalization
      expect(compendium.otherEntries.first.category,
          equals('Eldritch Invocation'));
    });

    test(
        '8. CompendiumJsonIngestionPipeline revitalizes existing HomebrewBundle exports',
        () {
      final pipeline = CompendiumJsonIngestionPipeline();
      ...
      final bundleExportJson = {
        ...
        'feats': [
          {
            'id': {'slug': 'mind-force', 'ruleset': 'homebrew'},
            'name': 'Mind Force',
      ...
      expect(compendium.races.first.speed, equals('30 ft. (fly 30 ft.)'));
      expect(
          compendium.feats.first.selectableAbilities,
          containsAll([
    ...
    test(
        '9. _copy monster revitalization inherits base stats, extracts markdown actions, and applies replaceArr',
        () {
      final pipeline = CompendiumJsonIngestionPipeline();
...
| 10 (+0) | 10 (+0) | 10 (+0) | 10 (+0) | 10 (+0) | 10 (+0) |''',
                  ...
                  'action': [
                    {
                      ...
                      'items': {
                        'name': 'Multiattack',
                        'entries': [
                          'The troglodyte makes two attacks with its longsword.'
                        ],
                      },
                    },
                    {
                      ...
                      'items': {
                        'name': 'Longsword',
                        'entries': [
                          '{@atk mw} {@hit 4} to hit. {@h}6 ({@damage 1d8 + 2}) slashing damage.'
                        ],
                      },
                    },
                  ],
      ...
      final armored =
          result.monsters.firstWhere((m) => m.name == 'Armored Troglodyte');
      ...
      expect(armored.actionsMarkdown,
          contains('Chameleon Skin')); // Inherited trait
      expect(armored.actionsMarkdown, contains('Longsword')); // Replaced action
      expect(armored.actionsMarkdown,
          contains('two attacks with its longsword')); // Replaced Multiattack
    });

    test(
        '10. EntryNodeTransformer and Monster Parser infer damageType from trailing text and action context',
        () {
      final transformer = EntryNodeTransformer();
      const text =
          'Bite. {@atk mw} {@hit 5} to hit. {@h}5 ({@damage 1d4 + 3}) bludgeoning damage plus 2 ({@damage 1d4}) fire damage.';
      final result = transformer.transformEntries(text);

      expect(result.extractedMath.length, equals(2));
      expect(
          result.extractedMath[0].damageType, equals(DamageType.bludgeoning));
    ...
    test(
        '11. CompendiumSpellParser respects explicit metadata and leaves non-SRD unmetadata spells empty',
        () {
      final parser = CompendiumSpellParser();
      ...
      final customSpell = parser.parseSpell({
        ...
        'entries': ['A sphere of deadly energy drains vitality.'],
      });
      expect(customSpell.customProperties['classes'],
          containsAll(['Sorcerer', 'Wizard']));

      final unmetadataSpell = parser.parseSpell({
        'name': 'Dream Slumber',
        'level': 3,
        'school': 'E',
      ...
      expect(
          unmetadataSpell.customProperties['classes'], anyOf(isNull, isEmpty));
    });

    test(
        '12. Item revitalization cleans rechargeAmount and unparsed description tags',
        () {
      final pipeline = CompendiumJsonIngestionPipeline();
        ...
        'items': [
          {
            ...
            'rarity': 'Rare',
            'descriptionMarkdown':
                '> **Note:** Paired with {@item Silver Horn of Valhalla}',
            'customProperties': {
    ...
    test(
        '13. Spell Parser and Ingestion revitalize distance and geometry when rangeDistanceFeet is 0 but text has range',
        () {
      final parser = CompendiumSpellParser();
      ...
      final voidLance = parser.parseSpell({
        ...
        'entries': [
          'A beam of void energy 100 feet long and 5 feet wide shoots forth.'
        ],
      });
        ...
        'spells': [
          {
            ...
            'rangeDistanceFeet': 0,
            'entries': [
              'A wave of gravitational force ripples outward 30 feet.'
            ],
          }
        ]
      };
    ...
    test(
        '14. Feat Parser extracts nested daily/ritual bonus spells, skill choices, and tool proficiencies',
        () {
      final parser = CompendiumFeatParser();
      ...
      final bonusSpells =
          feat.grants.where((g) => g.type == GrantType.bonusSpell).toList();
      ...
      final skillChoices = feat.grants
          .where((g) => g.type == GrantType.bonusSkillChoice)
          .toList();
      expect(skillChoices, isNotEmpty);
      expect(skillChoices.first.payload['pool'],
          containsAll(['arcana', 'investigation', 'history']));
      expect(skillChoices.first.payload['count'], equals(1));

      // Tool proficiency extracted
      final toolProf =
          feat.grants.where((g) => g.type == GrantType.proficiency).toList();
      expect(
          toolProf.any((g) => (g.payload['proficiency']?.toString() ?? '')
              .toLowerCase()
              .contains('tinkers tools')),
          isTrue);
    });

    test(
        '15. Background Parser and Ingestion pipeline inherit skills, tools, and origin feat from base when empty lists are provided',
        () {
      final parser = CompendiumBackgroundParser();
      ...
      expect(shrineKeeper.skillProficiencies,
          containsAll(['Insight', 'Religion']));
      expect(
          shrineKeeper.toolProficiencies, contains('Calligrapher\'s Supplies'));
    });

    test(
        '16. SubclassSpellsLibrary resolveFilterSpells matches custom source filter and spell tags',
        () {
      const astralSpell = SpellItem(
        id: 'chrono-shift',
        name: 'Chrono Shift',
        level: 5,
        ...
        rules2014: SpellEditionDetails(
          ...
          duration: '1 round',
          description: [
            'You target the triggering creature and shift it across space.'
          ],
        ...
        tags: ['astral', 'source:astral_tome'],
      );
      ...
      SpellbookLibrary.setHomebrewSpells([astralSpell, normalSpell]);
      addTearDown(() => SpellbookLibrary.setHomebrewSpells([]));

      final matched =
          SubclassSpellsLibrary.resolveFilterSpells('source=astral_tome');

      expect(matched.map((s) => s.id), contains('chrono-shift'));
      expect(matched.map((s) => s.id), isNot(contains('custom-spark')));
    });

    test(
        '17. Feat Parser and Feat.fromMap extract bonus spells from nested choose/from structures and auto-heal empty grants',
        () {
      final parser = CompendiumFeatParser();
      ...
      final bonusSpells =
          parsed.grants.where((g) => g.type == GrantType.bonusSpell).toList();
      ...
      expect(
          fromMapFeat.grants.map((g) => g.payload['slug']), contains('shield'));
    });

    test(
        '18. Spell.toMap losslessly preserves classes and revitalized rangeDistanceFeet for round-trip exports',
        () {
      final spellMap = {
        'name': 'Cosmic Flare',
        'level': 4,
    ...
    test(
        '19. Background Parser resolves in-bundle background copies and applies SRD 5.1 customization fallback when base is external',
        () {
      final parser = CompendiumBackgroundParser();
      ...
      expect(childWithInBundleBase.skillProficiencies,
          containsAll(['Insight', 'Persuasion']));
      expect(
          childWithInBundleBase.toolProficiencies, contains('Smith\'s Tools'));
      ...
      final skillChoice = childWithExternalBase.grants
          .where((g) => g.type == GrantType.bonusSkillChoice)
          .toList();
