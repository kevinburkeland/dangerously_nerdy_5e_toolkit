...
void main() {
  group('Homebrew Backup Comprehensive Ingestion Tests', () {
    test(
        'ingests backup bundle and validates zero unparsed tags and revitalized _copy monsters',
        () {
      final bundleMap = {
        'schemaVersion': 1,
        'spells': [
          {
            ...
            'descriptionMarkdown':
                'Your weapon glows with astral fury dealing {@damage 2d6} damage on hit.',
            'higherLevelsMarkdown':
                'Deals {@scaledamage 2d6|2d6|1d6} additional damage per higher slot.',
            'customProperties': {
          ...
          },
          {
            ...
            'descriptionMarkdown':
                'A beam of energy dealing **`1d10 untyped`** force damage.',
            'customProperties': {},
          },
        ],
        'monsters': [
      ...
      for (final m in result.monsters) {
        ...
        if (m.actionsMarkdown.contains('{@hitYourSpellAttack}'))
          unparsedHitSpell++;
      ...
      for (final s in result.spells) {
        final fullDesc =
            '${s.descriptionMarkdown} ${s.higherLevelsMarkdown ?? ""}';
        if (fullDesc.contains('untyped')) untypedCount++;
        if (fullDesc.contains('{@damage')) unparsedH++;
      }

      expect(unparsedH, equals(0),
          reason: 'All {@h} and {@damage} tags should be transformed');
      expect(unparsedRecharge, equals(0),
          reason: 'All {@recharge} tags should be transformed');
      expect(unparsedHitSpell, equals(0),
          reason: 'All {@hitYourSpellAttack} tags should be transformed');
      expect(untypedCount, equals(0),
          reason: 'No "untyped" word should leak into markdown text');

      // 2. Verify _copy creature revitalization
      final statue = result.monsters
          .where((m) => m.name == 'Animated Guardian Statue')
          .firstOrNull;
      ...
      final serpent =
          result.monsters.where((m) => m.name == 'Serpent Drake').firstOrNull;
      ...
      final smiteSpell =
          result.spells.where((s) => s.name == 'Astral Smite').firstOrNull;
      expect(smiteSpell, isNotNull);
      final spellItem =
          HomebrewPersistenceService.spellToSpellItem(smiteSpell!);
