...
void main() {
  group('DprCalculatorEngine Math & Probabilities', () {
    test('expectedDieValue computes exact 5e dice expectations', () {
      ...
      expect(
          DprCalculatorEngine.expectedDieValue(6, gwf: GwfVersion.v2014Reroll),
          closeTo(4.1667, 0.001));
      expect(
          DprCalculatorEngine.expectedDieValue(8, gwf: GwfVersion.v2014Reroll),
          closeTo(5.25, 0.001));
      expect(
          DprCalculatorEngine.expectedDieValue(10, gwf: GwfVersion.v2014Reroll),
          closeTo(6.30, 0.001));
      expect(
          DprCalculatorEngine.expectedDieValue(12, gwf: GwfVersion.v2014Reroll),
          closeTo(7.3333, 0.001));

      // GWF 2024 floor 3 (1s & 2s treated as 3)
      expect(
          DprCalculatorEngine.expectedDieValue(6, gwf: GwfVersion.v2024Floor3),
          closeTo(4.0, 0.001));
      expect(
          DprCalculatorEngine.expectedDieValue(8, gwf: GwfVersion.v2024Floor3),
          closeTo(4.875, 0.001));
      expect(
          DprCalculatorEngine.expectedDieValue(10, gwf: GwfVersion.v2024Floor3),
          closeTo(5.8, 0.001));
      expect(
          DprCalculatorEngine.expectedDieValue(12, gwf: GwfVersion.v2024Floor3),
          closeTo(6.75, 0.001));
    });

    test(
        'calculateHitProbability correctly evaluates roll requirements and advantage states',
        () {
      // +7 attack bonus vs AC 15 -> needs roll of 8 -> 13/20 = 0.65 base hit chance
      final pNormal = DprCalculatorEngine.calculateHitProbability(
          7, 15, AdvantageType.normal);
      expect(pNormal, closeTo(0.65, 0.001));

      // Advantage: 1 - (1 - 0.65)^2 = 1 - 0.1225 = 0.8775
      final pAdv = DprCalculatorEngine.calculateHitProbability(
          7, 15, AdvantageType.advantage);
      expect(pAdv, closeTo(0.8775, 0.001));

      // Disadvantage: 0.65^2 = 0.4225
      final pDis = DprCalculatorEngine.calculateHitProbability(
          7, 15, AdvantageType.disadvantage);
      expect(pDis, closeTo(0.4225, 0.001));

      // Triple Advantage (Elven Focus): 1 - (1 - 0.65)^3 = 1 - 0.042875 = 0.957125
      final pElven = DprCalculatorEngine.calculateHitProbability(
          7, 15, AdvantageType.elvenAccuracy);
      expect(pElven, closeTo(0.9571, 0.001));

      // Clamped limits: Nat 1 always misses (max 95%), Nat 20 always hits (min 5%)
      expect(
          DprCalculatorEngine.calculateHitProbability(
              25, 5, AdvantageType.normal),
          closeTo(0.95, 0.001));
      expect(
          DprCalculatorEngine.calculateHitProbability(
              -10, 30, AdvantageType.normal),
          closeTo(0.05, 0.001));
    });
    ...
    test('calculateCritProbability correctly evaluates expanded crit ranges',
        () {
      expect(
          DprCalculatorEngine.calculateCritProbability(AdvantageType.normal,
              critThreshold: 20),
          closeTo(0.05, 0.001));
      expect(
          DprCalculatorEngine.calculateCritProbability(AdvantageType.normal,
      ...
      // Crit with Advantage
      expect(
          DprCalculatorEngine.calculateCritProbability(AdvantageType.advantage,
              critThreshold: 20),
          closeTo(0.0975, 0.001));
      expect(
          DprCalculatorEngine.calculateCritProbability(AdvantageType.advantage,
              critThreshold: 19),
          closeTo(0.19, 0.001));
    ...
    test('calculateSingleAttackDpr evaluates basic weapon attack DPR', () {
      ...
      final pt = DprCalculatorEngine.calculateSingleAttackDpr(
          attack, 15, AdvantageType.normal);
    ...
    test('WeaponMastery.graze deals ability modifier damage on miss', () {
      ...
      final ptGraze = DprCalculatorEngine.calculateSingleAttackDpr(
          attackWithGraze, 20, AdvantageType.normal);
      final ptNormal = DprCalculatorEngine.calculateSingleAttackDpr(
          attackWithoutGraze, 20, AdvantageType.normal);
    ...
    test('Sneak Attack is factored in once per turn across multiple attacks',
      ...
      final curve =
          DprCalculatorEngine.generateCurve(rogue, minAc: 10, maxAc: 20);

      final pt = curve.pointAt(15);
      expect(pt, isNotNull);
      expect(pt!.dpr,
          greaterThan(10.0)); // 1d6+4 + 3d6 sneak attack with advantage
    });
    ...
    test('Agonizing Blast adds ability modifier to cantrip damage', () {
      ...
      final ptWithout = DprCalculatorEngine.calculateSingleAttackDpr(
          ebWithoutAgonizing, 15, AdvantageType.normal);
      final ptWith = DprCalculatorEngine.calculateSingleAttackDpr(
          ebWithAgonizing, 15, AdvantageType.normal);

      // Hit chance = 0.65 (need 8+ on d20). Expected hit damage increases by 4.
      expect(ptWith.expectedDamageOnHit,
          equals(ptWithout.expectedDamageOnHit + 4.0));
      expect(ptWith.dpr, greaterThan(ptWithout.dpr));
    });

    test(
        'Halfling Luck rerolls natural 1s and boosts accuracy and crit chances',
        () {
      const attack = DprAttackAction(
        id: 'halfling_dagger',
        name: 'Dagger',
      ...
      final standardPt = DprCalculatorEngine.calculateSingleAttackDpr(
          attack, 15, AdvantageType.normal,
          hasHalflingLuck: false);
      final luckyPt = DprCalculatorEngine.calculateSingleAttackDpr(
          attack, 15, AdvantageType.normal,
    ...
    test(
        'DprWeaponPreset contains Shillelagh, Magic Stone, and Flame Tongue presets',
        () {
      final shillelagh =
          DprWeaponPreset.allPresets.firstWhere((p) => p.id == 'shillelagh');
      expect(shillelagh.name, contains('Shillelagh'));
      expect(shillelagh.diceSides, equals(8));
      expect(shillelagh.isCantrip, isTrue);

      final magicStone =
          DprWeaponPreset.allPresets.firstWhere((p) => p.id == 'magic_stone');
      expect(magicStone.name, contains('Magic Stone'));
      expect(magicStone.isRanged, isTrue);
      expect(magicStone.isCantrip, isTrue);

      final eb1 = DprWeaponPreset.allPresets
          .firstWhere((p) => p.id == 'eldritch_blast_1');
      final eb2 = DprWeaponPreset.allPresets
          .firstWhere((p) => p.id == 'eldritch_blast_2');
      final eb3 = DprWeaponPreset.allPresets
          .firstWhere((p) => p.id == 'eldritch_blast_3');
      final eb4 = DprWeaponPreset.allPresets
          .firstWhere((p) => p.id == 'eldritch_blast_4');
      ...
      final fb4 =
          DprWeaponPreset.allPresets.firstWhere((p) => p.id == 'fire_bolt_4');
      expect(fb4.diceCount, equals(4));
      expect(fb4.diceSides, equals(10));

      final flameTongue = DprWeaponPreset.allPresets
          .firstWhere((p) => p.id == 'flame_tongue_greatsword');
      expect(flameTongue.name, contains('Flame Tongue'));
      expect(flameTongue.diceCount, equals(2));
      expect(flameTongue.diceSides, equals(6));
      expect(flameTongue.damageType, equals('slashing'));
      expect(flameTongue.secondaryDamageType, equals('fire'));
    });
    ...
    test('calculateSaveFailureProbability computes exact 5e save failure rates',
      ...
      final failNormal = DprCalculatorEngine.calculateSaveFailureProbability(
          saveDc: 15, targetSaveBonus: 2);
      expect(failNormal, closeTo(0.60, 0.001));

      // With Advantage on Save: Pass prob = 1 - (1 - 0.40)^2 = 1 - 0.36 = 0.64 -> fail prob = 0.36
      final failAdv = DprCalculatorEngine.calculateSaveFailureProbability(
          saveDc: 15, targetSaveBonus: 2, targetHasAdvantage: true);
      expect(failAdv, closeTo(0.36, 0.001));

      // With Disadvantage on Save: Pass prob = 0.40^2 = 0.16 -> fail prob = 0.84
      final failDis = DprCalculatorEngine.calculateSaveFailureProbability(
          saveDc: 15, targetSaveBonus: 2, targetHasDisadvantage: true);
    ...
    test(
        'Vex Mastery chains advantage to subsequent attack in multi-attack rounds',
        () {
      ...
      final vexPt =
          DprCalculatorEngine.calculateProfileDpr(dualWielderWithVex, 15);
      final normalPt =
          DprCalculatorEngine.calculateProfileDpr(dualWielderWithoutVex, 15);
    ...
    test(
        'Saving Throw AoE damage accurately calculates half-on-save and target multipliers',
        () {
      ...
      expect(pt.expectedDamageOnMiss,
          closeTo(14.0, 0.01)); // half on successful save
    });
    ...
    test('DprMonsterAcPreset contains all standard CR benchmarks', () {
      expect(
          DprMonsterAcPreset.standardPresets.length, greaterThanOrEqualTo(8));
      final cr5 = DprMonsterAcPreset.standardPresets
          .firstWhere((p) => p.crDisplay == '5');
      expect(cr5.typicalAc, equals(15));
      expect(cr5.examples, contains('Troll'));

      final cr30 = DprMonsterAcPreset.standardPresets
          .firstWhere((p) => p.crDisplay == '30');
      expect(cr30.typicalAc, equals(25));
      expect(cr30.examples, contains('Tarrasque'));
    });

    test(
        'generateCurveAsync runs in background isolate and returns valid points',
        () async {
      ...
      final curve = await DprCalculatorEngine.generateCurveAsync(profile,
          minAc: 10, maxAc: 20);
    ...
    test('calculateGwmBreakEvenAsync runs in background isolate', () async {
      ...
      final result = await DprCalculatorEngine.calculateGwmBreakEvenAsync(
          profile,
          minAc: 10,
          maxAc: 20);
