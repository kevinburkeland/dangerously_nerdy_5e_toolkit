...
void main() {
  group('Combat Mechanics Comprehensive Audit & Edge Cases', () {
    ...
    group('AoeResolver Exhaustive Boundary Tests', () {
      test(
          'Base target caps calculate exact DMG p.249 geometry values and handle zero/negative sizes',
          () {
        ...
        expect(
            AoeResolver.getBaseTargetCap(AoeShape.sphere, 20), 4.0); // 20/5 = 4
        expect(
            AoeResolver.getBaseTargetCap(AoeShape.sphere, 40), 8.0); // 40/5 = 8
        expect(AoeResolver.getBaseTargetCap(AoeShape.cylinder, 10),
            2.0); // 10/5 = 2
        expect(AoeResolver.getBaseTargetCap(AoeShape.cone, 15),
            1.5); // 15/10 = 1.5
        expect(
            AoeResolver.getBaseTargetCap(AoeShape.cone, 30), 3.0); // 30/10 = 3
        expect(
            AoeResolver.getBaseTargetCap(AoeShape.cone, 60), 6.0); // 60/10 = 6
        expect(AoeResolver.getBaseTargetCap(AoeShape.cube, 15),
            1.5); // 15/10 = 1.5
        expect(AoeResolver.getBaseTargetCap(AoeShape.cube, 100),
            10.0); // 100/10 = 10
        expect(
            AoeResolver.getBaseTargetCap(AoeShape.line, 30), 1.0); // 30/30 = 1
        expect(AoeResolver.getBaseTargetCap(AoeShape.line, 100),
            closeTo(3.333, 0.001)); // 100/30
      });

      test(
          'calculateTargetCount handles boundary counts (0, 1, large) without throwing',
          () {
        final rng = Random(42);
        // 0 enemies returns 0
        expect(
      ...
      test(
          'selectTargets handles empty and single-target lists without mutation or error',
          () {
        final rng = Random(42);
      ...
      test(
          'selectTargets sorts candidates correctly for all 3 targeting heuristics',
          () {
        final rng = Random(42);
        final cLowHp = ArenaCombatant.fromMonster(
            id: 'c_low',
            monster: wolfMonster,
            team: ArenaTeam.teamB,
            hpOverride: 5,
            acOverride: 10);
        final cMidHp = ArenaCombatant.fromMonster(
            id: 'c_mid',
            monster: wolfMonster,
            team: ArenaTeam.teamB,
            hpOverride: 15,
            acOverride: 12);
        final cHighThreat = ArenaCombatant.fromMonster(
            id: 'c_high',
      ...
      test(
          'parseShapeAndSize extracts exact dimensions and ignores recharge numbers',
          () {
        // Explicit dimensions
        expect(
            AoeResolver.parseShapeAndSize('Fireball', 'A 20-foot radius sphere')
                .shape,
            AoeShape.sphere);
        expect(
            AoeResolver.parseShapeAndSize('Fireball', 'A 20-foot radius sphere')
        ...
        expect(
            AoeResolver.parseShapeAndSize('Cone of Cold (60-foot cone)').shape,
            AoeShape.cone);
        expect(
            AoeResolver.parseShapeAndSize('Cone of Cold (60-foot cone)')
                .sizeInFeet,
            60.0);
        ...
        expect(AoeResolver.parseShapeAndSize('Thunderwave 15-ft cube').shape,
            AoeShape.cube);
        expect(
            AoeResolver.parseShapeAndSize('Thunderwave 15-ft cube').sizeInFeet,
            15.0);

        expect(
            AoeResolver.parseShapeAndSize('Flame Strike 10-ft cylinder').shape,
            AoeShape.cylinder);
        expect(
            AoeResolver.parseShapeAndSize('Flame Strike 10-ft cylinder')
                .sizeInFeet,
            10.0);

        // Recharge strings like "(Recharge 5-6)" must not be mistaken for 5-foot dimensions!
        final breathAction =
            AoeResolver.parseShapeAndSize('Fire Breath (Recharge 5-6)');
    ...
    group('Aerial Combat & Condition State Transitions', () {
      test(
          'Combatant initializes flight and reach attributes from monster statblock',
          () {
        // Wolf has no flight, 5 ft reach
        final wolf = ArenaCombatant.fromMonster(
            id: 'w', monster: wolfMonster, team: ArenaTeam.teamA);
        ...
        final dragon = ArenaCombatant.fromMonster(
            id: 'd', monster: dragonMonster, team: ArenaTeam.teamA);
        ...
        final trex = ArenaCombatant.fromMonster(
            id: 't', monster: trexMonster, team: ArenaTeam.teamA);
        expect(trex.isAirborne, false);
        expect(trex.meleeReachInFeet, 10);
      });

      test(
          'applyCondition causes non-hovering fliers to fall, take fall damage, and land prone',
          () {
        ...
        expect(
            fallRes.log,
            contains(
                'fell 40 ft. from the air, taking 20 bludgeoning damage and landing Prone!'));
      });

      test(
          'Fall damage dice count is capped at 20d6 maximum per 5e rules for extreme altitudes',
          () {
        ...
        expect(diceRolled, 20,
            reason: 'Fall damage must be capped at 20d6 per standard 5e rules');
      ...
      test(
          'Hover capability keeps flier suspended in the air when suffering disruptive conditions',
          () {
        ...
        expect(
            fallRes.log, contains('hovered in place despite suffering Prone!'));
      });

      test(
          'Non-disruptive conditions (Blinded, Poisoned, Charmed) do not trigger fall mechanics',
          () {
        ...
        final blindRes = eagle.applyCondition(
            ArenaCondition.blinded, (s) => 6, DmRulesEdition.v2024);
        ...
        final poisonRes = eagle.applyCondition(
            ArenaCondition.poisoned, (s) => 6, DmRulesEdition.v2024);
      ...
      test('Condition getters and removal operate correctly', () {
        final combatant = ArenaCombatant.fromMonster(
            id: 'c', monster: wolfMonster, team: ArenaTeam.teamA);
    ...
    group('AttackType & Reach Validation', () {
      test(
          'DprAttackAction correctly infers AttackType from name and properties',
          () {
      ...
      test(
          'Grounded attacker switches to ranged attack when targeting airborne out-of-reach enemies',
          () {
    ...
    group('Condition Combat Advantage & Auto-Crit Mechanics', () {
      test(
          'Paralyzed or Unconscious target suffers automatic Critical Hit from 5-ft melee attacks',
          () {
      ...
      test(
          'Prone defender grants Advantage to melee and imposes Disadvantage to ranged attacks',
          () {
    ...
    group('Concentration & Shield Reaction Boundary Values', () {
      test('checkConcentration computes exact DC formula max(10, damage ~/ 2)',
        ...
        final resSmall = caster.checkConcentration(
            14, (s) => 20); // 14 ~/ 2 = 7 < 10 -> DC 10
        expect(resSmall.dc, 10);
        expect(resSmall.broken, false);

        // 3. Large damage (50): DC is 25 (50 ~/ 2)
        final resLarge =
            caster.checkConcentration(50, (s) => 5); // 5 + conMod < 25 -> fails
      ...
      test(
          'Shield reaction is NOT triggered on Nat 20 Critical Hits even if slots exist',
          () {
