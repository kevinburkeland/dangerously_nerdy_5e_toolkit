...
void main() {
  group('Variant Lineage & Species Spells Tests', () {
    test(
        'CompendiumRaceParser extracts innate and expanded spells from additionalSpells',
        () {
      ...
      final spellGrants =
          subrace.grants.where((g) => g.type.name == 'bonusSpell').toList();
      final slugs =
          spellGrants.map((g) => g.payload['slug'].toString()).toSet();

      expect(slugs, contains('mending'));
      expect(slugs, contains('magic-weapon'));
      expect(slugs, contains('identify'));
      expect(slugs, contains('floating-disk'));
      ...
      final mendingGrant =
          spellGrants.firstWhere((g) => g.payload['slug'] == 'mending');
      expect(mendingGrant.payload['isCantrip'], isTrue);
    });

    test(
        'SkillTraitResolver.getInnateSpeciesSpells returns innate cantrip and leveled spells',
        () {
      final parser = CompendiumRaceParser();
      final subrace = parser.parseSubrace({
        'name': 'Lineage of the Forge',
        'source': 'CUSTOM',
        'raceName': 'Human',
        'additionalSpells': [
          {
      ...
      final mending =
          level1Spells.firstWhere((s) => s.spellRef.slug == 'mending');
      expect(mending.isCantrip, isTrue);
    });

    test(
        'CharacterFactory.buildFromDraft auto-merges species and subrace spells into cantrips and spellsKnown',
        () {
      final parser = CompendiumRaceParser();
      final subrace = parser.parseSubrace({
        'name': 'Lineage of the Forge',
