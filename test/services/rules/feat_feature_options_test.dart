...
const mockHomebrewInvocationFeat = Feat(
  id: EntityId(
      slug: 'homebrew-invocation-adept', ruleset: RulesetVersion.v2014),
...
const mockHomebrewFightingStyleFeat = Feat(
  id: EntityId(
      slug: 'homebrew-fighting-style-initiate', ruleset: RulesetVersion.v2014),
...
void main() {
  group('Feat Feature Options (Homebrew & Optional Feature Progressions)', () {
    test(
        'FeatAsiExtension identifies invocation and fighting style choices on homebrew feats',
        () {
    ...
    test(
        'Homebrew feats with generic optionalfeatureProgression are recognized',
        () {
    ...
    test('RAW prerequisite evaluation for invocation granting feats', () {
      ...
      // 1. Non-warlock
      final nonWarlockArmor =
          mockHomebrewInvocationFeat.evaluateInvocationPrerequisite(
        armorOfShadows,
        isWarlock: false,
        warlockLevel: 0,
      );
      expect(nonWarlockArmor.isMet, isTrue,
          reason: 'Armor of Shadows has no prerequisites');

      final nonWarlockAgonizing =
          mockHomebrewInvocationFeat.evaluateInvocationPrerequisite(
        agonizingBlast,
        isWarlock: false,
        warlockLevel: 0,
      ...
      expect(nonWarlockAgonizing.isMet, isFalse,
          reason:
              'Non-warlock cannot choose invocation with prerequisites even if they know cantrip');

      // 2. Warlock meeting prerequisite
      final warlockAgonizingMet =
          mockHomebrewInvocationFeat.evaluateInvocationPrerequisite(
        agonizingBlast,
        isWarlock: true,
        warlockLevel: 2,
      ...
      final warlockAscendantLowLevel =
          mockHomebrewInvocationFeat.evaluateInvocationPrerequisite(
        ascendantStep,
        isWarlock: true,
        warlockLevel: 5,
      ...
      final warlockAscendantHighLevel =
          mockHomebrewInvocationFeat.evaluateInvocationPrerequisite(
        ascendantStep,
        isWarlock: true,
        warlockLevel: 9,
    ...
    test('getAllSelectedFeatureOptions aggregates feat- prefixed option keys',
      ...
      expect(allOptions['feat-homebrew-invocation-adept'],
          equals(['armor_of_shadows']));
      expect(allOptions['feat-homebrew-fighting-style-initiate'],
          equals(['defense']));
      expect(allOptions.containsKey('unrelated-invalid-key'), isFalse);
    });

    test(
        'Character hasCapabilityFlag detects feat-granted invocation capability',
        () {
    ...
    test(
        'CharacterActionsResolver extracts action from feat-granted invocation',
        () {
      final char = _createTestChar(
        slug: 'test-warlock',
        name: 'Occult Scholar',
      ...
      final armorAction = allActions
          .where((a) =>
              a.name.toLowerCase().contains('armor of shadows') ||
              a.name.toLowerCase().contains('mage armor'))
          .firstOrNull;
      expect(armorAction, isNotNull);
    });

    test(
        'CharacterProgressionEngine merges featureOptionGrants during level up',
        () {
      ...
      final updatedChar =
          CharacterProgressionEngine.applyLevelUp(baseChar, levelUpRequest);
      expect(updatedChar.totalLevel, equals(4));
      expect(
          updatedChar.feats.any((f) => f.slug == 'homebrew-invocation-adept'),
          isTrue);
    ...
    test(
        'CharacterSheetController.addFeat saves feat options and removeFeat cleans them up',
        () async {
      final baseChar = _createTestChar(
        slug: 'test-wizard',
        name: 'Gondor',
      ...
      expect(
          controller.character.feats
              .any((f) => f.slug == 'homebrew-invocation-adept'),
          isTrue);
      final optionsAfterAdd =
          controller.character.progression.getAllSelectedFeatureOptions();
      expect(optionsAfterAdd['feat-homebrew-invocation-adept'],
          equals(['armor_of_shadows']));
      ...
      expect(
          controller.character.feats
              .any((f) => f.slug == 'homebrew-invocation-adept'),
          isFalse);
      final optionsAfterRemove =
          controller.character.progression.getAllSelectedFeatureOptions();
      expect(optionsAfterRemove.containsKey('feat-homebrew-invocation-adept'),
          isFalse);
    });
  });
}
