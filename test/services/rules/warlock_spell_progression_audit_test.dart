...
void main() {
  group('Warlock Spell Progression & Untraining Audit', () {
    test(
        'Warlock untraining at level 3 must not prevent learning 5th spell at level 4',
        () {
      // 1. Level 1 Warlock
      ...
      expect(
          warlock.allocatedSpells['class-warlock-spells']
              ?.map((s) => s.slug)
              .toSet(),
          contains('spell_armor_of_agathys'));
      ...
      expect(warlock.spellsKnown.map((s) => s.slug).toSet(),
          isNot(contains('spell_hellish_rebuke')));
      expect(warlock.spellsKnown.length, 4,
          reason:
              'Warlock at level 3 should know exactly 4 spells after 1-for-1 swap');
      ...
      expect(delta, 1,
          reason:
              'Warlock leveling to level 4 should have delta of 1 new leveled spell');
    });

    test(
        'Untraining a spell previously learned at level up (stored in allocatedSpells) removes it from allocatedSpells',
        () {
      var warlock = const Character(
      ...
      expect(
          warlock.allocatedSpells['class-warlock-spells']
              ?.map((s) => s.slug)
              .toSet(),
          isNot(contains('spell_armor_of_agathys')));
      expect(warlock.spellsKnown.map((s) => s.slug).toSet(),
          isNot(contains('spell_armor_of_agathys')));
      expect(warlock.spellsKnown.length, 4);
    ...
    test(
        'College of Lore Bard at Level 6 receives 2 Additional Magical Secrets (+2 known)',
        () {
    ...
    test(
        'Eldritch Knight Fighter at Level 3 is recognized as 1/3 caster with Wizard spell list',
        () {
    ...
    testWidgets(
        'LevelUpWizardDialog correctly computes Level 4 Warlock quotas after Level 3 untrain',
        (tester) async {
    ...
    test(
      ...
      expect(
          controller.character.allocatedSpells['class-warlock-spells']?.length,
          2);
      ...
      expect(controller.character.spellsKnown.map((s) => s.slug).toSet(),
          contains('spell_hex'));
      expect(controller.character.spellsKnown.map((s) => s.slug).toSet(),
          isNot(contains('spell_darkness')));
      expect(
          controller.character.allocatedSpells['class-warlock-spells']
              ?.map((s) => s.slug)
              .toSet(),
          isNot(contains('spell_darkness')));
    });
