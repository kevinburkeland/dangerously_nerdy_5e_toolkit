...
void main() {
  group('CharacterProgressionEngine Unit Tests', () {
    ...
    test('Single-class progression (Fighter 1 -> 2 -> 3 Champion -> 4 ASI)',
      ...
      expect(fighter.progression.classes.first.subclassRef?.slug,
          equals('champion'));
    ...
    test('Retroactive CON HP recalculation when CON score increases at Level 4',
      ...
      expect(
          CharacterStatCalculator.compute(wizard, resolver).maxHp, equals(12));
      ...
      expect(
          CharacterStatCalculator.compute(wizard, resolver).maxHp, equals(17));
    ...
    test(
        'ASI milestones detected for Fighter (levels 4, 6, 8, 12, 14, 16, 19) and Rogue (10)',
        () {
    ...
    test('Multiclass validation rejects if attribute score is below 13', () {
      ...
      final barbValidation =
          CharacterProgressionEngine.validateMulticlass(weakMage, 'barbarian');
      expect(barbValidation.isValid, isFalse);

      // Paladin requires STR 13 & CHA 13
      final paladinValidation =
          CharacterProgressionEngine.validateMulticlass(weakMage, 'paladin');
      expect(paladinValidation.isValid, isFalse);

      // Rogue requires DEX 13 (current DEX 12)
      final rogueValidation =
          CharacterProgressionEngine.validateMulticlass(weakMage, 'rogue');
