...
void main() {
  group('CharacterHomebrewValidator Tests', () {
    ...
    test('Detects missing custom class when not loaded in compendium', () {
      ...
      expect(
          report.missingItems.first.type, equals(HomebrewEntityType.classType));
      expect(report.missingItems.first.slug, equals('blood-hunter'));
      expect(report.missingItems.first.name, equals('Blood Hunter'));
    });

    test(
        'Does NOT report missing class when custom class is loaded in compendium',
        () {
    ...
    test('Detects missing custom subclass on core class', () {
      ...
      expect(report.hasMissing, isTrue);
      expect(
          report.missingItems.any((i) =>
              i.type == HomebrewEntityType.subclassType &&
              i.slug == 'rift-warden'),
          isTrue);
    });
    ...
    test('Detects missing custom species', () {
      ...
      expect(
          report.missingItems.any((i) =>
              i.type == HomebrewEntityType.speciesType && i.slug == 'voidling'),
          isTrue);
    });
    ...
    test('Detects missing custom feat', () {
      ...
      expect(
          report.missingItems.any((i) =>
              i.type == HomebrewEntityType.featType &&
              i.slug == 'custom_ancient_rune'),
          isTrue);
    });
    ...
    test('Detects missing custom spell', () {
      ...
      expect(
          report.missingItems.any((i) =>
              i.type == HomebrewEntityType.spellType &&
              i.slug == 'custom_gravitic_crush'),
          isTrue);
    });
    ...
    test('Detects missing custom equipment / item in inventory', () {
      ...
      expect(
          report.missingItems.any((i) =>
              i.type == HomebrewEntityType.itemType &&
              i.slug == 'custom_blade_of_the_ruin'),
          isTrue);
    });

    test(
        'Validates explicit usedHomebrew manifest recorded in customProperties',
        () {
      final customChar = baseSrdCharacter.copyWith(
        customProperties: {
          'usedHomebrew': [
    ...
    test(
      ...
      final deps =
          CharacterHomebrewValidator.collectHomebrewDependencies(customChar);
      expect(deps.length, equals(2));
      expect(
          deps.any(
              (d) => d['type'] == 'speciesType' && d['slug'] == 'custom_grung'),
          isTrue);
      expect(
          deps.any((d) =>
              d['type'] == 'featType' && d['slug'] == 'custom_poisoner_expert'),
          isTrue);
    });
