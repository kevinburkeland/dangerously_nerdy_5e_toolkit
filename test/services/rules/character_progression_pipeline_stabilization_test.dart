...
void main() {
  group('Character Builder & Progression Pipeline Stabilization Tests', () {
    ...
    test(
      ...
      final validation =
          CharacterProgressionEngine.validateMulticlass(barbarian, 'wizard');
      expect(validation.isValid, isFalse);
      expect(
          validation.errors.any((e) =>
              e.contains('wizard') && e.contains('attribute prerequisite')),
          isTrue);
    });
    ...
    test(
      ...
      final validation = SpellAllocationValidator.validateSpellAllocations(
          character, activeGrants);
      expect(validation.isValid, isTrue);

      // If species changes and high-elf-cantrip is removed from active grants
      final invalidValidation =
          SpellAllocationValidator.validateSpellAllocations(character, []);
      expect(invalidValidation.isValid, isFalse);
      expect(
          invalidValidation.errors.any((e) =>
              e.contains('Orphan spell allocation key "high-elf-cantrip"')),
          isTrue);
    });

    test(
        'Phase 3: Historical ledger (manualHpRolls) preserves exact rolls across character modifications',
        () {
    ...
    test(
      ...
      final slots2014 =
          MulticlassSlotMatrix.calculateSpellSlots(multiclassChar2014);
      // 2014: Full (1) + Paladin (1 ~/ 2 = 0) = ECL 1 -> [2 Level 1 slots]
      expect(slots2014.maxSlots[1], equals(2));
      expect(slots2014.maxSlots[2], isNull);

      final multiclassChar2024 =
          multiclassChar2014.copyWith(rulesEdition: DmRulesEdition.v2024);
      final slots2024 =
          MulticlassSlotMatrix.calculateSpellSlots(multiclassChar2024);
      ...
      final slotsWithWarlock =
          MulticlassSlotMatrix.calculateSpellSlots(withWarlock);
    ...
    test(
      ...
      expect(() => (updatedChar.languages as dynamic).add('Draconic'),
