...
void main() {
  group('Eldritch Invocation & Feature Prerequisite Tests', () {
    test('Correctly extracts prerequisites from SRD descriptionMarkdown regex',
      ...
      // Level 12 + Pact of the Shroud
      const bondOfShroud = FeatureOption(
        id: 'bond_of_the_shroud',
        name: 'Bond of the Shroud',
        descriptionMarkdown:
            'Prerequisite: 12th level, Pact of the Shroud. You can teleport to the wearer of your shroud...',
      );
      expect(bondOfShroud.prerequisite.minLevel, equals(12));
      expect(bondOfShroud.prerequisite.requiredPact, equals('shroud'));
    });

    test(
        'Correctly extracts prerequisites from compendium structured JSON customProperties',
        () {
      const jsonEntry = FeatureOption(
        id: 'bond-of-the-shroud',
        name: 'Bond of the Shroud',
        descriptionMarkdown: 'Teleport to shroud wearer.',
        customProperties: {
          'prerequisite': [
            {
              'level': {
      ...
      expect(prereq.requiredPact, equals('shroud'));
      ...
      expect(
          jsonSpellEntry.prerequisite.requiredSpell, equals('eldritch blast'));
    });

    test(
        'FeaturePrerequisite.evaluate correctly evaluates level, pact, and spell requirements',
        () {
      const agonizingBlast = FeatureOption(
        id: 'agonizing_blast',
        name: 'Agonizing Blast',
      ...
      expect(evalAgonizing1.unmetReasons.first,
          contains('Requires Eldritch Blast'));
      ...
      expect(evalThirsting1.unmetReasons,
          contains('Requires Level 5 (Current: 2)'));
      expect(
          evalThirsting1.unmetReasons, contains('Requires Pact of the Blade'));
      ...
      expect(evalLifedrinker2.unmetReasons,
          contains('Requires Level 12 (Current: 5)'));
    ...
    test('Pact Boons and Invocations are separated in SrdFeatureOptions', () {
      ...
      // Pact Boons must contain exactly the 3 canonical SRD pacts
      final pactIds = pactBoons.map((p) => p.id).toSet();
      expect(
          pactIds,
          equals(
              {'pact_of_the_blade', 'pact_of_the_chain', 'pact_of_the_tome'}));
    });

    test(
        '2014 Warlock progression strictly separates Invocations (Lvl 2) and Pact Boon (Lvl 3)',
        () {
      final warlock = SrdClassesLibrary.warlock;

      // Level 2 decisions for 2014 ruleset
      final lvl2Decisions =
          warlock.getDecisionsForLevel(2, ruleset: RulesetVersion.v2014);
      ...
      expect(
          lvl2Invocations.availableOptions
              .any((o) => o.id.startsWith('pact_of_the_')),
          isFalse);

      // Level 3 decisions for 2014 ruleset
      final lvl3Decisions =
          warlock.getDecisionsForLevel(3, ruleset: RulesetVersion.v2014);
      final pactBoonDecision =
          lvl3Decisions.firstWhere((d) => d.type == FeatureChoiceType.pactBoon);
      expect(pactBoonDecision.levelRequired, equals(3));
      final pactIds =
          pactBoonDecision.availableOptions.map((o) => o.id).toSet();
      expect(
          pactIds,
          equals(
              {'pact_of_the_blade', 'pact_of_the_chain', 'pact_of_the_tome'}));
  ...
  group('LevelUpWizardDialog Gating Widget Tests', () {
    testWidgets(
        'Gated invocations display lock indicator and are non-selectable',
        (tester) async {
      ...
      expect(find.text('Step 1 of 6: Target Class & Multiclass Rules'),
          findsOneWidget);
      await tester.tap(find.text('Next Step'));
      await tester.pumpAndSettle();

      // Step 2: HP -> Tap Next Step
      expect(find.text('Step 2 of 6: Hit Points & Hit Die Scaling'),
          findsOneWidget);
      await tester.tap(find.text('Next Step'));
      await tester.pumpAndSettle();

      // Step 3: Class Features & Subclass Archetype (Level 2 Warlock decisions)
      expect(find.text('Step 3 of 6: Class Features & Subclass Archetype'),
          findsOneWidget);
      ...
      expect(find.text('Pact of the Chain'), findsNothing);
      ...
      expect(
          find.textContaining(
              'Cannot select One with Shadows: Requires Level 5'),
          findsOneWidget);
      ...
      expect(
          find.textContaining(
              'Cannot select Agonizing Blast: Requires Eldritch Blast'),
          findsOneWidget);
