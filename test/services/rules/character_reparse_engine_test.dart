...
void main() {
  group('CharacterReparseEngine Tests', () {
    test(
      ...
      expect(reparsed.customProperties['backgroundFeature'],
    ...
    test(
        'reparses arbitrary homebrew 2014 background and self-heals missing skills and background feature',
        () {
      const customBg = Background(
        id: EntityId(
            slug: 'homebrew-inquisitor', ruleset: RulesetVersion.v2014),
        name: 'Homebrew Inquisitor',
        skillProficiencies: ['Investigation', 'Insight'],
        descriptionMarkdown: 'You root out deception and hidden heretics.\n\n'
            '**Feature: Inquisitorial Eye**\n'
            'You can easily determine whether someone is withholding information under questioning.',
        customProperties: {
          'backgroundFeature': 'Inquisitorial Eye',
          'backgroundFeatureDescription':
              'You can easily determine whether someone is withholding information under questioning.',
        },
      );
      ...
      const char = Character(
        ...
        // Artificially missing background skills and empty feature
        skillProficiencies: {
      ...
      final reparsed = CharacterReparseEngine.reparse(char);

      // Verify self-healed skills: original class skill (Stealth) + backfilled background skills (Investigation, Insight)
      expect(reparsed.skillProficiencies[SkillType.stealth],
          equals(SkillProficiencyLevel.proficient));
      expect(reparsed.skillProficiencies[SkillType.investigation],
          equals(SkillProficiencyLevel.proficient));
      expect(reparsed.skillProficiencies[SkillType.insight],
          equals(SkillProficiencyLevel.proficient));

      // Verify self-healed background feature
      expect(reparsed.customProperties['backgroundFeature'],
          equals('Inquisitorial Eye'));
      expect(reparsed.customProperties['backgroundFeatureDescription'],
          contains('withholding information under questioning'));
    });
    ...
    test('heals contaminated 18-skill allowedSkills list to true class skills',
      ...
      expect((contaminated.customProperties['allowedSkills'] as List).length,
          equals(18));
