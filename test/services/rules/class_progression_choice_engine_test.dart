  ...
  group('Class Progression & Feature Choice Engine Tests', () {
    ...
    group('1. Declarative Schema & Serialization Contracts', () {
        ...
        const decision = ClassFeatureDecision(
          ...
          availableOptions: [
            FeatureOption(
                id: 'archery',
                name: 'Archery',
                descriptionMarkdown: '+2 ranged attacks'),
            FeatureOption(
                id: 'defense', name: 'Defense', descriptionMarkdown: '+1 AC'),
          ],
        );
      ...
      test('ClassLevelProgression serializes selectedFeatureOptions', () {
        ...
        expect(reconstructed.selectedFeatureOptions['fighter-fighting-style-1'],
            equals(['defense']));
      });
    });
    ...
    group('2. Ruleset-Aware Subclass Selection Matrix', () {
      test(
          '2014 Ruleset Subclass Milestones: Cleric, Sorcerer, Warlock at Level 1, Druid/Wizard at Level 2, Fighter at Level 3',
          () {
        ...
        expect(
            CharacterProgressionEngine.isSubclassMilestone('warlock', 1,
                ruleset: RulesetVersion.v2014),
            isTrue);
        expect(
            CharacterProgressionEngine.isSubclassMilestone('warlock', 2,
                ruleset: RulesetVersion.v2014),
            isFalse);
        expect(
            CharacterProgressionEngine.isSubclassMilestone('druid', 2,
                ruleset: RulesetVersion.v2014),
            isTrue);
        expect(
            CharacterProgressionEngine.isSubclassMilestone('fighter', 3,
                ruleset: RulesetVersion.v2014),
            isTrue);
      });

      test(
          '2024 Ruleset Subclass Milestones: Standardized across all classes at Level 3',
          () {
        const cleric = SrdClassesLibrary.cleric;
        const sorcerer = SrdClassesLibrary.sorcerer;
        final warlock = SrdClassesLibrary.warlock;
        ...
        expect(
            ...
            isFalse);
        expect(
            CharacterProgressionEngine.isSubclassMilestone('warlock', 3,
                ruleset: RulesetVersion.v2024),
            isTrue);
        expect(
            CharacterProgressionEngine.isSubclassMilestone('cleric', 1,
                ruleset: RulesetVersion.v2024),
            isFalse);
        expect(
            CharacterProgressionEngine.isSubclassMilestone('cleric', 3,
                ruleset: RulesetVersion.v2024),
            isTrue);
      });
    });
    ...
    group('3. 2024 Level 1 Class Decisions Engine', () {
      test(
          '2024 Cleric presents Divine Order choices (Protector vs Thaumaturge)',
          () {
        const cleric = SrdClassesLibrary.cleric;
        final decisions =
            cleric.getDecisionsForLevel(1, ruleset: RulesetVersion.v2024);

        expect(decisions, isNotEmpty);
        final orderDecision = decisions
            .firstWhere((d) => d.type == FeatureChoiceType.divineOrder);
        expect(orderDecision.availableOptions.map((o) => o.id),
            containsAll(['protector', 'thaumaturge']));
      });

      test('2024 Druid presents Primal Order choices (Magician vs Warden)', () {
        const druid = SrdClassesLibrary.druid;
        final decisions =
            druid.getDecisionsForLevel(1, ruleset: RulesetVersion.v2024);

        expect(decisions, isNotEmpty);
        final orderDecision = decisions
            .firstWhere((d) => d.type == FeatureChoiceType.primalOrder);
        expect(orderDecision.availableOptions.map((o) => o.id),
            containsAll(['magician', 'warden']));
      });

      test(
          '2024 Warlock presents Eldritch Invocations / Pact Boon choices at Level 1',
          () {
        final warlock = SrdClassesLibrary.warlock;
        final decisions =
            warlock.getDecisionsForLevel(1, ruleset: RulesetVersion.v2024);

        expect(decisions, isNotEmpty);
        final invocationDecision = decisions
            .firstWhere((d) => d.type == FeatureChoiceType.invocations);
        expect(
            invocationDecision.availableOptions.map((o) => o.id),
            containsAll([
              'pact_of_the_blade',
              'pact_of_the_tome',
              'pact_of_the_chain',
      ...
      test('Fighter presents Fighting Style at Level 1 in both 2014 and 2024',
        ...
        const fighter = SrdClassesLibrary.fighter;
        final decisions2024 =
            fighter.getDecisionsForLevel(1, ruleset: RulesetVersion.v2024);
        final decisions2014 =
            fighter.getDecisionsForLevel(1, ruleset: RulesetVersion.v2014);

        expect(decisions2024, isNotEmpty);
        expect(decisions2014, isNotEmpty);
        expect(decisions2024.first.availableOptions.map((o) => o.id),
            contains('defense'));
      });
    });
    ...
    group('4. Homebrew Ingestion & ACL Normalization', () {
      test(
          'Community compendium parser extracts custom subclass selection level and feature decisions',
          () {
          ...
          'featureDecisions': [
            {
              ...
              'availableOptions': [
                {
                  'id': 'order-of-the-silver-shield',
                  'name': 'Order of the Silver Shield',
                  'descriptionMarkdown': 'Shield guardian'
                },
                {
                  'id': 'order-of-the-moon-warden',
                  'name': 'Order of the Moon Warden',
                  'descriptionMarkdown': 'Moon guardian'
                }
              ]
            }
          ]
        ...
        expect(
            parsed.featureDecisions.first.availableOptions.length, equals(2));
      });

      test(
          'Compendium ingestion pipeline normalizes custom classes with bespoke level 1 choices',
          () {
              ...
              'availableOptions': [
                {
                  'id': 'telekinesis',
                  'name': 'Telekinesis',
                  'descriptionMarkdown': 'Move objects with your mind'
                },
                {
                  'id': 'psychokinesis',
                  'name': 'Psychokinesis',
                  'descriptionMarkdown': 'Energy projection'
                }
              ]
            }
          ]
        ...
        expect(psion.featureDecisions.first.availableOptions.first.id,
            equals('telekinesis'));
      });
    });
    ...
    group('5. Mechanical Grants & Live Character Sheet Evaluation', () {
      test('Defense Fighting Style grants +1 AC only when armor is equipped',
        ...
        final unarmoredStats =
            CharacterStatCalculator.compute(characterWithoutArmor, resolver);
        ...
        final armoredStats =
            CharacterStatCalculator.compute(characterWithArmor, resolver);
        // Armored: 11 (Leather) + 2 (DEX) + 1 (Defense style) = 14
        expect(armoredStats.armorClass, equals(14));
        expect(armoredStats.activeBuffNotes,
            contains('Defense Fighting Style: +1 AC'));
      });
      ...
      test(
        ...
        expect(fighterSlice.selectedFeatureOptions['fighter-fighting-style-1'],
            equals(['defense']));
      });
    });
  });
