...
void main() {
  ...
  group('Dual-Ruleset Divergence: Multiclass Spellcasting Math', () {
    test(
        'Paladin 3 / Sorcerer 4 spell slot progression divergence (2014 vs 2024)',
        () {
    ...
    test(
        'Paladin 1 / Sorcerer 4 multiclass: 2014 gives ECL 4 while 2024 gives ECL 5',
        () {
      final ecl2014 = MulticlassSlotMatrix.calculateEffectiveCasterLevel(
        fullCasterLevels: 4,
        paladinLevels: 1,
    ...
    test(
        'Warlock Pact Magic slots remain completely separate from multiclass pool',
        () {
      const classes = [
      ...
      final pool = CharacterProgressionEngine.computeSpellSlots(classes,
          edition: DmRulesEdition.v2014);
  ...
  group('Retroactive HP Scaling & Constitution Updates', () {
    test(
      ...
      final statsL16 =
          CharacterStatCalculator.compute(fighterWithCon16, resolver);
      // Level 1: 10 + 3 = 13. Levels 2-5: 4 * (6 + 3) = 36. Total = 49 (+5 HP).
      expect(statsL16.maxHp, 49);
    });

    test(
        'Equipping and attuning Amulet of Health (CON 19 = +4) dynamically scales HP',
        () {
    ...
    test('Tough feat correctly adds 2 HP per total level', () {
      ...
      final repo = LayeredPriorityRepository();
      final stats =
          CharacterStatCalculator.compute(hero, ReferenceResolver(repo));
  ...
  group('Exhaustion & Status Mechanics (2014 vs 2024)', () {
    test(
        '2014 discrete exhaustion tiers evaluate speed, HP, and disadvantage correctly',
        () {
    ...
    test(
        '2024 cumulative exhaustion inflicts -2 per stack to d20 tests and -5 ft speed',
        () {
    ...
    test(
      ...
      const character = Character(
        ...
        conditions: [
          CharacterCondition(
              conditionName: 'exhaustion', parameters: {'level': 2}),
        ],
      ...
      final repo = LayeredPriorityRepository();
      final stats =
          CharacterStatCalculator.compute(character, ReferenceResolver(repo));
  ...
  group('Derived DCs, Weapon Masteries & Dynamic Encumbrance', () {
    test(
        'Grapple & Shove calculates contested check for 2014 vs Save DC for 2024',
        () {
      const s2014 = Ruleset2014Strategy();
      final g2014 = s2014.calculateGrappleShoveDc(
          strengthModifier: 3, dexterityModifier: 1, proficiencyBonus: 2);
      expect(g2014.dc, isNull);
      expect(g2014.formulaDescription, contains('Contested Athletics (+5)'));

      const s2024 = Ruleset2024Strategy();
      final g2024 = s2024.calculateGrappleShoveDc(
          strengthModifier: 3, dexterityModifier: 1, proficiencyBonus: 2);
      // 8 + PB(2) + STR(3) = DC 13
      expect(g2024.dc, 13);
    ...
    test(
        '2024 Weapon Mastery validation succeeds for eligible classes and matching property',
        () {
      const s2024 = Ruleset2024Strategy();
      ...
      expect(
          s2024.canUseWeaponMastery(
              character: fighterChar,
              weapon: greatsword,
              mastery: WeaponMasteryProperty.graze),
          true);
      expect(
          s2024.canUseWeaponMastery(
              character: fighterChar,
    ...
    test(
        'Dynamic Encumbrance calculates weight, capacities, and variant encumbrance speed penalties',
        () {
      const strategy = Ruleset2014Strategy();
  ...
  group('Schema Hydration, State Immutability & Controller Persistence', () {
    ...
    test(
        'Legacy character map without rulesEdition defaults gracefully to v2014',
        () {
      final legacyMap = {
