...
import 'package:dangerously_nerdy_5e_toolkit/infrastructure/modules/dnd5e/dnd5e_ruleset_module.dart';

void main() {
  group('Breastplate and Standard Armor Disambiguation Tests', () {
    test(
        'SrdEquipmentLibrary resolves Breastplate with baseAc 14, medium armor, maxDexBonus 2',
        () {
      ...
      expect(breastplate.customProperties['defaultSlot'],
          equals(EquipmentSlot.armor));
    });

    test(
        'SrdEquipmentLibrary resolves Breastplate +1 with baseAc 14, medium armor, and +1 AC bonus',
        () {
      final all = SrdEquipmentLibrary.allEquipmentItems;
      final breastplatePlus1 =
          all.firstWhere((i) => i.name == 'Breastplate +1');
    ...
    test(
        'SrdEquipmentLibrary resolves Half Plate Armor with baseAc 15 and Plate Armor with baseAc 18',
        () {
      final all = SrdEquipmentLibrary.allEquipmentItems;
      final halfPlate = all.firstWhere((i) => i.name == 'Half Plate Armor');
      final fullPlate =
          all.firstWhere((i) => i.name == 'Plate Armor (Full Plate)');
    ...
    test(
        'Dnd5eRulesetModule.resolveStandardArmor resolves breastplate, half plate, plate, and leather correctly',
        () {
      final bp =
          Dnd5eRulesetModule.resolveStandardArmor('Breastplate', 'breastplate');
      expect(bp.baseAc, equals(14));
      expect(bp.armorType, equals('medium'));
      expect(bp.maxDex, equals(2));

      final hp =
          Dnd5eRulesetModule.resolveStandardArmor('Half Plate', 'half-plate');
      expect(hp.baseAc, equals(15));
      expect(hp.armorType, equals('medium'));
      expect(hp.maxDex, equals(2));

      final pl =
          Dnd5eRulesetModule.resolveStandardArmor('Plate Armor', 'plate-armor');
      ...
      final leatherLinedBp = Dnd5eRulesetModule.resolveStandardArmor(
          'Leather-Lined Breastplate', 'leather-lined-breastplate');
      expect(leatherLinedBp.baseAc, equals(14));
      expect(leatherLinedBp.armorType, equals('medium'));

      final studded = Dnd5eRulesetModule.resolveStandardArmor(
          'Studded Leather', 'studded-leather');
      expect(studded.baseAc, equals(12));
      expect(studded.armorType, equals('light'));

      final plainLeather = Dnd5eRulesetModule.resolveStandardArmor(
    ...
    test(
      ...
      expect(stats.armorClassBreakdown,
          contains('14 (Breastplate) + 2 DEX (capped at 2)'));
    ...
    test('CharacterStatCalculator calculates AC for Breastplate +1', () {
      final breastplatePlus1 = SrdEquipmentLibrary.allEquipmentItems
          .firstWhere((i) => i.name == 'Breastplate +1');
      ...
      final layer = PriorityLayer(
          layerId: 'test', name: 'Test', priority: LayerPriority.baseRuleset)
    ...
    test(
        'CommunityCompendiumAdapters parses medium armor breastplate JSON with correct AC and type',
        () {
    ...
    test(
      ...
      expect(InventoryTransactionService.resolveDefaultSlot(lightXbow),
          equals(EquipmentSlot.twoHand));
    });
  });
}
