...
void main() {
  group('CharacterActionsResolver Class Features Translation', () {
    test(
      ...
      final bonusActionNames =
          resolved.bonusActions.map((a) => a.name).toList();
    ...
    test(
      ...
      expect(
          bonusActionNames, contains('Channel Divinity: Harness Divine Power'));
    ...
    test('Druid Moon Druid gets Combat Wild Shape as Bonus Action and healing',
      ...
      expect(
          bonusActionNames, contains('Combat Wild Shape: Spell Slot Healing'));
    ...
    test(
      ...
      final sneakAtk =
          resolved.specialActions.firstWhere((a) => a.name == 'Sneak Attack');
    ...
    test(
      ...
      expect(specialNames, contains('Arcane Ward'));
    });
  ...
  group('Homebrew Parsing Robustness & Action Economy Extraction', () {
    test(
        'Header-based homebrew features (### Name (Level X)) translate correctly and obey level gating',
        () {
...
''';
      ...
      final bloodSurge =
          resolved.bonusActions.firstWhere((a) => a.name == 'Blood Surge');
      expect(bloodSurge.damageFormula, equals('2d6'));
      expect(bloodSurge.damageType, equals(DamageType.necrotic));
    });

    test(
        'Bold lead-in homebrew features (**Name.** Description) translate correctly',
        () {
      const boldMarkdown = '''
...
''';
      ...
      final spiritBurst =
          resolved.actions.firstWhere((a) => a.name == 'Spirit Burst');
...
''';
      ...
      final kd = resolved.bonusActions
          .firstWhere((a) => a.name == 'Kinetic Discharge');
    ...
    test('Community Compendium rawJson structure translates features properly',
            ...
            'classFeatures': [
              {
                ...
                'level': 1,
                'entries': [
                  'As an action, focus mental force to blast a target with 1d8 psychic damage.'
                ],
              },
              {
                'name': 'Thought Shield',
                'level': 2,
                'entries': [
                  'When targeted by an attack, you can use your reaction to raise a psychic barrier.'
                ],
              },
              {
                'name': 'Psionic Leap',
      ...
      final thrust =
          resolved.actions.firstWhere((a) => a.name == 'Psychic Thrust');
