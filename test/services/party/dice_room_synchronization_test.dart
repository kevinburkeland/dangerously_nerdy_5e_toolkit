...
void main() {
  ...
  group('RoomRoll Serialization & Firestore Rule Safety', () {
    test(
        'toMap omits null details and null droppedRolls to prevent Firestore permission-denied',
        () {
    ...
    test(
        'toMap includes details and droppedRolls only when populated and clamps bounds',
        () {
      final roll = RoomRoll(
        id: 'roll-456',
        roomCode: 'ROOM-BETA',
    ...
    test(
        'fromMap parses various timestamp types safely (String, Timestamp, int, DateTime)',
        () {
      ...
      expect(fromString.timestamp.difference(now).inSeconds.abs(),
          lessThanOrEqualTo(1));
      ...
      expect(
          fromInt.timestamp.millisecondsSinceEpoch, now.millisecondsSinceEpoch);
  ...
  group('DiceRoomService Unified Stream & Ingestion Tests', () {
    ...
    test(
        'ingestRemoteRoll pushes remote roll to subscribers and dedupes entries',
        () async {
      final service = DiceRoomService();
    ...
    test(
        'broadcastRoll forwards to IP2pTransportPort when registered in Service Locator',
        () async {
      ...
      final decoded =
          jsonDecode(mockTransport.broadcasted.first) as Map<String, dynamic>;
      expect(decoded['type'], 'dice_roll');
      expect(decoded['roomCode'], 'ROOM-MESH-3');
      expect(decoded['payload']['id'], 'mesh-1');
      expect(decoded['payload']['playerName'], 'Valen');
  ...
  group('RoomSyncOrchestrator P2P Dice Roll Routing', () {
    test(
        'receives type == "dice_roll" payload and ingests into DiceRoomService',
        () async {
      final mockTransport = MockP2pTransport();
      ...
      expect(emissions.any((list) => list.any((r) => r.id == 'orch-roll-99')),
          isTrue);
