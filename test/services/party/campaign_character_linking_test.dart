...
void main() {
  ...
  group('Campaign Character Linking & Vault Payout Synchronization', () {
    ...
    test('Player links character by importing as a new character on join',
        () async {
      ...
      expect(joined.sharedCharacters['hero_valeros']?['name'],
          equals('Valeros the Fighter'));
    ...
    test('Player links character to an existing campaign roster slot',
        () async {
      final session = await partyService.createCampaign(
      ...
      expect(linked.sharedCharacters['hero_seward']?['name'],
          equals('Dr. Seward'));
      expect(linked.sharedCharacters['Fighter Slot']?['name'],
          equals('Dr. Seward'));
    ...
    test('Coin dispersal automatically reflects on linked character purse',
        () async {
      final session = await partyService.createCampaign(
      ...
      final valeros =
          updatedChars.firstWhere((c) => c.id.slug == 'hero_valeros');
    ...
    test('Claiming a loot item adds it to linked character inventory',
        () async {
      final session = await partyService.createCampaign(
      ...
      expect(claimedItem.notes,
          equals('A glowing blade of pure radiant solar energy.'));
      ...
      final valerosAfterUnclaim =
          unclaimChars.firstWhere((c) => c.id.slug == 'hero_valeros');
      expect(valerosAfterUnclaim.inventory.isEmpty, isTrue);
    });

    test('Direct member purse update synchronizes with character purse',
        () async {
      final session = await partyService.createCampaign(
