...
Character _createHeroWithPurse(
    {required String id, required String name, required PartyPurse purse}) {
  return Character(
...
void main() {
  ...
  group('Money Synchronization Tests', () {
    test('CharacterSheetController modifies purse and persists changes',
        () async {
    ...
    test(
        'PartyRoomService.linkCharacterToCampaign initializes memberPurses from character.purse',
        () async {
      final hero = _createHeroWithPurse(
        id: 'hero_sylas',
        name: 'Sylas',
      ...
      final session = await partyService.linkCharacterToCampaign(
        roomCode: 'LOTHO1',
        character: hero,
        existingRosterName: 'Sylas',
      );

      // Verify session memberPurse has the character's exact money
      final memberPurse = session.getMemberPurse('Sylas');
    ...
    test(
        'DmDashboardController separates shared party treasury from individual character purses',
        () async {
      ...
      expect(controller.partyCharactersMap['hero_valen']?.purse.gp, equals(40));
      expect(
          controller.partyCharactersMap['hero_boromir']?.purse.gp, equals(80));
      ...
      await controller.modifyCharacterPurseCoin('hero_valen', 'gp', 10);
      expect(controller.partyCharactersMap['hero_valen']?.purse.gp, equals(50));
      expect(controller.partyPurse.gp, equals(550));

      final savedAragorn = await persistence.getCharacter('hero_valen');
      expect(savedAragorn?.purse.gp, equals(50));
    });

    testWidgets(
        'DM Dashboard UI displays both character purse and shared treasury',
        (tester) async {
      ...
      expect(find.text('Corin Hilltopper'), findsOneWidget);
      ...
      final updatedHero =
          await CharacterPersistenceService().getCharacter('hero_frodo');
