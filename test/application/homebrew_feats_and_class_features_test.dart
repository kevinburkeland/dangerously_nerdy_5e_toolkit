...
void main() {
  group('Homebrew Feats & Class Features Directives (SRD Compliant)', () {
    test('Feat parser extracts armor and tool proficiencies from homebrew JSON',
      ...
      final toolGrants = feat.grants
          .where((g) =>
              g.payload['tool'] != null || g.payload['proficiency'] != null)
          .toList();
      expect(
          toolGrants.any((g) =>
              (g.payload['tool'] ?? g.payload['proficiency']) ==
              "Smith's Tools"),
          isTrue);
    ...
    test(
        'HomebrewEntityDto recognizes armor, tool, and weapon proficiencies in feats',
        () {
    ...
    test(
        'SkillTraitResolver resolves armor and weapon proficiencies across feats',
        () {
    ...
    test(
        'CharacterSheetController.addFeat grants tools and armor/weapon proficiencies',
        () async {
      final testFeat = Feat(
        id: const EntityId(
            slug: 'ironclad-artisan', ruleset: RulesetVersion.v2014),
        name: 'Ironclad Artisan',
        descriptionMarkdown: 'Grants medium armor and artisan tools.',
        grants: [
          FeatureGrant.armorProficiency('Medium Armor',
              grantId: 'artisan-armor-1'),
          FeatureGrant.armorProficiency('Shields', grantId: 'artisan-armor-2'),
          FeatureGrant.bonusTool("Brewer's Supplies",
              grantId: 'artisan-tool-1'),
        ],
        customProperties: {
          'armorProficiencies': ['Medium Armor', 'Shields'],
      ...
      expect(
          controller.character.toolProficiencies.contains("Brewer's Supplies"),
          isTrue);
      expect(controller.character.toolProficiencies.contains("Cook's Utensils"),
          isTrue);

      // Armor proficiencies should now include Medium Armor and Shields
      final armors =
          controller.character.customProperties['armorProficiencies'] as List;
    ...
    test(
        'Homebrew class with saving throws and restricted skill choices does NOT default to all 18 skills',
        () {
      final rawClassJson = {
        ...
        'proficiency': [
          'con',
          'int'
        ], // Saving throws in community compendium format!
      ...
      expect(
          parsedClass.allowedSkills.contains(SkillType.investigation), isTrue);
      expect(parsedClass.allowedSkills.contains(SkillType.acrobatics), isFalse);
    });

    test(
        'CharacterActionsResolver parses pipe-delimited feature strings into combat actions',
        () {
      ...
      const customClass = CharacterClass(
        ...
        hitDie: 'd8',
        featuresMarkdown:
            'Helpful|Sidekick Expert|CUSTOM|1\nCunning Action|Sidekick Expert|CUSTOM|2\nReliable Talent|Sidekick Expert|CUSTOM|11',
      );
      ...
      final helpfulAction = resolvedWithCustom.bonusActions
          .where((a) => a.name.toLowerCase() == 'helpful')
          .firstOrNull;
      expect(helpfulAction, isNotNull,
          reason: 'Helpful should be resolved as a bonus action');
      expect(helpfulAction?.description,
          contains('Help action as a bonus action'));

      // Cunning Action should also be resolved as a Bonus Action at level 2+
      final cunningAction = resolvedWithCustom.bonusActions
          .where((a) => a.name.toLowerCase() == 'cunning action')
          .firstOrNull;
      expect(cunningAction, isNotNull,
          reason: 'Cunning Action should be resolved at level 3');

      // Reliable Talent requires Level 11, so at Level 3 it should be skipped
      final reliableTalent = resolvedWithCustom.allActions
          .where((a) => a.name.toLowerCase() == 'reliable talent')
          .firstOrNull;
      expect(reliableTalent, isNull,
          reason: 'Reliable Talent should be level-gated at level 3');
    });
  });
}
