...
void main() {
  group('CombatEncounterService Tests', () {
    ...
    test('applyCharacterDamage absorbs temp HP first', () async {
      ...
      expect(
          charRepo.storage[testChar.id.slug]?.resources.currentHp, equals(17));
    });

    test('applyCharacterDamage absorbs partial temp HP when damage < tempHp',
        () async {
    ...
    test('applyCharacterHealing clamps to effectiveMaxHp', () async {
      ...
      expect(
          charRepo.storage[testChar.id.slug]?.resources.currentHp, equals(25));
    });

    test(
        'modifyCharacterHp routes negative delta to damage and positive to healing',
        () async {
    ...
    test('modifyMinionHp reduces and heals minion HP safely', () async {
      ...
      final profile =
          CampaignProfile.defaultProfile(nodeId: 'test_node').copyWith(
    ...
    test(
        'modifyMinionHp returns a new instance of MinionInstance leaving original instance unmodified',
        () async {
      final originalMinion = MinionInstance(
    ...
    test(
        'applyCharacterHealing handles unconscious vs dead characters per 5e RAW',
        () async {
    ...
    test(
      ...
      final profile = CampaignProfile.defaultProfile(
          id: 'camp_clock_test', nodeId: 'test_node');
      ...
      final encounterItem =
          updatedProfile.roomState.activeEncounter.items['p1'];
      ...
      final tombstone =
          removedProfile.roomState.activeEncounter.tombstones['p1'];
      expect(tombstone, isNotNull);
      expect(tombstone!.physicalTime, equals(fixedNetworkTime));
      expect(tombstone.logicalCounter,
          greaterThan(encounterItem.timestamp.logicalCounter));
    });
  });
}
