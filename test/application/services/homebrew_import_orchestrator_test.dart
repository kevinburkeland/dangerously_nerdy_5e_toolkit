...
void main() {
  group('HomebrewImportOrchestrator CRDT Integration Tests', () {
    final source =
        GithubRepoSource.parse('https://github.com/my-group/campaign-homebrew');

    test(
        'Coordinates manifest discovery, ACL filtering, and commits to CRDT ledger',
        () async {
      ...
      expect(
          finalTelemetry.filesSkipped, equals(1)); // vorpal_dagger with mastery
    ...
    test(
        'instantiated with nodeId: null generates matching dynamic nodeId for orchestrator and HLC stamp',
        () async {
      final source =
          GithubRepoSource.parse('https://github.com/dnd/homebrew_dynamic');
      final mockClient = MockHttpFetchClient({
    ...
    test(
        'batchPersister receives flushed batches of entities upon stream completion',
        () async {
      final mockClient = MockHttpFetchClient({
        source.apiTreeUri.toString(): jsonEncode({
          'tree': [
    ...
    test(
        'batchPersister flushes multiple batches when entities exceed batchSize',
        () async {
      final mockClient = MockHttpFetchClient({
        source.apiTreeUri.toString(): jsonEncode({
          'tree': [
