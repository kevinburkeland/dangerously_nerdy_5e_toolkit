...
class MockCampaignRepository implements ICampaignRepository {
  ...
  List<CampaignProfile> get allProfiles =>
      _activeProfile != null ? [_activeProfile!] : [];
...
class ThrowingReconciliationService extends RoomStateReconciliationService {
  ThrowingReconciliationService() : super(networkTimeProvider: () => 0);
  ...
  CampaignProfile reconcileProfile({
    ...
    throw StateError('Simulated reconciliation failure');
  }
}
...
void main() {
  ...
  group('RoomSyncOrchestrator Tests', () {
    ...
    test(
        'Echo Cancellation Test: inbound save does not echo back to transport mesh',
        () async {
    ...
    test(
        'Outbound Sync Test: genuine local UI mutation triggers mesh broadcast',
        () async {
      orchestrator.startSynchronization();
      ...
      final broadcasted = jsonDecode(mockWebRtc.broadcastedPayloads.first)
          as Map<String, dynamic>;
      expect(broadcasted['type'], equals('room_sync_full'));
      expect(broadcasted['payload'], isNotNull);
      expect(broadcasted['payload']['name'],
          equals('Locally Mutated Campaign Title'));
      // Clock offset (500ms) should be reflected in timestamp
      expect(broadcasted['timestamp'], greaterThan(1700000000000));
    });

    test(
        'Microtask Lock Release Test: mutex unlocks after inbound cycle allowing subsequent broadcasts',
        () async {
      mockRepo.emitOnSaveImmediate = true;
      orchestrator.startSynchronization();
      ...
      final userEditProfile =
          remoteProfile.copyWith(name: 'Subsequent Local User Edit');
      mockRepo.emitProfile(userEditProfile);

      await Future<void>.delayed(Duration.zero);

      expect(mockWebRtc.broadcastedPayloads.length, equals(1));
      final decoded = jsonDecode(mockWebRtc.broadcastedPayloads.first)
          as Map<String, dynamic>;
      expect(decoded['payload']['name'], equals('Subsequent Local User Edit'));
    });

    test(
        'CvRDT Delta Merge Test: crdt_or_set_delta reconciles active values and tombstones',
        () async {
      orchestrator.startSynchronization();

      // Setup local tracked CRDT set with item 'grapple'
      const ts1 = HybridLogicalClock(
          physicalTime: 1000, logicalCounter: 0, nodeId: 'nodeA');
      const ts2 = HybridLogicalClock(
          physicalTime: 2000, logicalCounter: 0, nodeId: 'nodeB');

      orchestrator.trackedRulesSet =
          const CrdtOrSet<String>.empty().add('grapple', 'grapple', ts1);
      ...
      expect(
          orchestrator.trackedRulesSet.items.containsKey('grapple'), isFalse);
      expect(orchestrator.trackedRulesSet.tombstones.containsKey('grapple'),
          isTrue);
    ...
    test(
        'Telemetry Stream Test: watchTelemetry emits periodic telemetry reflecting router state and peerCount',
        () async {
      ...
      expect(
          emittedTelemetry.first.state, equals(TransportState.webRtc));
    ...
    test(
        'Teardown & Cleanup Test: stopSynchronization releases subscriptions and timers cleanly',
        () {
      orchestrator.startSynchronization();
    ...
    test(
        'TransportPort fallback test: generic IP2pTransportPort defaults to connecting and peerCount 0',
        () async {
      ...
      expect(genericOrchestrator.telemetryInterval,
          equals(const Duration(milliseconds: 20)));

      final telemetrySnapshots = <RoomConnectionTelemetry>[];
      final sub =
          genericOrchestrator.watchTelemetry().listen(telemetrySnapshots.add);
    ...
    test(
        'Interface Polymorphism Verification: generic mock IP2pTransportPort directly drives currentTelemetry',
        () {
      final genericMock = MockTransportPort()
    ...
    test(
        'Deadlock Prevention Test: broadcast failure steps down to fallbackRelay while fallback adapter emits inbound packet without deadlock',
        () async {
    ...
    test(
        'Causality Tracking & Offline Edits: valid offline edits older than 30s reconnect and reconcile cleanly while duplicate/stale sequence packets are dropped',
        () async {
      orchestrator.startSynchronization();
      ...
      expect(mockRepo.savedImmediateProfiles.last.partyCharacterIds,
          contains('char-offline-reconnect'));
      ...
      expect(mockRepo.savedImmediateProfiles.last.pinnedRuleIds,
    ...
    test(
        'LRU Cache Eviction: caps at 500 entries and evicts least recently used',
        () async {
      orchestrator.startSynchronization();
      const localTime = 1700000000500;
    ...
    test(
      ...
      final recentTombstoneWithinLookback = HybridLogicalClock(
          physicalTime: now - 5000, logicalCounter: 0, nodeId: 'dm-host-1');
    ...
    test(
        'Concurrent Conflict Resolution: concurrent edits to notes and purse merge without wholesale overwrite',
        () async {
    ...
    test(
        'Causality Tracking: drops self-broadcast packets matching origin_node_id',
        () async {
    ...
    test(
        'LRU Poisoning Prevention: Unknown or malformed payloads are discarded without polluting LRU cache',
        () async {
    ...
    test(
        'Dead Letter Routing: Schema mismatches and reconciliation errors route to deadLetterStream',
        () async {
      ...
      final sub = testOrchestrator.deadLetterStream.listen((event) {
        receivedError = event;
      });
      addTearDown(sub.cancel);
      addTearDown(testOrchestrator.dispose);

      final remoteDto = CampaignProfileDto.fromDomain(initialProfile);
      final inboundJson = jsonEncode({
        'type': 'room_sync_full',
        'origin_node_id': 'remote-peer-error',
        'origin_seq': 1,
      ...
      try {
        await testOrchestrator.handleIncomingPayload(inboundJson);
      } catch (_) {}

      expect(receivedError, isNotNull);
      expect(receivedError!.error, isA<StateError>());
    });
    ...
    test(
        'Dead Letter Routing: malformed HLC payload without nodeId routes to deadLetterStream',
        () async {
      final throwingReconciliation = _ThrowingHlcReconciliationService();
      final testOrchestrator = RoomSyncOrchestrator(
        transportPort: mockWebRtc,
        campaignRepo: mockRepo,
      ...
      expect(receivedError, isNotNull);
      expect(receivedError!.error, isA<FormatException>());
      expect(
        (receivedError!.error as FormatException).message,
        equals('Missing required nodeId in HLC payload'),
      );
    });
  });
}

class _ThrowingHlcReconciliationService extends RoomStateReconciliationService {
  _ThrowingHlcReconciliationService() : super(networkTimeProvider: () => 0);
  ...
  CampaignProfile reconcileProfile({
    ...
    // Malformed HLC missing node and nodeId triggers FormatException
    HybridLogicalClock.fromMap({'pt': 1700000000000, 'lc': 0});
    return local;
  }
}
