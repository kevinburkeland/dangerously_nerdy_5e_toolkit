...
class FakeCampaignRepository implements ICampaignRepository {
  ...
  @override
  CampaignProfile? get activeProfile => _activeId != null ? profiles[_activeId] : null;
...
void main() {
  ...
  group('StorageDurabilityCoordinator Lifecycle & Preflight', () {
    ...
    test('Silent preflight triggers requestPersistence on WebKit Standalone PWA', () async {
      final storagePort = FakeStorageDurabilityPort(
        currentProfile: const EngineProfile(
    ...
    test('Silent preflight suppresses prompt on Firefox Desktop and sets requiresContextualPrompt', () async {
      final storagePort = FakeStorageDurabilityPort(
        currentProfile: const EngineProfile(
  ...
  group('StorageDurabilityCoordinator Cold Storage Export & Hydration', () {
    test('Exports campaign profile into verified cold storage snapshot', () async {
      final storagePort = FakeStorageDurabilityPort(
        currentProfile: const EngineProfile(
          engine: BrowserEngine.chromium,
          os: PlatformOs.macos,
      ...
      expect(snapshotPort.exportedFileName, contains('vault_camp_frost_haven_'));
