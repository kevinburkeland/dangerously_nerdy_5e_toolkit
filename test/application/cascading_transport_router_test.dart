...
void main() {
  group('CascadingTransportRouter 4-Tier Waterfall Verification', () {
    ...
    test(
        'Tier 1: Initializes Local Wi-Fi first and enters localWifi state (Zero Cost)',
        () async {
    ...
    test(
        'Tier 3: Steps down to Firebase relay when both Local Wi-Fi and WebRTC fail',
        () async {
      mockLocalWifi.initializeShouldThrow = true;
    ...
    test('Tier 4: Enters offline state when all 3 adapters fail initialization',
        () async {
      mockLocalWifi.initializeShouldThrow = true;
      mockWebRtc.initializeShouldThrow = true;
    ...
    group('Signaling Lifecycle Verification', () {
      test(
          'Wipes ephemeral signaling documents upon WebRTC connection established',
          () async {
        ...
        await signalingAdapter.initialize(
            roomCode: 'ROOM-SIG', localNodeId: 'local-node');
        final offerId = await signalingAdapter.sendOffer(
            toNodeId: 'remote-node', sdp: 'fake-sdp');
        expect(signalingAdapter.trackedDocPaths,
            contains('rooms/ROOM-SIG/nodes/remote-node/signals/$offerId'));
        expect(deletedDocPaths, isEmpty);

        final realWebRtcAdapter =
            WebRtcMeshAdapter(signalingAdapter: signalingAdapter);
        ...
        expect(deletedDocPaths,
            contains('rooms/ROOM-SIG/nodes/remote-node/signals/$offerId'));
        expect(
            signalingAdapter.trackedDocPaths,
            isNot(
                contains('rooms/ROOM-SIG/nodes/remote-node/signals/$offerId')));
    ...
    group('Failover Step-Down & Payload Routing Test', () {
      test(
          'Relays incoming payloads from active adapter and filters protocol heartbeats',
          () async {
        ...
        mockWebRtc
            .emitPayload('{"_protocol":"heartbeat_ping","from":"peer-2"}');
      ...
      test(
          'Broadcast failure on WebRTC steps down to Firebase Relay after reaching sequential failure threshold',
          () async {
      ...
      test(
          'Transient broadcast failure followed by success resets consecutive failure count without step down',
          () async {
        mockLocalWifi.initializeShouldThrow = true;
      ...
      test(
          'Broadcast failure on Firebase relay steps down to total offline after reaching threshold',
          () async {
        mockLocalWifi.initializeShouldThrow = true;
        mockWebRtc.initializeShouldThrow = true;
        await router.initializeRoom('ROOM-1234', 'node-player-1');
    ...
    group('Zombie Node Pruning & Heartbeat Cascade Test', () {
      test(
          'Prunes dead peer nodes exceeding TTL and cascades waterfall when all peers expire',
          () async {
        ...
        testRouter.recordPeerHeartbeat('peer-fighter',
            timestamp: initialTime.millisecondsSinceEpoch);
        testRouter.recordPeerHeartbeat('peer-wizard',
            timestamp: initialTime.millisecondsSinceEpoch);
        ...
        testRouter.recordPeerHeartbeat('peer-wizard',
            timestamp: at5Seconds.millisecondsSinceEpoch);
      ...
      test(
          'Synchronizes peerLastSeen from active adapter preventing premature eviction during silent ping/pongs',
          () async {
        ...
        router.recordPeerHeartbeat('peer-silent',
            timestamp: initialTime.millisecondsSinceEpoch);
        expect(router.peerLastSeen['peer-silent'],
        ...
        expect(router.peerLastSeen['peer-silent'],
            at5Seconds.millisecondsSinceEpoch);
        expect(router.currentState, TransportState.webRtc);
      ...
      test(
          'Default heartbeat TTL is 15 seconds to prevent premature mobile fallback',
          () {
      ...
      test(
          'Architecture Test: UI state stays locked at 1 peer when left idle for 15 seconds with silent ping/pongs',
          () async {
        ...
        final initialTime = DateTime(2026, 9, 9, 12, 0, 0);
        archRouter.recordPeerHeartbeat('peer-party-member',
            timestamp: initialTime.millisecondsSinceEpoch);
    ...
    test(
        'Repeated initializeRoom with differing room codes cancels previous heartbeat timers without accumulating duplicate callbacks',
        () async {
    ...
    test(
        'Payload events emitted to watchIncomingPayloads() arrive asynchronously on the microtask queue',
        () async {
      ...
      expect(received, isEmpty,
          reason:
              'Payload must not arrive synchronously in the same call frame');

      // Drain event queue and microtasks
      await pumpEventQueue();

      expect(received, equals(['{"msg": "async_delivery"}']),
          reason: 'Payload must arrive after microtask execution');
    ...
    test(
        'Bi-directional Waterfall Recovery: steps up from fallbackRelay to localWifi when probe succeeds',
        () async {
    ...
    test(
        'Session preparation polymorphism: invokes prepareSession on active adapter during initialization',
        () async {
    ...
    test(
        'Clean disconnect tears down all adapters, subscriptions, and clears state',
        () async {
    ...
    group('Asymmetric Deduplication Cache Tests', () {
      test('silently drops duplicate incoming payloads and updates LRU order',
          () async {
      ...
      test('bounded LRU cache evicts oldest entry when exceeding 500 items',
          () async {
      ...
      test(
          'custom injected payloadHasher is invoked to compute deduplication keys',
          () async {
        int hashCallCount = 0;
