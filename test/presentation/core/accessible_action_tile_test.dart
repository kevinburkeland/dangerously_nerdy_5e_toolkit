...
void main() {
  group('AccessibleActionTile', () {
    testWidgets('exposes proper Semantics node and narrative speech',
        (tester) async {
      var tapped = false;
      ...
      expect(
        ...
        matchesSemantics(
          label:
              'Longsword Attack. One eight-sided die plus three slashing damage',
          isButton: true,
          hasTapAction: true,
        ),
      );
    ...
    testWidgets('ensures minimum touch target dimensions >= 48x48dp',
        (tester) async {
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
      ...
      final renderBox =
          tester.renderObject<RenderBox>(find.byType(AccessibleActionTile));
      expect(renderBox.size.width, greaterThanOrEqualTo(48.0));
      expect(renderBox.size.height, greaterThanOrEqualTo(48.0));
    });

    testWidgets('renders without overflow under 2.0x dynamic type scaling',
        (tester) async {
      await tester.pumpWidget(
        MaterialApp(
          home: MediaQuery(
    ...
    testWidgets('respects disableAnimations for reduced motion preferences',
        (tester) async {
      await tester.pumpWidget(
        MaterialApp(
          home: MediaQuery(
      ...
      expect(
          singleChildScrollView.physics, isA<NeverScrollableScrollPhysics>());
    });
  });
}
