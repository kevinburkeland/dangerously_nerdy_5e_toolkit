...
void main() {
  group('StorageDurabilityBanner & StorageLockOfflineAction Widget Tests', () {
    testWidgets('Zero-Friction Invariant: completely hidden if isPersisted is true', (tester) async {
      ...
      expect(find.textContaining('Running in temporary browser mode'), findsNothing);
      expect(find.textContaining('Storage capacity is reaching critical limits'), findsNothing);

      coordinator.dispose();
    });

    testWidgets('Zero-Friction Invariant: completely hidden if standalone PWA', (tester) async {
      final port = MockStorageDurabilityPort(
        profile: const EngineProfile(
          engine: BrowserEngine.webkit,
          os: PlatformOs.ios,
    ...
    testWidgets('Condition 1 (Safari Tab Risk): renders dismissible banner and dismisses cleanly', (tester) async {
      final port = MockStorageDurabilityPort(
        profile: const EngineProfile(
          engine: BrowserEngine.webkit,
          os: PlatformOs.ios,
      ...
      expect(
        find.text('Running in temporary browser mode. Add to Home Screen to ensure campaign data is never cleared by iOS.'),
        findsOneWidget,
      );
    ...
    testWidgets('Condition 3 (Storage Pressure >= 80%): renders critical alert with export action', (tester) async {
      final port = MockStorageDurabilityPort(
        profile: const EngineProfile(
      ...
      expect(find.textContaining('Storage capacity is reaching critical limits (85% used)'), findsOneWidget);
    ...
    testWidgets('Condition 2 (Firefox Pending): StorageLockOfflineAction renders and triggers requestPersistence', (tester) async {
      final port = MockStorageDurabilityPort(
        profile: const EngineProfile(
    ...
    testWidgets('Accessibility: supports 2.0x dynamic type scaling without RenderFlex overflow', (tester) async {
