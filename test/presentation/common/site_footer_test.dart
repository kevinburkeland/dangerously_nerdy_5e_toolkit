...
void main() {
  group('Repository Governance & Licensing Verification', () {
    test(
        'LICENSE file contains full AGPL-3.0 text, breakdown header, and SRD attribution',
        () {
      ...
      expect(
          content, contains('APPLICATION SOURCE CODE & SOFTWARE: GNU AGPLv3'));
      expect(
          content,
          contains(
              'D&D 5e GAME CONTENT & RULES MECHANICS: CREATIVE COMMONS CC-BY-4.0'));
      expect(content, contains('GNU AFFERO GENERAL PUBLIC LICENSE'));
      expect(content, contains('Version 3, 19 November 2007'));
      expect(content, contains('System Reference Document 5.1 & 5.2.1'));
    });

    test(
        'CONTRIBUTING.md explicitly rejects CLA in favor of DCO and defines dual licensing scope',
        () {
      ...
      expect(
          content,
          contains(
              'we deliberately do not use a Contributor License Agreement (CLA)'));
      expect(content, contains('git commit -s'));
      expect(content, contains('inbound = outbound'));
      expect(content,
          contains('Dual Licensing Scope: AGPL-3.0 vs. SRD CC-BY-4.0'));
    ...
    test(
        'README.md includes AGPLv3 badge and clearly distinguishes AGPL software from SRD CC-BY-4.0 mechanics',
        () {
  ...
  group('SiteFooter Widget Verification', () {
    ...
    testWidgets('renders copyright, AGPLv3 button, and GitHub source button',
        (tester) async {
      ...
      expect(find.textContaining('© $currentYear Dangerously Nerdy'),
          findsOneWidget);
    ...
    testWidgets(
        'enforces strict 48x48dp minimum touch targets on interactive buttons',
        (tester) async {
      await tester.pumpWidget(
        const MaterialApp(
          home: Scaffold(
    ...
    testWidgets(
        'Semantics wrappers announce external links with full descriptive targets',
        (tester) async {
      ...
      expect(
        tester.getSemantics(find
            .bySemanticsLabel('External Link: View AGPLv3 License details')),
        matchesSemantics(
    ...
    testWidgets(
        'tapping buttons triggers url_launcher with external application mode',
        (tester) async {
      await tester.pumpWidget(
        const MaterialApp(
          home: Scaffold(
      ...
      expect(fakeUrlLauncher.launchedUrl,
          'https://www.gnu.org/licenses/agpl-3.0.html');
      expect(
          fakeUrlLauncher.launchMode, PreferredLaunchMode.externalApplication);
    ...
    testWidgets(
        'renders without layout overflow under TextScaler.linear(2.0) across viewport widths',
        (tester) async {
