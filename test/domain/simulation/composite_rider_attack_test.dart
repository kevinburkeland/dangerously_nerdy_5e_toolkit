...
void main() {
  ...
  ArenaCombatant createTarget({int tempHp = 15, int altitude = 30, bool hasHover = true}) {
  ...
  group('Composite Action Rider AST & Multi-Effect Execution Pipeline', () {
    test('PrecomputedAttack enforces riders.length <= 8 invariant', () {
      final validAttack = PrecomputedAttack(
        ...
        riders: const [
          ...
          AttributeDrainRider(targetAbility: AbilityType.strength),
          MaxHpReductionRider(),
    ...
    test('Executes Kraken/Vampire synthetic composite attack with 6 stacked riders upon hit', () {
      ...
      expect(target.getAbilitySavingThrowBonus(AbilityType.strength), equals(3));
      ...
      final compositeAttack = PrecomputedAttack(
        ...
        riders: const [
          ...
          AttributeDrainRider(
            ...
            diceCount: 0,
            flatBonus: 3, // Drains exactly 3 STR for deterministic scaling assertion
            deathAtZero: true,
          ),
          MaxHpReductionRider(
      ...
      expect(target.getAbilitySavingThrowBonus(AbilityType.strength), equals(1));
    ...
    test('Triggers instant death when Strength drops to 0 regardless of current HP', () {
    ...
    test('Triggers instant death when effectiveMaxHp drops to 0 regardless of current HP', () {
      final target = createTarget();
      expect(target.currentHp, equals(100));
      expect(target.isAlive, isTrue);
    ...
    test('StatBlockAclParser extracts all 6 stacked riders from unstructured action text', () {
      const rawAction =
      ...
      expect(drainRider.targetAbility, equals(AbilityType.strength));
