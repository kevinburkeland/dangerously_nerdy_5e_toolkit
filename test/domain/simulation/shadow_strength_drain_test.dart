...
void main() {
  group('Shadow Strength Drain & Instant Death Hardening', () {
    ...
    test('StatBlockAclParser parses Shadow Strength Drain action text with deathAtZero=true', () {
      const shadowAction =
    ...
    test('StatBlockAclParser flags deathAtZero=true on regex variations of ability score death', () {
      const variations = [
      ...
      for (final text in variations) {
        ...
        final drainRider = riders.whereType<AttributeDrainRider>().firstOrNull;
        expect(drainRider, isNotNull, reason: 'Failed to extract drain from: $text');
        expect(drainRider!.deathAtZero, isTrue, reason: 'deathAtZero should be true for: $text');
      }
    });

    test('Reducing Fire Giant Strength (25) to 0 forces currentHp = 0, isAlive = false, isDefeated = true', () {
    ...
    test('Shadow Strength Drain via applyAttackHit logs instant death when Strength reaches 0', () {
      final giant = createFireGiant();
      expect(giant.isAlive, isTrue);
      ...
      expect(result.riderLogs, anyElement(contains("STR was reduced to 0! Fire Giant dies instantly!")));
    });

    test('ArenaCombatant.isAlive immediately returns false whenever Strength is <= 0', () {
      final giant = createFireGiant();
      expect(giant.isAlive, isTrue);
