...
void main() {
  ...
  group('Wolf Knockdown Action Rider & Saving Throw Simulation Tests', () {
    test('StatBlockAclParser extracts PrecomputedAttack with ConditionRider(prone) from Wolf bite', () {
      final parsed = StatBlockAclParser.parseStatBlockBoundary(wolfStatBlock, challengeRating: 0.25);
      ...
      expect(proneRider.saveAbility, equals(AbilityType.strength));
    });

    test('MonsterCombatProfile.fromStatBlock populates attacks map seamlessly', () {
      final profile = MonsterCombatProfile.fromStatBlock(wolfStatBlock, challengeRating: 0.25);
      expect(profile.attacks.containsKey('bite'), isTrue);
      final bite = profile.attacks['bite']!;
      expect(bite.riders.any((r) => r is ConditionRider && r.condition == ArenaCondition.prone), isTrue);
    });

    test('Defender fails DC 11 STR save -> falls Prone, logs event and updates summaryText', () {
      ...
      final attackEvent = ArenaAttackEvent(
        ...
        summaryText: 'Wolf hits Defender Target with Bite for 7 piercing damage.',
        appliedRiderLogs: result.riderLogs,
      );

      expect(attackEvent.summaryText, contains('Wolf hits Defender Target with Bite for 7 piercing damage.'));
      expect(attackEvent.summaryText, contains('Defender Target failed DC 11 STR save (4) and fell Prone!'));
    });

    test('Defender succeeds on DC 11 STR save -> resists knockdown and remains standing', () {
      final target = createDefender();
      expect(target.isProne, isFalse);
