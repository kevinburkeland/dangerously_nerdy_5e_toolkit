...
void main() {
  group('2014 Background Skills, Features, and Equipment Compilation Suite', () {
    test('SrdBackgroundsLibrary helpers extract canonical 2014 starting equipment and features', () {
      final equipment = SrdBackgroundsLibrary.extractStartingEquipment('acolyte');
      ...
      expect(feature.description, contains('free healing and care at a temple'));

      final hydratedBg = SrdBackgroundsLibrary.findBySlug('acolyte');
      expect(hydratedBg, isNotNull);
      expect(hydratedBg!.skillProficiencies, containsAll(['Insight', 'Religion']));
      expect(hydratedBg.customProperties['feature'], equals('Shelter of the Faithful'));
      expect(hydratedBg.customProperties['startingEquipment'], contains('Holy Symbol'));
    });

    test('Compiling a 2014 character draft with Acolyte background grants skills, equipment, and Shelter of the Faithful feature with 0 background ASIs', () {
      ...
      expect(character.skillProficiencies[SkillType.insight], equals(SkillProficiencyLevel.proficient));
      expect(character.skillProficiencies[SkillType.religion], equals(SkillProficiencyLevel.proficient));

      // 2. 2014 Starting Equipment: Instantiated in character.inventory
      final itemNames = character.inventory.map((i) => i.itemRef.displayName).toList();
      ...
      expect(character.customProperties['backgroundFeature'], equals('Shelter of the Faithful'));
      expect(character.customProperties['backgroundFeatureDescription'], contains('free healing and care at a temple'));
