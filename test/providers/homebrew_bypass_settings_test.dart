...
void main() {
  group('Homebrew Bypass State Injection & SettingsProvider Tests', () {
    ...
    test(
        'AppSettings defaults bypassedHomebrewSlugs to empty set and supports copyWith / equals',
        () {
      ...
      expect(updated.bypassedHomebrewSlugs,
          containsAll(['custom-dragon-breath', 'fireball-homebrew']));
    ...
    test('SettingsProvider toggles and queries homebrew bypass state correctly',
        () async {
      ...
      expect(provider.settings.bypassedHomebrewSlugs,
          contains('eldritch-blast-mod'));

      provider.toggleHomebrewBypass('eldritch-blast-mod');
      expect(provider.isHomebrewBypassed('eldritch-blast-mod'), isFalse);
      expect(provider.settings.bypassedHomebrewSlugs,
          isNot(contains('eldritch-blast-mod')));
    });

    test('hydrateFromPrefs reconstitutes bypassedHomebrewSlugs from storage',
        () async {
      SharedPreferences.setMockInitialValues({
        'setting_bypassed_homebrew_slugs': [
          'homebrew_shield_spell',
          'custom_sword'
        ],
      });
      ...
      expect(settings.bypassedHomebrewSlugs,
          containsAll(['homebrew_shield_spell', 'custom_sword']));
    });
  });
}
