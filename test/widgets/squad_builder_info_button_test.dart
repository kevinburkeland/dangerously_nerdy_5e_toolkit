...
void main() {
  testWidgets(
      'SquadBuilderBottomSheet displays creature stat block info button that opens full stat block dialog',
  ...
  testWidgets(
      'SpellReferenceWidget creature profile card displays stat block button that opens full stat block dialog',
      (WidgetTester tester) async {
    final undeadPreset =
        SrdSummonsLibrary.allPresets.firstWhere((p) => p.id == 'animate_dead');
    ...
    expect(find.byType(CreatureStatBlockDialog), findsOneWidget);
    expect(find.widgetWithText(CreatureStatBlockDialog, 'Skeleton'),
        findsOneWidget);
