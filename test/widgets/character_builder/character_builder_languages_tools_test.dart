...
void main() {
  group('CharacterBuilderScreen Languages and Tools Tests', () {
    testWidgets(
        'Step 2 displays Dwarf tool proficiency choice when Dwarf is selected',
        (tester) async {
      ...
      await tester.scrollUntilVisible(dwarfTile, 150,
      ...
      final brewerChip = tester.widget<ChoiceChip>(
          find.widgetWithText(ChoiceChip, "Brewer's Supplies"));
      expect(brewerChip.selected, isTrue);
    });

    testWidgets(
        'Step 2 displays bonus language selection when Human is selected',
        (tester) async {
      tester.view.physicalSize = const Size(1280, 1000);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
