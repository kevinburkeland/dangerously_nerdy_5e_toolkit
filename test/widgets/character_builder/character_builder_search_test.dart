...
void main() {
  group('Character Builder & Level Up Search Widget Tests', () {
    testWidgets(
        'BackgroundStep filters backgrounds by search query and restores on clear',
        (tester) async {
    ...
    testWidgets(
        'CharacterBuilderScreen filters Species step by search query and restores on clear',
        (tester) async {
      ...
      final speciesSearch =
          find.widgetWithText(TextField, 'Search Species / Races');
      ...
      final clearBtn = find.descendant(
          of: speciesSearch, matching: find.byIcon(Icons.clear));
    ...
    testWidgets(
        'LevelUpWizardDialog Step 4 filters Feats by search query and restores on clear',
        (tester) async {
      ...
      const alertFeat = Feat(
        ...
        category: 'General',
        descriptionMarkdown:
            'Always on the lookout for danger. +5 to initiative.',
      );
      ...
      // Clear search
      final clearBtn =
          find.descendant(of: featSearch, matching: find.byIcon(Icons.clear));
      expect(clearBtn, findsOneWidget);
      await tester.tap(clearBtn);
      await tester.pumpAndSettle();
    ...
    testWidgets(
        'LevelUpWizardDialog Step 5 Spells search filters and clears via suffix icon',
        (tester) async {
      tester.view.physicalSize = const Size(1280, 1600);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      ...
      // Step 4 (Spells)
      final spellSearch =
          find.widgetWithText(TextField, 'Search Available Class Spells');
      ...
      final clearBtn =
          find.descendant(of: spellSearch, matching: find.byIcon(Icons.clear));
      expect(clearBtn, findsOneWidget);
