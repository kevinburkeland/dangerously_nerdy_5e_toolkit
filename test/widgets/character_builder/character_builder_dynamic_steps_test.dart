...
void main() {
  ...
  group('CharacterBuilderScreen Dynamic Steps Widget Tests', () {
    testWidgets('Wizard renders dynamic Class Decisions step for Fighter',
        (tester) async {
      ...
      expect(
          find.text('Step 3: Choose Class & Starting Skills'), findsOneWidget);
    ...
    testWidgets('Wizard caps cantrips and leveled spells to class quotas',
        (tester) async {
      tester.view.physicalSize = const Size(1280, 1000);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      ...
      expect(
          find.textContaining(
              'A Level 1 Wizard must choose exactly 6 1st-level spells'),
          findsOneWidget);
    ...
    testWidgets(
        'Wizard detects skill collision with background and offers compensatory picks',
        (tester) async {
      tester.view.physicalSize = const Size(1280, 1000);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      ...
      await tester.pumpAndSettle();
      if (find
          .text('Fighter Decisions & Specializations')
          .evaluate()
          .isNotEmpty) {
        await tester.drag(find.byType(ListView), const Offset(0, -1000));
        await tester.pumpAndSettle();
      ...
      expect(
          find.text('Step 3: Choose Class & Starting Skills'), findsOneWidget);
      expect(find.textContaining('Skill Collision Detected: Insight'),
          findsOneWidget);
      expect(find.textContaining('Compensatory Pick(s):'), findsOneWidget);
    });

    testWidgets(
        'Attributes First preset under 2014 rules does not default attributes to 20',
        (tester) async {
      tester.view.physicalSize = const Size(1280, 1000);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
    ...
    testWidgets(
        'Attributes First preset under 2014 rules prompts for racial skills and flexible attributes on species step (Variant Human)',
        (tester) async {
      tester.view.physicalSize = const Size(1280, 1000);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      ...
      final dexChipFinder =
          find.widgetWithText(FilterChip, 'DEXTERITY (+1 Bonus)');
    ...
    testWidgets(
        'Attributes First preset under 2014 rules with Custom Lineage grants +2 to only ONE attribute instead of two',
        (tester) async {
      tester.view.physicalSize = const Size(1280, 1000);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      ...
      final customLineage = Race(
        id: const EntityId(
            slug: 'custom-lineage', ruleset: RulesetVersion.v2014),
        name: 'Custom Lineage',
        size: 'Medium or Small',
        speed: '30 ft.',
        abilityScoreSummary:
            '+2 to One Score, 1 Feat, Darkvision or 1 Skill (Lineage / Homebrew)',
        traitsMarkdown: '**Creature Type.** You are a Humanoid.\n\n'
      ...
      addTearDown(() =>
          HomebrewPersistenceService().deleteCustomRace('custom-lineage'));
      ...
      expect(
          find.textContaining('Custom Lineage Lineage Ability Choices (+2):'),
          findsOneWidget);
      expect(find.textContaining('0 / 1 selected'),
          findsNWidgets(2)); // Both skills and ability choices
      ...
      expect(find.text('Racial Attribute Modifiers (Custom Lineage): +2 STR'),
          findsOneWidget);
      ...
      expect(find.text('Racial Attribute Modifiers (Custom Lineage): +2 DEX'),
          findsOneWidget);
    ...
    testWidgets(
        'Attributes First preset under 2024 rules prompts for 2024 Human Skillful racial bonus skill',
        (tester) async {
      tester.view.physicalSize = const Size(1280, 1000);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      ...
      expect(
          find.textContaining('Species Bonus Skills (Human):'), findsOneWidget);
