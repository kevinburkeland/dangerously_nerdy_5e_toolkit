...
void main() {
  group('LevelUpWizardDialog Selection List Omission Tests', () {
    ...
    testWidgets(
        'Already selected invocations are omitted and unselected/novel invocations appear',
        (tester) async {
      ...
      // Register custom invocation Frost Web Blast from homebrew
      const lance = FeatureOption(
        id: 'frost-web-blast',
        name: 'Frost Web Blast',
        descriptionMarkdown:
            'Once on each of your turns when you hit a creature with your eldritch blast, reduce speed by 10 ft.',
        customProperties: {
          'page': 57,
          'featureType': ['EI'],
          'prerequisite': [
      ...
      // Frost Web Blast (unselected homebrew invocation) MUST BE PRESENT!
      expect(find.text('Frost Web Blast'), findsOneWidget);
      ...
      await tester.tap(find.text('Next Step'));
      await tester.pumpAndSettle();
      expect(find.text('Step 5 of 6: Spells & Invocations Management'),
          findsOneWidget);
    ...
    test(
        'Ingests custom homebrew Infusions and Invocations and syncs to runtime libraries',
        () async {
      ...
      const jsonCompendium = '''
{
  "infusion": [
    {
      "name": "Boots of the Phantom Step",
      "source": "CUSTOM",
      "entries": [
...
''';
      ...
      final infusionEntry = ingestion.otherEntries
          .firstWhere((e) => e.name == 'Boots of the Phantom Step');
      expect(infusionEntry.category, equals('Infusion'));

      final invocationEntry =
          ingestion.otherEntries.firstWhere((e) => e.name == 'Frost Web Blast');
      ...
      // Verify custom Invocations includes Frost Web Blast
      expect(
          SrdFeatureOptions.warlockInvocations
              .any((i) => i.name == 'Frost Web Blast'),
          isTrue);

      // Verify custom Infusions includes Boots of the Phantom Step
      expect(
          SrdFeatureOptions.customInfusions
              .any((i) => i.name == 'Boots of the Phantom Step'),
          isTrue);
    });
  });
}
