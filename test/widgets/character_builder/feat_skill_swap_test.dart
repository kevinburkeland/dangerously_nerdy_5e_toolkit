...
void main() {
  ...
  group('Feat Skill Swapping Tests', () {
    testWidgets(
        'AddFeatDialog: swapping skill from default updates expertise to swapped skill',
        (tester) async {
      ...
      const versatileSpecialist = Feat(
        id: EntityId(
            slug: 'versatile-specialist', ruleset: RulesetVersion.v2014),
        name: 'Versatile Specialist',
        category: 'General',
        descriptionMarkdown:
            'Increase ability, gain skill proficiency and expertise.',
        customProperties: {
      ...
      SrdFeatsLibrary.addCustomFeat(versatileSpecialist);
      ...
      // Search for Versatile Specialist
      await tester.enterText(
          find.byType(TextField).first, 'Versatile Specialist');
      await tester.pumpAndSettle();

      // Tap Versatile Specialist in the feat list
      final featTile = find.widgetWithText(ListTile, 'Versatile Specialist');
      expect(featTile, findsOneWidget);
      await tester.tap(featTile);
      await tester.pumpAndSettle();

      // Find skill dropdown and swap to Deception
      final skillDropdown = find
          .byWidgetPredicate(
            (w) =>
                w is DropdownButtonFormField<SkillType> &&
                !w.key.toString().contains('add_feat_expertise_'),
          )
          .first;
      ...
      expect(controller.character.skillProficiencies[SkillType.acrobatics],
          isNull);
    });

    testWidgets(
        'LevelUpWizardDialog: swapping skill updates expertise to swapped skill and not athletics',
        (tester) async {
      tester.view.physicalSize = const Size(1280, 1600);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      ...
      // Select Versatile Specialist
      final featPicker = find.byType(DropdownButtonFormField<String>).first;
      await tester.tap(featPicker);
      await tester.pumpAndSettle();

      final featItem = find.text('Versatile Specialist').last;
      await tester.tap(featItem);
      await tester.pumpAndSettle();
      ...
      final skillDropdown = find.byWidgetPredicate(
        (w) =>
            w is DropdownButtonFormField<SkillType> &&
            w.key.toString().contains('levelup_feat_skill_'),
      );
