...
void main() {
  group('Level Up Invocations, Spells, and Patron Tests', () {
    test('SubclassSpellsLibrary identifies Warlock Fiend expanded spells', () {
      final burningHands =
          SpellbookLibrary.getSpellById('spell_burning_hands')!;
      ...
      expect(
          SubclassSpellsLibrary.isExpandedSpell(
              'warlock', 'fiend-patron', burningHands, DmRulesEdition.v2024),
          isTrue);
      expect(
          SubclassSpellsLibrary.isExpandedSpell(
              'warlock', 'the_fiend', fireball, DmRulesEdition.v2024),
          isTrue);
      expect(
          SubclassSpellsLibrary.isExpandedSpell(
              'warlock', 'the_fiend', cureWounds, DmRulesEdition.v2024),
          isFalse);
    });

    testWidgets(
        'LevelUpWizardDialog shows Eldritch Invocations when leveling Warlock to Level 2',
        (tester) async {
      ...
      expect(
          find.textContaining('Choose 2 Eldritch Invocations'), findsOneWidget);
      ...
      expect(warlockProgression.selectedFeatureOptions['warlock-invocations-2'],
          contains('agonizing_blast'));
      expect(warlockProgression.selectedFeatureOptions['warlock-invocations-2'],
          contains('repelling_blast'));
    });

    testWidgets(
        'LevelUpWizardDialog shows Fiend Patron expanded spells (Burning Hands, Command) in Spells step',
        (tester) async {
      tester.view.physicalSize = const Size(1024, 1200);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      const character = Character(
        ...
        baseScores: AbilityScores(
            ...
            charisma: 16),
        resources:
            CharacterResourcePool(currentHp: 10, currentHitDice: {'d8': 1}),
      );
    ...
    testWidgets(
        'LevelUpWizardDialog shows No Spellcasting Advancement for Barbarian',
        (tester) async {
      tester.view.physicalSize = const Size(1024, 1200);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      const barbarian = Character(
        ...
        baseScores: AbilityScores(
            ...
            charisma: 8),
        resources:
            CharacterResourcePool(currentHp: 15, currentHitDice: {'d12': 1}),
      );
      ...
      expect(find.textContaining('No Spellcasting Advancement at Level 2'),
          findsOneWidget);
    });

    testWidgets(
        'LevelUpWizardDialog shows Magical Secrets chip and spell list switcher for Bard leveling to 10',
        (tester) async {
      tester.view.physicalSize = const Size(1024, 1200);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      const bard = Character(
        ...
        baseScores: AbilityScores(
            ...
            charisma: 18),
        resources:
            CharacterResourcePool(currentHp: 55, currentHitDice: {'d8': 9}),
      );
    ...
    testWidgets(
        'LevelUpWizardDialog shows Mystic Arcanum banner for Warlock leveling to 11',
        (tester) async {
      tester.view.physicalSize = const Size(1024, 1200);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      const warlock = Character(
        ...
        baseScores: AbilityScores(
            ...
            charisma: 18),
        resources:
            CharacterResourcePool(currentHp: 65, currentHitDice: {'d8': 10}),
      );
      ...
      expect(find.textContaining('Mystic Arcanum Milestone (Level 6 Spell)'),
          findsOneWidget);
    });

    testWidgets(
        'LevelUpWizardDialog allows selecting both traditional spell and Mystic Arcanum without conflict at Level 11',
        (tester) async {
      tester.view.physicalSize = const Size(1024, 1200);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      final warlock = Character(
        ...
        resources: const CharacterResourcePool(
            currentHp: 65, currentHitDice: {'d8': 10}),
      );
      ...
      expect(
          find.textContaining('Quota for Level 11: 0/1 New Leveled Spell(s)'),
          findsOneWidget);
      expect(
          find.textContaining('+ 1 Mystic Arcanum (Level 6)'), findsOneWidget);
      ...
      await tester
          .ensureVisible(find.textContaining('Burning Hands (L1)').first);
      await tester.tap(find.textContaining('Burning Hands (L1)').first);
      await tester.pumpAndSettle();

      // Quota is now 1/1
      expect(
          find.textContaining('Quota for Level 11: 1/1 New Leveled Spell(s)'),
          findsOneWidget);
      ...
      expect(leveledResult!.allocatedSpells['class-warlock-mystic-arcanum'],
          isNotNull);
      expect(
          leveledResult!.allocatedSpells['class-warlock-mystic-arcanum']!
              .any((s) => s.slug == 'spell_eyebite'),
          isTrue);
    });

    testWidgets(
        'LevelUpWizardDialog displays Subclass Granted Spells for Cleric',
        (tester) async {
      tester.view.physicalSize = const Size(1024, 1200);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      const cleric = Character(
        ...
        baseScores: AbilityScores(
            ...
            charisma: 12),
        resources:
            CharacterResourcePool(currentHp: 18, currentHitDice: {'d8': 2}),
      );
    ...
    testWidgets(
        'LevelUpWizardDialog shows Eldritch Invocations for 2014 Warlock leveling to Level 2',
        (tester) async {
      tester.view.physicalSize = const Size(1024, 1200);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
    ...
    testWidgets(
        'LevelUpWizardDialog unlocks Spellcasting and Fighting Style for 2014 Paladin leveling to Level 2',
        (tester) async {
      tester.view.physicalSize = const Size(1024, 1200);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      const paladin2014 = Character(
        ...
        baseScores: AbilityScores(
            ...
            charisma: 16),
        resources:
            CharacterResourcePool(currentHp: 12, currentHitDice: {'d10': 1}),
      );
      ...
      expect(find.textContaining('Available Leveled Spells (Up to Level 1)'),
          findsOneWidget);
      expect(find.textContaining('Bless (L1)'), findsOneWidget);
    });

    testWidgets(
        'LevelUpWizardDialog enforces spell quota cap and prevents infinite spell selection',
        (tester) async {
      tester.view.physicalSize = const Size(1024, 1200);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      const wizard = Character(
        ...
        baseScores: AbilityScores(
            ...
            charisma: 10),
        resources:
            CharacterResourcePool(currentHp: 8, currentHitDice: {'d6': 1}),
      );
      ...
      expect(find.textContaining('Quota for Level 2: 0/2 New Leveled Spell(s)'),
          findsOneWidget);
      ...
      await tester.tap(find.textContaining('Burning Hands (L1)').first);
      await tester.pumpAndSettle();
      expect(find.textContaining('Quota for Level 2: 1/2 New Leveled Spell(s)'),
          findsOneWidget);

      // Select 2nd spell
      await tester
          .ensureVisible(find.textContaining('Charm Person (L1)').first);
      await tester.tap(find.textContaining('Charm Person (L1)').first);
      await tester.pumpAndSettle();
      expect(find.textContaining('Quota for Level 2: 2/2 New Leveled Spell(s)'),
          findsOneWidget);

      // Attempt to select 3rd spell (should be blocked by quota)
      await tester
          .ensureVisible(find.textContaining('Detect Magic (L1)').first);
      ...
      expect(
          find.textContaining(
              'Cannot select more than 2 new spell(s) at Level 2.'),
          findsOneWidget);
    });

    testWidgets(
        'LevelUpWizardDialog supports Spell Replacement (swapping known spell)',
        (tester) async {
      tester.view.physicalSize = const Size(1024, 1200);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      await tester
          .ensureVisible(find.textContaining('Hellish Rebuke (L1)').first);
      ...
      final resultKnownSlugs =
          leveledResult!.spellsKnown.map((s) => s.slug).toList();
