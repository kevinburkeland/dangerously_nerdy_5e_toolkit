...
void main() {
  group('LevelUpWizardDialog Spell Level Separation & Info Button Tests', () {
    testWidgets(
        'Separates leveled spells into distinct sections by spell level (1st-Level, 2nd-Level)',
        (tester) async {
      ...
      const wizard = Character(
        ...
        baseScores: AbilityScores(
            ...
            charisma: 10),
        resources:
            CharacterResourcePool(currentHp: 14, currentHitDice: {'d6': 2}),
      );
      ...
      expect(
          find.textContaining('Step 5 of 6: Spells & Invocations Management'),
          findsOneWidget);

      // Verify overall leveled spells header with max level
      expect(find.textContaining('Available Leveled Spells (Up to Level 2)'),
          findsOneWidget);
    ...
    testWidgets(
        'Tapping spell info button opens SpellComparisonDialog without selecting spell',
        (tester) async {
      tester.view.physicalSize = const Size(1024, 1200);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      expect(find.textContaining('Quota for Level 3: 0/2 New Leveled Spell(s)'),
          findsOneWidget);

      // Find info button for Burning Hands
      final infoBtnFinder =
          find.byKey(const Key('spell_info_btn_spell_burning_hands'));
      ...
      // Quota is now 1/2
      expect(find.textContaining('Quota for Level 3: 1/2 New Leveled Spell(s)'),
          findsOneWidget);
    ...
    testWidgets(
        'Replaceable spell section displays info button for known spells',
        (tester) async {
      tester.view.physicalSize = const Size(1024, 1200);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      final infoBtnFinder =
          find.byKey(const Key('replaceable_spell_info_spell_hellish_rebuke'));
      expect(infoBtnFinder, findsOneWidget);
