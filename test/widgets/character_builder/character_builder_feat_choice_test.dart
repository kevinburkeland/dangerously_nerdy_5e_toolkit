...
void main() {
  group('CharacterBuilderScreen Feat ASI & Rider Choice Widget Tests', () {
    testWidgets(
        '2014 Variant Human bonus feat allows choosing Tenacious, selecting CON chip, and viewing rider',
        (tester) async {
      ...
      const tenacious = Feat(
        ...
        customProperties: {
          ...
          'statIncrease': 1,
          'riderDescription':
              'Gain saving throw proficiency in the chosen ability.',
        },
      );
      ...
      await tester.scrollUntilVisible(variantHumanTile, 150,
      ...
      await tester.scrollUntilVisible(acolyteFinder, 150,
          scrollable: find.byType(Scrollable).first);
      await tester.pumpAndSettle();
      ...
      await tester.scrollUntilVisible(tenaciousFinder, 200,
          scrollable: find.byType(Scrollable).first);
      await tester.pumpAndSettle();
      ...
      expect(find.byKey(const Key('builder_feat_ability_constitution')),
          findsOneWidget);
      expect(
          find.byKey(const Key('builder_feat_ability_wisdom')), findsOneWidget);

      // Select Constitution
      await tester
          .tap(find.byKey(const Key('builder_feat_ability_constitution')));
      await tester.pumpAndSettle();

      // Verify saving throw badge is shown
      expect(
          find.text('Grants Proficiency: CON Saving Throws'), findsOneWidget);
      ...
      final reviewFinder =
          find.textContaining('Tenacious (+1 CON, CON Save Prof)');
    ...
    testWidgets(
        'Chef feat from homebrew allows picking between CON and WIS chips in builder',
        (tester) async {
      tester.view.physicalSize = const Size(1280, 1100);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      ...
      await tester.scrollUntilVisible(chefFinder, 200,
          scrollable: find.byType(Scrollable).first);
      await tester.pumpAndSettle();
      ...
      expect(
          find.byKey(const Key('builder_feat_ability_wisdom')), findsOneWidget);
      expect(
          find.byKey(const Key('builder_feat_ability_strength')), findsNothing);
