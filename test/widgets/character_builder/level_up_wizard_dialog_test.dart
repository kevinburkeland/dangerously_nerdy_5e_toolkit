...
void main() {
  group('LevelUpWizardDialog & Sheet Integration Widget Tests', () {
    ...
    testWidgets(
        'Header banner contains Level Up button and launches Wizard dialog',
        (tester) async {
      ...
      expect(find.byKey(const Key('character_sheet_level_up_button')),
          findsOneWidget);

      // Tap level up button
      await tester
          .tap(find.byKey(const Key('character_sheet_level_up_button')));
    ...
    testWidgets(
        'Wizard steps navigation and confirmation updates character level',
        (tester) async {
      tester.view.physicalSize = const Size(1280, 900);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      ...
      expect(find.text('Step 6 of 6: Review & Final Confirmation'),
          findsOneWidget);
      ...
      expect(upgradedResult!.resources.currentHp,
          equals(20)); // 12 base + 8 (6 avg + 2 con)
    });

    testWidgets(
        'Level 4: Choosing Feat Tenacious prompts for ability chips and applies save proficiency',
        (tester) async {
      tester.view.physicalSize = const Size(1280, 900);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      ...
      expect(find.text('Step 4 of 6: Ability Score Improvement or Feat'),
          findsOneWidget);
      ...
      expect(find.byKey(const Key('feat_ability_chip_constitution')),
          findsOneWidget);
      ...
      expect(upgradedResult!.savingThrowProficiencies,
          contains(AbilityType.constitution));
