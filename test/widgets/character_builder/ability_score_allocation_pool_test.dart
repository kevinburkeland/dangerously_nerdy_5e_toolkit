...
void main() {
  ...
  group('CharacterBuilderController Consumable Pool State Tests', () {
    ...
    test(
        'Reassigning an ability returns previous score to pool and consumes new score',
        () {
      controller.assignScore(AbilityType.strength, 15);
      expect(controller.availableScores, equals([14, 13, 12, 10, 8]));
    ...
    test(
        'Unassigning score returns it to pool and keeps pool sorted high to low',
        () {
      controller.assignScore(AbilityType.dexterity, 14);
      expect(controller.availableScores, equals([15, 13, 12, 10, 8]));

      controller.unassignScore(AbilityType.dexterity);
      expect(controller.assignedScores.containsKey(AbilityType.dexterity),
          isFalse);
      expect(controller.availableScores, equals([15, 14, 13, 12, 10, 8]));
    });

    test(
        'Duplicate rolled scores can only be assigned up to their rolled frequency',
        () {
      controller.setMode('rolled');
      controller.populateRolledScores([14, 14, 12, 10, 10, 8]);

      expect(
          controller.availableScores.where((s) => s == 14).length, equals(2));

      // Assign first 14
      controller.assignScore(AbilityType.strength, 14);
      expect(
          controller.availableScores.where((s) => s == 14).length, equals(1));

      // Assign second 14
      controller.assignScore(AbilityType.dexterity, 14);
      expect(
          controller.availableScores.where((s) => s == 14).length, equals(0));
    ...
    test(
        'Two-tap selection workflow updates selectedPoolScore and assigns cleanly',
        () {
    ...
    test(
        'isAbilityAllocationComplete is true only when all 6 abilities are allocated',
        () {
      expect(controller.isAbilityAllocationComplete, isFalse);
    ...
    test(
        'Down the line rolling assigns scores sequentially and completes allocation',
        () {
      controller.setMode('rolled');
    ...
    test(
        'Manual entry mode validates all 6 abilities have integer in 3-30 range',
        () {
  ...
  group('CharacterBuilderScreen Consumable Pool Widget & Progression Tests',
    ...
    testWidgets(
        'Ability Scores step disables Next Step button until all 6 are allocated',
        (tester) async {
    ...
    testWidgets(
        'Unassigning score returns it to pool and re-disables Next Step',
        (tester) async {
      tester.view.physicalSize = const Size(1280, 2000);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
