...
void main() {
  ...
  group('CharacterBuilderController Skill Overlap & Refund State Tests', () {
    ...
    test(
        'Resolving refunded skill adds to bonusReplacementSkills and decrements counter',
        () {
      controller.setSelectedSkills({SkillType.insight, SkillType.history});
      controller.setBackgroundSlug('acolyte');
      expect(controller.refundedSkillChoices, equals(1));
    ...
    test(
        'Unresolving refunded skill removes from replacements and restores refund count',
        () {
      controller.setSelectedSkills({SkillType.insight, SkillType.history});
      controller.setBackgroundSlug('acolyte');
      controller.resolveRefundedSkill(SkillType.stealth);
    ...
    test('Multiple overlaps from background grant corresponding refund count',
      ...
      expect(controller.collidingSkills,
          equals({SkillType.insight, SkillType.religion}));
      ...
      expect(controller.refundedSkillChoices, equals(0));
      expect(controller.bonusReplacementSkills,
          equals({SkillType.survival, SkillType.acrobatics}));
    });
    ...
    test(
        'availableReplacementSkills excludes class, background, species, and selected replacements',
        () {
  ...
  group(
      'CharacterBuilderScreen Skill Overlap & Wizard Progression Widget Tests',
      () {
    testWidgets(
        'Acolyte background colliding with Fighter Insight locks Next Step until replacement is chosen',
        (tester) async {
      ...
      final acrobaticsChipFinder =
          find.widgetWithText(FilterChip, 'Acrobatics').first;
      await tester.tap(acrobaticsChipFinder);
      await tester.pumpAndSettle();

      // Select Insight chip for Fighter
      final insightChipFinder =
          find.widgetWithText(FilterChip, 'Insight').first;
      ...
      expect(find.textContaining('Skill Proficiency Overlap Detected!'),
          findsOneWidget);
      expect(find.textContaining('Please select 1 replacement skill(s)'),
          findsOneWidget);
      ...
      expect(find.textContaining('All skill overlap replacements resolved!'),
          findsOneWidget);
