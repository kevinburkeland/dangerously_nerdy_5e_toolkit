...
void main() {
  ...
  group('Agonizing Blast & Rod of the Pact Keeper on Spells Page', () {
    testWidgets(
      ...
      const eldritchBlast = Spell(
        id: EntityId(
            slug: 'spell-eldritch-blast', ruleset: RulesetVersion.v2024),
        name: 'Eldritch Blast',
        level: 0,
        school: 'evocation',
  ...
  group('Actions Tab Full Rework & Action Economy Tests', () {
    test(
      ...
      // Standard Actions
      final stdActionNames =
          resolved.standardActions.map((a) => a.name).toList();
      ...
      // Special & Magic Items
      final specialActionNames =
          resolved.specialActions.map((a) => a.name).toList();
      expect(specialActionNames.any((n) => n.contains('Pact Keeper')), isTrue);
    });

    testWidgets(
        'Actions tab UI renders Round Economy quick tracker, Filter Chips, and organized sections',
        (tester) async {
