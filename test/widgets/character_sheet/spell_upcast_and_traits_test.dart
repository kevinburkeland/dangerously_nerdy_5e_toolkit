...
void main() {
  ...
  group('SpellUpcastSheet UI & Accessibility Tests', () {
    testWidgets(
        'Renders actionable rows with >= spellLevel, disables depleted slots, min 48dp height',
        (tester) async {
    ...
    testWidgets('Dismissing SpellUpcastSheet returns null gracefully',
        (tester) async {
      SpellCastSelection? selectedSlot =
          const SpellCastSelection(slotLevel: 999);
  ...
  group('InteractiveSpellTile & SpellCard Upcasting Flow Tests', () {
    testWidgets(
      ...
      expect(
          controller.character.resources.spellSlots.currentSlots[1], equals(4));
    });
    ...
    testWidgets(
      ...
      const leveledSpell = Spell(
        ...
        components: SpellComponents(),
        descriptionMarkdown:
            'Make three ranged spell attacks. Each deals 2d6 fire damage.',
        damageMath: [
          EvaluationMath(
            diceFormula: '2d6',
            damageType: DamageType.fire,
      ...
      expect(
          controller.character.resources.spellSlots.currentSlots[4], equals(0));
      // 2nd level slot left untouched at 3
      expect(
          controller.character.resources.spellSlots.currentSlots[2], equals(3));
    });

    testWidgets(
        'Select a 3rd-level slot for a 1st-level spell via upcast bottom sheet: decrements 3rd slot, leaves 1st slot untouched, and scales damage dice',
        (tester) async {
      ...
      expect(
          controller.character.resources.spellSlots.currentSlots[3], equals(1));
      ...
      final cachedRolls =
          roomService.getCachedRolls(roomService.activeRoomCode ?? 'LOCAL');
    ...
    testWidgets(
      ...
      expect(
          controller.character.resources.spellSlots.currentSlots[1], equals(3));
    });

    testWidgets(
        'Warlock casting Armor of Agathys auto-upcasts to Pact Magic level without warning or modal',
        (tester) async {
    ...
    testWidgets(
        'Depleted Pact Magic slots block casting and display helpful notification',
        (tester) async {
      final warlockChar = baseCharacter.copyWith(
        resources: baseCharacter.resources.copyWith(
          spellSlots: const SpellSlotPool(
      ...
      expect(
          find.textContaining('No Pact Magic slots remaining'), findsOneWidget);
      expect(controller.character.resources.spellSlots.pactMagicCurrent,
          equals(0));
    });

    testWidgets(
        'Multiclass character shows both regular slots and Pact Magic slot in SpellUpcastSheet',
        (tester) async {
      ...
      expect(
          controller.character.resources.spellSlots.currentSlots[2], equals(2));
  ...
  group('AbilitiesAndTraitsTab Component Tests', () {
    testWidgets(
        'Renders Active Features, tracking pips, decrement button, passive traits, and feats',
        (tester) async {
