...
void main() {
  group('ShortRestDialog Widget Tests', () {
    ...
    testWidgets('Prevents incrementing beyond available Hit Dice count',
        (tester) async {
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: Builder(
              builder: (context) => ElevatedButton(
                onPressed: () => ShortRestDialog.show(context,
                    controller: singleDieController),
                child: const Text('Open Short Rest'),
              ),
            ),
          ),
    ...
    testWidgets(
        'CharacterVitalsHud Long Rest opens confirmation dialog and restores character vitals',
        (tester) async {
      ...
      expect(testController.character.resources.currentHp,
          equals(testController.stats.maxHp));
      ...
      expect(
          testController.character.resources.currentHitDice['d10'], equals(3));
    });
  });
}
