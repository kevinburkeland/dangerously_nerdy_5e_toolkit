...
void main() {
  testWidgets(
      'FeatureListItem renders and opens bottom sheet modal with Semantics',
      (tester) async {
    ...
    expect(
        find.textContaining('You can push yourself beyond your normal limits'),
        findsOneWidget);
    ...
    final semanticsFinder = find.byWidgetPredicate(
      (w) =>
          w is Semantics &&
          w.properties.label?.contains('Feature Details: Action Surge') == true,
    );
    expect(semanticsFinder, findsOneWidget);
  });

  testWidgets('SpellListItem renders and opens spell detail modal',
      (tester) async {
    const testSpell = Spell(
      ...
      castingTime: CastingTime(
          ...
          triggerCondition: 'when hit by an attack'),
      range: 'Self',
      components: SpellComponents(v: true, s: true),
      duration: SpellDuration(
          type: DurationType.timed, durationSeconds: 6, rawText: '1 round'),
      descriptionMarkdown:
    ...
    expect(find.textContaining('An invisible barrier of magical force appears'),
        findsOneWidget);
    ...
    final semanticsFinder = find.byWidgetPredicate(
      (w) =>
          w is Semantics &&
          w.properties.label?.contains('Spell Details: Shield') == true,
    );
    expect(semanticsFinder, findsOneWidget);
  });
}
