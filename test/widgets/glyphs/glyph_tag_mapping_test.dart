...
void main() {
  group('Glyph & Tag Mapping Tests', () {
    ...
    test('Spell tag semantics map correctly to ActionTraitRings', () {
      ...
      final fireball =
          SpellbookLibrary.allSpells.firstWhere((s) => s.name == 'Fireball');
      final fireballRings = fireball.getGlyphActionRings(DmRulesEdition.v2014);
      expect(fireballRings.any((r) => r.ringType == ActionRingType.recharge),
          isTrue);
      expect(fireball.getGlyphPrimaryDamageAccent(DmRulesEdition.v2014),
          DamageAccent.fire);

      // Hold Person -> Concentration & Control rings
      final holdPerson =
          SpellbookLibrary.allSpells.firstWhere((s) => s.name == 'Hold Person');
      final holdRings = holdPerson.getGlyphActionRings(DmRulesEdition.v2014);
      expect(holdRings.any((r) => r.ringType == ActionRingType.concentration),
          isTrue);
      expect(
          holdRings.any((r) => r.ringType == ActionRingType.control), isTrue);

      // Healing Word -> Sustain ring
      final healingWord = SpellbookLibrary.allSpells
          .firstWhere((s) => s.name == 'Healing Word');
      final healRings = healingWord.getGlyphActionRings(DmRulesEdition.v2014);
      expect(
          healRings.any((r) => r.ringType == ActionRingType.sustain), isTrue);

      // Shield -> Reaction ring
      final shield =
          SpellbookLibrary.allSpells.firstWhere((s) => s.name == 'Shield');
      final shieldRings = shield.getGlyphActionRings(DmRulesEdition.v2014);
      expect(shieldRings.any((r) => r.ringType == ActionRingType.reaction),
          isTrue);
    });

    test(
        'Magic Item tag semantics map correctly to ActionTraitRings and DamageAccents',
        () {
      ...
      expect(flameRings.any((r) => r.ringType == ActionRingType.attunement),
          isTrue);
      expect(flameRings.any((r) => r.ringType == ActionRingType.melee), isTrue);
      expect(flameTongue.getGlyphPrimaryDamageAccent(DmRulesEdition.v2024),
          DamageAccent.fire);

      // Staff of Healing -> Attunement + Sustain + Recharge rings
      final staffOfHealing = MagicItemLibrary.findByName('Staff of Healing')!;
      final staffRings =
          staffOfHealing.getGlyphActionRings(DmRulesEdition.v2024);
      expect(staffRings.any((r) => r.ringType == ActionRingType.attunement),
          isTrue);
      expect(
          staffRings.any((r) => r.ringType == ActionRingType.sustain), isTrue);
      expect(
          staffRings.any((r) => r.ringType == ActionRingType.recharge), isTrue);
      ...
      expect(
          wandRings.any((r) => r.ringType == ActionRingType.recharge), isTrue);
      expect(wandMm.getGlyphPrimaryDamageAccent(DmRulesEdition.v2024),
          DamageAccent.force);
      ...
      expect(ringProtRings.any((r) => r.ringType == ActionRingType.attunement),
          isTrue);
      expect(ringProtRings.any((r) => r.ringType == ActionRingType.reaction),
          isTrue);

      // Longsword -> Melee ring with Slashing damage
      final longsword = MagicItemLibrary.findByName('Longsword')!;
      final longswordRings =
          longsword.getGlyphActionRings(DmRulesEdition.v2024);
      expect(longswordRings.any((r) => r.ringType == ActionRingType.melee),
          isTrue);
      expect(longsword.getGlyphPrimaryDamageAccent(DmRulesEdition.v2024),
          DamageAccent.slashing);
    });

    test(
        'Monster type display strings map accurately to CreatureType and CR Tiers',
        () {
      ...
      expect(
          vampStat.glyphActionRings
              .any((r) => r.ringType == ActionRingType.legendary),
          isTrue);
      expect(
          vampStat.glyphActionRings
              .any((r) => r.ringType == ActionRingType.sustain),
          isTrue);

      final dragon =
          MonsterCodexLibrary.getMonsterByName('Ancient Red Dragon')!;
      ...
      expect(
          dragonStat.glyphActionRings
              .any((r) => r.ringType == ActionRingType.legendary),
          isTrue);
      expect(
          dragonStat.glyphActionRings
              .any((r) => r.ringType == ActionRingType.recharge),
          isTrue);
