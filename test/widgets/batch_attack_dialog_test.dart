...
void main() {
  ...
  testWidgets('BatchAttackDialog renders batch attack title and living count',
      (WidgetTester tester) async {
    final session = SpellSession(spellLevel: 5);
    session.addObject(ObjectSize.tiny, customName: 'Coin #1');

    await tester
        .pumpWidget(createTestableWidget(BatchAttackDialog(session: session)));

    expect(find.text('Batch Attack Roller'), findsOneWidget);
    expect(find.text('Target AC'), findsOneWidget);
    expect(find.widgetWithText(ElevatedButton, 'ROLL ALL 1 ATTACKS'),
        findsOneWidget);
  });

  testWidgets(
      'BatchAttackDialog displays broadcasting status when room code is set',
      (WidgetTester tester) async {
    final session = SpellSession(spellLevel: 5);
    session.addObject(ObjectSize.tiny, customName: 'Coin #1');
  ...
  testWidgets('Tapping Roll Attacks produces batch summary results',
      (WidgetTester tester) async {
    final session = SpellSession(spellLevel: 5);
    session.addObject(ObjectSize.tiny, customName: 'Coin #1');
    final roomService = DiceRoomService();
    ...
    final rollButton =
        find.widgetWithText(ElevatedButton, 'ROLL ALL 1 ATTACKS');
  ...
  testWidgets(
      'Batch attack broadcasts RoomRoll with full to-hit papertrail details',
      (WidgetTester tester) async {
    final session = SpellSession(spellLevel: 5);
    ...
    expect(
        capturedRoll.details!.first, contains('Silver Coin #1 (Tiny): d20 ['));
  ...
  testWidgets(
      'BatchAttackDialog renders cleanly on mobile screen without overflow',
      (WidgetTester tester) async {
    ...
    for (int i = 1; i <= 5; i++) {
      session.addObject(ObjectSize.tiny,
          customName: 'Silver Coin #$i with a very long name');
    }
    final roomService = DiceRoomService();
