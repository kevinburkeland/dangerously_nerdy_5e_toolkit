...
void main() {
  group('Subrace & Character Builder Support', () {
    test(
        'SrdSpeciesLibrary correctly exposes subraces and attaches custom subraces',
        () {
      ...
      expect(updatedElf.subraces.any((s) => s.id.slug == 'eladrin-custom'),
          isTrue);
    ...
    test(
        '2014 mode subrace attributes replace base species ability score increases (not additive)',
        () {
      final elf = SrdSpeciesLibrary.findBySlug('elf')!;
      ...
      final mountainDwarf =
          dwarf.subraces.firstWhere((s) => s.id.slug == 'mountain-dwarf');
    ...
    test(
        'Drow subrace grants dancing-lights cantrip at level 1 and scales at levels 3 and 5',
        () {
      ...
      expect(
          lvl1.any((s) => s.spellRef.slug == 'dancing-lights' && s.isCantrip),
          isTrue);
  ...
  group('MagicItemLibrary & Homebrew Item Integration', () {
    test('Homebrew items can be converted and loaded into MagicItemLibrary',
      ...
      final magicItem =
          HomebrewPersistenceService.equipmentItemToMagicItem(equip);
      ...
      expect(MagicItemLibrary.allItems.any((i) => i.id == 'sunblade-custom'),
          isTrue);
  ...
  group('Markdown & Tag Parsing Robustness', () {
    test(
        'cleanRawTags accurately parses hit bonus, h, hom, recharge, dc, and nested tags',
        () {
      ...
      expect(CompendiumJsonIngestionPipeline.cleanRawTags('{@hom}'),
          '*Hit or Miss:* ');
      expect(CompendiumJsonIngestionPipeline.cleanRawTags('{@recharge 5}'),
          '*(Recharge 5–6)*');
      expect(CompendiumJsonIngestionPipeline.cleanRawTags('{@recharge}'),
          '*(Recharge 6)*');
      expect(CompendiumJsonIngestionPipeline.cleanRawTags('{@dc 15}'), 'DC 15');
      expect(CompendiumJsonIngestionPipeline.cleanRawTags('{@damage 2d6+3}'),
          '**`2d6+3`**');
      expect(CompendiumJsonIngestionPipeline.cleanRawTags('{@scaledice 3d6}'),
          '**`3d6`**');
    });

    testWidgets(
        'FormattedMarkdownText renders deep headings, blockquotes, and tables',
        (tester) async {
      const sample = '''
#### Subaction Heading
...
''';
    ...
    testWidgets('RaceCard renders traits using FormattedMarkdownText',
        (tester) async {
      final elf = SrdSpeciesLibrary.findBySlug('elf')!;
