...
void main() {
  group('RoomConnectionBadge Widget & a11y Tests', () {
    Widget buildTestWidget({
      ...
      return MaterialApp(
        home: Scaffold(
          body: MediaQuery(
            data:
                MediaQueryData(textScaler: TextScaler.linear(textScaleFactor)),
            child: Center(
    ...
    testWidgets(
        'a11y: Semantics label properly expands WebRTC P2P with 3 peers',
        (tester) async {
    ...
    testWidgets(
        'a11y: Semantics label properly expands Offline state with 0 peers',
        (tester) async {
      final controller = StreamController<RoomConnectionTelemetry>();
      const telemetry = RoomConnectionTelemetry(
    ...
    testWidgets(
        'a11y: Semantics label properly expands Firebase Relay with 2 peers',
        (tester) async {
      final controller = StreamController<RoomConnectionTelemetry>();
      const telemetry = RoomConnectionTelemetry(
      ...
      expect(
        find.bySemanticsLabel(
            'Network Status: Firebase Relay, 2 peers connected.'),
        findsOneWidget,
      );
    ...
    testWidgets(
        'Renders Connecting icon when state is connecting with peers > 0',
        (tester) async {
      final controller = StreamController<RoomConnectionTelemetry>();
      const telemetry = RoomConnectionTelemetry(
        state: TransportState.connecting,
      ...
      expect(
        find.bySemanticsLabel(
            'Network Status: Connecting..., 1 peers connected.'),
        findsOneWidget,
      );
    ...
    testWidgets(
        'Dynamically updates UI as telemetry stream emits new snapshots',
        (tester) async {
    ...
    testWidgets(
        'Renders cleanly at TextScaler 2.0 in a constrained width without overflow errors',
        (tester) async {
      final controller = StreamController<RoomConnectionTelemetry>();
      const telemetry = RoomConnectionTelemetry(
        state: TransportState.webRtc,
    ...
    testWidgets(
        'Dynamically transitions from Connecting (0) to WebRTC P2P (1) to Firebase Relay (1)',
        (tester) async {
      final controller = StreamController<RoomConnectionTelemetry>.broadcast();
