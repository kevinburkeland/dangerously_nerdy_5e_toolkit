...
void main() {
  group('ArenaCombatantToken & Condition Chips Widget Tests', () {
    testWidgets('renders ArenaCombatantToken with glyph and condition chips',
        (tester) async {
      ...
      combatant.applyActiveCondition(
        const ActiveCondition(
            condition: ArenaCondition.poisoned, durationRounds: 2),
      );
      combatant.applyActiveCondition(
    ...
    testWidgets(
        'ArenaConditionChipsBar handles overflow with +N badge when >3 conditions active',
        (tester) async {
      final conditions = [
    ...
    testWidgets(
        'ArenaConditionChip displays tooltip with mechanical penalty reminder',
        (tester) async {
    ...
    testWidgets('ArenaConditionToggleDialog allows DM to toggle conditions',
        (tester) async {
      final wolfMonster = MonsterCodexLibrary.allMonsters
          .firstWhere((m) => m.name.toLowerCase() == 'wolf');
      final combatant = ArenaCombatant.fromMonster(
    ...
    testWidgets(
        'ArenaCombatantCard renders token and dynamic condition chips in roster',
        (tester) async {
      final tRex = MonsterCodexLibrary.allMonsters
          .firstWhere((m) => m.name.toLowerCase().contains('tyrannosaurus'));
      final combatant = ArenaCombatant.fromMonster(
        id: 'card_trex_1',
        monster: tRex,
        team: ArenaTeam.teamA,
    ...
    testWidgets(
        'LandingScreen renders Monster Fighting Arena tool card with custom DndGlyph',
        (tester) async {
      await tester.pumpWidget(
        const MaterialApp(
          home: LandingScreen(),
