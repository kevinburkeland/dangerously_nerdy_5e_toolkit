...
void main() {
  group('AppLogo Widget Tests', () {
    testWidgets('renders properly with default parameters', (tester) async {
      ...
      expect(find.bySemanticsLabel('DangerouslyNerdy 5e Toolkit Logo'),
          findsOneWidget);
    });

    testWidgets('renders at different sizes without layout overflow',
        (tester) async {
    ...
    testWidgets('spins and calls onTap when tapped in interactive mode',
        (tester) async {
      bool tapped = false;
      await tester.pumpWidget(
        MaterialApp(
    ...
    testWidgets('inherits primary and secondary colors dynamically from Theme',
        (tester) async {
      await tester.pumpWidget(
        MaterialApp(
          theme: ThemeData(
