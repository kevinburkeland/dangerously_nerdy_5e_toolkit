...
void main() {
  group('MissingHomebrewWarningWidget Tests', () {
    ...
    testWidgets(
        'MissingHomebrewBanner renders when homebrew is missing and opens dialog on tap',
        (tester) async {
      ...
      expect(find.byKey(const Key('missing_homebrew_banner_details_btn')),
          findsOneWidget);

      // Tap Details button to open dialog
      await tester
          .tap(find.byKey(const Key('missing_homebrew_banner_details_btn')));
      await tester.pumpAndSettle();

      // Verify dialog is open and shows missing details
      expect(find.byType(Dialog), findsOneWidget);
      expect(find.text('Character: Alden the Homebrewed'), findsOneWidget);
    ...
    testWidgets(
        'MissingHomebrewBanner does not render when no homebrew is missing',
        (tester) async {
    ...
    testWidgets(
        'MissingHomebrewBadge renders on roster card and opens dialog on tap',
        (tester) async {
      const report = MissingHomebrewReport(
        missingItems: [
          MissingHomebrewItem(
      ...
      expect(find.byKey(const ValueKey('missing_homebrew_badge_alden_custom')),
          findsOneWidget);
      expect(find.text('Missing Homebrew (1)'), findsOneWidget);

      // Tap badge
      await tester.tap(
          find.byKey(const ValueKey('missing_homebrew_badge_alden_custom')));
      await tester.pumpAndSettle();
