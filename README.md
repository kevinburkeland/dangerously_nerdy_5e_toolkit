...
[![Tests](https://img.shields.io/badge/Tests-1946%20Passing-brightgreen.svg)](test)
[![SRD 5.1 & 5.2.1](https://img.shields.io/badge/Rules-SRD%205.1%20%26%205.2.1%20CC--BY--4.0-blueviolet.svg)](LEGAL_ATTRIBUTION_MODAL.md)

A modern, high-performance Flutter application designed for 5th Edition (5e) tabletop RPG players and Game Masters. Built for seamless cross-edition play (supporting both **2014 RAW** and **2024 Revised SRD 5.1 & 5.2.1** rulesets), the toolkit provides a complete ecosystem of **core tabletop apps, character progression pipelines, compendiums, combat simulators, and real-time campaign hubs**.
...
* **Species, Lineage & Background Integration**:
  ...
  - Subrace flexible ability choice pool constraints (`flexibleAbilityPool`), preventing unselected choices from defaulting to Strength in 2014 mode.
...
* **Multiclassing & Level-Up Studio Pipeline**:
  ...
  - Unified multiclass spell slot progression matrix (Full-caster, half-caster, round-up half-caster, and third-caster progression support).
...
* **Dynamic Action Economy Framework**:
  ...
  - Full propagation and level-gating of homebrew and compendium class features (e.g. `Helpful`, `Cunning Action`, `Second Wind`, `Action Surge`, `Rage`) directly into the combat action economy.
...
* **Clean Room Structural Ingestion & Anti-Corruption Layer (ACL)**:
  - **Platform-Agnostic Polymorphic Ingestion**: Dynamically discovers and normalizes tabletop entities via structural JSON inspection (`hitDie`, `entries`, `stats`, `cr`, `action`, etc.) rather than hardcoded proprietary schema keys or third-party bundle catalogs. Completely isolated from any specific external toolchain or platform (such as 5etools).
  - **Encapsulated Pipe Token Parsing (`CompendiumPipeParser`)**: Isolates external pipe-delimited syntax (`Feature|Class|Source|Level`, `Spell|Source#c`) strictly within the ingestion ACL boundary, mapping external references into pure domain `FeatureGrant` entities.
  - **AST & EntryNodeTransformer**: Decodes nested JSON structures, entries, lists, tables, and tag syntax (e.g. `{@spell ...}`, `{@item ...}`, `{@dice ...}`, `{@creature ...}`, `{@condition ...}`, `{@damage ...}`).
  - **Automated Monster Extraction & Feature Verification**: Parses abbreviated sizes, alignments, multiattack routines, damage types, and spellcasting blocks into fully playable statblocks. Traits, actions, and features are parsed and checked against our internal parser (`StatBlockAclParser`), resolving any discrepancies in mobility, reach, riders, and saves strictly in favor of the internal ACL parser.
...
* **Sanitized Manifest Discovery**: Ingests directly from remote GitHub releases or source repos while strictly bypassing redundant utility scripts, generator lookups, non-tabletop assets (crochet/recipes), and prose narrative book chapters to preserve database integrity.
...
* **Explicit-Ruleset GitHub Repository Ingestor (Expert Options)**:
  ...
  - Strict Anti-Corruption Layer (ACL) schema validation enforcing distinct 2014 SRD 5.1 vs. 2024 SRD 5.2.1 contracts (forbidding heuristic auto-detection).
...
* **Active Character Personas & Roster Manager**:
  - Create and manage custom character personas (e.g. *"Alden (Wizard)"*, *"Corin (Rogue)"*).
  ...
  - **Mobile-Responsive Banner & Identity HUD**: Responsive `LayoutBuilder` architecture guarantees the *"ACTIVE CHARACTER / SESSION IDENTITY"* title, character name, and linked badges span full width on mobile viewports without vertical letter squishing, cleanly wrapping action buttons beneath.
...
* **Decentralized CvRDT State Synchronization**:
  ...
  - **Explicit Causality & Deduplication**: Outbound payloads are mapped and serialized via `RoomSyncPayloadMapper`, stamped with `origin_node_id` and monotonic sequence numbers, dropping stale frames within a 30s sliding window while eliminating recursive echo loops and self-echo broadcasts.
...
* **4-Tier Cost-Optimized Transport Waterfall (`CascadingTransportRouter`)**:
  - **Tier 1 (Local Wi-Fi / LAN)**: Zero-latency, zero-cloud-cost direct local network communication with non-destructive socket re-use and UDP health probing.
  - **Tier 2 (WebRTC P2P Mesh)**: Zero-cost DataChannels with dedicated recipient mailbox signaling (`rooms/{roomCode}/nodes/{toNodeId}/signals/{signalId}`) in Firestore, eliminating wildcard race conditions and multi-peer signaling collisions. W3C polite peer glare rollback (`RTCSessionDescription('', 'rollback')`) prevents simultaneous-offer collisions.
  ...
  - **Asymmetric Bounded Deduplication Cache**: Inbound payloads are indexed in an in-memory 500-entry LRU cache using injected SHA-256 digests (`payloadHasher`), silently discarding duplicate transmissions and updating LRU order without redundant processing.
...
* **Strict 5e Action Economy Resolution**:
  ...
  - **Action & Spell Saving Throw Riders**: Precomputed attacks extract condition saving throw riders (e.g. Wolf knockdown prone DC 11 STR save) with dynamic fallback DC calculation, delegating HP management to domain `HitPoints` and formatting condition pass/fail outcomes into turn event logs.
...
* **Dual 2014 RAW vs 2024 Revised Rules**: Toggle between 2014 rules (Power Attack -5/+10, GWF reroll 1s/2s) and 2024 rules (Heavy Weapon +PB damage, GWF floor 3), or unlock **"Anything Goes"** mode to mix cross-edition mechanics and masteries.
* **Power Attack (-5/+10) Break-Even Analysis**: Automatic calculation of the exact crossover armor class (AC) where power attack options outperform normal strikes, highlighted by a golden beacon and diamond marker.
* **Multi-Curve Situational Triangle**:
  - Baseline Strike (Cyan)
  - Power Attack (-5/+10) (Amber)
...
* **Cantrip & Weapon Damage Presets**: Built-in SRD damage actions including *Eldritch Blast* (with **Agonizing Blast** +Mod toggle), *Fire Bolt*, *Ray of Frost*, *Sacred Flame*, *Chill Touch*, *Shocking Grasp*, *Poison Spray*, *Acid Splash*, *Vicious Mockery*, *Produce Flame*, *Thorn Whip*, *Shillelagh*, and *Magic Stone*, plus standard weapons and magic weapons (*Flame Tongue*, *Sun Blade*, *Frost Brand*, *Holy Avenger*). Arbitrary custom or homebrew attack profiles are fully supported.
...
* **Capability-Driven Mechanics & Zero Platform Tying**:
  - Combat resolution, stat calculation, and character progression are 100% capability-driven via domain `FeatureGrant` entities (`attackAbilitySubstitution`, `capabilityFlag`, `armorClassModifier`, `bonusSpell`).
  - Ingestion Anti-Corruption Layer (`lib/services/acl/`) isolates external JSON schemas and pipe tokens, ensuring the domain core is pristine and completely decoupled from any external tool or repository structure.
...
├── test/                           # Unit, widget, accessibility, & resilience test suites (1,946 tests)
│   ├── core/                       # Standalone ruleset-agnostic core engine tests
│   ├── modules/dnd5e/              # D&D 5e loadable module tests (SRD 5.1/5.2.1)
...
├── LEGAL_ATTRIBUTION_MODAL.md      # SRD 5.1 & 5.2.1 Creative Commons attribution & IP safeguards
...
To execute the automated unit, widget, accessibility, spellcasting mechanics, character progression pipeline, conflict resolution, domain purity, and resilience test suite (**1,946 tests with 100% pass rate**):
```bash
flutter test
```

To run ruleset-agnostic core engine tests in isolation:
```bash
flutter test test/core/
```

To run D&D 5e loadable module tests:
```bash
flutter test test/modules/dnd5e/
```
...
- Writing comprehensive unit, widget, domain purity, and resilience tests (1,946 automated tests).
...
- **Game Content & Mechanics (CC-BY-4.0)**: All tabletop RPG rules, spell matrices, creature stat blocks, equipment tables, and game mechanics derived from the System Reference Document 5.1 & 5.2.1 are licensed under the **[Creative Commons Attribution 4.0 International License (CC-BY-4.0)](LEGAL_ATTRIBUTION_MODAL.md)**.
