...
typedef EntityBatchPersister = Future<void> Function(
...
class HomebrewImportOrchestrator {
  ...
  Stream<HomebrewImportTelemetry> runImport({
    ...
    final controller =
        StreamController<HomebrewImportTelemetry>.broadcast(sync: false);
    ...
    scheduleMicrotask(() async {
      ...
      try {
          ...
          if (result is IngestionSuccessResult) {
            ...
            if (_batchPersister != null) {
              ...
              if (pendingBatch.length >= _batchSize) {
                ...
                pendingBatch.clear();
                // Yield to the event loop so frame rendering and UI progress stay fluid
            ...
            if (accumulatedErrors.length > 100) {
              accumulatedErrors.removeAt(0);
            }
          }

          final now = DateTime.now();
          if (now.difference(lastTelemetryEmission).inMilliseconds >=
              telemetryThrottleIntervalMs) {
            lastTelemetryEmission = now;
            if (_retainLedger && pendingLedgerEntries.isNotEmpty) {
        ...
        final friendlyMsg = (rawMsg.contains('minified:') ||
                rawMsg.contains('ProgressEvent') ||
