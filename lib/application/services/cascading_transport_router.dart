...
export 'package:vtt_engine_core/ports/transport_state.dart';
...
class CascadingTransportRouter implements IP2pTransportPort {
  ...
  Future<void> initializeRoom(String roomCode, String localNodeId) async {
    ...
    try {
      await _activateAdapter(
          firebaseFallbackAdapter, TransportState.fallbackRelay);
      return;
    } catch (_) {}
  ...
  Future<void> broadcastPayload(String jsonPayload) async {
    if (_activeAdapter == null || _currentState == TransportState.offline)
      return;
  ...
  @visibleForTesting
  Future<void> probeHigherTiers({bool force = true}) =>
      _attemptStepUpRecovery(force: force);
  ...
  Future<void> _attemptStepUpRecovery({bool force = false}) async {
    ...
    final dynamicCooldownMs = stepUpCooldown.inMilliseconds *
        (_stepDownCountInWindow > 3
            ? 3
            : (_stepDownCountInWindow > 0 ? _stepDownCountInWindow : 1));
