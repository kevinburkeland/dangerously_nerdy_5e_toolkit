...
class SettingsProvider extends ChangeNotifier {
  ...
  SettingsProvider(
      {AppSettings initialSettings = const AppSettings(), bool autoLoad = true})
  ...
  static AppSettings hydrateFromPrefs(SharedPreferences prefs) {
    try {
      ...
      if (accentStr != null) {
        ...
        if (legacyIndex != null &&
            legacyIndex >= 0 &&
            legacyIndex < FantasyAccent.values.length) {
      ...
      if (hapticStr != null) {
        ...
        if (legacyIndex != null &&
            legacyIndex >= 0 &&
            legacyIndex < HapticFeedbackLevel.values.length) {
      ...
      if (editionStr != null) {
        ...
        if (legacyIndex != null &&
            legacyIndex >= 0 &&
            legacyIndex < DmRulesEdition.values.length) {
      ...
      WizardOrderingPreset resolvedWizardOrder =
          WizardOrderingPreset.classic2014;
      ...
      LoggingService().logNonFatal(
        ...
        stackTrace,
        reason:
            'Failed to hydrate AppSettings from SharedPreferences; using defaults',
      );
  ...
  Future<void> updateSettings(AppSettings newSettings) async {
    ...
    try {
      ...
      await Future.wait([
        ...
        prefs.setString(
            _kWizardOrderingPreset, newSettings.wizardOrderingPreset.name),
        ...
        prefs.setStringList(
            _kPinnedRuleIds, newSettings.pinnedRuleIds.toList()),
        prefs.setStringList(
            _kPinnedSpellIds, newSettings.pinnedSpellIds.toList()),
        prefs.setStringList(
            _kPinnedMonsterIds, newSettings.pinnedMonsterIds.toList()),
        prefs.setStringList(
            _kPinnedItemIds, newSettings.pinnedItemIds.toList()),
        prefs.setStringList(
            _kPinnedFeatIds, newSettings.pinnedFeatIds.toList()),
        prefs.setStringList(
            _kPinnedClassIds, newSettings.pinnedClassIds.toList()),
        prefs.setStringList(
            _kPinnedRaceIds, newSettings.pinnedRaceIds.toList()),
        prefs.setStringList(_kBypassedHomebrewSlugs,
            newSettings.bypassedHomebrewSlugs.toList()),
      ]);
    } catch (e, stackTrace) {
      LoggingService().logNonFatal(
        e,
  ...
  void setFantasyAccent(FantasyAccent accent) =>
      updateSettings(_settings.copyWith(fantasyAccent: accent));
  void setOledMode(bool value) =>
      updateSettings(_settings.copyWith(oledPitchBlack: value));
  void setHapticLevel(HapticFeedbackLevel level) =>
      updateSettings(_settings.copyWith(hapticLevel: level));
  void setWizardOrderingPreset(WizardOrderingPreset preset) =>
      updateSettings(_settings.copyWith(wizardOrderingPreset: preset));
  void setCritFumbleFx(bool value) =>
      updateSettings(_settings.copyWith(enableCritFumbleFx: value));
  void setSpellParticles(bool value) =>
      updateSettings(_settings.copyWith(enableSpellParticles: value));
  void set3dDiceOverlays(bool value) =>
      updateSettings(_settings.copyWith(enable3dDiceOverlays: value));
  void setGlyphAnimations(bool value) =>
      updateSettings(_settings.copyWith(enableGlyphAnimations: value));
  void setPerformanceMode(bool value) =>
      updateSettings(_settings.copyWith(performanceMode: value));
  void setRulesEdition(DmRulesEdition edition) =>
      updateSettings(_settings.copyWith(rulesEdition: edition));
  ...
  void togglePinRule(String ruleId) => updateSettings(_settings.copyWith(
      pinnedRuleIds: _toggleSetId(_settings.pinnedRuleIds, ruleId)));
  void pinRule(String ruleId) => updateSettings(_settings.copyWith(
      pinnedRuleIds: _addSetId(_settings.pinnedRuleIds, ruleId)));
  void unpinRule(String ruleId) => updateSettings(_settings.copyWith(
      pinnedRuleIds: _removeSetId(_settings.pinnedRuleIds, ruleId)));
  void clearPinnedRules() => _settings.pinnedRuleIds.isEmpty
      ? null
      : updateSettings(_settings.copyWith(pinnedRuleIds: const <String>{}));

  // --- Spells Pinning ---
  bool isSpellPinned(String spellId) =>
      _settings.pinnedSpellIds.contains(spellId);
  void togglePinSpell(String spellId) => updateSettings(_settings.copyWith(
      pinnedSpellIds: _toggleSetId(_settings.pinnedSpellIds, spellId)));
  void pinSpell(String spellId) => updateSettings(_settings.copyWith(
      pinnedSpellIds: _addSetId(_settings.pinnedSpellIds, spellId)));
  void unpinSpell(String spellId) => updateSettings(_settings.copyWith(
      pinnedSpellIds: _removeSetId(_settings.pinnedSpellIds, spellId)));
  void clearPinnedSpells() => _settings.pinnedSpellIds.isEmpty
      ? null
      : updateSettings(_settings.copyWith(pinnedSpellIds: const <String>{}));

  // --- Monsters Pinning ---
  bool isMonsterPinned(String monsterId) =>
      _settings.pinnedMonsterIds.contains(monsterId);
  void togglePinMonster(String monsterId) => updateSettings(_settings.copyWith(
      pinnedMonsterIds: _toggleSetId(_settings.pinnedMonsterIds, monsterId)));
  void pinMonster(String monsterId) => updateSettings(_settings.copyWith(
      pinnedMonsterIds: _addSetId(_settings.pinnedMonsterIds, monsterId)));
  void unpinMonster(String monsterId) => updateSettings(_settings.copyWith(
      pinnedMonsterIds: _removeSetId(_settings.pinnedMonsterIds, monsterId)));
  void clearPinnedMonsters() => _settings.pinnedMonsterIds.isEmpty
      ? null
  ...
  void togglePinItem(String itemId) => updateSettings(_settings.copyWith(
      pinnedItemIds: _toggleSetId(_settings.pinnedItemIds, itemId)));
  void pinItem(String itemId) => updateSettings(_settings.copyWith(
      pinnedItemIds: _addSetId(_settings.pinnedItemIds, itemId)));
  void unpinItem(String itemId) => updateSettings(_settings.copyWith(
      pinnedItemIds: _removeSetId(_settings.pinnedItemIds, itemId)));
  void clearPinnedItems() => _settings.pinnedItemIds.isEmpty
      ? null
  ...
  void togglePinFeat(String featId) => updateSettings(_settings.copyWith(
      pinnedFeatIds: _toggleSetId(_settings.pinnedFeatIds, featId)));
  void pinFeat(String featId) => updateSettings(_settings.copyWith(
      pinnedFeatIds: _addSetId(_settings.pinnedFeatIds, featId)));
  void unpinFeat(String featId) => updateSettings(_settings.copyWith(
      pinnedFeatIds: _removeSetId(_settings.pinnedFeatIds, featId)));
  void clearPinnedFeats() => _settings.pinnedFeatIds.isEmpty
      ? null
      : updateSettings(_settings.copyWith(pinnedFeatIds: const <String>{}));

  // --- Classes Pinning ---
  bool isClassPinned(String classId) =>
      _settings.pinnedClassIds.contains(classId);
  void togglePinClass(String classId) => updateSettings(_settings.copyWith(
      pinnedClassIds: _toggleSetId(_settings.pinnedClassIds, classId)));
  void pinClass(String classId) => updateSettings(_settings.copyWith(
      pinnedClassIds: _addSetId(_settings.pinnedClassIds, classId)));
  void unpinClass(String classId) => updateSettings(_settings.copyWith(
      pinnedClassIds: _removeSetId(_settings.pinnedClassIds, classId)));
  void clearPinnedClasses() => _settings.pinnedClassIds.isEmpty
      ? null
      : updateSettings(_settings.copyWith(pinnedClassIds: const <String>{}));

  // --- Species / Races Pinning ---
  bool isRacePinned(String raceId) => _settings.pinnedRaceIds.contains(raceId);
  void togglePinRace(String raceId) => updateSettings(_settings.copyWith(
      pinnedRaceIds: _toggleSetId(_settings.pinnedRaceIds, raceId)));
  void pinRace(String raceId) => updateSettings(_settings.copyWith(
      pinnedRaceIds: _addSetId(_settings.pinnedRaceIds, raceId)));
  void unpinRace(String raceId) => updateSettings(_settings.copyWith(
      pinnedRaceIds: _removeSetId(_settings.pinnedRaceIds, raceId)));
  void clearPinnedRaces() => _settings.pinnedRaceIds.isEmpty
      ? null
      : updateSettings(_settings.copyWith(pinnedRaceIds: const <String>{}));

  // --- Homebrew Override Bypass ---
  bool isHomebrewBypassed(String slug) =>
      _settings.bypassedHomebrewSlugs.contains(slug);
