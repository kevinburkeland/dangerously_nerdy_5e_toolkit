...
class SessionGraphService {
  ...
  static RoomNodeState addContainerToRoom(
      RoomNodeState room, LootContainer container) {
    final containers = List<LootContainer>.from(room.containers);
    final idx =
        containers.indexWhere((c) => c.containerId == container.containerId);
    if (idx != -1) {
  ...
  static EncounterParticipant instantiateCombatParticipant({
    ...
    final participantId =
        'part-${entityLink.entityId}-${DateTime.now().millisecondsSinceEpoch}';
  ...
  static ({RoomNodeState updatedRoom, Character updatedCharacter})
      transferRoomLoot({
    ...
    if (containerIdx == -1) {
      throw ArgumentError(
          'Container $containerId not found in room ${room.roomCode}.');
    }
