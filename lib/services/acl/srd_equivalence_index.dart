...
class SrdEquivalenceIndex {
  ...
  SrdMatchResult checkEntity({
    ...
    // Unify hyphenated possessives (e.g. 'hunter-s-mark' -> 'hunters-mark')
    ...
    final stripped = normSlug.replaceAll(
      RegExp(
          r'[-_](phb|dmg|mm|xge|tce|srd|srd52|srd51|2014|2024|v2014|v2024|xphb)$'),
      '',
    );
  ...
  bool isCanonSrd(String slug, EntityType type, {String? name}) {
    ...
    if (name != null && name.isNotEmpty) {
      return checkEntity(slug: slug, name: name, type: type) !=
          SrdMatchResult.notSrd;
    }
    final slugSet = _slugsByType[type] ?? const {};
    final normSlug = slug.toLowerCase().trim();
    final unifiedSlug = normSlug.replaceAll(RegExp(r'-s(-|$)'), r's$1');
    if (slugSet.contains(normSlug) || slugSet.contains(unifiedSlug))
      return true;
    final stripped = normSlug.replaceAll(
      RegExp(
          r'[-_](phb|dmg|mm|xge|tce|srd|srd52|srd51|2014|2024|v2014|v2024|xphb)$'),
  ...
  void build() {
    ...
    for (final m
        in MonsterCodexLibrary.allMonsters.where((m) => !m.isHomebrew)) {
