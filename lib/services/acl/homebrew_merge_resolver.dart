...
class ImportAnalysisItem<T extends DomainEntity> {
  ...
  ImportAnalysisItem({
    ...
    this.isSrdCanon = false,
            ...
            (disposition != ImportDisposition.identical && !isSrdCanon);
...
class ImportAnalysisResult {
  ...
  final List<ImportAnalysisItem<Subrace>> subraces;
  ...
  int get novelCount => allItems
      .where((i) => i.disposition == ImportDisposition.novel && !i.isSrdCanon)
      .length;
  int get identicalCount => allItems
      .where(
          (i) => i.disposition == ImportDisposition.identical && !i.isSrdCanon)
      .length;
  int get srdDuplicateCount => allItems.where((i) => i.isSrdCanon).length;
  int get collisionCount => allItems
      .where((i) => i.disposition == ImportDisposition.collision)
      .length;
...
class HomebrewMergeResolver {
  ...
  ImportAnalysisResult analyzeBundle({
    ...
    List<Subrace> localSubraces = const [],
    ...
    final subraceItems = _analyzeCategory<Subrace>(
      incoming: incomingBundle.subraces,
      local: localSubraces,
      srdIndex: srdIndex,
    );
    ...
    if (srdCount > 0) {
      warnings.add(
          '$srdCount canonical SRD entities detected and excluded to prevent duplicate content.');
    }
    ...
    for (final inc in incoming) {
      ...
      if (srdMatch != SrdMatchResult.notSrd) {
        results.add(ImportAnalysisItem<T>(
          ...
          isSelected:
              false, // Unselected by default so zero SRD duplicates are imported!
          isSrdCanon: true,
  ...
  String _generateDiffSummary(
      Map<String, dynamic> local, Map<String, dynamic> incoming) {
    final changedKeys = <String>[];
    for (final key in incoming.keys) {
      if (key != 'id' &&
          json.encode(local[key]) != json.encode(incoming[key])) {
        changedKeys.add(key);
      }
    }
