...
class EntryNodeTransformer {
  ...
  void _parseNode(
    ...
    if (node is Map) {
      _parseMapNode(Map<String, dynamic>.from(node), buffer, math, refs, depth,
          defaultRuleset);
      return;
    }
  }
  ...
  void _parseMapNode(
    ...
    switch (type) {
      ...
      case 'insetReadaloud':
        ...
        final insetContent = _captureNode(
            node['entries'] ?? node['entry'] ?? node['desc'],
            depth + 1,
            math,
            refs,
            defaultRuleset);
      ...
      case 'image':
        final alt =
            node['title']?.toString() ?? node['altText']?.toString() ?? 'Image';
        buffer.writeln('*[$alt]*');
        buffer.writeln();
      ...
      // -----------------------------------------------------------------------
      case 'quote':
        final quoteContent = _captureNode(
            node['entries'] ?? node['entry'] ?? node['desc'],
            depth,
            math,
            refs,
      ...
      // -----------------------------------------------------------------------
      // Referenced features fallback handler
      // -----------------------------------------------------------------------
      case 'refSubclassFeature':
      case 'refClassFeature':
        final ptr = (node['subclassFeature'] ??
                node['classFeature'] ??
                node['name'] ??
                '')
            .toString()
            .trim();
        final title = ptr.contains('|') ? ptr.split('|').first.trim() : ptr;
        if (title.isNotEmpty) {
          final prefix = '#' * (depth + 3).clamp(1, 6);
          buffer.writeln('$prefix $title');
          buffer.writeln();
        }
        if (node.containsKey('entries') || node.containsKey('entry')) {
          _parseNode(node['entries'] ?? node['entry'], buffer, math, refs,
              depth + 1, defaultRuleset);
        }
      ...
      case 'options':
        ...
        _parseNode(node['entries'] ?? node['items'] ?? node['options'], buffer,
            math, refs, depth + 1, defaultRuleset);
      ...
      default:
        ...
        if (node.containsKey('entries')) {
          _parseNode(
              node['entries'], buffer, math, refs, depth + 1, defaultRuleset);
        } else if (node.containsKey('entry')) {
          _parseNode(node['entry'], buffer, math, refs, depth, defaultRuleset);
        } else if (node.containsKey('desc')) {
          _parseNode(
              node['desc'], buffer, math, refs, depth + 1, defaultRuleset);
        } else if (node.containsKey('description')) {
          _parseNode(node['description'], buffer, math, refs, depth + 1,
              defaultRuleset);
        } else if (node.containsKey('text')) {
          _parseNode(
              node['text'], buffer, math, refs, depth + 1, defaultRuleset);
        } else if (node.containsKey('subclassFeatures')) {
          _parseNode(node['subclassFeatures'], buffer, math, refs, depth + 1,
              defaultRuleset);
        } else if (node.containsKey('features')) {
          _parseNode(
              node['features'], buffer, math, refs, depth + 1, defaultRuleset);
        } else if (node.containsKey('items')) {
          _parseNode(
              node['items'], buffer, math, refs, depth + 1, defaultRuleset);
        } else if (node.containsKey('options')) {
          _parseNode(
              node['options'], buffer, math, refs, depth + 1, defaultRuleset);
        } else if (node.containsKey('subentries')) {
          _parseNode(node['subentries'], buffer, math, refs, depth + 1,
              defaultRuleset);
        }
    }
  }
  ...
  void _renderList(
    ...
    for (final item in items) {
      ...
      if (item is String) {
        ...
        final subContent = _captureNode(
            Map<String, dynamic>.from(item), depth, math, refs, defaultRuleset);
  ...
  void _renderTable(
    ...
    if (caption != null && caption.isNotEmpty) {
      final processedCaption =
          _processTags(caption, math, refs, defaultRuleset);
    ...
    if (colLabels.isNotEmpty) {
      ...
      final headers =
          List.generate(firstRow.length, (i) => 'Col ${i + 1}').join(' | ');
  ...
  String _resolveCellText(
    ...
    if (cell is List) {
      return cell
          .map((c) => _resolveCellText(c, math, refs, defaultRuleset))
          .join('');
    }
    if (cell is Map) {
    ...
    while (_tagRegex.hasMatch(current) && passes < 10) {
      ...
      current = current.replaceAllMapped(_tagRegex, (match) {
        ...
        switch (tag) {
          ...
          case 'hit':
            final pfx =
                primary.startsWith('+') || primary.startsWith('-') ? '' : '+';
            return '**`$pfx$primary`**';
            ...
            switch (cleanAtk) {
              ...
              default:
                if (RegExp(r'^[+-]?\d+$').hasMatch(cleanAtk)) {
                  final pfx =
                      cleanAtk.startsWith('+') || cleanAtk.startsWith('-')
                          ? ''
                          : '+';
                  return '**`$pfx$cleanAtk`**';
                }
  ...
  RulesetVersion _mapSourceToRuleset(
      String? source, RulesetVersion defaultRuleset) {
