...
class CompendiumRaceParser {
  ...
  Race parseRace(Map<String, dynamic> raw, {RulesetVersion? forceRuleset}) {
    ...
    final abilityResult =
        _parseAbilityScores(raw['ability'], isModernLineage: isModernLineage);

    // Traits Markdown
    final traitsData = raw['trait'] ??
        raw['traits'] ??
        raw['entries'] ??
        raw['desc'] ??
        raw['description'];
    ...
    if (rawSubList is List) {
      for (final rawSub in rawSubList) {
        if (rawSub is Map) {
          try {
            subraces.add(parseSubrace(Map<String, dynamic>.from(rawSub),
                defaultRaceSlug: slug, forceRuleset: ruleset));
          } catch (_) {}
        }
      }
    }
    ...
    if (raw.containsKey('darkvision'))
      customProperties['darkvision'] = raw['darkvision'];
    if (raw.containsKey('skillProficiencies'))
      customProperties['skillProficiencies'] = raw['skillProficiencies'];
    if (raw.containsKey('toolProficiencies'))
      customProperties['toolProficiencies'] = raw['toolProficiencies'];
    if (raw.containsKey('languageProficiencies'))
      customProperties['languageProficiencies'] = raw['languageProficiencies'];
  ...
  Subrace parseSubrace(
    ...
    } else if (raw['raceSlug'] != null &&
        raw['raceSlug'].toString().isNotEmpty) {
    ...
    final abilityResult = _parseAbilityScores(
        raw['ability'] ?? raw['abilities'],
        isModernLineage: isModernLineage);
  ...
  String _parseSpeed(dynamic speedData) {
    if (speedData is Map) {
      ...
      final walk = rawWalk is num
          ? rawWalk.toInt()
          : (int.tryParse(rawWalk.toString()) ?? 30);
      final other = <String>[];
  ...
  ({
    ...
    return (
      summary: parts.isNotEmpty
          ? parts.join(', ')
          : (isModernLineage ? '+2/+1 or +1/+1/+1 to any' : null),
      ...
      for (final t in tools) {
        if (t is String && t.isNotEmpty) {
          grants.add(FeatureGrant.weaponArmorProficiency(
            t,
            grantId:
                'race-$slug-tool-${t.toLowerCase().trim().replaceAll(" ", "-")}',
            label: '$t Proficiency',
          ));
        }
      }
