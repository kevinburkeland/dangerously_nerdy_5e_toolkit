...
class CompendiumMonsterParser {
  ...
  Monster parseMonster(
    ...
    // Traits
    final traits =
        (raw['trait'] is List ? raw['trait'] : raw['traits']) as List?;
    if (traits != null && traits.isNotEmpty) {
      actionsBuffer.writeln('### Traits');
      for (final trait in traits) {
        _appendActionOrTrait(trait, actionsBuffer, attackMath, ruleset,
            defaultTitle: 'Trait');
      }
    }
      ...
      for (final sc in spellcasting) {
        if (sc is Map) {
          ...
          if (headerEntries != null) {
            final parsedHeader = transformer.transformEntries(headerEntries,
          ...
          if (daily != null) {
            daily.forEach((freq, list) {
              if (list is List) {
                final spellNames =
                    _extractSpellNames(list, innateSpells, ruleset);
                final cleanFreq = freq
                    .toString()
                    .replaceAll(RegExp(r'[eE]$'), '')
                    .replaceAll('/d', '')
                    .trim();
                actionsBuffer.writeln(
                    '- **$cleanFreq/day each:** ${spellNames.join(', ')}');
              }
            });
          }
          ...
          if (spellsObj != null) {
            spellsObj.forEach((lvl, data) {
              if (data is Map && data['spells'] is List) {
                final spellNames = _extractSpellNames(
                    data['spells'] as List, innateSpells, ruleset);
                final slots =
                    data['slots'] != null ? ' (${data['slots']} slots)' : '';
                final lvlNum = int.tryParse(lvl.toString()) ?? 0;
                final lvlLabel =
                    lvlNum == 0 ? 'Cantrips (at will)' : 'Level $lvl$slots';
                actionsBuffer
                    .writeln('- **$lvlLabel:** ${spellNames.join(', ')}');
              }
            });
          }
          actionsBuffer.writeln();
    ...
    // Actions
    final actions =
        (raw['action'] is List ? raw['action'] : raw['actions']) as List?;
    if (actions != null && actions.isNotEmpty) {
      actionsBuffer.writeln('### Actions');
      for (final action in actions) {
        _appendActionOrTrait(action, actionsBuffer, attackMath, ruleset,
            defaultTitle: 'Action');
      }
    }
    ...
    // Bonus Actions
    final bonus =
        (raw['bonus'] is List ? raw['bonus'] : raw['bonusActions']) as List?;
    if (bonus != null && bonus.isNotEmpty) {
      actionsBuffer.writeln('### Bonus Actions');
      for (final b in bonus) {
        _appendActionOrTrait(b, actionsBuffer, attackMath, ruleset,
            defaultTitle: 'Bonus Action');
      }
    }

    // Reactions
    final reactions =
        (raw['reaction'] is List ? raw['reaction'] : raw['reactions']) as List?;
    if (reactions != null && reactions.isNotEmpty) {
      actionsBuffer.writeln('### Reactions');
      for (final r in reactions) {
        _appendActionOrTrait(r, actionsBuffer, attackMath, ruleset,
            defaultTitle: 'Reaction');
      }
    }

    // Legendary Actions
    final legendary = (raw['legendary'] is List
        ? raw['legendary']
        : raw['legendaryActions']) as List?;
    if (legendary != null && legendary.isNotEmpty) {
      actionsBuffer.writeln('### Legendary Actions');
      final legHeader = raw['legendaryHeader'] as List?;
      if (legHeader != null) {
        final parsedHeader =
            transformer.transformEntries(legHeader, defaultRuleset: ruleset);
        actionsBuffer.writeln('${parsedHeader.markdown}\n');
      } else {
        final count = (raw['legendaryActions'] as num?)?.toInt() ?? 3;
        actionsBuffer.writeln(
            'The creature can take $count legendary actions, choosing from the options below.\n');
      }
      for (final leg in legendary) {
        _appendActionOrTrait(leg, actionsBuffer, attackMath, ruleset,
            defaultTitle: 'Option');
      }
    }
    ...
    if (raw['save'] is Map) {
      customProperties['savingThrows'] = (raw['save'] as Map)
          .entries
    ...
    if (raw['skill'] is Map) {
      customProperties['skills'] = (raw['skill'] as Map)
          .entries
          .map((e) => '${e.key} ${e.value}')
          .join(', ');
    ...
    if (raw['vulnerable'] != null) {
      customProperties['damageVulnerabilities'] =
          _formatList(raw['vulnerable']);
    } else if (raw['damageVulnerabilities'] != null) {
      customProperties['damageVulnerabilities'] =
          raw['damageVulnerabilities'].toString();
    }
    if (raw['resist'] != null) {
      customProperties['damageResistances'] = _formatList(raw['resist']);
    } else if (raw['damageResistances'] != null) {
      customProperties['damageResistances'] =
          raw['damageResistances'].toString();
    }
    if (raw['immune'] != null) {
    ...
    if (raw['conditionImmune'] != null) {
      customProperties['conditionImmunities'] =
          _formatList(raw['conditionImmune']);
    } else if (raw['conditionImmunities'] != null) {
      customProperties['conditionImmunities'] =
          raw['conditionImmunities'].toString();
    }
    final senses = raw['senses'] != null ? _formatList(raw['senses']) : '';
    final passive =
        raw['passive'] != null ? 'passive Perception ${raw['passive']}' : '';
    final sensesStr = [
      if (senses.isNotEmpty) senses,
      if (passive.isNotEmpty) passive
    ].join(', ');
  ...
  void _appendActionOrTrait(
    ...
    if (node is Map) {
      final rawName =
          (node['name'] ?? node['title'] ?? defaultTitle).toString();
      aName = transformer
          .transformEntries(rawName, defaultRuleset: ruleset)
          .markdown;
      final entriesData = node['entries'] ??
          node['desc'] ??
          node['description'] ??
          node['text'] ??
          node['entry'];
      parsed =
          transformer.transformEntries(entriesData, defaultRuleset: ruleset);
  ...
  ({int average, String formula}) _parseHitPoints(dynamic hpData) {
    ...
    if (hpData is Map) {
      avg = (hpData['average'] as num?)?.toInt() ??
          (hpData['special'] != null ? 1 : 10);
      ...
      speedData.forEach((mode, val) {
        if (mode == 'walk' || mode == 'speed') {
          if (val is Map) {
            ...
            final cond =
                val['condition'] != null ? ' (${val['condition']})' : '';
  ...
  void _appendDefensesAndSenses(Map<String, dynamic> raw, StringBuffer buffer) {
    ...
    if (raw['vulnerable'] != null) {
      buffer.writeln(
          '**Damage Vulnerabilities:** ${_formatList(raw['vulnerable'])}');
    }
    if (raw['resist'] != null) {
    ...
    if (raw['conditionImmune'] != null) {
      buffer.writeln(
          '**Condition Immunities:** ${_formatList(raw['conditionImmune'])}');
    }
    final senses = raw['senses'] != null ? _formatList(raw['senses']) : '';
    final passive =
        raw['passive'] != null ? 'passive Perception ${raw['passive']}' : '';
  ...
  List<String> _extractSpellNames(List list,
      List<EntityReference<Spell>> innateSpells, RulesetVersion ruleset) {
    final names = <String>[];
    for (final item in list) {
      ...
      if (item is String) {
        ...
        rawText =
            (item['entry'] ?? item['name'] ?? item['item'] ?? '').toString();
      }
  ...
  Map<String, dynamic> resolveCopy(
    ...
    final copy = (raw['_copy'] ??
        (raw['customProperties'] is Map
            ? raw['customProperties']['_copy']
            : null)) as Map?;
    if (copy == null) {
      return raw;
    }
    ...
    final hasDefaultStats = raw['actionsMarkdown'] != null &&
        (raw['actionsMarkdown'].toString().contains(
    ...
    raw.forEach((key, value) {
      if (key != '_copy' && key != 'customProperties') {
        if (hasDefaultStats) {
          ...
          if (key == 'hp' &&
              (value == 10 || (value is Map && value['average'] == 10))) {
            return;
          }
          if (key == 'ac' &&
              (value == 10 || (value is List && value.contains(10)))) {
            return;
          }
      ...
      for (final rule in ruleList) {
        ...
        if (key == '*' || mode == 'replaceTxt') {
          ...
          if (replace != null && replace.isNotEmpty) {
            ...
            _replaceTextInNode(target, replace, withTxt,
          ...
          if (arr is List && replace != null && withItem != null) {
            final matchName = (replace is Map
            ...
            final idx = arr.indexWhere((it) =>
                it is Map &&
                ((it['name']?.toString() ?? it['title']?.toString())
                        ?.toLowerCase()
                        .trim() ==
                    matchName));
            if (idx != -1) {
          ...
          if (skills is Map) {
            final current = target['skill'] is Map
                ? Map<String, dynamic>.from(target['skill'] as Map)
                : <String, dynamic>{};
  ...
  void _replaceTextInNode(dynamic node, String search, String replace,
    ...
    if (node is Map) {
      for (final key in node.keys.toList()) {
        ...
        if (val is String) {
          ...
          _replaceTextInNode(val, search, replace,
              caseSensitive: caseSensitive);
        }
      }
  ...
  static Map<String, dynamic> _monsterItemToRawMap(MonsterItem item) {
    ...
    return {
      ...
      'wis': sb.wisScore,
      'cha': sb.chaScore,
      'cr': item.sourceStatBlock.crDisplay
          .replaceAll(RegExp(r'^CR\s*', caseSensitive: false), '')
          .trim(),
      'trait': sb.traits
          .map((t) => {
                'name': t.name,
                'entries': [t.description]
              })
          .toList(),
      'action': sb.actions
          .map((a) => {
