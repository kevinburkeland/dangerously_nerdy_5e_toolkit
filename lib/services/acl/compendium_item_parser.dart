...
class CompendiumItemParser {
  ...
  EquipmentItem parseItem(Map<String, dynamic> raw,
    ...
    if (raw.containsKey('ac')) {
      ...
      final parsedAc = (raw['ac'] is num)
          ? (raw['ac'] as num).toInt()
          : int.tryParse(raw['ac'].toString());
      if (parsedAc != null) {
    ...
    final tCode =
        raw['type']?.toString().split('|').first.toUpperCase().trim() ?? '';
    if (tCode == 'LA') {
    ...
    if (raw.containsKey('weaponCategory'))
      customProperties['weaponCategory'] = raw['weaponCategory'];
    if (raw.containsKey('property'))
      customProperties['property'] = raw['property'];
    if (raw.containsKey('dmg1')) customProperties['dmg1'] = raw['dmg1'];
    if (raw.containsKey('dmgType'))
      customProperties['dmgType'] = raw['dmgType'];
    if (raw.containsKey('dmg2')) customProperties['dmg2'] = raw['dmg2'];
    if (raw.containsKey('range')) customProperties['range'] = raw['range'];
    if (raw.containsKey('bonusWeapon'))
      customProperties['bonusWeapon'] = raw['bonusWeapon'];
    if (raw.containsKey('bonusAc'))
      customProperties['bonusAc'] = raw['bonusAc'];
    if (raw.containsKey('bonusSpellAttack'))
      customProperties['bonusSpellAttack'] = raw['bonusSpellAttack'];
    if (raw.containsKey('bonusSpellSaveDc'))
      customProperties['bonusSpellSaveDc'] = raw['bonusSpellSaveDc'];
    if (raw.containsKey('charges'))
      customProperties['charges'] = raw['charges'];
    if (raw.containsKey('recharge'))
      customProperties['recharge'] = raw['recharge'];
    if (raw.containsKey('rechargeAmount'))
      customProperties['rechargeAmount'] = raw['rechargeAmount'];
    if (customProperties.containsKey('rechargeAmount')) {
      final rc = customProperties['rechargeAmount']?.toString() ?? '';
      if (rc.contains('{@')) {
        customProperties['rechargeAmount'] = transformer
            .transformEntries(rc)
            .markdown
            .replaceAll('`', '')
            .replaceAll('*', '')
    ...
    if (description.isEmpty) {
      ...
      if (reqAttune.requiresAttunement) {
        details.add(
            '**Requires Attunement:** Yes${reqAttune.prerequisite != null ? " (${reqAttune.prerequisite})" : ""}');
      }
      if (customProperties['ac'] != null)
        details.add('**AC:** ${customProperties['ac']}');
      if (customProperties['bonusAc'] != null)
        details.add('**AC Bonus:** ${customProperties['bonusAc']}');
      if (customProperties['dmg1'] != null) {
        details.add(
            '**Damage:** ${customProperties['dmg1']}${customProperties['dmgType'] != null ? " ${customProperties['dmgType']}" : ""}');
      }
      if (customProperties['dmg2'] != null)
        details.add('**Versatile Damage:** ${customProperties['dmg2']}');
      ...
      if (customProperties['weight'] != null)
        details.add('**Weight:** ${customProperties['weight']} lb.');
      if (customProperties['vehHp'] != null)
        details.add('**Hull HP:** ${customProperties['vehHp']}');
      if (customProperties['vehAc'] != null)
        details.add('**Hull AC:** ${customProperties['vehAc']}');
      if (customProperties['vehSpeed'] != null)
        details.add('**Speed:** ${customProperties['vehSpeed']} mph');
      if (customProperties['crew'] != null)
        details.add('**Crew:** ${customProperties['crew']}');
      if (customProperties['capPassenger'] != null)
        details.add('**Passengers:** ${customProperties['capPassenger']}');
      if (customProperties['capCargo'] != null)
        details.add('**Cargo Capacity:** ${customProperties['capCargo']} tons');
  ...
  List<FeatureGrant> _extractGrants(
      String slug, Map<String, dynamic> raw, Map<String, dynamic> custom) {
    ...
    if (rawBonusAc != null) {
      final val =
          int.tryParse(rawBonusAc.toString().replaceAll('+', '').trim());
      if (val != null && val != 0) {
    ...
    final rawBonusWpn = raw['bonusWeapon'] ??
        custom['bonusWeapon'] ??
        raw['bonusWeaponAttack'] ??
        custom['bonusWeaponAttack'];
    if (rawBonusWpn != null) {
      final val =
          int.tryParse(rawBonusWpn.toString().replaceAll('+', '').trim());
      if (val != null && val != 0) {
    ...
    if (rawSaveBonus != null) {
      final val =
          int.tryParse(rawSaveBonus.toString().replaceAll('+', '').trim());
      if (val != null && val != 0) {
        grants.add(FeatureGrant.passiveBonus(
    ...
    if (rawSpellAtk != null) {
      final val =
          int.tryParse(rawSpellAtk.toString().replaceAll('+', '').trim());
      if (val != null && val != 0) {
        grants.add(FeatureGrant.passiveBonus(
    ...
    if (rawSpellDc != null) {
      final val =
          int.tryParse(rawSpellDc.toString().replaceAll('+', '').trim());
      if (val != null && val != 0) {
        grants.add(FeatureGrant.passiveBonus(
    ...
    if (modifySpeed is Map && modifySpeed['bonus'] != null) {
      final bonus = int.tryParse(
          modifySpeed['bonus'].toString().replaceAll('+', '').trim());
  ...
  ({bool requiresAttunement, String? prerequisite}) _parseAttunement(
      dynamic attuneData) {
    ...
    if (attuneData is String) {
      ...
      final prereq =
          (clean.toLowerCase() == 'true' || clean.toLowerCase() == 'optional')
              ? null
              : clean;
