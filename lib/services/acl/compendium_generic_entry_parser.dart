...
class CompendiumGenericEntryParser {
  ...
  HomebrewCompendiumEntry parseGenericEntry(
    ...
    final rawCat = (raw['category'] ??
            ...
            (raw['customProperties'] is Map
                ? (raw['customProperties'] as Map)['entityType']
                : null) ??
            raw['type'] ??
            defaultCategory)
        .toString()
    ...
    if (nameLower.startsWith('pact of the') ||
        nameLower.contains('pact boon')) {
      category = 'Pact Boon';
      ...
      category = 'Deity';
        ...
        raw.containsKey('upgradeType') ||
        raw.containsKey('vehicleType')) {
      category = 'Vehicle';
    } else if (rawCat.contains('trap') || raw.containsKey('trapType')) {
      category = 'Trap';
    } else if (rawCat.contains('hazard') || raw.containsKey('hazardType')) {
      category = 'Hazard';
    } else if (rawCat.contains('reward') ||
        rawCat.contains('boon') ||
        rawCat.contains('cult')) {
      category = 'Reward';
      ...
      category = 'Infusion';
    } else if (rawCat.contains('charoption') ||
        rawCat.contains('optionalfeature')) {
      ...
      category = 'Deck';
    } else if (rawCat.contains('monsterfeature') ||
        rawCat.contains('monster feature') ||
        rawCat.contains('monster trait') ||
        rawCat == 'trait' ||
        rawCat == 'traits' ||
        defaultCategory == 'Monster Feature') {
      category = 'Monster Feature';
    } else if (raw['category'] != null &&
        raw['category'].toString().isNotEmpty) {
      category = _cleanCategoryString(raw['category'].toString());
    } else if (raw['entityType'] != null &&
        raw['entityType'].toString().isNotEmpty) {
    ...
    if (desc.isEmpty && raw['rows'] is List) {
      ...
      final colLabels =
          raw['colLabels'] is List ? (raw['colLabels'] as List) : [];
      if (rows.isNotEmpty) {
        final buffer = StringBuffer();
        if (colLabels.isNotEmpty) {
          final processedCols = colLabels.map((c) {
            final text = transformer
                .transformEntries(
                    c is Map
                        ? (c['entry'] ?? c['entries'] ?? c.toString())
                        : c.toString(),
                    defaultRuleset: ruleset)
                .markdown;
          ...
          final colCount =
              (rows.first is List) ? (rows.first as List).length : 1;
          buffer.writeln(
              '| ${List.generate(colCount, (i) => 'Col ${i + 1}').join(' | ')} |');
          buffer.writeln(
              '| ${List.generate(colCount, (_) => ':---').join(' | ')} |');
        }
        for (final r in rows) {
          if (r is List) {
            final processedRow = r.map((c) {
              final cellContent = c is Map
                  ? (c['entry'] ?? c['entries'] ?? c['roll'] ?? c.toString())
                  : c.toString();
              final cellText = transformer
                  .transformEntries(cellContent, defaultRuleset: ruleset)
                  .markdown;
            ...
            final cellText = transformer
                .transformEntries(r.toString(), defaultRuleset: ruleset)
                .markdown
                .replaceAll('\n', ' ')
                .replaceAll('|', '\\|');
    ...
    if (category == 'Deity' || raw.containsKey('pantheon')) {
      category = 'Deity';
      final parts = <String>[];
      if (raw['pantheon'] != null)
        parts.add('**Pantheon:** ${raw['pantheon']}');
      if (raw['alignment'] != null) {
        final align = raw['alignment'] is List
            ? (raw['alignment'] as List).join('')
            : raw['alignment'].toString();
        parts.add('**Alignment:** $align');
      }
      if (raw['domains'] != null) {
        final doms = raw['domains'] is List
            ? (raw['domains'] as List).join(', ')
            : raw['domains'].toString();
    ...
    if (category == 'Monster Feature' || defaultCategory == 'Monster Feature') {
      ...
      if (aclFeature.riders.isNotEmpty) {
        customProperties['riders'] = aclFeature.riders
            .map((r) => switch (r) {
                  ConditionRider c => {
                      'type': 'condition',
                      ...
                      if (c.saveAttributeKey != null)
                        'saveAbility': c.saveAttributeKey,
                    },
                  AttributeDrainRider a => {
                      'type': 'attributeDrain',
                      'targetAbility': a.targetAttributeKey,
      ...
      if (aclFeature.hasNimbleEscape)
        customProperties['hasNimbleEscape'] = true;
      if (aclFeature.hasPackTactics) customProperties['hasPackTactics'] = true;
      if (aclFeature.maxReachFt > 5)
        customProperties['maxReachFt'] = aclFeature.maxReachFt;
      if (aclFeature.spellSaveDc != null)
        customProperties['spellSaveDc'] = aclFeature.spellSaveDc;
      if (aclFeature.spellAttackBonus != null)
        customProperties['spellAttackBonus'] = aclFeature.spellAttackBonus;
    }
      ...
      customProperties['rows'] = (customProperties['rows'] as List).map((row) {
        if (row is List) {
          return row.map((cell) {
            if (cell is String) {
              return transformer
                  .transformEntries(cell, defaultRuleset: ruleset)
                  .markdown
                  .replaceAll('\n', ' ')
                  .trim();
            }
    ...
    if (customProperties['colLabels'] is List) {
      customProperties['colLabels'] =
          (customProperties['colLabels'] as List).map((col) {
        if (col is String) {
          return transformer
              .transformEntries(col, defaultRuleset: ruleset)
              .markdown
              .replaceAll('\n', ' ')
              .trim();
        }
  ...
  String _decodeFeatureType(dynamic ftData) {
    final str = (ftData is List ? ftData.join(', ') : ftData.toString())
        .toUpperCase()
        .trim();
  ...
  String _cleanCategoryString(String cat) {
    ...
    if (upper.contains('MV') ||
        upper.contains('MANEUVER') ||
        upper.contains('BM')) {
      return 'Maneuver';
    }
