...
class CompendiumFeatParser {
  ...
  Feat parseFeat(Map<String, dynamic> raw, {RulesetVersion? forceRuleset}) {
      ...
      prereq = rawPrereq.map((e) {
        if (e is Map) {
          ...
          if (e['ability'] is List) {
            final ab =
                (e['ability'] as List).map((a) => a.toString()).join(' or ');
            parts.add(ab);
          }
    ...
    if (raw.containsKey('repeatable'))
      customProperties['repeatable'] = raw['repeatable'];
    if (raw.containsKey('category'))
      customProperties['category'] = raw['category'];

    // Parse structured Ability Score Improvements (ASI)
    final asi =
        _parseAbilityScores(raw['ability'] ?? customProperties['ability']);
  ...
  String _parseCategory(Map<String, dynamic> raw) {
    ...
    if (ftRaw is Map) {
      ...
      if (first.toLowerCase().contains('fighting style'))
  ...
  ({List<String> selectableAbilities, int amount}) _parseAbilityScores(
      dynamic rawAbility) {
        ...
        if ([
          ...
          if (v['from'] is List) {
            for (final item in v['from']) {
              final mapped =
                  _expandAbility(item.toString().toLowerCase().trim());
              if (!abilities.contains(mapped)) abilities.add(mapped);
            }
          }
  ...
  List<FeatureGrant> _extractGrants(
      ...
      if (node is String) {
        ...
        if (spName.isNotEmpty && !spName.startsWith('{')) {
          ...
          if (spSlug.isNotEmpty &&
              !grants.any((g) => g.grantId == 'feat-$slug-spell-$spSlug')) {
      ...
      for (final s in skills) {
        if (s is String && s.isNotEmpty) {
          ...
          grants.add(FeatureGrant.skillChoice(
            ...
            count: count,
            pool:
                fromList.map((f) => f.toString().toLowerCase().trim()).toList(),
    ...
    // Armor Proficiencies
    final armors = raw['armorProficiencies'] ??
        custom['armorProficiencies'] ??
        raw['armor'] ??
        custom['armor'];
    if (armors is List) {
    ...
    // Tool Proficiencies
    final tools = raw['toolProficiencies'] ??
        custom['toolProficiencies'] ??
        raw['tools'] ??
        custom['tools'];
    if (tools is List) {
    ...
    final weapons = raw['weaponProficiencies'] ??
        custom['weaponProficiencies'] ??
        raw['weapons'] ??
        custom['weapons'];
    if (weapons is List) {
