...
class CompendiumBackgroundParser {
  ...
  Background parseBackground(
    ...
    var skillProficiencies = _parseProficiencies(
      _isNonEmpty(raw['skillProficiencies']) ??
          _isNonEmpty(raw['skills']) ??
          _isNonEmpty(raw['skill']) ??
    ...
    final toolProficiencies = _parseProficiencies(
      _isNonEmpty(raw['toolProficiencies']) ??
          _isNonEmpty(raw['tools']) ??
    ...
    final modEntries = raw['_copy'] is Map
        ? _extractModEntries((raw['_copy'] as Map)['_mod'])
        : null;
    ...
    if (description.isEmpty && base != null) {
      description = base.descriptionMarkdown;
        ...
        base != null &&
        !description.contains(base.name)) {
    ...
    if (raw.containsKey('ability'))
      customProperties['ability'] = raw['ability'];
    if (raw.containsKey('backgroundFeature'))
      customProperties['backgroundFeature'] = raw['backgroundFeature'];
    if (raw.containsKey('additionalSpells'))
      customProperties['additionalSpells'] = raw['additionalSpells'];

    // Agnostic structural narrative feature extraction from entries AST or raw
    final bgFeature = _extractBackgroundFeature(entriesData, raw, ruleset);
    if (bgFeature != null) {
      customProperties['backgroundFeature'] = bgFeature.name;
      customProperties['backgroundFeatureDescription'] = bgFeature.description;
      customProperties['feature'] = bgFeature.name;
      customProperties['featureDescription'] = bgFeature.description;
    }

    final grants = _extractGrants(
      slug,
      skillProficiencies,
      toolProficiencies,
      languages,
  ...
  Background? _resolveBaseBackground(Map<String, dynamic> raw,
    ...
    final copyName =
        (raw['_copy'] as Map)['name']?.toString().toLowerCase().trim() ?? '';
    ...
    if (localLookup != null) {
      ...
      for (final entry in localLookup.entries) {
        ...
        if (k == copyName ||
            k == copySlug ||
            entry.value.name.toLowerCase() == copyName) {
          return entry.value;
        }
      }
    }
  ...
  String? _parseOriginFeat(dynamic featData) {
    ...
    if (featData is String) {
      ...
      if (first is String) {
        ...
        rawFeat = first['feat']?.toString() ??
            first['name']?.toString() ??
            first.keys.first.toString();
      }
    } else if (featData is Map) {
      rawFeat = featData['feat']?.toString() ??
          featData['name']?.toString() ??
          featData.keys.first.toString();
    }
  ...
  String? _parseAbilitySummary(dynamic abilityData) {
    ...
    if (abilityData is List) {
      ...
      for (final item in abilityData) {
        if (item is Map) {
          final keys =
              item.keys.map((k) => k.toString().toUpperCase()).toList();
    ...
    if (abilityData is Map) {
      final keys =
          abilityData.keys.map((k) => k.toString().toUpperCase()).toList();
    ...
    if (profData is List) {
      for (final item in profData) {
        if (item is String) {
          ...
          _extractFromProfMap(item, results);
        }
      }
    } else if (profData is Map) {
      _extractFromProfMap(profData, results);
    } else if (profData is String) {
      results.add(profData.trim());
    }
    return results;
  }

  void _extractFromProfMap(Map item, List<String> results) {
    item.forEach((k, v) {
      final keyStr = k.toString().trim();
      if (v == true) {
        results.add(_formatProficiencyName(keyStr));
      } else if (keyStr == 'choose' && v is Map) {
        final count = v['count'] ?? 1;
        results.add('Choose $count');
      } else if (v is List) {
        for (final subItem in v) {
          if (subItem is String) {
            results.add(subItem.trim());
          }
        }
      }
    });
  ...
  String _formatProficiencyName(String rawName) {
    final clean = rawName.trim();
    if (clean.isEmpty || clean.toLowerCase().startsWith('choose')) return clean;
    return clean
        .split(' ')
        .map((w) => w.isNotEmpty ? w[0].toUpperCase() + w.substring(1) : '')
        .join(' ');
  ...
  ({String name, String description})? _extractBackgroundFeature(
    dynamic entriesData,
    Map<String, dynamic> raw,
    RulesetVersion ruleset,
  ) {
    // 1. Direct explicit keys in raw
    final explicitName =
        raw['backgroundFeature']?.toString() ?? raw['feature']?.toString();
    final explicitDesc = raw['backgroundFeatureDescription']?.toString() ??
        raw['featureDescription']?.toString();
    if (explicitName != null && explicitName.trim().isNotEmpty) {
      return (
        name: explicitName.trim(),
        description: explicitDesc?.trim() ?? '',
      );
    }

    // 2. Search entries structurally
    if (entriesData != null) {
      final found = _searchEntriesForFeature(entriesData, ruleset);
  ...
  ({String name, String description})? _searchEntriesForFeature(
    dynamic node,
    RulesetVersion ruleset,
  ) {
    if (node is List) {
      for (final item in node) {
        final found = _searchEntriesForFeature(item, ruleset);
        if (found != null) return found;
      }
    } else if (node is Map) {
      final entryName = node['name']?.toString().trim();
      if (entryName != null && entryName.isNotEmpty) {
        ...
        if (lower.startsWith('feature:') ||
          ...
          final cleanName = entryName
              .replaceFirst(
                  RegExp(r'^(?:###\s*)?Feature:\s*', caseSensitive: false), '')
              .trim();
          final descData =
              node['entries'] ?? node['desc'] ?? node['text'] ?? '';
          final descMarkdown = transformer
              .transformEntries(descData, defaultRuleset: ruleset)
              .markdown;
          return (name: cleanName, description: descMarkdown);
        }
      }
      if (node.containsKey('entries')) {
        final found = _searchEntriesForFeature(node['entries'], ruleset);
        if (found != null) return found;
      }
  ...
  List<FeatureGrant> _extractGrants(
    ...
    String? originFeat,
    Map<String, dynamic> raw, {
    ({String name, String description})? feature,
  }) {
    ...
    for (final t in tools) {
      ...
      if (clean.isNotEmpty && !clean.startsWith('choose')) {
        grants.add(FeatureGrant.weaponArmorProficiency(
          clean,
          grantId:
              'bg-$slug-tool-${clean.toLowerCase().replaceAll(RegExp(r"[^a-z0-9]+"), "-")}',
          label: '$clean Proficiency',
        ));
      }
    }
    if (feature != null && feature.name.isNotEmpty) {
      grants.add(FeatureGrant(
        type: GrantType.actionDefinition,
        grantId: 'bg-$slug-feature',
        payload: {
    ...
    if (addSpells is List) {
      for (final spGroup in addSpells) {
        if (spGroup is Map) {
          final expanded =
              spGroup['expanded'] ?? spGroup['innate'] ?? spGroup['known'];
          if (expanded is Map) {
