...
class CompendiumSpellParser {
  ...
  Spell parseSpell(Map<String, dynamic> raw, {RulesetVersion? forceRuleset}) {
    ...
    final entriesData =
        raw['entries'] ?? raw['desc'] ?? raw['description'] ?? raw['text'];
    ...
    final hlData =
        raw['entriesHigherLevel'] ?? raw['higherLevel'] ?? raw['higherLevels'];
    if (hlData != null) {
    ...
    if (damageMath.isNotEmpty) {
      ...
      } else if (raw['damageInflict'] is List &&
          (raw['damageInflict'] as List).isNotEmpty) {
        inferred = DamageType.fromLooseString(
            (raw['damageInflict'] as List).first?.toString());
      }
      if (inferred == null || inferred == DamageType.untyped) {
        for (final dt in DamageType.values) {
          if (dt != DamageType.untyped &&
              dt != DamageType.variable &&
              descLower.contains('${dt.name} damage')) {
            inferred = dt;
            break;
          }
        }
      ...
      if (inferred != null && inferred != DamageType.untyped) {
        damageMath = damageMath.map((m) {
          if (m.damageType == DamageType.untyped ||
              (isVariable && m.damageType == DamageType.acid)) {
    ...
    if (raw['scalingLevelDice'] is Map) {
      ...
      if (scaling is Map && scaling.isNotEmpty) {
        ...
        if (firstScale != null && damageMath.isNotEmpty) {
          damageMath = [
            damageMath.first
                .copyWith(scalingFormula: '+$firstScale per higher slot'),
            ...damageMath.skip(1),
          ];
        }
      }
      ...
      final extracted =
          HomebrewIngestor.extractHigherLevelsDice(higherLevelsMarkdown);
      if (extracted != null) {
        bool applied = false;
        for (var i = 0; i < damageMath.length; i++) {
          ...
          if (dm.scalingFormula == null || dm.scalingFormula!.isEmpty) {
            ...
            final mentionsType = typeName != 'untyped' &&
                higherLevelsMarkdown.toLowerCase().contains(typeName);
    ...
    final rangeType =
        (rangeNorm['rangeType'] != null && rangeNorm['rangeType'] != 'ranged')
            ? rangeNorm['rangeType'] as String
            : (raw['rangeType']?.toString() ??
                (rangeNorm['rangeType'] as String? ?? 'ranged'));
    final resolvedDamageType = isVariable
        ? 'variable'
        : (damageMath.isNotEmpty
            ? damageMath.first.damageType.name
            : 'untyped');
    ...
    if (raw['customProperties'] is Map) {
      customProperties
          .addAll(Map<String, dynamic>.from(raw['customProperties'] as Map));
      if (customProperties['customProperties'] is Map) {
        final inner = Map<String, dynamic>.from(
            customProperties['customProperties'] as Map);
    ...
    final rawClasses = raw['classes'] ??
        (raw['customProperties'] is Map
            ? (raw['customProperties'] as Map)['classes']
            : null) ??
        customProperties['classes'];
    if (rawClasses is Map && rawClasses.isNotEmpty) {
      customProperties['classes'] = rawClasses;
      final hasClassList = (rawClasses['fromClassList'] is List &&
          (rawClasses['fromClassList'] as List).isNotEmpty);
      final hasClassVariant = (rawClasses['fromClassListVariant'] is List &&
          (rawClasses['fromClassListVariant'] as List).isNotEmpty);
      final hasSubclass = (rawClasses['fromSubclass'] is List &&
          (rawClasses['fromSubclass'] as List).isNotEmpty);
      if (!hasClassList && !hasClassVariant && !hasSubclass) {
        final parsed = _extractClasses(rawClasses, slug, name, ruleset);
        if (parsed.isNotEmpty) {
          rawClasses['fromClassList'] =
              parsed.map((c) => {'name': c, 'source': 'SRD'}).toList();
        }
      }
    } else {
  ...
  SpellDuration _parseDurationString(String str) {
    ...
    final match =
        RegExp(r'(\d+)\s*(round|minute|hour|day)s?').firstMatch(lower);
    if (match != null) {
    ...
    if (compData is Map) {
      ...
      if (rawM is String) {
        ...
        final rawCost = rawM['cost'] is num
            ? (rawM['cost'] as num)
            : (rawM['cost'] is String
                ? num.tryParse(rawM['cost'].toString())
                : null);
        cost = rawCost != null
      ...
      for (final item in compData) {
        ...
        if (str.startsWith('M')) {
          m = true;
          final match = RegExp(r'M\s*\((.*?)\)', caseSensitive: false)
              .firstMatch(item.toString());
      ...
      if (upper.contains('M')) {
        m = true;
        final match =
            RegExp(r'M\s*\((.*?)\)', caseSensitive: false).firstMatch(compData);
  ...
  List<String> extractClassesForSpell(
      String slug, String name, RulesetVersion ruleset) {
    return _extractClasses(null, slug, name, ruleset);
  }

  List<String> _extractClasses(
      dynamic rawClasses, String slug, String name, RulesetVersion ruleset) {
    ...
    void addClass(String cName) {
      ...
      final match = SpellClass.values
          .where(
            (sc) =>
                sc.name.toLowerCase() == clean.toLowerCase() ||
                sc.label.toLowerCase() == clean.toLowerCase(),
          )
          .firstOrNull;
    ...
    if (classes.isEmpty) {
      final srdMatch = SpellbookLibrary.allSpells
          .where(
              (s) => s.name.toLowerCase() == name.toLowerCase() || s.id == slug)
          .firstOrNull;
      if (srdMatch != null) {
        final srdClasses = (ruleset == RulesetVersion.v2024
                ? srdMatch.rules2024
                : srdMatch.rules2014)
            .classes;
