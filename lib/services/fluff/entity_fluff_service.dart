...
class EntityFluff {
  final String
      entityType; // 'monster', 'spell', 'item', 'class', 'feat', 'race', 'background'
  final String slug;
...
class EntityFluffService extends ChangeNotifier {
      ...
      if (rawFluff != null && rawFluff.isNotEmpty) {
        ...
        decoded.forEach((k, v) {
          if (v is Map<String, dynamic>) {
            ...
            _fluffRegistry[k] =
                EntityFluff.fromMap(Map<String, dynamic>.from(v));
          }
        });
      }
  ...
  void setFluff(
    ...
    final updatedLore = existing != null &&
            existing.loreMarkdown.isNotEmpty &&
            existing.loreMarkdown != loreMarkdown
  ...
  void batchRegisterFluff(List<EntityFluff> items, {bool saveToDisk = true}) {
    for (final item in items) {
      ...
      if (existing == null) {
        ...
        final combinedLore = existing.loreMarkdown.isNotEmpty &&
                existing.loreMarkdown != item.loreMarkdown
            ? '${existing.loreMarkdown}\n\n${item.loreMarkdown}'.trim()
            : (item.loreMarkdown.isNotEmpty
                ? item.loreMarkdown
                : existing.loreMarkdown);
        _fluffRegistry[key] = EntityFluff(
  ...
  void setUserNotes(String entityType, String slug, String notes,
      {bool saveToDisk = true}) {
    final key = _buildKey(entityType, slug);
  ...
  void _persistUserNotes() {
    DebouncedStorageService().scheduleWrite(_kUserNotesStorageKey, () async {
      ...
      await prefs.setString(
          _kUserNotesStorageKey, json.encode(_userNotesRegistry));
    });
  }
