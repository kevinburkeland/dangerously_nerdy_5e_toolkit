...
class CompendiumTagParser {
  static final RegExp tagPattern =
  ...
  ParsedTextResult parseEntries(
    ...
    _parseNode(
        entries, buffer, mathList, refsList, 0, defaultRuleset, maxDepth);
  ...
  void _parseNode(
    ...
    if (node is String) {
      buffer.writeln(processTags(node,
          math: math, refs: refs, defaultRuleset: defaultRuleset));
      buffer.writeln();
      ...
      switch (type) {
        case 'entries':
        case 'section':
          _parseNode(node['entries'], buffer, math, refs, depth + 1,
              defaultRuleset, maxDepth);
        ...
        case 'list':
          ...
          for (final item in items) {
            buffer.write('- ');
            _parseNode(
                item, buffer, math, refs, depth, defaultRuleset, maxDepth);
          }

        case 'table':
          buffer.writeln(parseTableNode(node,
              math: math, refs: refs, defaultRuleset: defaultRuleset));
          buffer.writeln();
