...
class CommunityCompendiumAdapters {
  ...
  Spell parseSpell(Map<String, dynamic> json, {RulesetVersion? forceRuleset}) {
    ...
    if (json.containsKey('entriesHigherLevel')) {
      ...
      higherLevels = parsedHigher.cleanMarkdown.isNotEmpty
          ? parsedHigher.cleanMarkdown
          : null;
    }
  ...
  Monster parseMonster(Map<String, dynamic> json,
    ...
    final actionEntries = rawAction is List
        ? rawAction
        : (rawAction != null ? [rawAction] : const []);
    final rawTrait = json['trait'] ?? json['traits'];
    final traitEntries = rawTrait is List
        ? rawTrait
        : (rawTrait != null ? [rawTrait] : const []);
    final combined = <dynamic>[];
    if (traitEntries.isNotEmpty) {
      combined
          .add({'type': 'section', 'name': 'Traits', 'entries': traitEntries});
    }
    if (actionEntries.isNotEmpty) {
      combined.add(
          {'type': 'section', 'name': 'Actions', 'entries': actionEntries});
    }

    final parsedActions =
        parser.parseEntries(combined, defaultRuleset: ruleset);
  ...
  EquipmentItem parseItem(Map<String, dynamic> json,
      {RulesetVersion? forceRuleset}) {
    ...
    final itemType = switch (typeCode.toUpperCase()) {
      ...
      'WD' => 'Wand',
      'G' => 'Adventuring Gear',
      _ => (name.toLowerCase().contains('armor') ||
              name.toLowerCase().contains('shield'))
          ? 'Armor'
    ...
    final parsedEntries =
        parser.parseEntries(json['entries'], defaultRuleset: ruleset);
    ...
    return EquipmentItem(
      ...
      customProperties: {
        ...
        if (json.containsKey('ac'))
          'ac': (json['ac'] is num)
              ? (json['ac'] as num).toInt()
              : int.tryParse(json['ac'].toString()),
        if (json.containsKey('ac'))
          'baseAc': (json['ac'] is num)
              ? (json['ac'] as num).toInt()
              : int.tryParse(json['ac'].toString()),
  ...
  CharacterClass parseClass(
    ...
    final parsed = parser.parseEntries(json['classFeatures'] ?? json['entries'],
        defaultRuleset: ruleset);
    ...
    if (json['featureDecisions'] is List) {
      ...
      for (final prog in json['optionalfeatureProg']) {
        if (prog is Map) {
          ...
          final progTypeStr =
              prog['featureType']?.toString().toLowerCase() ?? '';
          final progLevel = (prog['progression'] as Map?)
                  ?.keys
          ...
          if (progTypeStr.contains('fs') ||
              progName.toLowerCase().contains('fighting style')) {
            choiceType = FeatureChoiceType.fightingStyle;
          } else if (progTypeStr.contains('ei') ||
              progName.toLowerCase().contains('invocation')) {
            choiceType = FeatureChoiceType.invocations;
          } else if (progTypeStr.contains('pb') ||
              progName.toLowerCase().contains('pact boon')) {
    ...
    return CharacterClass(
      ...
      customProperties: {
        ...
        if (json.containsKey('startingProficiencies'))
  ...
  Background parseBackground(Map<String, dynamic> json,
      {RulesetVersion? forceRuleset}) {
