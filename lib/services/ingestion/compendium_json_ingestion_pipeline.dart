...
class CompendiumJsonIngestionPipeline {
  ...
  CompendiumJsonIngestionPipeline({
    ...
    CompendiumGenericEntryParser? genericParser,
        ...
        spellParser = spellParser ??
            CompendiumSpellParser(
                transformer: transformer ?? EntryTagTransformer()),
        monsterParser = monsterParser ??
            CompendiumMonsterParser(
                transformer: transformer ?? EntryTagTransformer()),
        itemParser = itemParser ??
            CompendiumItemParser(
                transformer: transformer ?? EntryTagTransformer()),
        classParser = classParser ??
            CompendiumClassParser(
                transformer: transformer ?? EntryTagTransformer()),
        raceParser = raceParser ??
            CompendiumRaceParser(
                transformer: transformer ?? EntryTagTransformer()),
        featParser = featParser ??
            CompendiumFeatParser(
                transformer: transformer ?? EntryTagTransformer()),
        backgroundParser = backgroundParser ??
            CompendiumBackgroundParser(
                transformer: transformer ?? EntryTagTransformer()),
        genericParser = genericParser ??
            CompendiumGenericEntryParser(
                transformer: transformer ?? EntryTagTransformer());

  /// Parses an in-memory Map containing single entity maps, multi-entity bundles, or [HomebrewBundle] envelopes.
  IngestionBatchResult ingestJsonMap(Map<String, dynamic> map,
  ...
  IngestionBatchResult _ingestMap(Map<String, dynamic> map,
      ...
      try {
        ...
        final revitalizedMonsters = bundle.monsters.map((m) {
          ...
          final hasDefaultStats = m.actionsMarkdown.isEmpty ||
              m.actionsMarkdown.contains(
                  '| 10 (+0) | 10 (+0) | 10 (+0) | 10 (+0) | 10 (+0) | 10 (+0) |') ||
              m.actionsMarkdown
                  .startsWith('**Speed:** 30 ft.\n\n| STR | DEX') ||
          ...
          if (hasCopy && hasDefaultStats) {
            ...
            if (copyObj is Map && copyObj['name'] != null) {
              ...
              return monsterParser.parseMonster(
                ...
                baseLookup: (name, src) =>
                    baseMonsterLookup[name.toLowerCase().trim()],
              );
            }
          }
          ...
          if (actions != m.actionsMarkdown ||
              updatedAttackMath != m.attackMath) {
            return m.copyWith(
                actionsMarkdown: actions, attackMath: updatedAttackMath);
          }
          return m;
        }).toList();
        ...
        final revitalizedSpells = bundle.spells.map((s) {
          ...
          var updated =
              s.copyWith(descriptionMarkdown: desc, higherLevelsMarkdown: hl);

          if (!updated.customProperties.containsKey('classes') ||
              (updated.customProperties['classes'] is List &&
                  (updated.customProperties['classes'] as List).isEmpty)) {
            final parsedClasses = spellParser.parseSpell({
              'id': {
                'slug': updated.id.slug,
                'ruleset': updated.id.ruleset.name
              },
              'name': updated.name,
          ...
          if (updated.rangeDistanceFeet <= 0 && updated.range.isNotEmpty) {
            final norm = HomebrewIngestor.normalizeRange(
                updated.range, updated.descriptionMarkdown);
            ...
            if (dist > 0 || rType != updated.rangeType) {
              updated =
                  updated.copyWith(rangeDistanceFeet: dist, rangeType: rType);
            }
          }
          return updated;
        }).toList();
        ...
        final revitalizedItems = bundle.items.map((i) {
          ...
          if (customProps.containsKey('rechargeAmount')) {
            ...
            if (rc is String && rc.contains('{@')) {
              customProps['rechargeAmount'] =
                  cleanRawTags(rc).replaceAll('`', '').trim();
            }
          }
          ...
          final reparsed = itemParser.parseItem(raw,
              forceRuleset: forceRuleset ?? i.id.ruleset);
          return reparsed.copyWith(
            descriptionMarkdown:
                desc.isNotEmpty ? desc : reparsed.descriptionMarkdown,
            customProperties: customProps,
          );
        }).toList();

        final revitalizedRaces = bundle.races
            .where((r) => r.id.slug.isNotEmpty && r.name.isNotEmpty)
            .map((r) {
          final raw = <String, dynamic>{
            ...r.customProperties,
            'name': r.name,
            ...
            if (r.abilityScoreSummary != null)
              'abilityScoreSummary': r.abilityScoreSummary,
          };
          final reparsed = raceParser.parseRace(raw,
              forceRuleset: forceRuleset ?? r.id.ruleset);
          final validSubraces = r.subraces
              .where((s) => s.id.slug.isNotEmpty || s.name.isNotEmpty)
              .toList();
          return reparsed.copyWith(
            traitsMarkdown: r.traitsMarkdown.isNotEmpty
                ? r.traitsMarkdown
                : reparsed.traitsMarkdown,
            subraces:
                validSubraces.isNotEmpty ? validSubraces : reparsed.subraces,
          );
        }).toList();
        ...
        final revitalizedFeats = bundle.feats.map((f) {
          final raw = <String, dynamic>{
            ...
            'category': f.category,
            if (f.descriptionMarkdown.isNotEmpty)
              'entries': [f.descriptionMarkdown],
          };
          final reparsed = featParser.parseFeat(raw,
              forceRuleset: forceRuleset ?? f.id.ruleset);
          return reparsed.copyWith(
            descriptionMarkdown: f.descriptionMarkdown.isNotEmpty
                ? f.descriptionMarkdown
                : reparsed.descriptionMarkdown,
        ...
        final revitalizedBackgrounds = bundle.backgrounds.map((b) {
          final raw = <String, dynamic>{
            ...
            if (b.originFeat != null && b.originFeat!.isNotEmpty)
              'feat': b.originFeat,
            if (b.abilityScoreSummary != null &&
                b.abilityScoreSummary!.isNotEmpty)
              'ability': b.abilityScoreSummary,
            if (b.descriptionMarkdown.isNotEmpty)
              'entries': [b.descriptionMarkdown],
          };
          ...
          return reparsed.copyWith(
            descriptionMarkdown: b.descriptionMarkdown.isNotEmpty
                ? b.descriptionMarkdown
                : reparsed.descriptionMarkdown,
            skillProficiencies: reparsed.skillProficiencies.isNotEmpty
                ? reparsed.skillProficiencies
                : b.skillProficiencies,
            toolProficiencies: reparsed.toolProficiencies.isNotEmpty
                ? reparsed.toolProficiencies
                : b.toolProficiencies,
            languages: reparsed.languages.isNotEmpty
                ? reparsed.languages
                : b.languages,
            originFeat: reparsed.originFeat ?? b.originFeat,
            abilityScoreSummary:
                reparsed.abilityScoreSummary ?? b.abilityScoreSummary,
        ...
        final revitalizedOtherEntries = bundle.otherEntries.where((o) {
          ...
          if (customProps['rows'] is List) {
            customProps['rows'] = (customProps['rows'] as List).map((row) {
              if (row is List) {
                return row
                    .map((cell) => cell is String ? cleanRawTags(cell) : cell)
                    .toList();
              }
              return row;
            }).toList();
          ...
          if (customProps['colLabels'] is List) {
            customProps['colLabels'] =
                (customProps['colLabels'] as List).map((col) {
          ...
          final raw = <String, dynamic>{
            ...
            'name': o.name,
            'category': o.category,
            if (o.descriptionMarkdown.isNotEmpty)
              'entries': [cleanRawTags(o.descriptionMarkdown)],
          };
          final reparsed = genericParser.parseGenericEntry(raw,
              forceRuleset: forceRuleset ?? o.id.ruleset,
              defaultCategory: o.category);
          return reparsed.copyWith(
            descriptionMarkdown: cleanRawTags(o.descriptionMarkdown.isNotEmpty
                ? o.descriptionMarkdown
                : reparsed.descriptionMarkdown),
            customProperties: customProps,
          );
        }).toList();
        ...
        final revitalizedClasses = bundle.classes.map((c) {
          ...
          if (c.featuresMarkdown.contains('{@')) {
            updated = updated.copyWith(
                featuresMarkdown: cleanRawTags(updated.featuresMarkdown));
          }
          if (c.grants.isEmpty &&
              (c.customProperties.containsKey('additionalSpells') ||
                  c.customProperties.containsKey('spells'))) {
            final grants = classParser.extractSpellsGrants(
              c.customProperties['additionalSpells'] ??
            ...
            if (grants.isNotEmpty) {
              updated =
                  updated.copyWith(grants: [...updated.grants, ...grants]);
            }
          }
          return updated;
        }).toList();
        ...
        final revitalizedSubclasses = bundle.subclasses.map((s) {
          ...
          if (s.grants.isEmpty &&
              (s.customProperties.containsKey('additionalSpells') ||
                  s.customProperties.containsKey('subclassSpells'))) {
            final grants = classParser.extractSpellsGrants(
              s.customProperties['additionalSpells'] ??
    ...
    final isSingleClass =
        lowerKeys.contains('hd') && lowerKeys.contains('name');
    final isSingleMonster =
        lowerKeys.contains('cr') && lowerKeys.contains('name');
    final isSingleSpell =
        lowerKeys.contains('school') && lowerKeys.contains('name');
    ...
    final hasMechanicalKeys = lowerKeys.contains('cr') ||
        ...
        lowerKeys.contains('subclassfeature') ||
        lowerKeys.contains('subclassshortname') ||
        lowerKeys.contains('classfeature') ||
    ...
    final isSingleFluff =
        (lowerKeys.contains('name') || lowerKeys.contains('title')) &&
            hasFluffIndicators &&
            (lowerKeys.contains('_fluff') ||
                lowerKeys.contains('flufftype') ||
                !hasMechanicalKeys);
    ...
    if (!hasBundleKeys ||
        ...
        isSingleFluff) {
      final singleResult =
          _ingestSingleEntityMap(map, forceRuleset: forceRuleset);
    ...
    List<dynamic>? findListForKeys(List<String> candidateKeys) {
      for (final candidate in candidateKeys) {
        ...
        for (final entry in map.entries) {
          ...
          if ((entryLower == candLower ||
                  (candLower.length >= 6 && entryLower.endsWith(candLower))) &&
              entry.value is List) {
    ...
    void ingestKeys(List<String> candidateKeys,
        void Function(Map<String, dynamic>) handler, String entityLabel) {
        ...
        for (final raw in list) {
          if (raw is Map) {
            try {
              ...
              errors
                  .add('$entityLabel error (${raw['name'] ?? 'unnamed'}): $e');
            }
          }
        }
      }
    ...
    ingestKeys(
        ['spell', 'spells'],
        (raw) =>
            spells.add(spellParser.parseSpell(raw, forceRuleset: forceRuleset)),
        'Spell');

    // Monsters / Bestiary
    final rawMonsterList = findListForKeys([
      'monster',
      'monsters',
      'bestiary',
      'creature',
    ...
    ingestKeys(
        ...
        (raw) =>
            items.add(itemParser.parseItem(raw, forceRuleset: forceRuleset)),
        'Item');
    ...
    ingestKeys(['subclass', 'subclasses'], (raw) {
      ...
      final matchClass = classes
          .where((c) =>
              c.id.slug == sub.classSlug ||
              c.name.toLowerCase() == sub.classSlug.toLowerCase())
          .firstOrNull;
      if (matchClass != null) {
        if (!matchClass.subclasses.any((s) => s.id.slug == sub.id.slug)) {
          final idx = classes.indexOf(matchClass);
          classes[idx] =
              matchClass.copyWith(subclasses: [...matchClass.subclasses, sub]);
        }
      }
    }, 'Subclass');
    ...
    // Races / Species
    ingestKeys(
        ['race', 'races', 'species', 'lineage', 'lineages'],
        (raw) =>
            races.add(raceParser.parseRace(raw, forceRuleset: forceRuleset)),
        'Race');
    ingestKeys(['subrace', 'subraces'], (raw) {
      ...
      if (match != null) {
        ...
        races.add(Race(
          id: EntityId(
              slug: sub.raceSlug, ruleset: forceRuleset ?? sub.id.ruleset),
    ...
    ingestKeys(
        ['feat', 'feats'],
        (raw) =>
            feats.add(featParser.parseFeat(raw, forceRuleset: forceRuleset)),
        'Feat');
    ...
    ingestKeys([
      ...
      if (name.startsWith('pact of the') ||
          name.contains('pact boon') ||
          featureType.contains('PB')) {
        category = 'Pact Boon';
      }
      otherEntries.add(genericParser.parseGenericEntry(raw,
          forceRuleset: forceRuleset, defaultCategory: category));
    }, 'Eldritch Invocation');
    ...
    ingestKeys([
      ...
      otherEntries.add(genericParser.parseGenericEntry(raw,
          forceRuleset: forceRuleset, defaultCategory: 'Infusion'));
    }, 'Infusion');
    ...
    ingestKeys(['optionalfeature', 'optionalfeatures'], (raw) {
      ...
      if (name.startsWith('pact of the') ||
          ...
          featureType.contains('PB')) {
        category = 'Pact Boon';
      } else if (featureType.contains('EI') ||
          name.contains('invocation') ||
          raw['isInvocation'] == true) {
        category = 'Eldritch Invocation';
      } else if (featureType.contains('MM') || name.contains('metamagic')) {
        category = 'Metamagic';
      } else if (featureType.contains('MAN') ||
          featureType.contains('BM') ||
          name.contains('maneuver')) {
        category = 'Maneuver';
      } else if (featureType.contains('AI') ||
          featureType.contains('INF') ||
    ...
    ingestKeys(
        ['table', 'tables'],
        (raw) => otherEntries.add(genericParser.parseGenericEntry(raw,
            forceRuleset: forceRuleset, defaultCategory: 'Table')),
        'Table');
      ...
      if (raw['tables'] is List) {
        for (final table in raw['tables'] as List) {
          if (table is Map) {
            ...
            tableMap['name'] = caption ??
                (opt != null
                    ? '$parentName Names – $opt'
                    : '$parentName Names');
            tableMap['source'] = tableMap['source'] ?? raw['source'];
            otherEntries.add(genericParser.parseGenericEntry(tableMap,
                forceRuleset: forceRuleset, defaultCategory: 'Table'));
          }
        }
    ...
    ingestKeys(
        ['deity', 'deities'],
        (raw) => otherEntries.add(genericParser.parseGenericEntry(raw,
            forceRuleset: forceRuleset, defaultCategory: 'Deity')),
        'Deity');
    ingestKeys(
        ['vehicle', 'vehicles', 'vehicleupgrade', 'vehicleupgrades'],
        (raw) => otherEntries.add(genericParser.parseGenericEntry(raw,
            forceRuleset: forceRuleset, defaultCategory: 'Vehicle')),
        'Vehicle');
    ingestKeys(
        ['trap', 'traps'],
        (raw) => otherEntries.add(genericParser.parseGenericEntry(raw,
            forceRuleset: forceRuleset, defaultCategory: 'Trap')),
        'Trap');
    ingestKeys(
        ['hazard', 'hazards'],
        (raw) => otherEntries.add(genericParser.parseGenericEntry(raw,
            forceRuleset: forceRuleset, defaultCategory: 'Hazard')),
        'Hazard');
    ingestKeys(
        ['reward', 'rewards', 'boon', 'boons', 'cult', 'cults'],
        (raw) => otherEntries.add(genericParser.parseGenericEntry(raw,
            forceRuleset: forceRuleset, defaultCategory: 'Reward')),
        'Reward');
    ingestKeys(
        ['charm', 'charms'],
        (raw) => otherEntries.add(genericParser.parseGenericEntry(raw,
            forceRuleset: forceRuleset, defaultCategory: 'Charm')),
        'Charm');
    ingestKeys(
        ['condition', 'conditions', 'status', 'statuses'],
        (raw) => otherEntries.add(genericParser.parseGenericEntry(raw,
            forceRuleset: forceRuleset, defaultCategory: 'Condition')),
        'Condition');
    ingestKeys(
        ['disease', 'diseases'],
        (raw) => otherEntries.add(genericParser.parseGenericEntry(raw,
            forceRuleset: forceRuleset, defaultCategory: 'Disease')),
        'Disease');
    ingestKeys(
        ['charoption', 'charoptions', 'characteroption', 'characteroptions'],
        (raw) => otherEntries.add(genericParser.parseGenericEntry(raw,
            forceRuleset: forceRuleset, defaultCategory: 'Character Option')),
        'Character Option');
    ingestKeys(
        ['action', 'actions'],
        (raw) => otherEntries.add(genericParser.parseGenericEntry(raw,
            forceRuleset: forceRuleset, defaultCategory: 'Action')),
        'Action');
    ingestKeys(
        ['variantrule', 'variantrules', 'rule', 'rules'],
        (raw) => otherEntries.add(genericParser.parseGenericEntry(raw,
            forceRuleset: forceRuleset, defaultCategory: 'Rule')),
        'Rule');
    ingestKeys(
    ...
    ingestKeys(
        ['psionic', 'psionics'],
        (raw) => otherEntries.add(genericParser.parseGenericEntry(raw,
            forceRuleset: forceRuleset, defaultCategory: 'Psionic')),
        'Psionic');
    ingestKeys(
        ['language', 'languages', 'languagescript', 'languagescripts'],
        (raw) => otherEntries.add(genericParser.parseGenericEntry(raw,
            forceRuleset: forceRuleset, defaultCategory: 'Language')),
        'Language');
    ingestKeys(
        ['sense', 'senses'],
        (raw) => otherEntries.add(genericParser.parseGenericEntry(raw,
            forceRuleset: forceRuleset, defaultCategory: 'Sense')),
        'Sense');
    ingestKeys(
        ['skill', 'skills'],
        (raw) => otherEntries.add(genericParser.parseGenericEntry(raw,
            forceRuleset: forceRuleset, defaultCategory: 'Skill')),
        'Skill');
    ingestKeys(
        ['deck', 'decks', 'card', 'cards'],
        (raw) => otherEntries.add(genericParser.parseGenericEntry(raw,
            forceRuleset: forceRuleset, defaultCategory: 'Deck')),
        'Deck');
    ingestKeys(
        ['monsterfeature', 'monsterfeatures'],
        (raw) => otherEntries.add(genericParser.parseGenericEntry(raw,
            forceRuleset: forceRuleset, defaultCategory: 'Monster Feature')),
        'Monster Feature');
    ingestKeys(
        ['otherentry', 'otherentries', 'genericentry', 'genericentries'],
        (raw) => otherEntries.add(
            genericParser.parseGenericEntry(raw, forceRuleset: forceRuleset)),
        'Generic Entry');
    ...
    ingestFluffKeys(
        ['monsterfluff', 'bestiaryfluff', 'creaturefluff'], 'monster');
    ...
    for (final entry in map.entries) {
      ...
      if (keyLower.endsWith('fluff') &&
          !handledFluffKeys.contains(keyLower) &&
          entry.value is List) {
        final inferredType = keyLower.length > 5
            ? keyLower.substring(0, keyLower.length - 5)
            : 'generic';
    ...
    if (rawSubclassFeatures.isNotEmpty) {
      ...
      for (int i = 0; i < subclasses.length; i++) {
        ...
        if (matchingFeatures.isNotEmpty) {
          ...
          for (final feat in matchingFeatures) {
            ...
            final fContent = entryTransformer
      ...
      for (int i = 0; i < classes.length; i++) {
        ...
        for (final sub in cls.subclasses) {
          final matchingSub =
              subclasses.where((s) => s.id.slug == sub.id.slug).firstOrNull;
    ...
    if (rawClassFeatures.isNotEmpty) {
      ...
      for (int i = 0; i < classes.length; i++) {
        ...
        if (matchingFeatures.isNotEmpty) {
          ...
          if (featureBlocks.isNotEmpty) {
            final combinedMarkdown = cls.featuresMarkdown.isEmpty ||
                    (cls.featuresMarkdown.contains('|') &&
                        !cls.featuresMarkdown.contains('\n'))
                ? featureBlocks.join('\n\n')
                : '${cls.featuresMarkdown}\n\n${featureBlocks.join('\n\n')}';
            classes[i] =
                cls.copyWith(featuresMarkdown: combinedMarkdown.trim());
          }
        }
      }
    }
  ...
  IngestionBatchResult _ingestSingleEntityMap(Map<String, dynamic> rawInputMap,
      {RulesetVersion? forceRuleset}) {
    var map = rawInputMap;
    if (rawInputMap['customProperties'] is Map &&
        (rawInputMap['customProperties'] as Map)['rawJson'] is Map) {
      map = {
        ...((rawInputMap['customProperties'] as Map)['rawJson'] as Map)
            .cast<String, dynamic>(),
        ...rawInputMap,
      };
    }
    ...
    if (hasExplicitFluffKey || isPureFluffMap) {
      ...
      if (lowerKeys.contains('monsterfluff') ||
          map['fluffType'] == 'monster' ||
          map['type'] == 'monster') {
        entityType = 'monster';
      } else if (lowerKeys.contains('spellfluff') ||
          map['fluffType'] == 'spell' ||
          map['type'] == 'spell') {
        entityType = 'spell';
      } else if (lowerKeys.contains('itemfluff') ||
          map['fluffType'] == 'item' ||
          map['type'] == 'item') {
        entityType = 'item';
      } else if (lowerKeys.contains('racefluff') ||
          map['fluffType'] == 'race' ||
          map['type'] == 'race') {
        entityType = 'race';
      } else if (lowerKeys.contains('classfluff') ||
          map['fluffType'] == 'class' ||
          map['type'] == 'class') {
        entityType = 'class';
      } else if (lowerKeys.contains('subclassfluff') ||
          map['fluffType'] == 'subclass' ||
          map['type'] == 'subclass') {
        entityType = 'subclass';
      } else if (lowerKeys.contains('featfluff') ||
          map['fluffType'] == 'feat' ||
          map['type'] == 'feat') {
        entityType = 'feat';
      } else if (lowerKeys.contains('backgroundfluff') ||
          map['fluffType'] == 'background' ||
          map['type'] == 'background') {
        entityType = 'background';
      } else if (map.containsKey('className') ||
          map.containsKey('subclassShortName')) {
        entityType = 'subclass';
          ...
          for (final img in imagesRaw) {
            ...
            if (img is Map) {
              ...
              if (href is Map && href['path'] != null)
                imagePaths.add(href['path'].toString());
        ...
        if (imagePaths.any((p) => p.toLowerCase().contains('bestiary/'))) {
          ...
          entityType = 'race';
            ...
            .any((p) => p.toLowerCase().contains('classes/'))) {
          entityType = 'class';
        }
      }
      ...
      final count =
          _ingestFluffEntry(entityType, map, createdFluff: singleFluff);
      if (count > 0) {
        return IngestionBatchResult(
          fluff: singleFluff,
          attachedFluffCount: count,
    ...
    if (lowerKeys.contains('school') ||
        (lowerKeys.contains('level') &&
            (lowerKeys.contains('time') ||
                lowerKeys.contains('duration') ||
                lowerKeys.contains('range')))) {
      try {
    ...
    if (lowerKeys.contains('cr') ||
        (lowerKeys.contains('hp') &&
            (lowerKeys.contains('ac') || lowerKeys.contains('speed')))) {
      try {
        final monster =
            monsterParser.parseMonster(map, forceRuleset: forceRuleset);
    ...
    if (lowerKeys.contains('hd') ||
        lowerKeys.contains('classfeatures') ||
        (lowerKeys.contains('proficiency') &&
            lowerKeys.contains('subclasses'))) {
      try {
    ...
    // 3b. Identify Subclass Features
    final isSubclassFeature = lowerKeys.contains('subclassfeature') ||
        lowerKeys.contains('gainsubclassfeature') ||
        (lowerKeys.contains('subclassshortname') &&
            !lowerKeys.contains('subclassfeatures'));
    if (isSubclassFeature) {
      try {
        SrdClassesLibrary.registerSubclassFeature(map);
        final entry = genericParser.parseGenericEntry(
          map,
          forceRuleset: forceRuleset,
          defaultCategory: 'Subclass Feature',
        );
        return IngestionBatchResult(otherEntries: [entry]);
      } catch (e) {
        return IngestionBatchResult(
            errors: ['Failed to parse subclass feature: $e']);
      }
    }

    // 3c. Identify Class Features
    final isClassFeature = lowerKeys.contains('classfeature') ||
        (lowerKeys.contains('classname') &&
            lowerKeys.contains('level') &&
            !lowerKeys.contains('hd') &&
            !lowerKeys.contains('hitdie') &&
            !lowerKeys.contains('subclassfeatures') &&
            !lowerKeys.contains('subclasses') &&
            !lowerKeys.contains('shortname') &&
    ...
    if (isClassFeature) {
      try {
        final entry = genericParser.parseGenericEntry(
          map,
          forceRuleset: forceRuleset,
          defaultCategory: 'Class Feature',
        );
        return IngestionBatchResult(otherEntries: [entry]);
      } catch (e) {
        return IngestionBatchResult(
            errors: ['Failed to parse class feature: $e']);
      }
    }

    // 4. Identify Subclasses (has subclassFeatures, subclassTitle, or shortName + className)
    if (lowerKeys.contains('subclassfeatures') ||
        lowerKeys.contains('subclasstitle') ||
        (lowerKeys.contains('shortname') &&
            (lowerKeys.contains('classname') ||
        ...
        ((lowerKeys.contains('classname') ||
            ...
            !lowerKeys.contains('subclassshortname') &&
            (lowerKeys.contains('features') ||
                lowerKeys.contains('desc') ||
                lowerKeys.contains('entries')))) {
      try {
    ...
    if (lowerKeys.contains('skillproficiencies') ||
        lowerKeys.contains('backgroundfeature')) {
      try {
        final bg =
            backgroundParser.parseBackground(map, forceRuleset: forceRuleset);
    ...
    if (lowerKeys.contains('name')) {
      ...
      try {
        final entry = genericParser.parseGenericEntry(map,
            forceRuleset: forceRuleset, defaultCategory: 'Custom');
        ...
        return IngestionBatchResult(
            errors: ['Failed to parse custom compendium entry: $e']);
      }
    }
  ...
  int _ingestFluffEntry(
    ...
    final name =
        raw['name']?.toString().trim() ?? raw['title']?.toString().trim() ?? '';
    ...
    final rawEntries = raw['entries'] ??
        ...
        raw['description'] ??
        raw['lore'];
    var loreMarkdown = rawEntries != null
        ? transformer.transformEntries(rawEntries).markdown.trim()
        : '';
  ...
  static String cleanRawTags(String input) {
    ...
    var current = input.replaceAllMapped(
    ...
    while (tagRegex.hasMatch(current) && passes < 10) {
      ...
      current = current.replaceAllMapped(tagRegex, (match) {
        ...
        final display = parts.length > 2 && parts[2].trim().isNotEmpty
            ? parts[2].trim()
            : primary;
        ...
        switch (tag) {
          ...
          case 'm':
            ...
            if (p.contains('m,r') || p.contains('r,m'))
