  ...
  Spell ingestSpell(Map<String, dynamic> raw) {
    ...
    if (durationList.isNotEmpty && durationList.first is Map) {
      ...
      if (type == 'instant') {
        ...
        final amount =
            (durationMap['duration']?['amount'] as num?)?.toInt() ?? 1;
        final unit =
            durationMap['duration']?['type']?.toString().toLowerCase() ??
                'minute';
    ...
    final spellComponents = SpellComponents(
      ...
      materialCostGp:
          (compMap['m'] is Map ? compMap['m']['cost'] as num? : null)
                  ?.toInt() ??
              0,
    ...
    if (damageMath.isEmpty && raw['damageInflict'] is List) {
      final inflictList = (raw['damageInflict'] as List)
