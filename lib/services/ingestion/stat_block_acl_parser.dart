...
class StatBlockAclParser {
  ...
  static final _reachPattern =
      RegExp(r'reach\s*(\d+)\s*ft', caseSensitive: false);
  static final _slotPattern = RegExp(
      r'(\d+)(?:st|nd|rd|th)\s*level\s*\((\d+)\s*slots?\)',
      caseSensitive: false);
  static final _casterLvlPattern = RegExp(
      r'(\d+)(?:st|nd|rd|th)[- ]level\s+spellcaster',
      caseSensitive: false);
  static final _spellSaveDcPattern =
      RegExp(r'spell\s*save\s*dc\s*(\d+)', caseSensitive: false);
  static final _spellAttackPattern = RegExp(
      r'([+-]?\s*\d+)\s*to\s*hit\s*with\s*spell\s*attacks',
      caseSensitive: false);
  static final _rechargeDayPattern = RegExp(r'(\d+)/day', caseSensitive: false);
  static final _staticAbilitySavePatterns = {
    AbilityType.strength:
        RegExp(r'\b(?:str|strength)\s*([+-]?\s*\d+)', caseSensitive: false),
    AbilityType.dexterity:
        RegExp(r'\b(?:dex|dexterity)\s*([+-]?\s*\d+)', caseSensitive: false),
    AbilityType.constitution:
        RegExp(r'\b(?:con|constitution)\s*([+-]?\s*\d+)', caseSensitive: false),
    AbilityType.intelligence:
        RegExp(r'\b(?:int|intelligence)\s*([+-]?\s*\d+)', caseSensitive: false),
    AbilityType.wisdom:
        RegExp(r'\b(?:wis|wisdom)\s*([+-]?\s*\d+)', caseSensitive: false),
    AbilityType.charisma:
        RegExp(r'\b(?:cha|charisma)\s*([+-]?\s*\d+)', caseSensitive: false),
  };
  ...
  static ({
    ...
    if (spellCorpus.isNotEmpty) {
      ...
      if (slots.isEmpty &&
          (spellCorpus.contains('/day') || spellCorpus.contains('at will'))) {
    ...
    final traitsText = sb.traits
        .map((t) => '${t.name} ${t.description}')
        .join(' ')
        .toLowerCase();
    final actionsText = sb.actions
    ...
    final hasHover =
        speedLower.contains('hover') || traitsText.contains('hover');
    ...
    final hasEvasion =
        traitsText.contains('evasion') || actionsText.contains('evasion');
    ...
    if (sb.legendaryActions.isNotEmpty ||
        fullCorpus.contains('legendary action')) {
    ...
    for (final a in allActions) {
      ...
      if (atkMatch != null) {
        final parsed = int.tryParse(
            atkMatch.group(1)!.replaceAll('+', '').replaceAll(' ', ''));
        if (parsed != null) atkBonus = parsed;
      } else if (a.attackBonus == null &&
          (desc.contains('Attack:') || desc.contains('Hit:'))) {
  ...
  static List<CombatEffectRider> extractRiders(
    ...
    if (drainMatch != null) {
      ...
      riders.add(AttributeDrainRider(
        targetAttributeKey: ability.name,
        diceCount: diceCount,
        diceSides: diceSides,
        flatBonus: flatBonus,
    ...
    if (maxHpMatch != null) {
      ...
      final deathAtZero =
          textLower.contains('to 0') || textLower.contains('dies');
    ...
    if (saveMatch != null) {
      ...
      final ability = abilityStr != null
          ? AbilityType.fromLooseString(abilityStr)
          : defaultAbility;
      ...
      if (condition != null &&
          !riders.any((r) => r is ConditionRider && r.condition == condition)) {
        riders.add(ConditionRider(
          condition: condition,
          requiresSave: true,
          saveDc: dcVal,
          saveAttributeKey: ability?.name,
        ));
      }
    } else {
      ...
      if (proneMatch != null &&
        ...
        riders.add(ConditionRider(
          ...
          saveDc: defaultDc,
          saveAttributeKey: defaultAbility?.name,
        ));
      }
    }
