...
class SkillCollisionReport {
  ...
  final int
      compensatoryPicksEarned; // Count of free compensatory selections granted
  final Set<SkillType>
      availableSkillPool; // Unassigned skills eligible for compensatory pick
...
class SkillTraitResolver {
  ...
  static SkillCollisionReport resolveSkills({
      ...
      if (race != null) {
        for (final grant in race.grants) {
          if (grant.type == GrantType.bonusSkill) {
            final skillName =
                grant.payload['skill']?.toString() ?? grant.label ?? '';
        ...
        if (race.customProperties['skillProficiencies'] is List) {
          for (final sp
              in race.customProperties['skillProficiencies'] as List) {
            if (sp is Map) {
      ...
      if (subrace != null) {
        for (final grant in subrace.grants) {
          if (grant.type == GrantType.bonusSkill) {
            ...
            final skill = SkillType.tryParse(skillName);
            if (skill != null) {
              _addOrCollide(granted, collisions, skill,
                  'Subrace: ${subrace.name} ($skillName)');
            }
          }
        }
        if (subrace.customProperties['skillProficiencies'] is List) {
          for (final sp
              in subrace.customProperties['skillProficiencies'] as List) {
            if (sp is Map) {
              for (final k in sp.keys) {
                final skill = SkillType.tryParse(k.toString());
                if (skill != null) {
                  _addOrCollide(
                      granted, collisions, skill, 'Subrace: ${subrace.name}');
                }
              }
            }
          }
    ...
    // 2. Ingest Background Fixed Skills dynamically from library or legacy fallback
    if (backgroundSlug != null) {
      final bg = backgroundSlug.toLowerCase();
      final bgEntity = SrdBackgroundsLibrary.findBySlug(bg);
      bool resolvedFromLibrary = false;
      if (bgEntity != null && bgEntity.skillProficiencies.isNotEmpty) {
        for (final s in bgEntity.skillProficiencies) {
          final clean = s
              .toLowerCase()
              .replaceAll(' ', '')
              .replaceAll('_', '')
          ...
          for (final st in SkillType.values) {
            if (st.name.toLowerCase() == clean ||
              ...
              _addOrCollide(
                  granted, collisions, st, 'Background: ${bgEntity.name}');
      ...
      if (!resolvedFromLibrary) {
        switch (bg) {
          case 'acolyte':
            _addOrCollide(
                granted, collisions, SkillType.insight, 'Background: Acolyte');
            _addOrCollide(
                granted, collisions, SkillType.religion, 'Background: Acolyte');
          case 'criminal':
          case 'spy':
            _addOrCollide(granted, collisions, SkillType.deception,
                'Background: Criminal');
            _addOrCollide(
                granted, collisions, SkillType.stealth, 'Background: Criminal');
          case 'entertainer':
            _addOrCollide(granted, collisions, SkillType.acrobatics,
                'Background: Entertainer');
            _addOrCollide(granted, collisions, SkillType.performance,
                'Background: Entertainer');
          case 'folk-hero':
          case 'folk_hero':
          case 'guide':
            _addOrCollide(granted, collisions, SkillType.animalHandling,
                'Background: Folk Hero');
            _addOrCollide(granted, collisions, SkillType.survival,
                'Background: Folk Hero');
          case 'guild-artisan':
          case 'guild_artisan':
          case 'merchant':
            _addOrCollide(granted, collisions, SkillType.insight,
                'Background: Guild Artisan');
            _addOrCollide(granted, collisions, SkillType.persuasion,
                'Background: Guild Artisan');
          case 'noble':
            _addOrCollide(
                granted, collisions, SkillType.history, 'Background: Noble');
            _addOrCollide(
                granted, collisions, SkillType.persuasion, 'Background: Noble');
          case 'sage':
            _addOrCollide(
                granted, collisions, SkillType.arcana, 'Background: Sage');
            _addOrCollide(
                granted, collisions, SkillType.history, 'Background: Sage');
          case 'sailor':
            _addOrCollide(
                granted, collisions, SkillType.athletics, 'Background: Sailor');
            _addOrCollide(granted, collisions, SkillType.perception,
                'Background: Sailor');
          case 'soldier':
            _addOrCollide(granted, collisions, SkillType.athletics,
                'Background: Soldier');
            _addOrCollide(granted, collisions, SkillType.intimidation,
                'Background: Soldier');
          case 'urchin':
            _addOrCollide(granted, collisions, SkillType.sleightOfHand,
                'Background: Urchin');
            _addOrCollide(
                granted, collisions, SkillType.stealth, 'Background: Urchin');
        }
      }
    }
    ...
    final available =
        SkillType.values.where((s) => !granted.containsKey(s)).toSet();
  ...
  static int getSpeciesBonusSkillCount(
      String? speciesSlug, DmRulesEdition edition) {
  ...
  static List<InnateSpeciesSpell> getInnateSpeciesSpells({
    ...
    void processGrantsAndSpells(
        List<FeatureGrant> grants, Map<String, dynamic> customProps) {
      for (final g in grants) {
        if (g.type == GrantType.bonusSpell) {
          final spellSlug = g.payload['slug']?.toString() ?? '';
          final spellName =
              g.payload['displayName']?.toString() ?? g.label ?? spellSlug;
          final spellFromLib = SpellbookLibrary.getSpellById(spellSlug);
          final isCantrip =
              g.payload['isCantrip'] == true || spellFromLib?.level == 0;
          if (spellSlug.isNotEmpty &&
              !spells.any((s) => s.spellRef.slug == spellSlug)) {
      ...
      final rawAddSpells =
          customProps['additionalSpells'] ?? customProps['spells'];
                ...
                section.forEach((lvlKey, spellList) {
                  ...
                  if (sectionKey == 'expanded') {
                    if (lvlStr == 's1' || lvlStr == '1' || lvlStr == '_') {
                      ...
                      reqLevel = int.tryParse(
                              lvlStr.replaceAll(RegExp(r'[^0-9]'), '')) ??
                          1;
                    }
                  } else {
                    if (lvlStr == '_') {
                  ...
                  void addSpellItems(dynamic listOrMap) {
                    if (listOrMap is List) {
                      for (final item in listOrMap) {
                        ...
                        final isCantripRaw = str.contains('#c') ||
                            lvlStr == '_' ||
                            lvlStr == '0' ||
                            lvlStr == 's0';
                        final cleanName =
                            str.replaceAll('#c', '').split('|').first.trim();
                        final cleanSlug = cleanName
                            .toLowerCase()
                        ...
                        final isCantrip =
                            isCantripRaw || spellFromLib?.level == 0;
                        final finalMinLevel = isCantrip ? 1 : reqLevel;
                        if (totalCharacterLevel >= finalMinLevel &&
                            !spells.any((s) => s.spellRef.slug == cleanSlug)) {
                          spells.add(InnateSpeciesSpell(
                            spellRef: EntityReference<Spell>(
                              refType: EntityType.spell,
                              slug: cleanSlug,
  ...
  static List<String> resolveTools({
    ...
    if (customProperties != null) {
      final dynamicTools = customProperties['tools'] ??
          customProperties['startingTools'] ??
          (customProperties['startingProficiencies'] is Map
              ? customProperties['startingProficiencies']['tools']
              : null);
      if (dynamicTools is List) {
        for (final t in dynamicTools) {
          final tStr = t.toString().trim();
          if (tStr.isNotEmpty &&
              !tools.any((x) => x.toLowerCase() == tStr.toLowerCase())) {
            tools.add(tStr);
          }
        }
      }
    ...
    if (speciesSlug != null && speciesSlug.isNotEmpty) {
      ...
      if (sp != null) {
        for (final g in sp.grants) {
          if (g.type == GrantType.proficiency &&
              g.payload['proficiency'] != null) {
            final prof = g.payload['proficiency'].toString();
            if (_isToolProficiency(prof) &&
                !tools.any((t) => t.toLowerCase() == prof.toLowerCase())) {
              tools.add(prof);
            }
          }
        }
      ...
      if (feat != null) {
        for (final g in feat.grants) {
          if (g.type == GrantType.bonusTool && g.payload['tool'] != null) {
            ...
            if (tName.isNotEmpty &&
                !tools.any((t) => t.toLowerCase() == tName.toLowerCase())) {
              tools.add(tName);
            }
            ...
            if (_isToolProficiency(pName) &&
                !tools.any((t) => t.toLowerCase() == pName.toLowerCase())) {
              tools.add(pName);
            }
          }
        }
  ...
  static List<String> resolveArmorProficiencies({
    ...
    for (final fSlug in featSlugs) {
      ...
      if (feat != null) {
        ...
        final featArmors = feat.customProperties['armorProficiencies'] ??
            (feat.customProperties['rawJson'] is Map
                ? feat.customProperties['rawJson']['armorProficiencies']
                : null);
    ...
    // 4. Custom Properties
    if (customProperties != null &&
        customProperties['armorProficiencies'] is List) {
  ...
  static List<String> resolveWeaponProficiencies({
    ...
    for (final fSlug in featSlugs) {
      ...
      if (feat != null) {
        ...
        final featWeapons = feat.customProperties['weaponProficiencies'] ??
            (feat.customProperties['rawJson'] is Map
                ? feat.customProperties['rawJson']['weaponProficiencies']
                : null);
    ...
    // 4. Custom Properties
    if (customProperties != null &&
        customProperties['weaponProficiencies'] is List) {
  ...
  static ({
    ...
    if (subrace?.speed != null && subrace!.speed!.isNotEmpty) {
      final parsedSubSpeed = int.tryParse(
          RegExp(r'\d+').firstMatch(subrace.speed!)?.group(0) ?? '');
      if (parsedSubSpeed != null && parsedSubSpeed > 0) {
        speed = parsedSubSpeed;
      }
    } else if (race != null) {
      final parsedSpeed =
          int.tryParse(RegExp(r'\d+').firstMatch(race.speed)?.group(0) ?? '');
    ...
    if (subrace?.darkvision != null && subrace!.darkvision! > 0) {
      ...
      darkvision =
          (race?.customProperties['darkvisionFeet'] as num?)?.toInt() ?? 60;
    } else if (slug.contains('elf') ||
        slug.contains('dwarf') ||
        slug.contains('gnome') ||
        slug.contains('half-orc') ||
        slug.contains('tiefling')) {
      darkvision =
          (subSlug.contains('drow') || slug.contains('drow')) ? 120 : 60;
    }

    // Powerful build
    if (race?.traitsMarkdown.toLowerCase().contains('powerful build') == true ||
        subrace?.traitsMarkdown.toLowerCase().contains('powerful build') ==
            true ||
        slug == 'goliath') {
      powerfulBuild = true;
    }
    ...
    if (subrace?.traitsMarkdown.toLowerCase().contains('dwarven toughness') ==
            true ||
        subrace?.traitsMarkdown
                .toLowerCase()
                .contains('hit point maximum increases by 1') ==
            true ||
        (slug.contains('dwarf') &&
            (edition == DmRulesEdition.v2024 || subSlug.contains('hill')))) {
      hpBonus = 1;
    }
