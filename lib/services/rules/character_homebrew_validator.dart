...
class CharacterHomebrewValidator {
  ...
  static MissingHomebrewReport validate(Character character) {
    ...
    for (final clsProg in character.progression.classes) {
      ...
      final isHomebrewClass =
          classRef.rulesetPreferred == RulesetVersion.homebrew ||
    ...
    if (subraceProp != null) {
      ...
      if (subraceStr.isNotEmpty) {
        final foundSubrace = SrdSpeciesLibrary.allSpecies.any((r) => r.subraces
            .any((s) =>
    ...
    for (final featRef in character.feats) {
      ...
      final isHomebrewFeat =
          featRef.rulesetPreferred == RulesetVersion.homebrew ||
    ...
    for (final itemInstance in character.inventory) {
      ...
      if (isHomebrewItem) {
        ...
        final foundPackage = SrdEquipmentLibrary.allPackages.any((p) =>
            p.items.any((i) => i.itemRef.slug.toLowerCase() == itemSlug));
  ...
  static bool _isItemMissing(
      HomebrewEntityType type, String slug, String name) {
    ...
    switch (type) {
      ...
      case HomebrewEntityType.subspeciesType:
        final found = SrdSpeciesLibrary.allSpecies.any((r) => r.subraces.any(
            (s) =>
      ...
      case HomebrewEntityType.itemType:
        ...
        final foundPackage = SrdEquipmentLibrary.allPackages.any((p) =>
            p.items.any((i) => i.itemRef.slug.toLowerCase() == cleanSlug));
  ...
  static List<Map<String, dynamic>> collectHomebrewDependencies(
      Character character) {
    ...
    for (final clsProg in character.progression.classes) {
      ...
      if (classRef.rulesetPreferred == RulesetVersion.homebrew ||
        ...
        addDep(
          ...
          classRef.slug,
          classRef.displayName.isNotEmpty
              ? classRef.displayName
              : classRef.slug,
        );
      }
