...
class DprCalculatorEngine {
  ...
  static double expectedDieValue(int sides,
      {GwfVersion gwf = GwfVersion.none}) {
  ...
  static double calculateEffectiveAttackBonus(DprAttackAction attack) {
    var bonus =
        attack.attackBonus.toDouble() + attack.attackBuffFlat.toDouble();
    if (attack.hasArchery) bonus += 2.0;
    if (attack.gwmMode == GwmMode.v2014PowerAttack) bonus -= 5.0;
    if (attack.attackBuffDiceSides > 0) {
      bonus +=
          (attack.attackBuffDiceSides + 1.0) / 2.0; // e.g. +2.5 for Bless (1d4)
    }
    return bonus;
  }
  ...
  static double calculateHitProbability(
    ...
    return switch (advantage) {
      ...
      AdvantageType.advantage =>
        1.0 - math.pow(1.0 - clampedBase, 2).toDouble(),
      AdvantageType.disadvantage => math.pow(clampedBase, 2).toDouble(),
      AdvantageType.elvenAccuracy =>
        1.0 - math.pow(1.0 - clampedBase, 3).toDouble(),
    };
  }
  ...
  static double calculateCritProbability(
    ...
    return switch (advantage) {
      ...
      AdvantageType.elvenAccuracy =>
        1.0 - math.pow(1.0 - baseCritP, 3).toDouble(),
    };
  }
  ...
  static DprPoint calculateSingleAttackDpr(
    ...
    if (attack.deliveryType == DprActionDeliveryType.savingThrow) {
      final saveDc = attack.saveDc ??
          (8 + proficiencyBonus + math.max(0, attack.damageBonus));
      final targetSaveBonus =
          targetSaveBonusOverride ?? ((targetAc - 13).clamp(-2, 12));

      final targetHasAdv = advantage == AdvantageType.disadvantage;
      final targetHasDisadv = advantage == AdvantageType.advantage ||
          advantage == AdvantageType.elvenAccuracy;
      ...
      final primaryDieEv =
          expectedDieValue(attack.diceSides, gwf: attack.gwfVersion);
      ...
      return DprPoint(
        ...
        expectedDamageOnMiss:
            attack.halfDamageOnSave ? (fullDamage * 0.5) : 0.0,
      );
    }
    ...
    final totalHitChance = calculateHitProbability(
        effBonus, targetAc, advantage,
        hasHalflingLuck: luck);
    final critChance = calculateCritProbability(advantage,
        critThreshold: attack.critThreshold, hasHalflingLuck: luck);
    ...
    if (attack.gwmMode == GwmMode.v2024ProficiencyBonus)
      flatDamage += proficiencyBonus;
    ...
    if (attack.extraCritDiceCount > 0) {
      final extraSides = attack.extraCritDiceSides > 0
          ? attack.extraCritDiceSides
          : attack.diceSides;
      extraCritDieEv = attack.extraCritDiceCount *
          expectedDieValue(extraSides, gwf: attack.gwfVersion);
    }
  ...
  static DprPoint calculateProfileDpr(
    ...
    for (int i = 0; i < profile.attacks.length; i++) {
      ...
      for (int count = 0; count < attack.attacksPerRound; count++) {
        ...
        if (attack.weaponMastery == WeaponMastery.vex &&
            baseAdv == AdvantageType.normal) {
    ...
    if (profile.sneakAttackDiceCount > 0 && hitChances.isNotEmpty) {
      final probMissAll =
          hitChances.fold<double>(1.0, (acc, p) => acc * (1.0 - p));
      final probAtLeastOneHit = 1.0 - probMissAll;

      final probNoCrit =
          critChances.fold<double>(1.0, (acc, c) => acc * (1.0 - c));
      ...
      final sneakDpr =
          (probAtLeastOneHit + probAtLeastOneCrit) * singleSneakDamage;
      totalDpr += sneakDpr;
    }
  ...
  /// Performs full Break-Even Analysis comparing baseline attacks vs Power Attack (-5/+10).
  static DprBreakEvenAnalysis calculatePowerAttackBreakEven(
    ...
    // Create power attack profile copy with Power Attack 2014 (-5/+10) applied to all attacks
    ...
    final powerProfile = baselineProfile.copyWith(
      id: '${baselineProfile.id}_power_attack',
      name: '${baselineProfile.name} (Power Attack Active)',
      attacks: powerAttacks,
    );

    final baselineCurve = generateCurve(baselineProfile,
        advantageOverride: adv, minAc: minAc, maxAc: maxAc);
    final powerCurve = generateCurve(powerProfile,
        advantageOverride: adv, minAc: minAc, maxAc: maxAc);

    int? maxOptimalAc;
    bool hadPowerAttackAdvantage = false;

    for (int ac = minAc; ac <= maxAc; ac++) {
      final baseDpr = baselineCurve.pointAt(ac)?.dpr ?? 0;
      final powerDpr = powerCurve.pointAt(ac)?.dpr ?? 0;

      if (powerDpr >= baseDpr) {
        if (!hadPowerAttackAdvantage || maxOptimalAc == ac - 1) {
          maxOptimalAc = ac;
          hadPowerAttackAdvantage = true;
        }
      } else {
        if (hadPowerAttackAdvantage) {
          // First time power attack falls below baseline - this establishes the standard break-even threshold
          break;
        }
      }
    }
    ...
    if (maxOptimalAc == null) {
      rec =
          'Power Attack (-5/+10) is suboptimal across all evaluated ACs for this build.';
    } else if (maxOptimalAc >= maxAc) {
      rec =
          'Power Attack (-5/+10) is optimal against ALL target ACs (up to AC $maxAc)!';
    } else {
      rec =
          'Use Power Attack (-5/+10) against target AC $maxOptimalAc or lower. Switch to Normal attacks against AC ${maxOptimalAc + 1}+.';
    }
    ...
    return DprBreakEvenAnalysis(
      ...
      maxOptimalAc: maxOptimalAc,
      recommendation: rec,
    );
  }

  /// Backwards-compatible alias for [calculatePowerAttackBreakEven].
  ...
  static DprBreakEvenAnalysis _isolateBreakEvenRunner(
      _DprBreakEvenPayload payload) {
    return calculatePowerAttackBreakEven(
      payload.profile,
  ...
  /// Offloads Power Attack break-even analysis to a background isolate.
  static Future<DprBreakEvenAnalysis> calculatePowerAttackBreakEvenAsync(
    DprCombatantProfile baselineProfile, {
    AdvantageType? advantage,
    int minAc = 5,
    int maxAc = 30,
  ...
  /// Backwards-compatible alias for [calculatePowerAttackBreakEvenAsync].
  static Future<DprBreakEvenAnalysis> calculateGwmBreakEvenAsync(
    ...
    int maxAc = 30,
      ...
      calculatePowerAttackBreakEvenAsync(
        baselineProfile,
        advantage: advantage,
        minAc: minAc,
        maxAc: maxAc,
