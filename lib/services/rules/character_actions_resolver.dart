...
class CharacterActionsResolver {
  ...
  static ResolvedCharacterActions resolve({
    ...
    void dispatchRoll(BuildContext context,
        {required String label, required String formula}) {
    ...
    for (final atk in stats.attackProfiles) {
      ...
      actionList.add(
        CharacterCombatAction(
          ...
          subtitle:
              '${isRanged ? "Ranged" : "Melee"} Weapon • ${atk.damageType.name.toUpperCase()}',
          ...
          isAttack: true,
          dealsDamage: true,
          onRollAttack: (ctx) => dispatchRoll(ctx,
              label: '${atk.weaponName} Attack', formula: '1d20$bonusStr'),
          onRollDamage: (ctx) => dispatchRoll(ctx,
              label: '${atk.weaponName} Damage', formula: atk.damageFormula),
        ),
      );
    }
    ...
    actionList.add(
      CharacterCombatAction(
        ...
        description:
            'Instead of using a weapon, you punch, kick, or headbutt a target within 5 feet.',
        range: 'Reach 5 ft',
        ...
        dealsDamage: true,
        onRollAttack: (ctx) => dispatchRoll(ctx,
            label: 'Unarmed Strike Attack', formula: '1d20$unarmedAtkStr'),
        onRollDamage: (ctx) => dispatchRoll(ctx,
            label: 'Unarmed Strike Damage', formula: '$unarmedDmg'),
      ),
    );
    ...
    for (final s in allSpells) {
      ...
      final castingTime =
          (rules?.castingTime ?? item?.rules2014.castingTime ?? '1 Action')
              .toLowerCase();
      ...
      final isEB = cleanSlug == 'eldritch-blast' ||
          s.displayName.toLowerCase() == 'eldritch blast';
      ...
      if (isEB &&
          (controller.hasAgonizingBlast ||
              controller.hasCapabilityFlag('eldritchBlastChaDamage') ||
              controller.hasCapabilityFlag('agonizing_blast'))) {
        final chaMod =
            character.effectiveAbilityScores.getModifier(AbilityType.charisma);
        final chaStr = chaMod >= 0 ? '+ $chaMod' : '- ${chaMod.abs()}';
        dmgFormula = dmgFormula != null && dmgFormula.isNotEmpty
            ? '$dmgFormula $chaStr'
            : '1d10 $chaStr';
      ...
      final hasAtk = isEB ||
          castingTime.contains('attack') ||
          (rules?.description
                  .any((d) => d.toLowerCase().contains('spell attack')) ??
              false);
      final dealsDmg = dmgFormula != null &&
          dmgFormula.isNotEmpty &&
          dmgFormula.toLowerCase() != 'none';
      ...
      final combatAction = CharacterCombatAction(
        ...
        subtitle:
            '${item?.level == 0 ? "Cantrip" : "Level ${item?.level ?? 1}"} • ${item?.school.displayName ?? "Magic"}',
        description: rules != null && rules.description.isNotEmpty
        ...
        onRollAttack: hasAtk
            ? (ctx) => dispatchRoll(ctx,
                label: '${s.displayName} Spell Attack',
                formula: '1d20$spellAtkStr')
            : null,
        onRollDamage: dealsDmg
            ? (ctx) => dispatchRoll(ctx,
                label: '${s.displayName} Damage', formula: dmgFormula!)
            : null,
      );
    ...
    final weapons = character.equippedItems
        .where((i) =>
            i.equippedSlot == EquipmentSlot.mainHand ||
            i.equippedSlot == EquipmentSlot.offHand)
        .toList();
    if (weapons.length >= 2) {
      ...
      bonusActionList.add(
        CharacterCombatAction(
          ...
          description:
              'When you take the Attack action with a light melee weapon, use a bonus action to attack with your other light weapon (${offWeapon.displayName}).',
          range: 'Reach 5 ft',
          isAttack: true,
          dealsDamage: true,
          onRollAttack: (ctx) => dispatchRoll(ctx,
              label: 'Off-Hand Attack',
              formula: '1d20+${stats.proficiencyBonus}'),
          onRollDamage: (ctx) =>
              dispatchRoll(ctx, label: 'Off-Hand Damage', formula: '1d6'),
        ),
      );
    }
    ...
    for (final cls in character.progression.classes) {
        ...
        if (lvl >= 1) {
          ...
          registerAction(
            CharacterCombatAction(
              ...
              description:
                  'Enter a rage as a bonus action. Gain advantage on STR checks/saves, +$rageDmg melee weapon damage, and resistance to bludgeoning, piercing, and slashing damage.',
        ...
        if (lvl >= 2) {
          registerAction(
            const CharacterCombatAction(
              ...
              description:
                  'When making your first attack on your turn, gain advantage on melee weapon attack rolls using STR, but attack rolls against you have advantage until your next turn.',
            ),
          );
        }
      }
        ...
        if (lvl >= 1) {
          ...
          registerAction(
            CharacterCombatAction(
              ...
              description:
                  'Use a bonus action to inspire one creature within 60 feet. Once within the next 10 minutes, the creature can roll $die and add it to one d20 test.',
              range: '60 ft',
              resourceCost: 'Charisma Mod / Long Rest',
              onExecute: (ctx) => dispatchRoll(ctx,
                  label: 'Bardic Inspiration Die', formula: die),
            ),
          );
        }
        if (lvl >= 6) {
          registerAction(
            const CharacterCombatAction(
              ...
              description:
                  'Start a performance that lasts until the end of your next turn. You and friendly creatures within 30 feet have advantage on saving throws against being frightened or charmed.',
              range: '30 ft',
            ),
          );
        }
        if (lvl >= 3 &&
            (cls.subclassRef?.slug.toLowerCase().contains('lore') ?? false)) {
          registerAction(
            const CharacterCombatAction(
              id: 'cutting-words',
              name: 'Cutting Words',
              ...
              description:
                  'When a creature within 60 feet rolls an attack roll, ability check, or damage roll, use your reaction and expend one Bardic Inspiration die to subtract the roll from the creature\'s total.',
              range: '60 ft',
      ...
      if (classSlug.contains('cleric')) {
        if (lvl >= 2) {
          registerAction(
            CharacterCombatAction(
              ...
              description:
                  'Present your holy symbol and speak a prayer. Each undead that can see or hear you within 30 feet must make a Wisdom saving throw or be turned for 1 minute or until it takes damage.',
              range: '30 ft',
          ...
          registerAction(
            CharacterCombatAction(
              ...
              description:
                  'Touch your holy symbol and expend a use of Channel Divinity to regain one expended spell slot (up to half your proficiency bonus rounded up).',
              resourceCost: '1 Channel Divinity',
            ),
          );
        }
        ...
        if (lvl >= 2 && subSlug.contains('life')) {
          ...
          registerAction(
            CharacterCombatAction(
              ...
              description:
                  'Present your holy symbol and evoke healing energy that can restore up to $preserveHp hit points divided among bloodied creatures within 30 feet (cannot heal above half max HP).',
              range: '30 ft',
              resourceCost: '1 Channel Divinity',
            ),
          );
        ...
        if (lvl >= 2 && subSlug.contains('light')) {
          registerAction(
            CharacterCombatAction(
              ...
              actionType: ActionType.action,
              category: CombatActionCategory.feature,
              subtitle: 'Light Domain • 30 ft Sunlight Burst',
              description:
                  'Present your holy symbol and evoke sunlight. Magical darkness within 30 feet is dispelled, and hostile creatures must make a Constitution saving throw or take 2d10 + $lvl radiant damage (half on success).',
              range: '30 ft',
              dealsDamage: true,
              ...
              onExecute: (ctx) {
                dispatchRoll(ctx,
                    label: 'Radiance of the Dawn Damage',
                    formula: '2d10+$lvl');
              },
            ),
          );
        ...
        if (subSlug.contains('light')) {
          registerAction(
            const CharacterCombatAction(
              ...
              description:
                  'When you are attacked by a creature within 30 feet that you can see, you can use your reaction to impose disadvantage on the attack roll.',
              range: '30 ft',
              resourceCost: '1 Use (WIS mod/Long Rest)',
            ),
          );
        }
        ...
        if (lvl >= 10) {
          registerAction(
            CharacterCombatAction(
              ...
              description:
                  'Implore your deity for aid. Roll percentile dice (d100); if you roll a number equal to or lower than your cleric level ($lvl), your deity intervenes.',
              resourceCost: '1/7 Days if successful',
              onExecute: (ctx) => dispatchRoll(ctx,
                  label: 'Divine Intervention Check', formula: '1d100'),
            ),
          );
        }
      }
      ...
      if (classSlug.contains('druid')) {
        if (lvl >= 2) {
          final isBonusAction = edition == DmRulesEdition.v2024 ||
              character.customProperties['bonusActionWildShape'] == true;
          registerAction(
            CharacterCombatAction(
              id: 'wild-shape',
              name: 'Wild Shape',
              actionType:
                  isBonusAction ? ActionType.bonusAction : ActionType.action,
              category: CombatActionCategory.feature,
              subtitle: 'Druid Level $lvl • Beast Transformation',
              description: isBonusAction
          ...
          registerAction(
            CharacterCombatAction(
              ...
              description:
                  'As an action, expend a use of Wild Shape to cast the find familiar spell without material components.',
              resourceCost: '1 Wild Shape Use',
            ),
          );

          final subSlug = cls.subclassRef?.slug.toLowerCase() ?? '';
          if (subSlug.contains('moon')) {
            registerAction(
              const CharacterCombatAction(
                ...
                actionType: ActionType.bonusAction,
                category: CombatActionCategory.feature,
                subtitle: 'Circle of the Moon • Bonus Action Transformation',
                description:
                    'You gain the ability to use Wild Shape on your turn as a bonus action, rather than an action.',
                resourceCost: '1 Wild Shape Use',
              ),
            );
            registerAction(
              const CharacterCombatAction(
                id: 'combat-wild-shape-healing',
                name: 'Combat Wild Shape: Spell Slot Healing',
                actionType: ActionType.bonusAction,
                category: CombatActionCategory.feature,
                subtitle: 'Circle of the Moon • Heal 1d8/Slot Level',
                description:
        ...
        if (lvl >= 1) {
          registerAction(
            CharacterCombatAction(
              ...
              description:
                  'On your turn, you can use a bonus action to regain hit points equal to 1d10 + your fighter level ($lvl).',
              resourceCost: '1/Short Rest',
              dealsDamage: false,
              onExecute: (ctx) {
                dispatchRoll(ctx,
                    label: 'Second Wind Healing', formula: '1d10+$lvl');
              },
            ),
          );
        }
        ...
        if (lvl >= 9) {
          registerAction(
            CharacterCombatAction(
              ...
              description:
                  'Reroll a saving throw that you fail. You must use the new roll.',
        ...
        if (lvl >= 1) {
          registerAction(
            const CharacterCombatAction(
              ...
              description:
                  'When you take the Attack action with an unarmed strike or monk weapon, you can make one unarmed strike as a bonus action.',
            ),
          );
        }
        if (lvl >= 2) {
          registerAction(
            const CharacterCombatAction(
              ...
              description:
                  'Immediately after you take the Attack action, spend 1 Ki point to make two unarmed strikes as a bonus action.',
              resourceCost: '1 Ki',
            ),
          );
          registerAction(
          ...
          registerAction(
            const CharacterCombatAction(
              ...
              description:
                  'Spend 1 Ki point to take the Disengage or Dash action as a bonus action on your turn, doubling your jump distance for the turn.',
              resourceCost: '1 Ki',
            ),
          );
        }
        if (lvl >= 3) {
          ...
          registerAction(
            CharacterCombatAction(
              ...
              description:
                  'You can use your reaction to deflect or catch a missile when you are hit by a ranged weapon attack. Damage is reduced by 1d10 + $dexMod + $lvl. If reduced to 0, spend 1 Ki to make a ranged attack with it.',
              resourceCost: 'Reaction (1 Ki to throw)',
              onExecute: (ctx) => dispatchRoll(ctx,
                  label: 'Deflect Missiles Reduction',
                  formula: '1d10+$dexMod+$lvl'),
            ),
          );
        }
        if (lvl >= 5) {
          registerAction(
            const CharacterCombatAction(
              ...
              description:
                  'When you hit another creature with a melee weapon attack, spend 1 Ki point to attempt to stun the target. It must make a Constitution saving throw or be stunned until the end of your next turn.',
              resourceCost: '1 Ki',
            ),
          );
        }
        ...
        if (lvl >= 1) {
          ...
          registerAction(
            CharacterCombatAction(
              ...
              description:
                  'As an action, touch a creature and draw power from your pool to restore up to $pool hit points, or expend 5 hit points to cure one disease or neutralize one poison.',
              range: 'Touch',
              resourceCost: '$pool HP / Long Rest',
            ),
          );
          final chaMod = character.effectiveAbilityScores
              .getModifier(AbilityType.charisma);
          ...
          registerAction(
            CharacterCombatAction(
              ...
              description:
                  'As an action, detect the location of any celestial, fiend, or undead within 60 feet that is not behind total cover, and sense consecrated or desecrated ground.',
              range: '60 ft',
        ...
        if (lvl >= 2) {
          ...
          registerAction(
            CharacterCombatAction(
              ...
              name: 'Divine Smite',
              actionType:
                  is2024Smite ? ActionType.bonusAction : ActionType.special,
              category: CombatActionCategory.feature,
              subtitle: is2024Smite
                  ? 'Paladin Level $lvl • Bonus Action'
                  : 'Paladin Level $lvl • On-Hit Special',
              ...
              dealsDamage: true,
              onRollDamage: (ctx) => dispatchRoll(ctx,
                  label: 'Divine Smite (1st Level Slot)', formula: '2d8'),
            ),
          );
        }
        if (lvl >= 3) {
          registerAction(
            CharacterCombatAction(
              ...
              description:
                  'As an action, imbue one weapon that you are holding with positive energy for 1 minute. Add your Charisma modifier to attack rolls made with that weapon, and it emits bright light in a 20-foot radius.',
              resourceCost: '1 Channel Divinity',
            ),
          );
          registerAction(
            CharacterCombatAction(
              ...
              description:
                  'As an action, present your holy symbol. Each fiend or undead within 30 feet of you that can see or hear you must make a Wisdom saving throw or be turned for 1 minute or until it takes damage.',
              range: '30 ft',
              resourceCost: '1 Channel Divinity',
            ),
          );
        ...
        if (lvl >= 14) {
          registerAction(
            const CharacterCombatAction(
              ...
              description:
                  'As an action, end one spell on yourself or on one willing creature that you touch.',
              range: 'Touch',
              resourceCost: 'Charisma Mod / Long Rest',
            ),
          );
        ...
        if (lvl >= 1) {
          registerAction(
            CharacterCombatAction(
              ...
              description:
                  'Choose a creature you can see within 90 feet and mark it as your quarry. Deal an extra 1d6 damage to the target whenever you hit it with a weapon attack.',
              range: '90 ft',
              damageFormula: '1d6',
              dealsDamage: true,
              onRollDamage: (ctx) => dispatchRoll(ctx,
                  label: "Hunter's Mark Extra Damage", formula: '1d6'),
            ),
          );
        }
        if (lvl >= 3) {
          registerAction(
            CharacterCombatAction(
              ...
              description:
                  'As an action, expend one ranger spell slot to sense whether aberrations, celestials, dragons, elementals, fey, fiends, or undead are present within 1 mile (or 6 miles in favored terrain).',
        ...
        if (lvl >= 1) {
          ...
          registerAction(
            CharacterCombatAction(
              ...
              description:
                  'Once per turn, you can deal an extra ${sneakD6}d6 damage to one creature you hit with an attack if you have advantage on the attack roll, or if an ally is within 5 feet of the target.',
              damageFormula: '${sneakD6}d6',
              dealsDamage: true,
              onRollDamage: (ctx) => dispatchRoll(ctx,
                  label: 'Sneak Attack Damage', formula: '${sneakD6}d6'),
            ),
          );
        }
        if (lvl >= 2) {
        ...
        if (lvl >= 3) {
          registerAction(
            CharacterCombatAction(
              ...
              description:
                  'As a bonus action, give yourself advantage on your next attack roll on the current turn. Usable only if you haven\'t moved this turn, and your speed becomes 0 until the end of the turn.',
            ),
          );
        }
        if (lvl >= 5) {
        ...
        if (edition == DmRulesEdition.v2024 && lvl >= 1) {
          registerAction(
            CharacterCombatAction(
              ...
              description:
                  'As a bonus action, unleash an aura of sorcery for 1 minute. Your spell save DC increases by 1 and you have advantage on sorcerer spell attack rolls.',
        ...
        if (lvl >= 2) {
          registerAction(
            CharacterCombatAction(
              ...
              description:
                  'As a bonus action, transform unexpended sorcery points into one spell slot, or expend a spell slot to gain sorcery points equal to the slot level.',
            ),
          );
        }
        if (lvl >= 3) {
          ...
          if (lvl >= 6) {
            registerAction(
              const CharacterCombatAction(
                ...
                actionType: ActionType.reaction,
                category: CombatActionCategory.feature,
                subtitle: "The Fiend • Ability Check or Saving Throw",
                description:
                    'When you make an ability check or saving throw, you can add 1d10 to your roll. You can do so after seeing the initial roll but before any of the roll’s effects occur.',
                resourceCost: '1/Short Rest',
              ),
            );
          }
        }
        final hasPactBlade = controller
                .hasCapabilityFlag('pact_of_the_blade') ||
            character.progression.getAllSelectedFeatureOptions().values.any(
                (opts) => opts.any((o) => o.toLowerCase().contains('blade')));
        if (hasPactBlade) {
          registerAction(
            const CharacterCombatAction(
              ...
              description:
                  'You can use your action to create a pact weapon in your empty hand. You are proficient with it while you wield it, and it counts as magical for overcoming resistance.',
            ),
          );
        }
      }
        ...
        if (lvl >= 1) {
          ...
          registerAction(
            CharacterCombatAction(
              ...
              description:
                  'Once per day when you finish a short rest, you can choose expended spell slots to recover with a combined level equal to or less than $recoveryMax (no slot can be 6th level or higher).',
              resourceCost: '1/Day (Short Rest)',
            ),
          );
        }
        final subSlug = cls.subclassRef?.slug.toLowerCase() ?? '';
        if (lvl >= 2 && subSlug.contains('abjuration')) {
          final intMod = stats.abilityModifiers[AbilityType.intelligence] ?? 0;
          final wardHp = (lvl * 2) + intMod;
          registerAction(
            CharacterCombatAction(
              id: 'arcane-ward',
              name: 'Arcane Ward',
              actionType: ActionType.special,
              category: CombatActionCategory.feature,
              subtitle: 'School of Abjuration • $wardHp HP Barrier',
              description:
                  'When you cast an abjuration spell of 1st level or higher, you can create a magical ward on yourself that has hit points equal to twice your wizard level + your Intelligence modifier ($wardHp HP).',
            ),
          );
        }
      }
    ...
    // ==========================================
    final hasProtection =
        controller.hasCapabilityFlag('hasProtectionReaction') ||
            character.progression.classes.any((cls) {
              final classSlug = cls.classRef.slug.toLowerCase();
              final isFightingStyleClass = classSlug == 'fighter' ||
                  classSlug == 'paladin' ||
                  classSlug == 'ranger';
              if (!isFightingStyleClass) return false;
              return cls.selectedFeatureOptions.entries.any((entry) =>
                  (entry.key.toLowerCase().contains('fighting-style') ||
                      entry.key.toLowerCase().contains('fighting_style')) &&
                  entry.value.any((v) =>
                      v.toLowerCase() == 'protection' ||
                      v.toLowerCase() == 'fighting_style_protection'));
            }) ||
    ...
    final hasInterception =
        controller.hasCapabilityFlag('hasInterceptionReaction') ||
            character.progression.classes.any((cls) {
              final classSlug = cls.classRef.slug.toLowerCase();
              final isFightingStyleClass = classSlug == 'fighter' ||
                  classSlug == 'paladin' ||
              ...
              return cls.selectedFeatureOptions.entries.any((entry) =>
                  (entry.key.toLowerCase().contains('fighting-style') ||
                      entry.key.toLowerCase().contains('fighting_style')) &&
                  entry.value.any((v) =>
                      v.toLowerCase() == 'interception' ||
                      v.toLowerCase() == 'fighting_style_interception'));
            }) ||
            character.feats.any((f) {
              final slug = f.slug.toLowerCase();
              final name = f.displayName.toLowerCase();
    ...
    if (hasInterception) {
      registerAction(
        CharacterCombatAction(
          ...
          description:
              'When a creature you can see hits a target within 5 feet of you with an attack, you can use your reaction to reduce the damage by 1d10 + ${stats.proficiencyBonus}.',
          damageFormula: '1d10+${stats.proficiencyBonus}',
          onExecute: (ctx) => dispatchRoll(ctx,
              label: 'Interception Damage Reduction',
              formula: '1d10+${stats.proficiencyBonus}'),
        ),
      );
    }
    ...
    for (final cls in character.progression.classes) {
      ...
      if (cls.subclassRef != null) {
        ...
        final resolvedSubclass = SrdClassesLibrary.findSubclass(
          ...
          ruleset: edition == DmRulesEdition.v2014
        ...
        if (resolvedSubclass != null &&
          ...
          final stitched = SrdClassesLibrary.buildFeaturesMarkdownForSubclass(
            resolvedSubclass,
            ruleset: edition == DmRulesEdition.v2014
                ? RulesetVersion.v2014
                : RulesetVersion.v2024,
        ...
        if (subFeaturesMarkdown.isNotEmpty) {
          _extractActionsFromMarkdown(
            markdown: subFeaturesMarkdown,
            classLevel: cls.level,
            sourceName: subName,
            dispatchRoll: dispatchRoll,
            registerAction: registerAction,
      ...
      for (final optionIdList in cls.selectedFeatureOptions.values) {
        for (final optId in optionIdList) {
          final opt = SrdFeatureOptions.allOptions
              .where(
                (o) =>
                    o.id == optId ||
                    o.id == optId.replaceAll('-', '_') ||
                    o.name.toLowerCase() == optId.toLowerCase(),
              )
              .firstOrNull;
      ...
      CharacterCombatAction(
        ...
        onRollAttack: stats.attackProfiles.isNotEmpty
            ? (ctx) {
                ...
                final b = atk.attackBonus >= 0
                    ? '+${atk.attackBonus}'
                    : '${atk.attackBonus}';
                dispatchRoll(ctx,
                    label: 'Opportunity Attack (${atk.weaponName})',
                    formula: '1d20$b');
              }
            : (ctx) => dispatchRoll(ctx,
                label: 'Opportunity Attack (Unarmed)',
                formula: '1d20$unarmedAtkStr'),
      ),
    );
      ...
      if (nameLower.contains('rod of the pact keeper')) {
        specialList.add(
          CharacterCombatAction(
            ...
            description:
                'While holding this rod, you can regain one warlock spell slot as an action. You can’t use this property again until you finish a long rest.',
            resourceCost: '1/Long Rest',
            onExecute: (ctx) {
              HapticService.heavyImpact(ctx);
              final pool = controller.character.resources.spellSlots;
              if (pool.pactMagicMax > 0 &&
                  pool.pactMagicCurrent < pool.pactMagicMax) {
                controller.togglePactSlot(false); // restores 1 pact slot
                ScaffoldMessenger.of(ctx).showSnackBar(
                  const SnackBar(
                    content: Text(
                        'Rod of the Pact Keeper: Restored 1 Warlock spell slot!'),
    ...
    standardList.addAll([
      const CharacterCombatAction(
        ...
        description:
            'Make one melee or ranged attack with an equipped weapon or unarmed strike. If you have the Extra Attack feature, you can make additional attacks with this action.',
      ),
      CharacterCombatAction(
        id: 'std-dash',
        name: 'Dash',
        ...
        description:
            'You gain extra movement for the current turn equal to your speed (${stats.speedFeet} feet) after applying any modifiers.',
      ),
      const CharacterCombatAction(
        id: 'std-disengage',
        name: 'Disengage',
      ...
      const CharacterCombatAction(
        ...
        description:
            'Until the start of your next turn, any attack roll made against you has disadvantage if you can see the attacker, and you make Dexterity saving throws with advantage. You lose this benefit if you are incapacitated.',
      ),
      const CharacterCombatAction(
        id: 'std-help',
        name: 'Help',
        ...
        description:
            'You lend your aid to another creature in the completion of a task. When you use the Help action, the creature you aid gains advantage on the next ability check it makes, or advantage on the next attack roll against a creature within 5 ft of you.',
      ),
      CharacterCombatAction(
        id: 'std-hide',
        name: 'Hide',
        ...
        description:
            'You make a Dexterity (Stealth) check in an attempt to become hidden, provided you are unseen and have cover.',
        onExecute: (ctx) {
          final stealthMod = stats.skillModifiers[SkillType.stealth] ??
              stats.abilityModifiers[AbilityType.dexterity] ??
              0;
          final strMod = stealthMod >= 0 ? '+$stealthMod' : '$stealthMod';
          dispatchRoll(ctx,
              label: 'Dexterity (Stealth) Check', formula: '1d20$strMod');
        },
      ),
      const CharacterCombatAction(
        id: 'std-ready',
        ...
        description:
            'You wait for a particular circumstance before you act. Decide what perceivable circumstance will trigger your reaction and the action you will take in response.',
      ),
      CharacterCombatAction(
        id: 'std-search',
        name: 'Search',
        ...
        description:
            'You devote your attention to finding something. Make a Wisdom (Perception) check or an Intelligence (Investigation) check.',
        onExecute: (ctx) {
          final percMod = stats.skillModifiers[SkillType.perception] ??
              stats.abilityModifiers[AbilityType.wisdom] ??
              0;
          final strMod = percMod >= 0 ? '+$percMod' : '$percMod';
          dispatchRoll(ctx,
              label: 'Wisdom (Perception) Check', formula: '1d20$strMod');
        },
      ),
      const CharacterCombatAction(
        id: 'std-use-object',
        ...
        description:
            'You normally interact with an object while doing something else. When an object requires your action for its use, you take the Use an Object action.',
      ),
    ]);
  ...
  static ActionType? _detectActionType(String title, String body) {
    ...
    if (baRegex.hasMatch(combined) ||
        title.toLowerCase().contains('(bonus action)')) {
    ...
    if (rxRegex.hasMatch(combined) ||
        title.toLowerCase().contains('(reaction)')) {
    ...
    if (actRegex.hasMatch(combined) ||
        title.toLowerCase().contains('(action)')) {
  ...
  static void _extractActionsFromMarkdown({
    ...
    required void Function(BuildContext context,
            {required String label, required String formula})
        dispatchRoll,
    ...
    if (markdown.contains(RegExp(r'(?:^|\n)#{1,4}\s+'))) {
      ...
      for (final block in blocks) {
        ...
        if (subMatches.length >= 2 ||
            (subMatches.length == 1 &&
                subMatches.first.group(1)?.trim().toLowerCase() !=
                    name.toLowerCase())) {
    ...
    if (boldMatches.isNotEmpty) {
      for (final match in boldMatches) {
        ...
        if (rawTitle.isEmpty ||
            _nonActionTitles.contains(rawTitle.toLowerCase())) continue;
    ...
    for (final match in pipeRegex6.allMatches(markdown)) {
      ...
      if (featName.isNotEmpty &&
          !processedNames.contains(featName.toLowerCase())) {
  ...
  static void _processExtractedBlock({
    ...
    final nameLvlMatch = RegExp(
            r'(.*?)(?:\s*\((?:Level|Lvl)?\s*(\d+)(?:st|nd|rd|th)?(?:\s*Level)?\))',
            caseSensitive: false)
    ...
    if (detectedLevel == null) {
      final bodyLvlMatch = RegExp(
              r'(?:starting at|beginning at|at|when you reach)\s+(\d+)(?:st|nd|rd|th)\s+level',
              caseSensitive: false)
          .firstMatch(body);
    ...
    for (final dt in DamageType.values) {
      ...
      if (RegExp('\\b${dt.name}\\s+damage\\b', caseSensitive: false)
          .hasMatch(lowerBody)) {
        dmgType = dt;
        break;
      }
    ...
    if (dmgType == DamageType.untyped) {
      for (final dt in DamageType.values) {
        if (dt == DamageType.untyped) continue;
        if (RegExp('\\b${dt.name}\\b', caseSensitive: false)
            .hasMatch(lowerBody)) {
          dmgType = dt;
          break;
        }
    ...
    final resMatch = RegExp(
            r'\b(1\/(?:short\s+or\s+long\s+rest|short\s+rest|long\s+rest|day))\b',
            caseSensitive: false)
        .firstMatch(body);
    if (resMatch != null) {
    ...
    final lvlSubtitle =
        detectedLevel != null ? 'Level $detectedLevel' : 'Level $classLevel';
    final typeLabel = actionType == ActionType.bonusAction
        ? 'Bonus Action'
        : (actionType == ActionType.reaction
            ? 'Reaction'
            : (actionType == ActionType.special ? 'Special' : 'Action'));

    var finalBody = body;
    final fallback = _wellKnownFeatureFallbacks[cleanName.toLowerCase()];
    if (fallback != null &&
        (body.startsWith('Feature from ') ||
            body.startsWith('Class Feature from ') ||
            body.trim().length < 30)) {
    ...
    registerAction(
      CharacterCombatAction(
        ...
        onRollDamage: formula != null
            ? (ctx) =>
                dispatchRoll(ctx, label: '$cleanName Damage', formula: formula)
            : null,
      ),
    );
  }
  ...
  static void _extractActionsFromJson({
    ...
    List entriesList = [];
    if (rawJson is Map) {
      final cf = rawJson['classFeatures'] ??
          rawJson['subclassFeatures'] ??
          rawJson['entries'];
    ...
    for (final entry in entriesList) {
      if (entry is String) {
        final pipeMatch6 = RegExp(
                r"^([A-Za-z0-9\s\(\)'-]+?)\|([A-Za-z0-9\s\(\)'-]*)\|([A-Za-z0-9\s\(\)'-]*)\|([A-Za-z0-9\s\(\)'-]*)\|([A-Za-z0-9\s\(\)'-]*)\|(\d+)")
            .firstMatch(entry.trim());
        final pipeMatch4 = RegExp(
                r"^([A-Za-z0-9\s\(\)'-]+?)\|([A-Za-z0-9\s\(\)'-]*)\|([A-Za-z0-9\s\(\)'-]*)\|(\d+)")
            .firstMatch(entry.trim());
        final match = pipeMatch6 ?? pipeMatch4;
        if (match != null) {
          final featName = match.group(1)?.trim() ?? '';
          final lvlStr = match.groupCount >= 6 && pipeMatch6 != null
              ? match.group(6)
              : match.group(4);
          final lvl = int.tryParse(lvlStr ?? '1') ?? 1;
          final fallback = _wellKnownFeatureFallbacks[featName.toLowerCase()];
          final bodyText =
              fallback?.$2 ?? 'Class Feature from $sourceName (Level $lvl)';
          _processExtractedBlock(
            rawName: featName,
            body: bodyText,
            explicitLevel: lvl,
        ...
        final level = (entry['level'] as num?)?.toInt() ??
            (entry['subclassLevel'] as num?)?.toInt();
