  ...
  static Character reparse(Character character) {
    ...
    for (final fRef in character.feats) {
      final feat = SrdFeatsLibrary.allFeats.firstWhereOrNull(
        (f) => f.id.slug == fRef.slug || f.name.toLowerCase() == fRef.displayName.toLowerCase(),
      );
      if (feat != null) {
        for (final g in feat.grants) {
          if (g.type == GrantType.bonusTool && g.payload['tool'] != null) {
            final tName = g.payload['tool'].toString().trim();
            if (tName.isNotEmpty && !compiledTools.any((t) => t.toLowerCase() == tName.toLowerCase())) {
            ...
            if (_isToolProficiency(pName) && !compiledTools.any((t) => t.toLowerCase() == pName.toLowerCase())) {
        ...
        if (featTools is List) {
          for (final t in featTools) {
            if (t is String && t.trim().isNotEmpty) {
              final tStr = t.trim();
              if (!compiledTools.any((existing) => existing.toLowerCase() == tStr.toLowerCase())) {
              ...
              t.forEach((toolName, enabled) {
                if (enabled == true || enabled == 1) {
                  ...
                  if (tStr != 'any' && tStr != 'anyArtisansTool' &&
                      !compiledTools.any((existing) => existing.toLowerCase() == tStr.toLowerCase())) {
                    compiledTools.add(tStr);
                  }
                }
              });
    ...
    for (final s in innateSpells) {
      if (s.isCantrip) {
        if (!updatedCantrips.any((existing) => existing.slug == s.spellRef.slug)) {
          updatedCantrips.add(s.spellRef);
        }
      } else {
        if (!updatedSpellsKnown.any((existing) => existing.slug == s.spellRef.slug)) {
      ...
      if (feat != null) {
        for (final g in feat.grants) {
          if (g.type == GrantType.bonusSpell && g.payload['spell'] != null) {
            ...
            if (isCantrip) {
              if (!updatedCantrips.any((existing) => existing.slug == spellSlug)) {
                updatedCantrips.add(spellRef);
              }
            } else {
              if (!updatedSpellsKnown.any((existing) => existing.slug == spellSlug)) {
    ...
    if (classSlug != null && classSlug.isNotEmpty) {
      ...
      if (cls != null) {
        ...
        if (existingAllowed is List && existingAllowed.length >= 18 && cls.allowedSkills.isNotEmpty && cls.allowedSkills.length < 18) {
          customProps['allowedSkills'] = cls.allowedSkills.map((s) => s.name).toList();
    ...
    // 6. 2014 Background Feature
    if (edition == DmRulesEdition.v2014 && backgroundSlug != null && backgroundSlug.isNotEmpty) {
      if (customProps['backgroundFeature'] == null || customProps['backgroundFeature'].toString().isEmpty) {
        final feat = SrdBackgroundsLibrary.get2014Feature(backgroundSlug);
        if (feat != null) {
