...
class InventoryTransactionService {
  ...
  static EquipmentSlot resolveDefaultSlot(
    ...
    final categoryLower =
        (props['category']?.toString() ?? resolvedItem?.itemType ?? '')
            .toLowerCase();
  ...
  static Character equipItem(
    ...
    if (targetIndex == -1) {
      throw ArgumentError(
          'Item instance $instanceId not found in character inventory.');
    }
  ...
  static Character attuneItem(
    ...
    if (attune) {
      if (!item.requiresAttunement) {
        throw ArgumentError(
            'Item "${item.displayName}" does not require attunement.');
      }
      if (item.isAttuned) {
  ...
  static LootTransferResult transferFromContainerToCharacter({
    ...
    final itemIndex =
        containerItems.indexWhere((i) => i.instanceId == instanceId);
    if (itemIndex == -1) {
      throw ArgumentError(
          'Item instance $instanceId not found in container "${sourceContainer.name}".');
    }

    final sourceItem = containerItems[itemIndex];
    if (quantity > sourceItem.quantity) {
      throw ArgumentError(
          'Requested quantity $quantity exceeds container quantity ${sourceItem.quantity}.');
    }
    ...
    if (quantity == sourceItem.quantity) {
      ...
      containerItems[itemIndex] =
          sourceItem.copyWith(quantity: sourceItem.quantity - quantity);
    }

    // Add to character
    final charInventory =
        List<InventoryItemInstance>.from(destinationCharacter.inventory);
  ...
  static LootTransferResult transferFromCharacterToContainer({
    ...
    final itemIndex =
        charInventory.indexWhere((i) => i.instanceId == instanceId);
    if (itemIndex == -1) {
      throw ArgumentError(
          'Item instance $instanceId not found in character inventory.');
    }
    ...
    if (quantity > sourceItem.quantity) {
      throw ArgumentError(
          'Requested quantity $quantity exceeds character quantity ${sourceItem.quantity}.');
    }
    ...
    if (quantity == sourceItem.quantity) {
      ...
      charInventory[itemIndex] =
          sourceItem.copyWith(quantity: sourceItem.quantity - quantity);
    }

    // Add to container
    final containerItems =
        List<InventoryItemInstance>.from(destinationContainer.items);
