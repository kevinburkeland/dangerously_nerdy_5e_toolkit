...
class ArenaCombatEngine {
  ...
  bool _canReachTarget(ArenaCombatant attacker, ArenaCombatant defender,
  ...
  ArenaCombatant? selectTarget(
    ...
    final reachableEnemies = attack != null
        ? livingEnemies
            .where((e) => _canReachTarget(attacker, e, attack))
            .toList()
        : livingEnemies;
    final candidates =
        reachableEnemies.isNotEmpty ? reachableEnemies : livingEnemies;
  ...
  ArenaTurnStep executeTurn({
    ...
    if (attacker.isDefeated) {
      return ArenaTurnStep(
        ...
        attackEvents: const [],
        specialEventSummary:
            '${attacker.displayName} is defeated and skips their turn.',
    ...
    if (hasRechargeAttack && !attacker.isRechargeReady) {
      ...
      final rechargeThreshold =
          attacks.firstWhere((a) => a.rechargeRoll != null).rechargeRoll ?? 5;
      if (rechargeD6 >= rechargeThreshold) {
        attacker.isRechargeReady = true;
        specialSummary =
            '${attacker.displayName} recharged their special ability (Rolled $rechargeD6 on d6)!';
      }
    }
    ...
    if (attacker.isSpellcaster) {
      ...
      if (spellResult != null &&
          (spellResult.events.isNotEmpty ||
              spellResult.specialSummary != null)) {
        ...
        if (events.isNotEmpty) {
          ...
          if (expired.isNotEmpty) {
            final expireMsg =
                '${attacker.displayName} is no longer ${expired.map((c) => c.label).join(", ")}.';
            specialSummary = specialSummary != null
                ? '$specialSummary\n$expireMsg'
                : expireMsg;
          }
          ...
          return ArenaTurnStep(
            ...
            combatantHpSnapshot: {
              for (final c in allCombatants) c.id: c.currentHp
            },
          );
        }
      }
    ...
    final actionsToExecute =
        _selectAttacksForTurn(attacker, attacks, sb, environment: environment);
    ...
    for (final attack in actionsToExecute) {
      ...
      if (isAoE) {
        ...
        if (aoeEvents.isNotEmpty && aoeEvents.length > 1) {
          final aoeNote =
              '${attacker.displayName} unleashed ${attack.name} catching ${aoeEvents.length} enemies in the area!';
          specialSummary =
              specialSummary != null ? '$specialSummary\n$aoeNote' : aoeNote;
        }
      } else {
        ...
        if (target.isAirborne &&
            !attacker.isAirborne &&
            attack.attackType == AttackType.meleeStandard &&
            attacker.meleeReachInFeet < target.altitudeInFeet) {
          // Check for fallback ranged attack
          final rangedFallback = attacks
              .where((a) =>
                  a.attackType == AttackType.rangedWeapon ||
                  a.attackType == AttackType.rangedSpell)
              .firstOrNull;
          if (rangedFallback != null) {
            ...
            final passNote =
                '${attacker.displayName} cannot reach airborne ${target.displayName} (${target.altitudeInFeet} ft. altitude) with melee reach (${attacker.meleeReachInFeet} ft.) and takes the Dodge action.';
            specialSummary = specialSummary != null
                ? '$specialSummary\n$passNote'
                : passNote;
          }
  ...
  ({List<ArenaAttackEvent> events, String? specialSummary})?
      _evaluateTacticalSpellcasting({
    required ArenaCombatant attacker,
    required List<ArenaCombatant> allCombatants,
    required ArenaTargetingStrategy strategy,
    ...
    if (attacker.hasSpell('spell_healing_word') && _hasSlotFor(attacker, 1)) {
      final injuredAllies =
          livingAllies.where((a) => a.currentHp < a.maxHp * 0.6).toList();
      if (injuredAllies.isNotEmpty) {
        ...
        final actualHealed =
            min(healTarget.maxHp - healTarget.currentHp, healAmount);
        healTarget.currentHp += actualHealed;
        bonusSummary =
            '${attacker.displayName} cast Healing Word (Slot $slotLvl) on ${healTarget.displayName}, restoring $actualHealed HP!';
      }
    }
    // 2. Misty Step (spell_misty_step)
    else if (attacker.hasSpell('spell_misty_step') &&
        _hasSlotFor(attacker, 2) &&
        !attacker.castBonusActionSpellThisTurn) {
      if (attacker.hpPercent < 0.5) {
        final slotLvl = _deductLowestSlot(attacker, 2)!;
        attacker.castBonusActionSpellThisTurn = true;
        bonusSummary =
            '${attacker.displayName} cast Misty Step (Slot $slotLvl) to teleport to safety!';
      }
    }
    ...
    if (attacker.castBonusActionSpellThisTurn) {
      ...
      return bonusSummary != null
          ? (events: const [], specialSummary: bonusSummary)
          : null;
    }
    ...
    if (attacker.activeConcentrationSpellId == null) {
      ...
      if (controlResult != null) {
        ...
        return (
          events: events,
          specialSummary: summary.isNotEmpty ? summary : null
        );
      }
    }
  ...
  List<ArenaAttackEvent>? _evaluateKillShot({
    ...
    if (attacker.hasSpell('spell_power_word_kill') &&
        _hasSlotFor(attacker, 9)) {
      ...
      if (killable.isNotEmpty) {
        ...
        return [
          ArenaAttackEvent(
            ...
            summaryText:
                'EXECUTION! ${attacker.displayName} uttered Power Word Kill — ${target.displayName} is instantly slain (took $dmg force damage)!',
          ),
        ];
      }
    }
    ...
    if (attacker.hasSpell('spell_disintegrate') && _hasSlotFor(attacker, 6)) {
      ...
      if (lowHpFoes.isNotEmpty) {
        ...
        final saveRes = _evaluateSaveWithLegendaryResistance(
            defender: target,
            saveRoll: totalSave,
        ...
        if (!saved) {
          ...
          damageDealt = _applyDefensiveModifiers(
              rawDmg, 'force', target, edition, environment);
        ...
        if (damageDealt > 0 && target.activeConcentrationSpellId != null) {
          final conRes =
              target.checkConcentration(damageDealt, _rollDie, edition);
          if (conRes.broken) {
            final lostName = _getSpellDisplayName(conRes.lostSpellId);
            summary +=
                ' [Concentration on $lostName broken! (CON save ${conRes.saveRoll} vs DC ${conRes.dc})]';
          }
        }
    ...
    if (attacker.hasSpell('spell_finger_of_death') &&
        _hasSlotFor(attacker, 7)) {
      ...
      if (lowHpFoes.isNotEmpty) {
        ...
        damageDealt = _applyDefensiveModifiers(
            damageDealt, 'necrotic', target, edition, environment);
        attacker.totalDamageDealt += damageDealt;
        target.applyDamage(damageDealt);
  ...
  List<ArenaAttackEvent>? _evaluateAoeSpells({
    ...
    for (final candidate in aoeCandidates) {
      if (attacker.hasSpell(candidate.id) &&
          _hasSlotFor(attacker, candidate.minLevel)) {
        ...
        final rawDamage =
            _rollDice(numDice, candidate.diceSides) + candidate.staticBonus;
        final events = <ArenaAttackEvent>[];

        for (final defender in caughtTargets) {
          final saveBonus =
              defender.getSavingThrowBonus(candidate.saveAbility, edition);
          final d20 = _rollDie(20);
          final totalSave = d20 + saveBonus;
          final saveRes = _evaluateSaveWithLegendaryResistance(
              defender: defender,
          ...
          final hasEvasion = defender.hasEvasion(edition) &&
              candidate.saveAbility.toLowerCase() == 'dex';
          ...
          effectiveDamage = _applyDefensiveModifiers(effectiveDamage,
              candidate.damageType, defender, edition, environment);
          ...
          if (defender.isDefeated) {
            ...
            defender.applyCondition(
                ArenaCondition.unconscious, _rollDie, edition);
          }

          String summary;
          if (evadedWithEvasion) {
            summary =
                'EVADED! (Rolled $d20 + $saveBonus = $totalSave vs DC ${attacker.spellSaveDc}) took 0 ${candidate.damageType} damage via Evasion!';
          } else if (saved) {
            final fatal = isKillShot ? ' — SLAIN!' : '';
            summary =
                'Saved (Rolled $d20 + $saveBonus = $totalSave vs DC ${attacker.spellSaveDc})${saveRes.summaryNote} took half: $effectiveDamage ${candidate.damageType} damage$fatal';
          } else {
            final fatal = isKillShot ? ' — SLAIN!' : '';
            summary =
                'Failed Save (Rolled $d20 + $saveBonus = $totalSave vs DC ${attacker.spellSaveDc}) took full: $effectiveDamage ${candidate.damageType} damage$fatal';
          }

          if (effectiveDamage > 0 &&
              defender.activeConcentrationSpellId != null) {
            final conRes =
                defender.checkConcentration(effectiveDamage, _rollDie, edition);
            if (conRes.broken) {
              final lostName = _getSpellDisplayName(conRes.lostSpellId);
              summary +=
                  ' [Concentration on $lostName broken! (CON save ${conRes.saveRoll} vs DC ${conRes.dc})]';
  ...
  ({List<ArenaAttackEvent> events, String? summary})?
      _evaluateConcentrationControl({
    required ArenaCombatant attacker,
    required List<ArenaCombatant> livingEnemies,
    ...
    if (attacker.hasSpell('spell_hold_monster') && _hasSlotFor(attacker, 5)) {
      livingEnemies.sort((a, b) =>
          b.monster.challengeRating.compareTo(a.monster.challengeRating));
      ...
      final totalSave = d20 + saveBonus;
      final saveRes = _evaluateSaveWithLegendaryResistance(
          defender: target, saveRoll: totalSave, saveDc: attacker.spellSaveDc);
      final saved = saveRes.saved;

      String note;
      if (saved) {
        note =
            '${target.displayName} resisted Hold Monster (Wis save $totalSave vs DC ${attacker.spellSaveDc})${saveRes.summaryNote}';
      } else {
        final condRes =
            target.applyCondition(ArenaCondition.paralyzed, _rollDie, edition);
        note =
            '${target.displayName} failed Wis save ($totalSave vs DC ${attacker.spellSaveDc}) and is PARALYZED by Hold Monster!';
      ...
      return (
        ...
        summary:
            '${attacker.displayName} is now concentrating on Hold Monster.',
      );
    }
    ...
    if (attacker.hasSpell('spell_slow') && _hasSlotFor(attacker, 3)) {
      ...
      for (final enemy in livingEnemies.take(6)) {
        ...
        final totalSave = d20 + saveBonus;
        final saveRes = _evaluateSaveWithLegendaryResistance(
            defender: enemy, saveRoll: totalSave, saveDc: attacker.spellSaveDc);
        final saved = saveRes.saved;
      ...
      return (
        events: events,
        summary: '${attacker.displayName} is concentrating on Slow.'
      );
    }
    ...
    if (attacker.hasSpell('spell_faerie_fire') && _hasSlotFor(attacker, 1)) {
      ...
      return (
        events: events,
        summary: '${attacker.displayName} is concentrating on Faerie Fire.'
      );
    }

    // 5. Shield of Faith (spell_shield_of_faith, Level 1)
    if (attacker.hasSpell('spell_shield_of_faith') &&
        _hasSlotFor(attacker, 1)) {
      final slotLvl = _deductLowestSlot(attacker, 1)!;
      ...
      return (
        events: const [],
        summary:
            '${attacker.displayName} cast Shield of Faith (Slot $slotLvl) on themselves (+2 AC, concentrating)!',
      );
    }
    ...
    if (attacker.hasSpell('spell_bless') && _hasSlotFor(attacker, 1)) {
      ...
      return (
        events: const [],
        summary:
            '${attacker.displayName} cast Bless (Slot $slotLvl) boosting allies with divine favor!',
      );
    }
  ...
  List<ArenaAttackEvent>? _evaluateSingleTargetBurst({
    ...
    if (attacker.hasSpell('spell_scorching_ray') && _hasSlotFor(attacker, 2)) {
      ...
      for (int i = 0; i < rayCount; i++) {
        ...
        if (isHit) {
          ...
          damageDealt = _applyDefensiveModifiers(
              rawDmg, 'fire', target, edition, environment);
          attacker.totalDamageDealt += damageDealt;
          target.applyDamage(damageDealt);
        ...
        if (shielded) {
          summary =
              'PARRIED BY SHIELD! ${target.displayName} cast Shield (+5 AC) deflecting Scorching Ray # ${i + 1} ($d20 + ${attacker.spellAttackBonus} = $totalAttack vs AC ${target.effectiveAc})!';
        } else if (isCrit) {
          final fatal = isKillShot ? ' — DEFENDER SLAIN!' : '';
          summary =
              'CRITICAL HIT! (Nat 20) Scorching Ray # ${i + 1} dealt $damageDealt fire damage$fatal';
        } else if (isHit) {
          final fatal = isKillShot ? ' — DEFENDER SLAIN!' : '';
          summary =
              'Hits (Roll $d20 + ${attacker.spellAttackBonus} = $totalAttack vs AC ${target.effectiveAc}) for $damageDealt fire damage$fatal';
        } else {
          summary =
              'Misses (Roll $d20 + ${attacker.spellAttackBonus} = $totalAttack vs AC ${target.effectiveAc}).';
        }
    ...
    if (attacker.hasSpell('spell_magic_missile') && _hasSlotFor(attacker, 1)) {
      ...
      if (!target.usedReactionThisRound &&
          target.hasSpell('spell_shield') &&
          target.hasAvailableSlots) {
        _deductLowestSlot(target, 1);
        target.usedReactionThisRound = true;
        ...
        return [
          ArenaAttackEvent(
            ...
            defenderMaxHp: target.maxHp,
            summaryText:
                'SHIELD INTERCEPT! ${target.displayName} cast Shield as a reaction, completely neutralizing all $dartCount Magic Missile darts!',
          ),
        ];
      }

      final rawDmg = _rollDice(dartCount, 4) + dartCount;
      final damageDealt = _applyDefensiveModifiers(
          rawDmg, 'force', target, edition, environment);
      attacker.totalDamageDealt += damageDealt;
      target.applyDamage(damageDealt);
      ...
      String summary =
          '$dartCount Magic Missiles automatically struck ${target.displayName} for $damageDealt force damage$fatal';
    ...
    if (attacker.hasSpell('spell_guiding_bolt') && _hasSlotFor(attacker, 1)) {
      ...
      if (isHit) {
        ...
        final rawDmg = _rollDice(dice, 6);
        damageDealt = _applyDefensiveModifiers(
            rawDmg, 'radiant', target, edition, environment);
        attacker.totalDamageDealt += damageDealt;
        target.applyDamage(damageDealt);
      ...
      if (shielded) {
        summary =
            'PARRIED BY SHIELD! ${target.displayName} cast Shield deflecting Guiding Bolt ($d20 + ${attacker.spellAttackBonus} = $totalAttack vs AC ${target.effectiveAc})!';
      } else if (isCrit) {
        final fatal = isKillShot ? ' — DEFENDER SLAIN!' : '';
        summary =
            'CRITICAL HIT! (Nat 20) Guiding Bolt dealt $damageDealt radiant damage$fatal';
      } else if (isHit) {
        final fatal = isKillShot ? ' — DEFENDER SLAIN!' : '';
        summary =
            'Hits (Roll $d20 + ${attacker.spellAttackBonus} = $totalAttack vs AC ${target.effectiveAc}) for $damageDealt radiant damage$fatal';
      } else {
        summary =
            'Misses (Roll $d20 + ${attacker.spellAttackBonus} = $totalAttack vs AC ${target.effectiveAc}).';
      }
    ...
    if (attacker.hasSpell('spell_inflict_wounds') && _hasSlotFor(attacker, 1)) {
      ...
      if (isHit) {
        ...
        damageDealt = _applyDefensiveModifiers(
            rawDmg, 'necrotic', target, edition, environment);
        attacker.totalDamageDealt += damageDealt;
        target.applyDamage(damageDealt);
      ...
      if (shielded) {
        summary =
            'PARRIED BY SHIELD! ${target.displayName} cast Shield deflecting Inflict Wounds ($d20 + ${attacker.spellAttackBonus} = $totalAttack vs AC ${target.effectiveAc})!';
      } else if (isCrit) {
        final fatal = isKillShot ? ' — DEFENDER SLAIN!' : '';
        summary =
            'CRITICAL HIT! (Nat 20) Inflict Wounds dealt $damageDealt necrotic damage$fatal';
      } else if (isHit) {
        final fatal = isKillShot ? ' — DEFENDER SLAIN!' : '';
        summary =
            'Hits (Roll $d20 + ${attacker.spellAttackBonus} = $totalAttack vs AC ${target.effectiveAc}) for $damageDealt necrotic damage$fatal';
      } else {
        summary =
            'Misses (Roll $d20 + ${attacker.spellAttackBonus} = $totalAttack vs AC ${target.effectiveAc}).';
      }
    ...
    // 6. Acid Arrow (spell_acid_arrow, Level 2)
    if (attacker.hasSpell('spell_acid_arrow') && _hasSlotFor(attacker, 2)) {
      final slotLvl = _deductHighestSlot(attacker, 2)!;
      attacker.attacksMade++;
      ...
      if (isHit) {
        ...
        damageDealt = _applyDefensiveModifiers(
            rawDmg, 'acid', target, edition, environment);
        attacker.totalDamageDealt += damageDealt;
        target.applyDamage(damageDealt);
      ...
      if (shielded) {
        summary =
            'PARRIED BY SHIELD! ${target.displayName} cast Shield deflecting Acid Arrow ($d20 + ${attacker.spellAttackBonus} = $totalAttack vs AC ${target.effectiveAc})!';
      } else if (isCrit) {
        final fatal = isKillShot ? ' — DEFENDER CORRODED & SLAIN!' : '';
        summary =
            'CRITICAL HIT! (Nat 20) Acid Arrow splashed for $damageDealt acid damage$fatal';
      } else if (isHit) {
        final fatal = isKillShot ? ' — DEFENDER SLAIN!' : '';
        summary =
            'Hits (Roll $d20 + ${attacker.spellAttackBonus} = $totalAttack vs AC ${target.effectiveAc}) for $damageDealt acid damage$fatal';
      } else {
        summary =
            'Misses (Roll $d20 + ${attacker.spellAttackBonus} = $totalAttack vs AC ${target.effectiveAc}) — splash dealt $damageDealt acid damage.';
      }
    ...
    if ((attacker.hasSpell('spell_fireball') ||
        ...
        _hasSlotFor(attacker, 2)) {
      final isFireball =
          attacker.hasSpell('spell_fireball') && _hasSlotFor(attacker, 3);
      final isLightning =
          attacker.hasSpell('spell_lightning_bolt') && _hasSlotFor(attacker, 3);
      final isConeOfCold =
          attacker.hasSpell('spell_cone_of_cold') && _hasSlotFor(attacker, 5);
      final isCloudkill =
          attacker.hasSpell('spell_cloudkill') && _hasSlotFor(attacker, 5);
      ...
      final spellName = isConeOfCold
          ...
          : (isCloudkill
              ...
              : (isFireball
                  ? 'Fireball'
                  : (isLightning ? 'Lightning Bolt' : 'Shatter')));
      final minLvl =
          isConeOfCold || isCloudkill ? 5 : (isFireball || isLightning ? 3 : 2);
      final saveAbility = isConeOfCold || isCloudkill
          ? 'con'
          : (isFireball || isLightning ? 'dex' : 'con');
      final dmgType = isConeOfCold
          ? 'cold'
          : (isCloudkill
              ? 'poison'
              : (isFireball
                  ? 'fire'
                  : (isLightning ? 'lightning' : 'thunder')));
      ...
      final numDice = minLvl == 5
          ? 8 + (slotLvl - 5)
          : (minLvl == 3 ? 8 + (slotLvl - 3) : 3 + (slotLvl - 2));
      ...
      damageDealt = _applyDefensiveModifiers(
          damageDealt, dmgType, target, edition, environment);
      attacker.totalDamageDealt += damageDealt;
      target.applyDamage(damageDealt);
      if (damageDealt > 0) attacker.hitsLanded++;
  ...
  List<ArenaAttackEvent>? _evaluateCantrips({
    ...
    if (attacker.hasSpell('spell_eldritch_blast')) {
      ...
      for (int i = 0; i < multiplier; i++) {
        ...
        if (shielded) {
          summary =
              'PARRIED BY SHIELD! ${target.displayName} deflected Eldritch Blast Beam # ${i + 1} ($d20 + ${attacker.spellAttackBonus} = $totalAttack vs AC ${target.effectiveAc})!';
        } else if (isCrit) {
          final fatal = isKillShot ? ' — DEFENDER SLAIN!' : '';
          summary =
              'CRITICAL HIT! (Nat 20) Eldritch Blast Beam # ${i + 1} dealt $damageDealt force damage$fatal';
        } else if (isHit) {
          final fatal = isKillShot ? ' — DEFENDER SLAIN!' : '';
          summary =
              'Hits (Roll $d20 + ${attacker.spellAttackBonus} = $totalAttack vs AC ${target.effectiveAc}) for $damageDealt force damage$fatal';
        } else {
          summary =
              'Misses (Roll $d20 + ${attacker.spellAttackBonus} = $totalAttack vs AC ${target.effectiveAc}).';
        }
    ...
    if (attacker.hasSpell('spell_fire_bolt')) {
      ...
      if (shielded) {
        summary =
            'PARRIED BY SHIELD! ${target.displayName} cast Shield deflecting Fire Bolt ($d20 + ${attacker.spellAttackBonus} = $totalAttack vs AC ${target.effectiveAc})!';
      } else if (isCrit) {
        final fatal = isKillShot ? ' — DEFENDER SLAIN!' : '';
        summary =
            'CRITICAL HIT! (Nat 20) Fire Bolt dealt $damageDealt fire damage$fatal';
      } else if (isHit) {
        final fatal = isKillShot ? ' — DEFENDER SLAIN!' : '';
        summary =
            'Hits (Roll $d20 + ${attacker.spellAttackBonus} = $totalAttack vs AC ${target.effectiveAc}) for $damageDealt fire damage$fatal';
    ...
    if (attacker.hasSpell('spell_ray_of_frost') ||
      ...
      if (isHit) {
        ...
        damageDealt = _applyDefensiveModifiers(
            rawDmg, 'cold', target, edition, environment);
        attacker.totalDamageDealt += damageDealt;
        target.applyDamage(damageDealt);
      ...
      if (shielded) {
        summary =
            'PARRIED BY SHIELD! ${target.displayName} deflected Ray of Frost ($d20 + ${attacker.spellAttackBonus} = $totalAttack vs AC ${target.effectiveAc})!';
      } else if (isCrit) {
        final fatal = isKillShot ? ' — FROZEN & SLAIN!' : '';
        summary =
            'CRITICAL HIT! (Nat 20) Ray of Frost dealt $damageDealt cold damage$fatal';
      } else if (isHit) {
        final fatal = isKillShot ? ' — FROZEN & SLAIN!' : '';
        summary =
            'Hits (Roll $d20 + ${attacker.spellAttackBonus} = $totalAttack vs AC ${target.effectiveAc}) for $damageDealt cold damage$fatal';
      } else {
        summary =
            'Misses (Roll $d20 + ${attacker.spellAttackBonus} = $totalAttack vs AC ${target.effectiveAc}).';
      }
  ...
  ({bool saved, bool usedLegendaryResistance, String summaryNote})
      _evaluateSaveWithLegendaryResistance({
    ...
    if (!saved && defender.useLegendaryResistance()) {
      ...
      note =
          ' [LEGENDARY RESISTANCE! ${defender.displayName} chose to succeed ($remaining left)]';
    }
  ...
  List<DprAttackAction> _selectAttacksForTurn(
    ...
    final rechargeAttack =
        candidateAttacks.where((a) => a.rechargeRoll != null).firstOrNull;
    ...
    final standardAttacks = candidateAttacks
        .where((a) => !a.isLegendaryAction && a.rechargeRoll == null)
        .toList();
    if (standardAttacks.isNotEmpty) {
      standardAttacks.sort(
          (a, b) => _estimateAttackDpr(b).compareTo(_estimateAttackDpr(a)));
      return [standardAttacks.first.copyWith(attacksPerRound: 1)];
    }

    final sortedCandidates = List<DprAttackAction>.from(candidateAttacks);
    sortedCandidates
        .sort((a, b) => _estimateAttackDpr(b).compareTo(_estimateAttackDpr(a)));
    return sortedCandidates
        .take(1)
        .map((a) => a.copyWith(attacksPerRound: 1))
        .toList();
  }

  double _estimateAttackDpr(DprAttackAction a) {
    final primaryAvg = (a.diceCount * (a.diceSides + 1) / 2.0) + a.damageBonus;
    final secAvg = (a.secondaryDiceCount * (a.secondaryDiceSides + 1) / 2.0) +
  ...
  List<ArenaAttackEvent> _resolveAoeAttack({
    ...
    final sb = attacker.getStatBlock(edition);
    final matchingAction = sb.actions
        .where((a) => a.name.toLowerCase() == attack.name.toLowerCase())
        .firstOrNull;
    final parsedShape =
        AoeResolver.parseShapeAndSize(attack.name, matchingAction?.description);
    ...
    final rawDamage =
        _rollDice(attack.diceCount, attack.diceSides) + attack.damageBonus;
    ...
    for (final defender in caughtDefenders) {
      ...
      final totalSave = d20 + saveBonus;
      final saveRes = _evaluateSaveWithLegendaryResistance(
          defender: defender, saveRoll: totalSave, saveDc: saveDc);
      final saved = saveRes.saved;

      final hasEvasion =
          defender.hasEvasion(edition) && saveAbility.toLowerCase() == 'dex';
      bool evadedWithEvasion = false;
      int effectiveDamage;
      ...
      effectiveDamage = _applyDefensiveModifiers(
          effectiveDamage, attack.damageType, defender, edition, environment);
      attacker.totalDamageDealt += effectiveDamage;
      defender.applyDamage(effectiveDamage);
      ...
      if (evadedWithEvasion) {
        summary =
            'EVADED! (Rolled $d20 + $saveBonus = $totalSave vs DC $saveDc) took 0 ${attack.damageType} damage via Evasion!';
      } else if (saved) {
        final fatal = isKillShot ? ' — SLAIN!' : '';
        summary =
            'Saved (Rolled $d20 + $saveBonus = $totalSave vs DC $saveDc)${saveRes.summaryNote} took half: $effectiveDamage ${attack.damageType} damage$fatal';
      } else {
        final fatal = isKillShot ? ' — SLAIN!' : '';
        summary =
            'Failed Save (Rolled $d20 + $saveBonus = $totalSave vs DC $saveDc) took full: $effectiveDamage ${attack.damageType} damage$fatal';
      }
  ...
  ArenaAttackEvent _resolveSingleAttack({
    ...
    final isRangedAttack = attack.attackType == AttackType.rangedWeapon ||
    ...
    if (attackerFlies &&
        !defenderFlies &&
        (isRangedAttack || attacker.hasFlyby(edition))) {
    ...
    if (defender.isParalyzed ||
        ...
        defender.hasCondition(ArenaCondition.restrained)) {
      conditionAdvantage = true;
    }

    if (defender.isProne) {
      if (attack.attackType == AttackType.meleeStandard ||
          attack.attackType == AttackType.meleeReach) {
        conditionAdvantage = true;
      } else {
    ...
    if (attacker.isProne ||
        attacker.hasCondition(ArenaCondition.restrained) ||
        attacker.hasCondition(ArenaCondition.blinded) ||
        attacker.hasCondition(ArenaCondition.poisoned)) {
      conditionDisadvantage = true;
    }

    final hasAdvantage = (packTacticsAdvantage ||
            flightAdvantage ||
            aquaticAdvantage ||
            conditionAdvantage) &&
        !flightDisadvantage &&
    ...
    final hasDisadvantage = (flightDisadvantage ||
            aquaticDisadvantage ||
            conditionDisadvantage ||
            cageMatchDisadvantage) &&
    ...
    bool isHit = (naturalRoll == 20) ||
        (!isFumble && totalAttack >= defender.effectiveAc);
    final isCrit = (naturalRoll == 20) ||
        (isHit &&
            (defender.isParalyzed || defender.isUnconscious) &&
            attack.attackType == AttackType.meleeStandard);
    ...
    if (isHit &&
        !isCrit &&
        !defender.usedReactionThisRound &&
        defender.hasSpell('spell_shield') &&
        defender.hasAvailableSlots) {
    ...
    if (isHit) {
      ...
      final precomputedAttack = profileAttacks[actionId] ??
          ...
          profileAttacks.values
              .where(
                (pa) =>
                    pa.attackId == attack.id.toLowerCase().trim() ||
                    pa.attackId ==
                        actionId.replaceAll(RegExp(r'[^a-z0-9]+'), '-'),
              )
              .firstOrNull ??
          PrecomputedAttack(
            ...
            damageGroups: [
              if (attack.diceCount > 0)
                DamageDieGroup(
                    count: attack.diceCount, faces: attack.diceSides),
              if (attack.secondaryDiceCount > 0)
                DamageDieGroup(
                    count: attack.secondaryDiceCount,
                    faces: attack.secondaryDiceSides),
            ],
          );
    ...
    if (shielded) {
      final advTag =
          hasAdvantage ? ' (Adv)' : (hasDisadvantage ? ' (Disadv)' : '');
      summary =
          'PARRIED BY SHIELD!$advTag ${defender.displayName} cast Shield (+5 AC) turning attack ($naturalRoll + ${attack.attackBonus} = $totalAttack vs AC ${defender.effectiveAc}) into a miss!';
    } else {
    ...
    if (damageDealt > 0 && defender.activeConcentrationSpellId != null) {
      final conRes =
          defender.checkConcentration(damageDealt, _rollDie, edition);
      if (conRes.broken) {
        final lostName = _getSpellDisplayName(conRes.lostSpellId);
        summary +=
            ' [Concentration on $lostName broken! (CON save ${conRes.saveRoll} vs DC ${conRes.dc})]';
  ...
  int _applyDefensiveModifiers(
    ...
    if (immunities.isNotEmpty &&
        (immunities.contains(dmgLower) || immunities.contains('all'))) {
      return 0;
    }
    ...
    if (resistances.isNotEmpty &&
        (resistances.contains(dmgLower) || resistances.contains('all'))) {
      workingDamage = (workingDamage / 2).floor();
    }
  ...
  List<ArenaAttackEvent> executeOffTurnLegendaryActions({
    ...
    final opposingLegendaries = allCombatants
        .where((c) =>
    ...
    for (final legendary in opposingLegendaries) {
      ...
      final hasDisruptLife = legActions
              .any((a) => a.name.toLowerCase().contains('disrupt life')) ||
          sb.traits
              .any((t) => t.description.toLowerCase().contains('disrupt life'));
      if (hasDisruptLife &&
        ...
        for (final enemy in livingEnemies) {
          ...
          final saveRes = _evaluateSaveWithLegendaryResistance(
              defender: enemy, saveRoll: totalSave, saveDc: dc);
          final saved = saveRes.saved;

          final rawDmg = _rollDice(6, 6);
          int damageDealt = saved ? (rawDmg / 2).floor() : rawDmg;
          damageDealt = _applyDefensiveModifiers(
              damageDealt, 'necrotic', enemy, edition, environment);
      ...
      final hasParalyzingTouch = legActions
          .any((a) => a.name.toLowerCase().contains('paralyzing touch'));
      if (hasParalyzingTouch && legendary.legendaryActionsRemaining >= 2) {
        ...
        if (target != null) {
          ...
          final isCrit = d20 == 20;
          final isFumble = d20 == 1;
          final atkBonus = legendary.spellAttackBonus > 0
              ? legendary.spellAttackBonus
              : sb.attackBonus;
          final totalAttack = d20 + atkBonus;
          final isHit =
              isCrit || (!isFumble && totalAttack >= target.effectiveAc);
          ...
          if (isHit) {
            ...
            if (target.isDefeated) {
              ...
              final dc =
                  legendary.spellSaveDc > 10 ? legendary.spellSaveDc : 18;
              final saveRes = _evaluateSaveWithLegendaryResistance(
                  defender: target, saveRoll: saveTotal, saveDc: dc);
              if (!saveRes.saved) {
                target.applyCondition(
                    ArenaCondition.paralyzed, _rollDie, edition,
                    durationRounds: 10, source: 'Paralyzing Touch');
                paralyzeMsg =
                    ' — FAILED Con Save ($saveTotal vs DC $dc) and is PARALYZED!';
              } else {
                paralyzeMsg =
                    ' — Saved against paralysis ($saveTotal vs DC $dc)${saveRes.summaryNote}.';
              }
            }
          }
      ...
      final hasCantrip =
          legActions.any((a) => a.name.toLowerCase().contains('cantrip')) ||
              legendary.isSpellcaster;
      ...
      final dprAttacks = sb
          ...
          .where((a) =>
              a.isLegendaryAction ||
              legActions.any(
                  (la) => la.name.toLowerCase().contains(a.name.toLowerCase())))
          .toList();
  ...
  ArenaSimulationResult simulateMatch({
    ...
    while (currentRound <= maxRounds) {
      ...
      for (final combatant in combatants) {
        ...
        final livingA = combatants
            .where((c) => c.team == ArenaTeam.teamA && c.isAlive)
            .length;
        final livingB = combatants
            .where((c) => c.team == ArenaTeam.teamB && c.isAlive)
            .length;
    ...
    final teamALiving =
        combatants.where((c) => c.team == ArenaTeam.teamA && c.isAlive).length;
    final teamBLiving =
        combatants.where((c) => c.team == ArenaTeam.teamB && c.isAlive).length;
  ...
  ArenaMonteCarloResult runMonteCarlo({
    ...
    for (int i = 0; i < iterations; i++) {
      ...
      if (res.totalRounds > maxRoundsObserved)
        maxRoundsObserved = res.totalRounds;
    ...
    return ArenaMonteCarloResult(
      ...
      avgTeamASurvivors:
          iterations > 0 ? (totalTeamASurvivors / iterations) : 0,
      avgTeamBSurvivors:
          iterations > 0 ? (totalTeamBSurvivors / iterations) : 0,
      avgTeamASurvivingHpPercent:
          iterations > 0 ? (totalTeamAHpPercent / iterations) * 100 : 0,
      avgTeamBSurvivingHpPercent:
          iterations > 0 ? (totalTeamBHpPercent / iterations) * 100 : 0,
      calculationDuration: stopwatch.elapsed,
    );
  }

  /// Background isolate runner for Monte Carlo simulation.
  static ArenaMonteCarloResult _isolateMonteCarloRunner(
      _ArenaMonteCarloPayload payload) {
    final engine = ArenaCombatEngine(
        rng: payload.seed != null ? Random(payload.seed) : Random());
  ...
  static ArenaSimulationResult _isolateMatchRunner(
      _ArenaMatchSimulationPayload payload) {
    final engine = ArenaCombatEngine(
        rng: payload.seed != null ? Random(payload.seed) : Random());
  ...
  String _formatAttackSummary({
    ...
    final advTag =
        hadAdvantage ? ' (Adv)' : (hadDisadvantage ? ' (Disadv)' : '');
