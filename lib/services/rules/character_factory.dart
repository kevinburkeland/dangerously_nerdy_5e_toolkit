...
export '../../models/domain/character_models.dart'
    show StartingEquipmentItemRequest;
...
class CharacterFactory {
  ...
  static Character buildFromDraft(CharacterDraft draft) {
    if (!draft.isReadyForCompilation) {
      ...
      if (draft.characterName == null || draft.characterName!.trim().isEmpty)
        missing.add('Character Name');
      throw StateError(
          'Cannot compile character from draft. Missing mandatory field(s): ${missing.join(", ")}');
    }

    final ruleset = draft.rulesEdition == DmRulesEdition.v2014
    ...
    if (draft.rulesEdition == DmRulesEdition.v2014) {
      if (finalBonusScores == const AbilityScores.zero() &&
          draft.speciesBonusScores != const AbilityScores.zero()) {
        finalBonusScores = draft.speciesBonusScores;
      }
    } else if (draft.rulesEdition == DmRulesEdition.v2024) {
      if (finalBonusScores == const AbilityScores.zero() &&
          draft.backgroundBonusScores != const AbilityScores.zero()) {
        finalBonusScores = draft.backgroundBonusScores;
      }
      if (finalBonusScores == const AbilityScores.zero() &&
          draft.backgroundRef != null) {
    ...
    final totalCon =
        draft.baseScores!.constitution + finalBonusScores.constitution;
        ...
        ? (draft.backgroundRef?.customProperties['startingEquipment'] ??
            (draft.rulesEdition == DmRulesEdition.v2014
                ? (resolvedBg?.customProperties['startingEquipment'] ??
                    (draft.backgroundRef != null
                        ? SrdBackgroundsLibrary.extractStartingEquipment(
                            draft.backgroundRef!.slug)
                        : null))
                : null))
        : null;
    if (bgEquip is List) {
      for (final item in bgEquip) {
        ...
        if (item is String) {
          ...
          itemName =
              (item['item'] ?? item['name'] ?? item['slug'] ?? '').toString();
        }
        if (itemName.isNotEmpty) {
          final slug = _slugify(itemName);
          final alreadyPresent =
              inventory.any((inv) => inv.itemRef.slug == slug);
          if (!alreadyPresent) {
    ...
    final compiledSkills =
        Map<SkillType, SkillProficiencyLevel>.from(draft.selectedSkills);
    if (draft.backgroundRef != null) {
      for (final skill in draft.backgroundRef!.grantedSkills) {
        if (!compiledSkills.containsKey(skill) ||
            compiledSkills[skill] == SkillProficiencyLevel.none) {
          compiledSkills[skill] = SkillProficiencyLevel.proficient;
        }
      }
      final bgSkillsRaw =
          draft.backgroundRef!.customProperties['skillProficiencies'] ??
        ...
        for (final s in bgSkillsRaw) {
          ...
          for (final st in SkillType.values) {
            if (st.name.toLowerCase() == sName) {
              if (!compiledSkills.containsKey(st) ||
                  compiledSkills[st] == SkillProficiencyLevel.none) {
    ...
    if (draft.rulesEdition == DmRulesEdition.v2014 &&
        draft.backgroundRef != null) {
      final feature = SrdBackgroundsLibrary.get2014Feature(
              draft.backgroundRef!.slug, resolvedBg) ??
          (draft.backgroundRef!.customProperties['backgroundFeature'] != null
              ? (
                  name: draft
                      .backgroundRef!.customProperties['backgroundFeature']
                      .toString(),
                  description: draft.backgroundRef!
              ...
              : (resolvedBg?.customProperties['feature'] != null
                  ? (
                      ...
                      description: resolvedBg
                              .customProperties['featureDescription']
                              ?.toString() ??
                          '',
                    )
                  : null));
    ...
    final compiledSpellsKnown =
        List<EntityReference<Spell>>.from(draft.spellsKnown);
    ...
    final character = Character(
      ...
      traitProficiencies:
          compiledSkills.map((k, v) => MapEntry(k.name, v.multiplier)),
      savingThrowProficiencies:
          AttributeKeySet.from(draft.savingThrowProficiencies),
  ...
  static Character createLevel1Character(CharacterCreationRequest request) {
    ...
    final hitDieSides = int.tryParse(cleanHitDie) ?? 8;
    final totalCon =
        request.baseScores.constitution + request.bonusScores.constitution;
    final conMod = totalCon.dndModifier;
    final startingHp = hitDieSides + conMod;
    ...
    // Initial Spell Slots Resource
    final edition = request.ruleset == RulesetVersion.v2014
    ...
    final speciesTraits = SkillTraitResolver.getSpeciesTraits(
      ...
      subraceSlug: null,
      edition: request.ruleset == RulesetVersion.v2014
          ? DmRulesEdition.v2014
          : DmRulesEdition.v2024,
    );
    ...
    final character = Character(
      ...
      traitProficiencies: request.skillProficiencies
          .map((k, v) => MapEntry(k.name, v.multiplier)),
      savingThrowProficiencies:
          AttributeKeySet.from(request.savingThrowProficiencies),
