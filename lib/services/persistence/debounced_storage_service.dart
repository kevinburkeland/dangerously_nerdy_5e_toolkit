...
class DebouncedStorageService {
  static final DebouncedStorageService _instance =
  ...
  DebouncedStorageService.custom({LoggingService? logger})
    ...
    final futures = tasksToFlush.entries.map((entry) async {
      try {
        ...
        _logger.logNonFatal(
          ...
          stackTrace,
          reason:
              'Failed to execute debounced write task during flushAll: ${entry.key}',
        );
      }
    });
      ...
      try {
        unawaited(entry.value().catchError((e, stackTrace) {
          _logger.logNonFatal(
            ...
            stackTrace,
            reason:
                'Failed to execute debounced write task during flushAllSync: ${entry.key}',
          );
        }));
      } catch (e, stackTrace) {
        _logger.logNonFatal(
          ...
          stackTrace,
          reason:
              'Synchronous failure initiating debounced task in flushAllSync: ${entry.key}',
        );
      }
    }
  }
