...
class StorageMigrationService {
  ...
  static final StorageMigrationService _instance =
  ...
  StorageMigrationService.custom({LoggingService? logger})
  ...
  Future<void> runMigrations(SharedPreferences prefs,
    ...
    try {
      ...
      log.logFatal(
        ...
        stackTrace,
        reason:
            'Critical failure during storage schema migration from v$storedVersion',
      );
    }
  }
  ...
  Future<void> _migrateV1ToV2(SharedPreferences prefs) async {
    ...
    if (oldTheme != null &&
        oldTheme >= 0 &&
        oldTheme < ThemeMode.values.length) {
      await prefs.setString(
          'setting_theme_mode_v2', ThemeMode.values[oldTheme].name);
    ...
    if (oldAccent != null &&
        oldAccent >= 0 &&
        oldAccent < FantasyAccent.values.length) {
      await prefs.setString(
          'setting_fantasy_accent_v2', FantasyAccent.values[oldAccent].name);
    ...
    if (oldHaptic != null &&
        oldHaptic >= 0 &&
        oldHaptic < HapticFeedbackLevel.values.length) {
      await prefs.setString('setting_haptic_level_v2',
          HapticFeedbackLevel.values[oldHaptic].name);
    ...
    if (oldEdition != null &&
        oldEdition >= 0 &&
        oldEdition < DmRulesEdition.values.length) {
      await prefs.setString(
          'setting_rules_edition_v2', DmRulesEdition.values[oldEdition].name);
