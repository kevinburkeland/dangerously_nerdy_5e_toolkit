...
class MinionSessionService {
  ...
  static final MinionSessionService _instance =
  ...
  SpellSession getOrCreateSession(SummonPreset preset,
      {int? defaultSpellLevel}) {
  ...
  static void populateDefaultMinions(
      SpellSession session, SummonPreset preset) {
    session.clearAll();
    final id = preset.id;
  ...
  void saveSessionDebounced(String presetId) {
    DebouncedStorageService().scheduleWrite('minion_session_$presetId',
        () async {
