...
class DiceRoomService {
  ...
  final ValueNotifier<RoomSession?> activeSessionNotifier =
      ValueNotifier<RoomSession?>(null);
  ...
  bool get isSessionRemembered =>
      activeSessionNotifier.value?.isRemembered ?? false;
  ...
  Future<void> _connectTransport(String roomCode, String playerName) async {
    try {
      if (sl.isRegistered<IP2pTransportPort>()) {
        final localNodeId =
            '$playerName-${roomCode.toLowerCase()}-${DateTime.now().millisecondsSinceEpoch % 100000}';
      ...
      LoggingService().logNonFatal(
        ...
        stackTrace,
        reason:
            'Failed to connect transport in DiceRoomService for room $roomCode',
      );
    }
  }
  ...
  Future<void> _persistSession(
      String roomCode, String playerName, bool remember) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      if (remember) {
        await Future.wait([
    ...
    try {
      ...
      if (raw != null && raw.isNotEmpty) {
        ...
        final loaded = decoded
            .map((item) =>
                RoomRoll.fromMap(Map<String, dynamic>.from(item as Map)))
        ...
        if (_localControllers.containsKey(cleanCode) &&
            !_localControllers[cleanCode]!.isClosed) {
    ...
    unawaited(() async {
      try {
        ...
        final raw = jsonEncode(
            rolls.map((r) => r.toMap(useFirestoreTimestamp: false)).toList());
  ...
  Stream<List<RoomRoll>> streamRoomRolls(String roomCode) {
    ...
    if (!_localControllers.containsKey(cleanCode) ||
        _localControllers[cleanCode]!.isClosed) {
      _localControllers[cleanCode] =
          StreamController<List<RoomRoll>>.broadcast();
    ...
    if (isFirebaseAvailable &&
        !_firestoreSubscriptions.containsKey(cleanCode)) {
      try {
        _firestoreSubscriptions[cleanCode] = FirebaseFirestore.instance
          ...
          LoggingService().logNonFatal(
            ...
            stackTrace,
            reason:
                'Firestore streamRoomRolls error for room $cleanCode; falling back to local rolls',
          );
        });
      } catch (e, stackTrace) {
        LoggingService().logNonFatal(
          ...
          stackTrace,
          reason:
              'Firestore stream initialization failed for room $cleanCode; falling back to in-memory',
        );
      }
    }
  ...
  Future<void> broadcastRoll(RoomRoll roll) async {
    ...
    try {
      ...
      LoggingService().logNonFatal(
        ...
        stackTrace,
        reason:
            'P2P broadcastRoll failed for room $cleanCode; cloud transport preserved',
      );
    }
    ...
    if (isFirebaseAvailable) {
      try {
        ...
        LoggingService().logNonFatal(
          ...
          stackTrace,
          reason:
              'Firestore broadcastRoll failed for room $cleanCode; local fallback preserved',
        );
      }
    }
  }
