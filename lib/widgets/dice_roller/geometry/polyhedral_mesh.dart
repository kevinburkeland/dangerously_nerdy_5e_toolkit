...
class PolyhedronMesh {
  ...
  static PolyhedronMesh createD4({double radius = 64.0}) {
    ...
    final vertices = [
      ...
      Vec3(-r * sqrt(3.0) / 2.0, -r / 2.0,
          zFront), // 1: Bottom-Left (front triangle)
      Vec3(r * sqrt(3.0) / 2.0, -r / 2.0,
          zFront), // 2: Bottom-Right (front triangle)
  ...
  static PolyhedronMesh createD8({double radius = 62.0}) {
    ...
    final alignAngle =
        atan2(topVert.x - centerFace.x, topVert.y - centerFace.y);
  ...
  static PolyhedronMesh createD10({double radius = 70.0}) {
    ...
    final rawVerts = <Vec3>[
      ...
      for (int k = 0; k < 5; k++)
        Vec3(r * cos(k * deg72), r * sin(k * deg72), z1), // 2..6: Upper Belt
      for (int k = 0; k < 5; k++)
        Vec3(r * cos(deg36 + k * deg72), r * sin(deg36 + k * deg72),
            -z1), // 7..11: Lower Belt
    ];
  ...
  static PolyhedronMesh createD12({double radius = 68.0}) {
    ...
    final rawVertices = <Vec3>[
      ...
      Vec3(0, -invPhi * scale, -phi * scale),
      Vec3(0, invPhi * scale, -phi * scale),
      Vec3(0, -invPhi * scale, phi * scale),
      Vec3(0, invPhi * scale, phi * scale),

      // 4 vertices in XY plane (indices 12..15)
      Vec3(-invPhi * scale, -phi * scale, 0),
      Vec3(invPhi * scale, -phi * scale, 0),
      Vec3(-invPhi * scale, phi * scale, 0),
      Vec3(invPhi * scale, phi * scale, 0),

      // 4 vertices in XZ plane (indices 16..19)
      Vec3(-phi * scale, 0, -invPhi * scale),
      Vec3(phi * scale, 0, -invPhi * scale),
      Vec3(-phi * scale, 0, invPhi * scale),
      Vec3(phi * scale, 0, invPhi * scale),
    ];
    ...
    const rawFaces = [
      ...
      Polygon3D([10, 5, 19, 7, 11],
          faceNumber: 12), // 0: Top Face (Facing camera)
      Polygon3D([8, 0, 16, 2, 9],
          faceNumber: 1), // 1: Bottom Face (Opposite 12)
    ...
    var rotated =
        rawVertices.map((v) => v.rotateAxis(rotAxis, rotAngle)).toList();
