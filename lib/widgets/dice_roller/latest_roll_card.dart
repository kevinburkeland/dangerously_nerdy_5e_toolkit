...
class _LatestRollCardState extends State<LatestRollCard>
  ...
  void didUpdateWidget(covariant LatestRollCard oldWidget) {
    ...
    if (widget.latestResult != null &&
        widget.latestResult != oldWidget.latestResult) {
      _triggerFeedback(widget.latestResult!);
      _animController.forward(from: 0.0);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final diceBreakdown = res.groupResults
        .map((g) => '${g.entry.dieLabel}: ${g.rolls.join(", ")}')
        .join("; ");
    final droppedStr =
        (res.droppedRolls != null && res.droppedRolls!.isNotEmpty)
            ? '. Dropped dice: ${res.droppedRolls!.join(", ")}'
            : '';
    final modStr = res.modifier != 0
      ...
      child: Semantics(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
                color: borderColor,
                width: res.isCrit || res.isFumble ? 2.5 : 1.5),
            boxShadow: [
              BoxShadow(
                color: borderColor.withValues(
                    alpha: res.isCrit ? 0.35 : (res.isFumble ? 0.25 : 0.15)),
