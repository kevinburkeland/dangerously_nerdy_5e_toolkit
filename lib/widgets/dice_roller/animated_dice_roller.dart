...
class AnimatedDiceRollOverlay extends StatefulWidget {
  ...
  @override
  State<AnimatedDiceRollOverlay> createState() =>
      _AnimatedDiceRollOverlayState();
}

class _AnimatedDiceRollOverlayState extends State<AnimatedDiceRollOverlay>
  ...
  void initState() {
    ...
    for (final group in widget.result.groupResults) {
      for (final val in group.rolls) {
        ...
        count++;
        if (count >= 10)
          break; // Display up to 10 simultaneous 3D tumbling dice
      }
      if (count >= 10) break;
    }
              ...
              if (isSettled)
                Positioned(
                  ...
                  child: TweenAnimationBuilder<double>(
                    tween: Tween(
                        begin: systemDisableAnimations ? 1.0 : 0.0, end: 1.0),
                    duration: systemDisableAnimations
                        ? Duration.zero
...
class _Simulated3DDie {
  ...
  factory _Simulated3DDie.create({
    ...
    switch (dieType) {
      ...
      case DieType.d100:
        mesh = PolyhedronMesh.createD100(
            radius: isSingle ? 80.0 : (totalCount <= 3 ? 62.0 : 48.0));
      case DieType.d12:
    ...
    return _Simulated3DDie(
      ...
      targetX: targetX,
      targetY: targetY,
      rotSpeedX: isMedallion
          ? (8.0 + rng.nextDouble() * 4.0)
          : (5.0 + rng.nextDouble() * 5.0),
      rotSpeedY: isMedallion
          ? (1.5 + rng.nextDouble() * 2.0)
          : (6.0 + rng.nextDouble() * 6.0),
      rotSpeedZ: isMedallion
          ? (0.5 + rng.nextDouble() * 1.0)
          : (3.0 + rng.nextDouble() * 4.0),
      bounceHeight: isMedallion
          ? (280.0 + rng.nextDouble() * 40.0)
          : (240.0 + rng.nextDouble() * 60.0),
    );
  }
}
...
class _Polyhedral3DDicePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final die in dice) {
      ...
      final currentX = (die.startX +
              (die.targetX - die.startX) *
                  Curves.easeOutCubic.transform(progress)) *
          size.width;
      final linearY = (die.startY +
              (die.targetY - die.startY) *
                  Curves.easeOutCubic.transform(progress)) *
          size.height;
      ...
      if (!isSettled) {
        final bounce1 = sin(progress * pi * 3.5).abs() *
            die.bounceHeight *
            (1.0 - progress);
      ...
      final shadowScale =
          (1.0 - (bounceOffset.abs() / (die.bounceHeight + 1.0)))
              .clamp(0.3, 1.0);
      canvas.drawOval(
        Rect.fromCenter(
      ...
      final isSphere =
          die.dieType == DieType.d100 || die.dieType == DieType.custom;
      ...
      for (final faceData in visibleFaces) {
        ...
        final faceColor =
            Color.lerp(Colors.black, baseColor, faceData.diffuse * 0.9 + 0.1)!;
        ...
        _edgePaint.color = isWinningFace && isSettled && !isSphere
            ...
            : Colors.white.withValues(
                alpha: isSphere ? 0.20 : (0.35 + (faceData.diffuse * 0.45)));
        _edgePaint.strokeWidth = (isWinningFace && isSettled && !isSphere)
            ? 2.4
            : (isSphere ? 1.0 : 1.6);
        ...
        if (shouldDrawNumber) {
          ...
          final fontSize = isWinningFace
              ? (die.mesh.radius * 0.44)
              : (die.mesh.radius * 0.28);
      ...
      if (isSphere && isSettled) {
        ...
        final discPaint = Paint()
          ..color =
              Color.lerp(Colors.black, baseColor, 0.45)!.withValues(alpha: 0.95)
        ...
        final rimPaint = Paint()
          ..color =
              isCrit ? Colors.amberAccent : Colors.white.withValues(alpha: 0.90)
          ..style = PaintingStyle.stroke
          ..strokeWidth = 2.4;
        ...
        final fontSize = textStr.length >= 3
            ...
            : (textStr.length == 2
                ? (die.mesh.radius * 0.50)
                : (die.mesh.radius * 0.60));
