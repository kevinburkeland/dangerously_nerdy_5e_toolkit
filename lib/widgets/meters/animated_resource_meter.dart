...
class _AnimatedResourceMeterState extends State<AnimatedResourceMeter>
  ...
  void _syncAnimation() {
    final systemDisableAnimations =
        MediaQuery.maybeDisableAnimationsOf(context) ?? false;
    final performanceMode =
        SettingsScope.settingsOf(context, listen: false).performanceMode;
    final effectiveCurrent = widget.currentValue > widget.maxValue
        ? widget.maxValue
        : widget.currentValue;
  ...
  Widget build(BuildContext context) {
    ...
    final excessTemp = (widget.currentValue > widget.maxValue
    ...
    final effectiveCurrent = widget.currentValue > widget.maxValue
        ...
        : widget.currentValue;
    final ratio = effectiveCurrent.ratioOf(widget.maxValue);
    final totalTempRatio = ((effectiveCurrent + excessTemp) /
            (widget.maxValue <= 0 ? 1 : widget.maxValue))
        .clamp(0.0, 1.0);
    final effectiveTempColor = widget.tempResourceColor ??
        tabletop?.tempHpCyan ??
                  ...
                  AnimatedFractionallySizedBox(
                    ...
                    child: AnimatedBuilder(
                      ...
                      builder: (context, child) {
                        final opacity = (!performanceMode &&
                                !systemDisableAnimations &&
                                isCritical)
