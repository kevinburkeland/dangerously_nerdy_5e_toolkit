...
class _LevelUpWizardDialogState extends State<LevelUpWizardDialog>
  ...
  final TextEditingController _customCantripController =
    ...
    'fighter': [
      {
        'slug': 'champion',
        'name': 'Champion',
        'desc': 'Improved Critical & physical mastery'
      },
    ],
    'wizard': [
      {
        'slug': 'school_of_evocation',
        'name': 'School of Evocation',
        'desc': 'Sculpt Spells & destructive blasts'
      },
    ],
    'rogue': [
      {
        'slug': 'thief',
        'name': 'Thief',
        'desc': 'Fast Hands & Supreme Sneak mobility'
      },
    ],
    'cleric': [
      {
        'slug': 'life_domain',
        'name': 'Life Domain',
        'desc': 'Disciple of Life empowered healing'
      },
    ],
    'paladin': [
      {
        ...
        'desc': 'Sacred Weapon & Aura of Devotion'
      },
    ],
    'barbarian': [
      {
        ...
        'desc': 'Frenzy bonus attacks & Mindless Rage'
      },
    ],
    'bard': [
      {
        ...
        'desc': 'Cutting Words & Additional Magical Secrets'
      },
    ],
    'druid': [
      {
        'slug': 'circle_of_the_land',
        'name': 'Circle of the Land',
        'desc': 'Natural Recovery & biome bonus spells'
      },
    ],
    'monk': [
      {
        'slug': 'way_of_the_open_hand',
        'name': 'Way of the Open Hand',
        'desc': 'Flurry of Blows knockdowns & push techniques'
      },
    ],
    'ranger': [
      {
        'slug': 'hunter',
        'name': 'Hunter',
        'desc': 'Colossus Slayer & defensive tactics'
      },
    ],
    'sorcerer': [
      {
        ...
        'desc': 'Dragon ancestor armor, HP & elemental affinity'
      },
    ],
    'warlock': [
      {
        'slug': 'the_fiend',
        'name': 'The Fiend',
        'desc': 'Dark One\'s Blessing temp HP & Hellfire'
      },
    ],
  };
  ...
  List<Feat> get _availableFeats {
    final allFeats =
        SrdFeatsLibrary.getFeatsForRuleset(widget.character.id.ruleset);
    final existingFeatSlugs =
        widget.character.feats.map((f) => f.slug.toLowerCase()).toSet();
    return allFeats.where((f) {
      if (f.id.slug == 'elemental_adept' || f.id.slug == 'elemental-adept')
        return true;
  ...
  void initState() {
    ...
    final starting = widget.character.progression.startingClass ??
        (widget.character.progression.classes.isNotEmpty
            ? widget.character.progression.classes.first
            : null);
    ...
    if (feats.isNotEmpty) {
      final initialFeat = feats.firstWhere((f) => f.id.slug == 'tough',
          orElse: () => feats.first);
  ...
  Set<String> _getAllAlreadySelectedFeatureOptionIds(
      {String? excludeDecisionId}) {
    final ids = <String>{};
    final existing =
        widget.character.progression.getAllSelectedFeatureOptions();
    ...
    if (widget.character.customProperties.containsKey('featInvocationChoice')) {
      ids.add(widget.character.customProperties['featInvocationChoice']
          .toString()
          .toLowerCase()
          .trim());
    }
  ...
  bool _isOptionRepeatable(FeatureOption opt) {
    if (opt.customProperties['repeatable'] == true ||
        opt.grants['repeatable'] == true) {
      return true;
    }
    final desc = opt.descriptionMarkdown.toLowerCase();
    if (desc.contains('you can take this') ||
        (desc.contains('you can choose this') &&
            desc.contains('more than once'))) {
      return true;
    }
    return false;
  }
  ...
  List<FeatureOption> _getEligibleInvocations(Feat feat) {
    ...
    final isWarlock = character.progression.classes
            .any((c) => c.classRef.slug.toLowerCase() == 'warlock') ||
        _selectedClassSlug.toLowerCase() == 'warlock';
    final warlockClass = character.progression.classes
        .where((c) => c.classRef.slug.toLowerCase() == 'warlock')
        .firstOrNull;
    final warlockLevel = (warlockClass?.level ?? 0) +
        (_selectedClassSlug.toLowerCase() == 'warlock' ? 1 : 0);
    final selectedPacts = character.progression.classes
        .expand((c) => c.selectedFeatureOptions.values.expand((opts) => opts));
    final knownSpells = {
      ...character.spellsKnown.map((s) => s.slug),
      ..._newSpells
    };
    ...
    return SrdFeatureOptions.warlockInvocations.where((opt) {
      if (!_isOptionRepeatable(opt)) {
        ...
        if (alreadySelected.contains(optId) ||
            alreadySelected.contains(optName)) {
          return false;
        }
      }
  ...
  int get _hitDieSides =>
      CharacterProgressionEngine.getHitDieSides(_currentHitDie);

  int get _fixedAverageHp =>
      CharacterProgressionEngine.getAverageHpForHitDie(_currentHitDie);

  bool get _isAsiEligible => CharacterProgressionEngine.isAsiMilestone(
      _selectedClassSlug, _targetClassNewLevel);

  bool get _isSubclassMilestone {
    final existingSubclass = widget.character.progression.classes
        .where((c) =>
            c.classRef.slug.toLowerCase() == _selectedClassSlug.toLowerCase())
        .map((c) => c.subclassRef)
        .firstOrNull;
    final cls = SrdClassesLibrary.findBySlug(_selectedClassSlug,
  ...
  int get _conModifier {
    var con = widget.character.baseScores.constitution +
        widget.character.bonusScores.constitution;
    if (_isAsiEligible && _isAsiSelected) {
      if (_isAsiPlusTwo && _asiSingleAbility == AbilityType.constitution) {
        con += 2;
      } else if (!_isAsiPlusTwo &&
          (_asiDualAbility1 == AbilityType.constitution ||
              _asiDualAbility2 == AbilityType.constitution)) {
        con += 1;
      }
    }
  ...
  LevelUpRequest _buildRequest() {
    ...
    if (_isAsiEligible) {
      if (_isAsiSelected) {
        ...
        final feat = _availableFeats
                .where((f) => f.id.slug == _selectedFeatSlug)
                .firstOrNull ??
    ...
    if (_selectedSubclass != null) {
      final customCls = SrdClassesLibrary.findBySlug(_selectedClassSlug,
          ruleset: widget.character.id.ruleset);
      ...
      final sub = subs.firstWhere((s) => s['slug'] == _selectedSubclass,
          orElse: () =>
              {'slug': _selectedSubclass!, 'name': _selectedSubclass!});
      final resolvedSub = SrdClassesLibrary.findSubclass(sub['slug']!,
          classSlug: _selectedClassSlug);
      subclassRef = EntityReference<DomainEntity>(
        refType: EntityType.subclass,
        slug: sub['slug']!,
        displayName: sub['name']!,
        customProperties: resolvedSub != null
            ? {
                'featuresMarkdown': resolvedSub.featuresMarkdown,
                'classSlug': resolvedSub.classSlug,
                'shortName': resolvedSub.shortName,
                'grants': resolvedSub.grants.map((g) => g.toMap()).toList(),
                ...resolvedSub.customProperties,
              }
            : const {},
      );
    }
    ...
    final maxAllowedNewCantrips =
        math.max(0, limits.maxCantrips - curCantripsCount);
    ...
    final arcanumRef = _selectedMysticArcanumSpellId != null
        ? () {
            final s =
                SpellbookLibrary.getSpellById(_selectedMysticArcanumSpellId!);
            return EntityReference<Spell>(
              refType: EntityType.spell,
              slug: s?.id ??
                  _selectedMysticArcanumSpellId!
                      .toLowerCase()
                      .replaceAll(' ', '_'),
              displayName:
                  s?.getName(edition) ?? _selectedMysticArcanumSpellId!,
            );
          }()
        : null;
    ...
    return LevelUpRequest(
      ...
      hpChoice: _useFixedAverage
          ? const HpProgressionChoice.average()
          : HpProgressionChoice.rolled(_rolledHp),
      asiOrFeat: asiChoice,
      newCantrips: cantripRefs,
      newSpells: spellRefs,
      replacedSpellIds:
          _replacedSpellId != null ? [_replacedSpellId!] : const [],
      selectedFeatureOptions:
          Map<String, List<String>>.from(_selectedFeatureOptions),
  ...
  void _onConfirmLevelUp() {
    ...
    final updated =
        CharacterProgressionEngine.applyLevelUp(widget.character, request);
    HapticService.selectionTick(context);
  ...
  Widget build(BuildContext context) {
    ...
    final validation = CharacterProgressionEngine.validateMulticlass(
        widget.character, _selectedClassSlug);
          ...
          children: [
            ...
            Expanded(
              child: SingleChildScrollView(
                ...
                child:
                    _buildCurrentStepContent(theme, customColors, validation),
              ),
            ),
            const Divider(height: 1),
  ...
  Widget _buildCurrentStepContent(ThemeData theme, TabletopColors? customColors,
      LevelUpValidationResult validation) {
    switch (_currentStep) {
      case 0:
  ...
  // STEP 1: CLASS SELECTION
  // --------------------------------------------------------------------------
  Widget _buildStep1ClassSelection(ThemeData theme,
      TabletopColors? customColors, LevelUpValidationResult validation) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
        ...
        DropdownButtonFormField<String>(
          ...
          decoration: InputDecoration(
            ...
            prefixIcon: Padding(
              ...
              child: DndGlyph.classFeature(
                classType: DndClassType.tryParse(_selectedClassSlug) ??
          ...
          items: _availableClasses.map((cls) {
            final isCurrent = widget.character.progression.classes
                .any((c) => c.classRef.slug.toLowerCase() == cls['slug']);
            final label =
                '${cls['name']} (${cls['hitDie']}) ${isCurrent ? "— Current" : "— Multiclass (Req: ${cls['prereq']})"}';
            final classType =
                DndClassType.tryParse(cls['slug']) ?? DndClassType.fighter;
          ...
          onChanged: (val) {
            if (val != null) {
              setState(() {
                ...
                _isMulticlass = !widget.character.progression.classes
                    .any((c) => c.classRef.slug.toLowerCase() == val);
          ...
          segments: [
            ...
            ButtonSegment(
              ...
              icon: DndGlyph.genericUi(
                uiType: GenericUiGlyphType.fromDie(_currentHitDie) ??
                    GenericUiGlyphType.d20,
                size: 16,
                isDarkMode: theme.brightness == Brightness.dark,
              ),
            ...
            children: [
              ...
              Row(
                children: [
                  Expanded(
                      child: Text(
                          'Hit Die Base (${_useFixedAverage ? "Average" : "Rolled"}):')),
                  Text('+${_useFixedAverage ? _fixedAverageHp : _rolledHp}',
              ...
              Row(
                children: [
                  ...
                  Text(_conModifier >= 0 ? "+$_conModifier" : "$_conModifier",
                      style: const TextStyle(fontWeight: FontWeight.bold)),
                ],
              ),
              ...
              Row(
                children: [
                  const Expanded(
                      child: Text('Total HP Gain this Level:',
  ...
  Set<String> _getAllSelectedPacts() {
    ...
    final pactRegex = RegExp(r'pact[-_ ](?:of[-_ ]the[-_ ])?([a-z0-9]+)',
        caseSensitive: false);
    void extract(Iterable<String> ids) {
      for (final id in ids) {
  ...
  // --------------------------------------------------------------------------
  Widget _buildStep3SubclassAndFeatures(
      ThemeData theme, TabletopColors? customColors) {
    final customCls = SrdClassesLibrary.findBySlug(_selectedClassSlug,
        ruleset: widget.character.id.ruleset);
    ...
    final decisions = customCls?.getDecisionsForLevel(_targetClassNewLevel,
            ruleset: widget.character.id.ruleset) ??
        [];
      ...
      children: [
              ...
              children: [
                Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    const Expanded(
                        child: Text('Subclass Archetype Milestone Unlocked!',
                            style: TextStyle(fontWeight: FontWeight.bold))),
                  ],
                ),
                const SizedBox(height: 6),
                const Text('Choose your Subclass archetype specialization:',
                  ...
                  Wrap(
                    ...
                    children: eligibleOptions.map((opt) {
                      ...
                      return Tooltip(
                        message: isGated
                            ? 'Locked: ${eval.summary}'
                            : (opt.descriptionMarkdown.isNotEmpty
                          ...
                          onSelected: (bool chosen) {
                            ...
                            setState(() {
                              final list = List<String>.from(
                                  _selectedFeatureOptions[decision.id] ?? []);
                              if (chosen) {
        ...
        Container(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Gains at Level $_targetClassNewLevel:',
                  style: const TextStyle(fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              Text('• Hit Die Pool: +1 $_currentHitDie'),
              Text(
                  '• Proficiency Bonus: +${_newTotalLevel.dndProficiencyBonus}'),
              if (_isAsiEligible)
                const Text('• Ability Score Improvement / Feat Eligible'),
              if (needsSubclass && _selectedSubclass != null)
                Text('• Specialization: $_selectedSubclass'),
              if (_selectedFeatureOptions.isNotEmpty)
                Text(
                    '• Chosen Specializations: ${_selectedFeatureOptions.values.expand((v) => v).join(', ')}'),
            ],
          ),
        ),
      ],
    ...
    setState(() {
      ...
      if (match.hasAbilityScoreIncrease) {
        if (_selectedFeatAbility == null ||
            !match.selectableAbilities.contains(_selectedFeatAbility)) {
  ...
  Widget _buildStep4AsiOrFeat(ThemeData theme, TabletopColors? customColors) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Ability Score Improvement (ASI) or Feat',
            style: theme.textTheme.titleSmall
                ?.copyWith(fontWeight: FontWeight.bold)),
        const SizedBox(height: 4),
        Text(
            'Milestone reached! Choose between increasing ability scores or gaining a new Feat.',
            style: theme.textTheme.bodySmall
                ?.copyWith(color: theme.colorScheme.onSurfaceVariant)),
        const SizedBox(height: 16),
        SegmentedButton<bool>(
          segments: const [
            ButtonSegment(
                value: true, label: Text('Ability Score Improvement')),
            ButtonSegment(value: false, label: Text('Choose Feat')),
          ],
          selected: {_isAsiSelected},
          onSelectionChanged: (set) =>
              setState(() => _isAsiSelected = set.first),
        ),
        const SizedBox(height: 16),
        if (_isAsiSelected) ...[
          SegmentedButton<bool>(
            ...
            onSelectionChanged: (set) =>
                setState(() => _isAsiPlusTwo = set.first),
          ),
          const SizedBox(height: 16),
          if (_isAsiPlusTwo) ...[
            DropdownButtonFormField<AbilityType>(
              ...
              decoration: const InputDecoration(
                  labelText: '+2 Ability Score', border: OutlineInputBorder()),
              items: AbilityType.values.map((a) {
                ...
                return DropdownMenuItem(
                  value: a,
                  child: Text(
                      '${a.name.toUpperCase()} (Current: $cur, Max: $max)'),
                );
              }).toList(),
              onChanged: (v) => setState(() => _asiSingleAbility = v!),
            ),
            if (widget.character.rawAbilityScores.getScore(_asiSingleAbility) +
                    2 >
              ...
              children: [
                Expanded(
                  child: DropdownButtonFormField<AbilityType>(
                    ...
                    decoration: const InputDecoration(
                        labelText: '+1 Score 1', border: OutlineInputBorder()),
                    items: AbilityType.values.map((a) {
                      final cur = widget.character.rawAbilityScores.getScore(a);
                      final max = widget.character.getAbilityScoreMaximum(a);
                      return DropdownMenuItem(
                ...
                Expanded(
                  child: DropdownButtonFormField<AbilityType>(
                    ...
                    decoration: const InputDecoration(
                        labelText: '+1 Score 2', border: OutlineInputBorder()),
                    items: AbilityType.values.map((a) {
                      final cur = widget.character.rawAbilityScores.getScore(a);
                      final max = widget.character.getAbilityScoreMaximum(a);
                      return DropdownMenuItem(
          ...
          TextField(
            ...
            onChanged: (v) => setState(() {
              ...
              if (matches.isNotEmpty &&
                  !matches.any((f) => f.id.slug == _selectedFeatSlug)) {
          ...
          () {
            ...
            final currentSelectionInList =
                filteredFeats.any((f) => f.id.slug == _selectedFeatSlug);
            final activeFeatSlug = currentSelectionInList
                ? _selectedFeatSlug
                : filteredFeats.first.id.slug;

            return DropdownButtonFormField<String>(
              key: ValueKey(
                  'feat_dropdown_${filteredFeats.length}_$activeFeatSlug'),
              isExpanded: true,
              initialValue: activeFeatSlug,
              decoration: InputDecoration(
                labelText: _featSearchQuery.isNotEmpty
                    ? 'Select Feat (${filteredFeats.length} matching)'
                    : 'Select Feat',
                border: const OutlineInputBorder(),
                prefixIcon: Padding(
                  padding: const EdgeInsets.all(10.0),
                  child: DndGlyph.feat(
                    category: FeatCategory.parse(
                      filteredFeats
                          .firstWhere((f) => f.id.slug == activeFeatSlug,
              ...
              items: filteredFeats.map((f) {
                final prereq =
                    f.prerequisite != null ? ' (${f.prerequisite})' : '';
                return DropdownMenuItem(
                  value: f.id.slug,
                  child: Row(
                    children: [
              ...
              children: [
                  ...
                  if (feat.requiresAbilityChoice) ...[
                    ...
                    Wrap(
                      ...
                      children: feat.selectableAbilities.map((ability) {
                        final isSelected = (_selectedFeatAbility ??
                                feat.selectableAbilities.first) ==
                            ability;
                        return ChoiceChip(
                          key: Key('feat_ability_chip_${ability.name}'),
                          label: Text(
                              '${ability.shortName} (+${feat.statIncreaseAmount})'),
                          selected: isSelected,
                          selectedColor:
                  ...
                  if (feat.grantsSavingThrowProficiency) ...[
                    ...
                    () {
                      final chosenSave = _selectedFeatAbility ??
                          feat.selectableAbilities.first;
                      final alreadyProficient = widget
                ...
                if (feat.hasSkillProficiencyChoice) ...[
                  ...
                  DropdownButtonFormField<SkillType>(
                    key: ValueKey(
                        'levelup_feat_skill_${_selectedFeatSkill}_$_selectedFeatSlug'),
                        ...
                        setState(() {
                          ...
                          if (feat.hasExpertiseChoice) {
                            final eligible = <SkillType>{
                              ...widget.character.skillProficiencies.entries
                                  .where((e) =>
                                      e.value != SkillProficiencyLevel.none)
                                  .map((e) => e.key),
                              s,
                            };
                            if (_selectedFeatExpertise == null ||
                  ...
                  () {
                    ...
                    final effectiveVal =
                        eligible.contains(_selectedFeatExpertise)
                            ? _selectedFeatExpertise
                            : (_selectedFeatSkill != null &&
                                    eligible.contains(_selectedFeatSkill)
                                ? _selectedFeatSkill
                                : eligible.first);

                    return DropdownButtonFormField<SkillType>(
                      key: ValueKey(
                          'levelup_feat_expertise_${effectiveVal}_$_selectedFeatSkill'),
                ...
                if (feat.hasInvocationChoice) ...[
                  const SizedBox(height: 12),
                  () {
                    final isWarlock = widget.character.progression.classes.any(
                            (c) =>
                                c.classRef.slug.toLowerCase() == 'warlock') ||
                        _selectedClassSlug.toLowerCase() == 'warlock';
                    ...
                    final selectedOpt =
                        eligible.where((o) => o.id == effectiveVal).firstOrNull;
                    ...
                    return Column(
                      ...
                      children: [
                        ...
                        if (selectedOpt != null &&
                            selectedOpt.descriptionMarkdown.isNotEmpty) ...[
                          const SizedBox(height: 6),
                          Container(
                            padding: const EdgeInsets.all(8),
                            decoration: BoxDecoration(
                ...
                if (feat.hasFightingStyleChoice) ...[
                  ...
                  () {
                    final alreadySelected =
                        _getAllAlreadySelectedFeatureOptionIds();
                    final eligibleStyles =
                        SrdFeatureOptions.fightingStyles.where((s) {
                      final sId = s.id.toLowerCase().trim();
                      final sName = s.name.toLowerCase().trim();
                      return !alreadySelected.contains(sId) &&
                          !alreadySelected.contains(sName);
                    }).toList();
                    final styles = eligibleStyles.isNotEmpty
                        ? eligibleStyles
                        : SrdFeatureOptions.fightingStyles;
                    final effectiveVal =
                        styles.any((o) => o.id == _selectedFeatOption)
                            ? _selectedFeatOption
                            : (styles.isNotEmpty ? styles.first.id : null);
                    final selectedOpt =
                        styles.where((o) => o.id == effectiveVal).firstOrNull;
                    ...
                    return Column(
                      ...
                      children: [
                        ...
                        DropdownButtonFormField<String>(
                          key:
                              const Key('levelup_feat_fighting_style_dropdown'),
                          initialValue: effectiveVal,
                          decoration: InputDecoration(
                            isDense: true,
                            filled: true,
  ...
  int _getCastingModifier(String classSlug) {
    ...
    var score = widget.character.baseScores.getScore(ability) +
        widget.character.bonusScores.getScore(ability);
    if (_isAsiEligible && _isAsiSelected) {
      if (_isAsiPlusTwo && _asiSingleAbility == ability) {
        score += 2;
      } else if (!_isAsiPlusTwo &&
          (_asiDualAbility1 == ability || _asiDualAbility2 == ability)) {
        score += 1;
      }
    }
  ...
  int get _maxAccessibleSpellLevel {
    ...
    if (effSub.contains('eldritch_knight') ||
  ...
  Widget _buildStep5Spells(ThemeData theme, TabletopColors? customColors) {
    ...
    if (edition == DmRulesEdition.v2014) {
      final isLore =
          _effectiveSubclassSlug?.toLowerCase().contains('lore') == true;
      final isLore6 = isLore && _targetClassNewLevel == 6;
      final isStandardSecrets = _selectedClassSlug.toLowerCase() == 'bard' &&
          (_targetClassNewLevel == 10 ||
              _targetClassNewLevel == 14 ||
              _targetClassNewLevel == 18);
      isMagicalSecretsActive = isLore6 || isStandardSecrets;
    } else {
      isMagicalSecretsActive = limits.magicalSecretsCount > 0 ||
          limits.allowedMagicalSecretClasses.isNotEmpty;
    }
    ...
    final allClassSpells = SpellbookLibrary.allSpells.where((s) {
      ...
      final isExpanded = SubclassSpellsLibrary.isExpandedSpell(
          _selectedClassSlug, _effectiveSubclassSlug, s, edition);
    ...
    final availableCantrips = filtered.where((s) {
      ...
      return !existingCantripSlugs.contains(id) &&
          !existingCantripSlugs.contains(name);
    }).toList();

    final existingKnownOrPrepared = <String>{};
    for (final s in widget.character.spellsKnown) {
      if (_replacedSpellId != null &&
          (_replacedSpellId == s.slug || _replacedSpellId == s.displayName)) {
        continue;
      }
    ...
    final availableLeveled = filtered.where((s) {
      ...
      final id = s.id.toLowerCase().trim();
      final name = s.getName(edition).toLowerCase().trim();
      return !existingKnownOrPrepared.contains(id) &&
          !existingKnownOrPrepared.contains(name);
    }).toList();
    ...
    final isMysticArcanumMilestone =
        _selectedClassSlug.toLowerCase() == 'warlock' &&
    ...
    final availableMysticArcanumSpells = isMysticArcanumMilestone
        ? SpellbookLibrary.allSpells.where((s) {
            ...
            final id = s.id.toLowerCase().trim();
            final name = s.getName(edition).toLowerCase().trim();
            if (existingKnownOrPrepared.contains(id) ||
                existingKnownOrPrepared.contains(name)) {
              return false;
            }
            final rules = s.getRules(edition);
    ...
    final gainedCantripsProgression =
        math.max(0, limits.maxCantrips - previousLimits.maxCantrips);
    final gainedSpellsProgression =
        math.max(0, limits.maxSpellsKnown - previousLimits.maxSpellsKnown);

    final curCantripsCount = widget.character.cantrips.length;
    final maxAllowedNewCantrips = math.max(
        gainedCantripsProgression, limits.maxCantrips - curCantripsCount);

    final effSub = _effectiveSubclassSlug?.toLowerCase() ?? '';
    final isWizard = _selectedClassSlug.toLowerCase() == 'wizard';
    final isSpontaneous = ['sorcerer', 'bard', 'warlock']
            .contains(_selectedClassSlug.toLowerCase()) ||
        (_selectedClassSlug.toLowerCase() == 'ranger' &&
    ...
    if (isWizard) {
      ...
      maxAllowedNewSpells =
          math.max(1, delta + (_replacedSpellId != null ? 1 : 0));
    }
    ...
    final replaceableSpells = baseReplaceable.where((s) {
      final spellItem = SpellbookLibrary.getSpellById(s.slug) ??
          SpellbookLibrary.findSpell(s.displayName);
      if (spellItem != null &&
          _selectedClassSlug.toLowerCase() == 'warlock' &&
          spellItem.level >= 6) {
        return false;
      ...
      children: [
                  ...
                  children: alwaysPreparedSpells.map((s) {
                    return ActionChip(
                      ...
                      label: Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          Text('${s.getName(edition)} (L${s.level})',
                ...
                Row(
                  children: [
                    ...
                    if (_replacedSpellId != null)
                      TextButton(
                        onPressed: () =>
                            setState(() => _replacedSpellId = null),
                ...
                if (isMysticArcanumMilestone) ...[
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      Text(
                        _selectedMysticArcanumSpellId != null
                            ? '1/1 Arcanum Selected'
                            : '0/1 Arcanum Selected',
                        style: TextStyle(
                          fontSize: 11,
                          fontWeight: FontWeight.bold,
                          color: _selectedMysticArcanumSpellId != null
                              ? Colors.amberAccent
                              : Colors.white60,
                        ),
                      ),
                  ...
                  if (filteredMysticArcanumSpells.isEmpty)
                    ...
                    Wrap(
                      ...
                      children: filteredMysticArcanumSpells.map((s) {
                        final isSelected =
                            _selectedMysticArcanumSpellId == s.id;
                        return ChoiceChip(
                          avatar: DndGlyph.spell(
                            school: s.school,
                            level: s.level,
        ...
        if (isMagicalSecretsActive) ...[
          SingleChildScrollView(
            ...
            child: Row(
              children: [
                const Text('Spell List: ',
                    style:
                        TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                ChoiceChip(
                  label: const Text('Bard List'),
                  selected: _selectedSpellListSource == null &&
                      !_showAllSpellListsForSecrets,
                  onSelected: (selected) {
                    setState(() {
                ...
                const SizedBox(width: 6),
                if (limits.allowedMagicalSecretClasses
                    .contains(SpellClass.wizard))
                  ChoiceChip(
                    label: const Text('Wizard'),
                    selected: _selectedSpellListSource == SpellClass.wizard,
                    onSelected: (selected) {
                      setState(() {
                        _selectedSpellListSource =
                            selected ? SpellClass.wizard : null;
                        _showAllSpellListsForSecrets = false;
                      });
                    },
                  ),
                ...
                if (limits.allowedMagicalSecretClasses
                  ...
                  ChoiceChip(
                    ...
                    onSelected: (selected) {
                      setState(() {
                        _selectedSpellListSource =
                            selected ? SpellClass.cleric : null;
                        _showAllSpellListsForSecrets = false;
                      });
                    },
                  ),
                ...
                if (limits.allowedMagicalSecretClasses
                  ...
                  ChoiceChip(
                    ...
                    onSelected: (selected) {
                      setState(() {
                        _selectedSpellListSource =
                            selected ? SpellClass.druid : null;
                        _showAllSpellListsForSecrets = false;
                      });
                    },
                  ),
                const SizedBox(width: 6),
                if (limits.allowedMagicalSecretClasses.length > 4 ||
                    edition == DmRulesEdition.v2014)
                  ChoiceChip(
              ...
              children: [
                ...
                Wrap(
                  ...
                  children: [
                    if (_selectedMysticArcanumSpellId != null)
                      () {
                        final spell = SpellbookLibrary.getSpellById(
                            _selectedMysticArcanumSpellId!);
                        return InputChip(
                          avatar: spell != null
                              ? DndGlyph.spell(
                          ...
                          label: Row(
                            ...
                            children: [
                              Text(
                                  'Mystic Arcanum: ${spell?.getName(edition) ?? _selectedMysticArcanumSpellId}'),
                              if (spell != null) ...[
                                const SizedBox(width: 4),
                                const Icon(Icons.info_outline,
                                    size: 12, color: Colors.amberAccent),
                    ...
                    ..._newSpells.map((s) {
                      ...
                      return InputChip(
                        ...
                        label: Row(
                          ...
                          children: [
                            Text(
                                '${spell?.levelLabel ?? "Spell"}: ${spell?.getName(edition) ?? s}'),
                            if (spell != null) ...[
                              const SizedBox(width: 4),
                              const Icon(Icons.info_outline,
                        ...
                        backgroundColor:
                            Colors.cyan.shade800.withValues(alpha: 0.5),
        ...
        if (availableLeveled.isNotEmpty) ...[
          ...
          ...(() {
            ...
            return sortedLevels.map((lvl) {
              ...
              final selectedAtThisLevel =
                  levelSpells.where((s) => _newSpells.contains(s.id)).length;
  ...
  Widget _buildStep6Summary(ThemeData theme, TabletopColors? customColors) {
    ...
    final updatedChar = CharacterProgressionEngine.applyLevelUp(
        widget.character, candidateRequest);
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Level-Up Summary & Diff Preview',
            style: theme.textTheme.titleSmall
                ?.copyWith(fontWeight: FontWeight.bold)),
        const SizedBox(height: 4),
        Text('Review changes before applying to your character sheet.',
            style: theme.textTheme.bodySmall
                ?.copyWith(color: theme.colorScheme.onSurfaceVariant)),
        const SizedBox(height: 16),
        ...
        Container(
          ...
          child: Column(
            children: [
              _buildDiffRow('Total Level', '${widget.character.totalLevel}',
                  '$_newTotalLevel', theme),
              _buildDiffRow(
                  'Class Progression',
              ...
              _buildDiffRow(
                  ...
                  '${updatedChar.resources.currentHitDice[_currentHitDie] ?? 0} $_currentHitDie',
                  theme),
              _buildDiffRow(
                  'Maximum HP',
                  '${oldStats.maxHp}',
                  '${newStats.maxHp} (+${newStats.maxHp - oldStats.maxHp})',
                  theme,
                  highlightNew: true),
              _buildDiffRow(
                  'Proficiency Bonus',
                  '+${oldStats.proficiencyBonus}',
                  '+${newStats.proficiencyBonus}',
                  theme),
              if (_isAsiEligible && _isAsiSelected)
                _buildDiffRow(
                    'ASI Bonus',
                    'None',
                    _isAsiPlusTwo
                        ? '+2 ${_asiSingleAbility.name}'
                        : '+1 ${_asiDualAbility1.name}, +1 ${_asiDualAbility2.name}',
                    theme,
                    highlightNew: true),
              if (_isAsiEligible && !_isAsiSelected) ...[
                _buildDiffRow('Feat Gained', 'None', _selectedFeatName, theme,
                    highlightNew: true),
                () {
                  final feat = _availableFeats
                          .where((f) => f.id.slug == _selectedFeatSlug)
                    ...
                    return Column(
                      children: [
                        _buildDiffRow(
                            'Feat Ability',
                            'None',
                            '+${feat.statIncreaseAmount} ${chosenAbility.shortName}',
                            theme,
                            highlightNew: true),
                        if (feat.grantsSavingThrowProficiency)
                          _buildDiffRow(
                              'Feat Saving Throw',
                              'None',
                              '${chosenAbility.shortName} Save Proficiency',
                              theme,
                              highlightNew: true),
                        if (feat.hasSkillProficiencyChoice &&
                            _selectedFeatSkill != null)
                          _buildDiffRow(
                              'Feat Skill',
                              'None',
                              '${_selectedFeatSkill!.displayName} Proficiency',
                              theme,
                              highlightNew: true),
                        if (feat.hasExpertiseChoice &&
                            _selectedFeatExpertise != null)
              ...
              if (_selectedSubclass != null)
                _buildDiffRow('Subclass', 'None', _selectedSubclass!, theme,
                    highlightNew: true),
              if (_selectedMysticArcanumSpellId != null)
                () {
                  final spell = SpellbookLibrary.getSpellById(
                      _selectedMysticArcanumSpellId!);
                  final name = spell?.getName(
                          widget.character.id.ruleset == RulesetVersion.v2024
                              ? DmRulesEdition.v2024
                              : DmRulesEdition.v2014) ??
                      _selectedMysticArcanumSpellId!;
                  return _buildDiffRow('Mystic Arcanum', 'None', name, theme,
                      highlightNew: true);
                }(),
            ],
          ),
              ...
              if (_newLanguages.isEmpty && _newToolProficiencies.isEmpty)
                ...
                if (_newLanguages.isNotEmpty) ...[
                  ...
                  Wrap(
                    spacing: 6,
                    runSpacing: 4,
                    children: _newLanguages
                        .map((l) => Chip(
                              label:
                                  Text(l, style: const TextStyle(fontSize: 11)),
                              deleteIcon: const Icon(Icons.close, size: 12),
                              onDeleted: () =>
                                  setState(() => _newLanguages.remove(l)),
                            ))
                        .toList(),
                  ),
                  const SizedBox(height: 6),
                ...
                if (_newToolProficiencies.isNotEmpty) ...[
                  ...
                  Wrap(
                    spacing: 6,
                    runSpacing: 4,
                    children: _newToolProficiencies
                        .map((t) => Chip(
                              label:
                                  Text(t, style: const TextStyle(fontSize: 11)),
                              deleteIcon: const Icon(Icons.close, size: 12),
                              onDeleted: () => setState(
                                  () => _newToolProficiencies.remove(t)),
                            ))
                        .toList(),
                  ),
                ],
  ...
  Widget _buildDiffRow(
      String label, String oldVal, String newVal, ThemeData theme,
      ...
      child: Row(
        ...
        children: [
          ...
          if (!isLastStep)
            ...
            FilledButton.icon(
              style:
                  FilledButton.styleFrom(backgroundColor: Colors.purpleAccent),
              onPressed: canAdvance ? _onConfirmLevelUp : null,
              icon: const Icon(Icons.check),
              label: const Text('CONFIRM LEVEL UP',
                  style: TextStyle(fontWeight: FontWeight.bold)),
            ),
        ],
      ),
