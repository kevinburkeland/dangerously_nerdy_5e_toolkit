...
class AbilityScoreStep extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final hasFlexibleLineageBonus = is2014 &&
        flexibleCount > 0 &&
        (curSpecies != null || curSubrace != null);
            ...
            SegmentedButton<String>(
              segments: const [
                ButtonSegment(
                  value: 'standard',
                  label:
                      Text('Standard Array', style: TextStyle(fontSize: 10.5)),
                ),
                ButtonSegment(
                  value: 'pointBuy',
  ...
  Widget _buildAllocationStatusBanner(BuildContext context) {
    ...
    if (mode == 'standard' ||
        (mode == 'rolled' && controller.rollMethod != 'classic_3d6_down')) {
    ...
    return Column(
      ...
      children: [
            ...
            children: [
              ...
              DropdownButtonFormField<String>(
                ...
                items: const [
                  DropdownMenuItem(
                      value: 'standard_4d6',
                      child: Text('Standard Roll (4d6 drop lowest)')),
                  DropdownMenuItem(
                      value: 'classic_3d6_down',
                      child: Text('Old School (3d6 down the line)')),
                  DropdownMenuItem(
                      value: 'classic_3d6_nice',
                      child: Text('Old School (Nice) (3d6, assignable)')),
                  DropdownMenuItem(
                      value: 'silly_d20',
                      child: Text('Silly, don\'t use this (1d20 per stat)')),
                ],
                onChanged: (v) {
                  if (v == null) return;
                  HapticService.selectionTick(context);
                    ...
                    children: controller.rolledBreakdowns
                        .map((b) => Text(b,
                            style: const TextStyle(
                                fontSize: 11, color: Colors.purpleAccent)))
                        .toList(),
                  ),
                ),
              ],
        ...
        if (controller.rollMethod == 'classic_3d6_down') ...[
          ...
          ...AbilityType.values.map((ab) {
            ...
            return Padding(
              ...
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('${ab.name.toUpperCase()} ($modStr)',
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(6),
                      border:
                          Border.all(color: Colors.purpleAccent, width: 0.8),
                    ),
                    child: Text('$score',
                        style: const TextStyle(
                            fontSize: 16,
          ...
          const SizedBox(height: 14),
          ...AbilityType.values
              .map((ab) => _buildConsumableSlotRow(context, ab)),
        ],
      ],
    );
  }
          ...
          children: [
            if (onAutoAssign != null)
              TextButton.icon(
                ...
                label:
                    const Text('Auto-Assign', style: TextStyle(fontSize: 11)),
        ...
        Wrap(
          ...
          children: controller.availableScores.map((score) {
            ...
            return Semantics(
              button: true,
              label:
                  'Available score $score${isSelected ? ', selected' : ', tap to select'}',
              child: InkWell(
                onTap: () {
                  HapticService.selectionTick(context);
              ...
              Expanded(
                ...
                child: Semantics(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: Text('Assign $selectedPool',
                        style: const TextStyle(
                            fontWeight: FontWeight.bold, fontSize: 12)),
                    onPressed: () {
                      HapticService.selectionTick(context);
                      A11yService.announce(
                          'Assigned $selectedPool to ${ab.name}');
                  ...
                  child: DropdownButton<int?>(
                    ...
                    items: [
                      ...
                      ...dropdownValues.map((val) {
                        ...
                        final itemModStr =
                            itemMod >= 0 ? '+$itemMod' : '$itemMod';
                        return DropdownMenuItem<int?>(
                          value: val,
                          child: Text('$val ($itemModStr)',
                              style: const TextStyle(
                ...
                children: [
                  ...
                  IconButton(
                    ...
                    constraints:
                        const BoxConstraints(minWidth: 48, minHeight: 48),
                    onPressed:
                        score < 15 && controller.pointBuyPointsRemaining > 0
                            ? () {
                                HapticService.selectionTick(context);
                                controller.adjustPointBuy(ab, 1);
                              }
            ...
            Wrap(
              ...
              children: (() {
                ...
                return availableAbilities.map((ab) {
                  ...
                  final isFixed =
                      fixedBonuses.containsKey(ab.name.toLowerCase());
                  ...
                  return FilterChip(
                    ...
                    onSelected: isFixed
                        ...
                        : (selected) {
                            HapticService.selectionTick(context);
                            final updated =
                                Set<AbilityType>.from(variantHumanBonuses);
                            if (selected) {
                              while (updated.length >= flexibleCount &&
                                  updated.isNotEmpty) {
                            ...
                            controller
                                .setFlexibleAbilityChoices(updated.toList());
                          },
                  );
                }).toList();
              })(),
                ...
                Expanded(
                  child: DropdownButtonFormField<AbilityType>(
                    ...
                    items: AbilityType.values
                        .map((ab) => DropdownMenuItem(
                            value: ab,
                            child: Text('${ab.name.toUpperCase()} (+2)')))
                        .toList(),
                    onChanged: (v) {
                      if (v == null) return;
                      HapticService.selectionTick(context);
                      ...
                      if (backgroundSecondaryBonus == v) {
                        onBackgroundSecondaryBonusChanged(
                            AbilityType.values.firstWhere((a) => a != v));
                      }
                    },
                  ),
                ),
                ...
                Expanded(
                  child: DropdownButtonFormField<AbilityType>(
                    ...
                    items: AbilityType.values
                        .map((ab) => DropdownMenuItem(
                            value: ab,
                            child: Text('${ab.name.toUpperCase()} (+1)')))
                        .toList(),
                    onChanged: (v) {
                      if (v == null) return;
                      HapticService.selectionTick(context);
                      ...
                      if (backgroundPrimaryBonus == v) {
                        onBackgroundPrimaryBonusChanged(
                            AbilityType.values.firstWhere((a) => a != v));
                      }
                    },
                  ),
  ...
  Widget _buildSpeciesRacialBonusCard(BuildContext context) {
    ...
    if (curSubrace != null &&
        (curSubrace!.abilityScoreSummary?.isNotEmpty == true)) {
                  ...
                  children: [
                    ...
                    if (bonus > 0)
                      Text(
                        '+$bonus bonus',
                        style: const TextStyle(
                            fontSize: 8.5, color: Colors.amberAccent),
                      ),
                  ],
                );
              }).toList(),
