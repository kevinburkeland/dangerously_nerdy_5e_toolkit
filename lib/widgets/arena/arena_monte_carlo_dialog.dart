          ...
          child: Column(
            ...
            children: [
              ...
              if (_isCalculating || res == null) ...[
                ...
                const SizedBox(height: 16),
                const Center(
                    child: Text('Simulating high-speed combat rounds...')),
                const SizedBox(height: 60),
              ] else ...[
                // Win Probability Bar
                Column(
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      _buildStatRow(
                          'Total Iterations', '${res.iterations} fights'),
                      _buildStatRow('Average Combat Length',
                          '${res.averageRounds.toStringAsFixed(1)} rounds (Min: ${res.minRounds}, Max: ${res.maxRounds})'),
                      _buildStatRow('Team Crimson Wins',
                          '${res.teamAWins} (${res.teamAWinRate.toStringAsFixed(1)}%)'),
                      _buildStatRow('Team Cobalt Wins',
                          '${res.teamBWins} (${res.teamBWinRate.toStringAsFixed(1)}%)'),
                      if (res.draws > 0)
                        _buildStatRow('Draws / Round Caps',
                            '${res.draws} (${res.drawRate.toStringAsFixed(1)}%)'),
                      _buildStatRow('Avg Surviving HP %',
                          'Crimson: ${res.avgTeamASurvivingHpPercent.toStringAsFixed(1)}% • Cobalt: ${res.avgTeamBSurvivingHpPercent.toStringAsFixed(1)}%'),
                      _buildStatRow('Compute Duration',
                          '${res.calculationDuration.inMilliseconds} ms'),
                    ],
                  ),
                ),
              ],
