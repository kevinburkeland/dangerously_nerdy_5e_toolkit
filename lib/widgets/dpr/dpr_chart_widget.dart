...
class _DprChartWidgetState extends State<DprChartWidget>
  ...
  Widget build(BuildContext context) {
    ...
    final tabletop = theme.extension<TabletopColors>() ??
        (isDark
            ? TabletopColors.dark
            : TabletopColors.createLight(FantasyAccent.paladinGold));
    ...
    if (widget.chartMode == DprChartMode.accuracy) {
      ...
      yMax =
          (math.max(8.0, (maxCritDamage * 1.08) / 5.0).ceil() * 5).toDouble();
    } else {
      double maxDpr = 0.0;
      for (int ac = minAc; ac <= maxAc; ac++) {
    ...
    final powerPt = widget.showPowerAttack
        ? widget.powerAttackCurve?.pointAt(activeAc)
        : null;
    final advPt =
        widget.showAdvantage ? widget.advantageCurve?.pointAt(activeAc) : null;
    final disPt = widget.showDisadvantage
        ? widget.disadvantageCurve?.pointAt(activeAc)
        : null;
    ...
    final powerColor =
        isDark ? const Color(0xFFFFB300) : const Color(0xFFD97706);
    ...
    final critColor =
        isDark ? const Color(0xFFFFD700) : const Color(0xFFB45309);
    final missColor =
        isDark ? const Color(0xFFB388FF) : const Color(0xFF7C3AED);
          ...
          child: LayoutBuilder(
            builder: (context, constraints) {
              return MouseRegion(
                ...
                onHover: (event) => _handleDrag(
                    event.localPosition, constraints.biggest, minAc, maxAc),
                onExit: (_) {
                  setState(() {
                    _hoverAc = null;
                  });
                ...
                child: GestureDetector(
                  onHorizontalDragStart: (details) => _handleDrag(
                      details.localPosition, constraints.biggest, minAc, maxAc),
                  ...
                  child: AnimatedBuilder(
                    animation:
                        Listenable.merge([_curveAnimation, _pulseAnimation]),
                    builder: (context, child) {
                      return CustomPaint(
                        size: Size(constraints.maxWidth, 250),
                        painter: _DprChartPainter(
                          baselineCurve: widget.baselineCurve,
                          powerCurve: widget.showPowerAttack
                              ? widget.powerAttackCurve
                              : null,
                          advantageCurve: widget.showAdvantage
                              ? widget.advantageCurve
                              : null,
                          disadvantageCurve: widget.showDisadvantage
                              ? widget.disadvantageCurve
                              : null,
                          minAc: minAc,
                          maxAc: maxAc,
                          yMax: yMax,
          ...
          Row(
            ...
            children: [
              ...
              if (mode == DprChartMode.dpr &&
                  widget.showPowerAttack &&
                  widget.breakEvenAc != null) ...[
                ...
                Container(
                  ...
                  child: Text(
                    activeAc <= widget.breakEvenAc!
                        ? '⚡ Power Attack Optimal'
                        : '🛡️ Normal Attack Optimal',
                    style: TextStyle(
                      fontSize: 11,
            ...
            Wrap(
              ...
              children: [
                _buildMetricChip(
                  label: 'Hit Damage',
                  value:
                      '${(basePt?.expectedDamageOnHit ?? 0).toStringAsFixed(1)} dmg',
                  subtext: 'Base + Flat',
                  color: baseColor,
                ),
                _buildMetricChip(
                  label: 'Crit Damage',
                  value:
                      '${(basePt?.expectedDamageOnCrit ?? 0).toStringAsFixed(1)} dmg',
                  subtext: 'Double Dice',
                  color: critColor,
                ),
                if ((basePt?.expectedDamageOnMiss ?? 0) > 0)
                  _buildMetricChip(
                    label: 'Miss (Graze)',
                    value:
                        '${(basePt!.expectedDamageOnMiss).toStringAsFixed(1)} dmg',
                    subtext: 'Guaranteed',
                    color: missColor,
                  ),
              ],
...
class _DprChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int ac = minAc; ac <= maxAc; ac++) {
      if (ac == minAc ||
          ac == maxAc ||
          (ac % step == 0 && (ac - minAc >= 2) && (maxAc - ac >= 2))) {
    ...
    for (final ac in xTicks) {
      ...
      final tp = TextPainter(text: span, textDirection: TextDirection.ltr)
        ..layout();
      tp.paint(
          canvas, Offset(x - (tp.width / 2), topPadding + chartHeight + 8));
    }
    ...
    if (chartMode == DprChartMode.dpr) {
      ...
      if (disadvantageCurve != null) {
        _drawSmoothCurve(
          ...
          curvePoints: [
            for (int ac = minAc; ac <= maxAc; ac++)
              Offset(ac.toDouble(),
                  (disadvantageCurve!.pointAt(ac)?.hitChance ?? 0.0) * 100),
          ],
          chartHeight: chartHeight,
          topPadding: topPadding,
          leftPadding: leftPadding,
      ...
      if (advantageCurve != null) {
        _drawSmoothCurve(
          ...
          curvePoints: [
            for (int ac = minAc; ac <= maxAc; ac++)
              Offset(ac.toDouble(),
                  (advantageCurve!.pointAt(ac)?.hitChance ?? 0.0) * 100),
          ],
          chartHeight: chartHeight,
          topPadding: topPadding,
          leftPadding: leftPadding,
      ...
      _drawSmoothCurve(
        ...
        curvePoints: [
          for (int ac = minAc; ac <= maxAc; ac++)
            Offset(ac.toDouble(),
                (baselineCurve.pointAt(ac)?.hitChance ?? 0.0) * 100),
        ],
        chartHeight: chartHeight,
        topPadding: topPadding,
        leftPadding: leftPadding,
      ...
      _drawSmoothCurve(
        ...
        curvePoints: [
          for (int ac = minAc; ac <= maxAc; ac++)
            Offset(ac.toDouble(),
                (baselineCurve.pointAt(ac)?.critChance ?? 0.0) * 100),
        ],
        chartHeight: chartHeight,
        topPadding: topPadding,
        leftPadding: leftPadding,
      ...
      _drawSmoothCurve(
        ...
        curvePoints: [
          for (int ac = minAc; ac <= maxAc; ac++)
            Offset(ac.toDouble(),
                baselineCurve.pointAt(ac)?.expectedDamageOnCrit ?? 0.0),
        ],
        chartHeight: chartHeight,
        topPadding: topPadding,
        leftPadding: leftPadding,
      ...
      _drawSmoothCurve(
        ...
        curvePoints: [
          for (int ac = minAc; ac <= maxAc; ac++)
            Offset(ac.toDouble(),
                baselineCurve.pointAt(ac)?.expectedDamageOnHit ?? 0.0),
        ],
        chartHeight: chartHeight,
        topPadding: topPadding,
        leftPadding: leftPadding,
      ...
      if (baselineCurve.points.values.any((p) => p.expectedDamageOnMiss > 0)) {
        _drawSmoothCurve(
          ...
          curvePoints: [
            for (int ac = minAc; ac <= maxAc; ac++)
              Offset(ac.toDouble(),
                  baselineCurve.pointAt(ac)?.expectedDamageOnMiss ?? 0.0),
          ],
          chartHeight: chartHeight,
          topPadding: topPadding,
          leftPadding: leftPadding,
    ...
    if (chartMode == DprChartMode.dpr &&
      ...
      while (startY < topPadding + chartHeight) {
        canvas.drawLine(
          ...
          Offset(
              beaconX, math.min(startY + dashHeight, topPadding + chartHeight)),
          beaconPaint,
        );
      ...
      if (basePt != null) {
        ...
        canvas.drawCircle(
          ...
          Paint()
            ..color = Colors.amber
                .withValues(alpha: 0.35 * (1.0 - (pulseValue * 0.5)))
    ...
    if (activeAc >= minAc && activeAc <= maxAc) {
      ...
      final scannerGlow = Paint()
        ..color =
            (isDark ? Colors.cyanAccent : Colors.cyan).withValues(alpha: 0.35)
        ..strokeWidth = 3.5;
      canvas.drawLine(
      ...
      if (chartMode == DprChartMode.dpr) {
        ...
        drawGlowingNode(
            (baselineCurve.pointAt(activeAc)?.hitChance ?? 0) * 100, baseColor);
        if (advantageCurve != null) {
          drawGlowingNode(
              (advantageCurve!.pointAt(activeAc)?.hitChance ?? 0) * 100,
              advColor);
        }
        if (disadvantageCurve != null) {
          drawGlowingNode(
              (disadvantageCurve!.pointAt(activeAc)?.hitChance ?? 0) * 100,
              disColor);
        }
        drawGlowingNode(
            (baselineCurve.pointAt(activeAc)?.critChance ?? 0) * 100,
            critColor);
      } else {
        drawGlowingNode(
            baselineCurve.pointAt(activeAc)?.expectedDamageOnHit, baseColor);
        drawGlowingNode(
            baselineCurve.pointAt(activeAc)?.expectedDamageOnCrit, critColor);
        if ((baselineCurve.pointAt(activeAc)?.expectedDamageOnMiss ?? 0) > 0) {
          drawGlowingNode(
              baselineCurve.pointAt(activeAc)?.expectedDamageOnMiss, missColor);
        }
      }
    }
  }
