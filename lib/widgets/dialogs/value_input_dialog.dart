...
class ValueInputDialog<T> extends StatefulWidget {
  ...
  static Future<int?> showCustomDie(BuildContext context,
          {int initialSides = 7}) =>
      showInt(
        context,
  ...
  static Future<int?> showGroupTempHp(BuildContext context) => showInt(
        context,
  ...
  static Future<int?> showMassDamage(BuildContext context) => showInt(
        context,
  ...
  static Future<String?> showRename(BuildContext context,
...
class _ValueInputDialogState<T> extends State<ValueInputDialog<T>> {
  ...
  Widget build(BuildContext context) {
    ...
    if (!isDark && widget.accentColor != null) {
      if (effectiveAccent == Colors.amber ||
          effectiveAccent == Colors.amberAccent) {
        effectiveAccent = const Color(0xFFB45309);
      } else if (effectiveAccent == Colors.cyanAccent ||
          effectiveAccent == Colors.cyan) {
      ...
      actions: [
        ...
        ElevatedButton(
          ...
          child: Text(
            widget.confirmLabel,
            style: TextStyle(
              color: ThemeData.estimateBrightnessForColor(effectiveAccent) ==
                      Brightness.dark
                  ? Colors.white
                  : Colors.black,
              fontWeight: FontWeight.bold,
