...
class _DmRuleComparisonDialogState extends State<DmRuleComparisonDialog> {
                  ...
                  IconButton(
                    ...
                    onPressed: () async {
                      await HomebrewPersistenceService()
                          .toggleOtherEntryEnabled(item.id, isEnabled: false);
                      if (context.mounted) {
                        Navigator.pop(context);
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                              content: Text(
                                  'Disabled homebrew rule: "${item.title}".')),
                        );
                      }
                    },
                  ),
  ...
  Widget _buildEditionBox(BuildContext context, String title,
      List<String> rules, Color accentColor) {
    final theme = Theme.of(context);
  ...
  List<Widget> _buildRuleWidgets(
      BuildContext context, List<String> rules, Color accentColor) {
    final theme = Theme.of(context);
