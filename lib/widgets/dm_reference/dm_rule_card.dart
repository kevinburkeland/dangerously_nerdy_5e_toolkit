...
class _DmRuleCardState extends State<DmRuleCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final isRollable = (item.linkedTableTabIndex != null ||
            ...
            (item.category == DmCategory.tables &&
            ...
            children: [
                      ...
                      children: [
                        Text(
                          item.category == DmCategory.tables &&
                              ...
                              ? item.subCategory!
                              : (item.subCategory != null
                                  ? '${item.category.label} • ${item.subCategory}'
                                  : item.category.label),
                          style: TextStyle(
                        ...
                        if (item.category == DmCategory.tables &&
                            item.subCategory != null)
                          Container(
                        ...
                        if (item.isChangedIn2024) const EditionDiffBadge(),
                      ],
                    ),
                  ],
                ),
              ...
              IconButton(
                icon: Icon(
                  ...
                  color:
                      isPinned ? diffColor : theme.colorScheme.onSurfaceVariant,
                  size: 20,
                ),
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    ...
                    Icon(_showTool ? Icons.expand_less : Icons.expand_more,
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        item.linkedTableLabel ??
                            (item.subCategory == 'Rollable Tables'
                                ? 'Roll on Table'
                                : 'Open in Table Roller'),
  ...
  List<Widget> _buildRuleContent(
    ...
    while (i < rules.length) {
      ...
      // Check if this starts a markdown table block or contains embedded table
      final isTableLine =
          rule.startsWith('|') || FormattedMarkdownText.isTableSeparator(rule);
      final hasEmbeddedTable = rule.contains('|') &&
          (rule.contains('\n') || FormattedMarkdownText.isTableSeparator(rule));

      if (isTableLine || hasEmbeddedTable) {
        final tableLines = <String>[];
        while (i < rules.length) {
          final current = rules[i].trim();
          if (current.startsWith('|') ||
              FormattedMarkdownText.isTableSeparator(current) ||
  ...
  Widget _buildHighlightedText({
    ...
    while (start < text.length) {
      ...
      if (nextMatchIndex > start) {
        spans.add(TextSpan(
            text: text.substring(start, nextMatchIndex), style: style));
      }
