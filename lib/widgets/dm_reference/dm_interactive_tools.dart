...
class ConcentrationCalculatorWidget extends StatefulWidget {
  ...
  @override
  State<ConcentrationCalculatorWidget> createState() =>
      _ConcentrationCalculatorWidgetState();
}

class _ConcentrationCalculatorWidgetState
    extends State<ConcentrationCalculatorWidget> {
  ...
  void _rollConSave() {
    ...
    setState(() {
      ...
      _lastRollResult =
          '$rollDesc $modSign = $total vs DC $_requiredDc (${success ? "PASS" : "FAIL"})';
    });
  }
...
class FallingDamageCalculatorWidget extends StatefulWidget {
  ...
  @override
  State<FallingDamageCalculatorWidget> createState() =>
      _FallingDamageCalculatorWidgetState();
}

class _FallingDamageCalculatorWidgetState
    extends State<FallingDamageCalculatorWidget> {
  int _fallFeet = 30;
  ...
  void _rollFallingDamage() {
    ...
    final msg =
        'Fell $_fallFeet ft: Rolled ${count}d6 -> [${dice.join(", ")}] = $sum bludgeoning damage.';
  ...
  Widget build(BuildContext context) {
    ...
    final isDark = theme.brightness == Brightness.dark;
    final accentColor =
        isDark ? Colors.deepOrangeAccent : const Color(0xFFC2410C);
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Fall Distance: $_fallFeet feet',
                        style: TextStyle(
          ...
          ElevatedButton.icon(
            ...
            icon: const Icon(Icons.casino, size: 16),
            label: Text('Roll ${_diceCount}d6 Impact Damage',
                overflow: TextOverflow.ellipsis),
          ),
...
class GrappleShoveCalculatorWidget extends StatefulWidget {
  ...
  @override
  State<GrappleShoveCalculatorWidget> createState() =>
      _GrappleShoveCalculatorWidgetState();
}

class _GrappleShoveCalculatorWidgetState
    extends State<GrappleShoveCalculatorWidget> {
  int _strModifier = 3;
...
class DcBenchmarkSelectorWidget extends StatelessWidget {
  ...
  static const List<(int, String, String)> benchmarks = [
    ...
    (
      30,
      'Nearly Impossible',
      'Tracking invisible flyer, surviving volcano dive'
    ),
  ];
  ...
  Widget build(BuildContext context) {
    ...
    final isDark = theme.brightness == Brightness.dark;
    final accentColor =
        isDark ? Colors.lightGreenAccent : const Color(0xFF15803D);
          ...
          ...benchmarks.map((b) {
            ...
            return InkWell(
              onTap: () {
                HapticService.selectionTick(context);
                Clipboard.setData(
                    ClipboardData(text: 'DC $dc ($label): $example'));
