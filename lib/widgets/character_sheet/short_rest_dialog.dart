...
/// Interactive modal allowing players to spend hit dice during a Short Rest,
  ...
  void _executeShortRest() {
    ...
    for (final entry in _spentDice.entries) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        allRolls.add(roll);
        final healFromDie = math.max<int>(1, roll + conMod);
        totalHealing += healFromDie;
        individualRollStrings.add('$dieTypeStr($roll) + $conMod = $healFromDie');
      }
    }
    ...
    if (activeRoom != null) {
      final roomRoll = RoomRoll(
        ...
        formulaString: 'Short Rest Hit Dice ($_totalDiceSelected dice)',
        total: totalHealing,
