...
class _ShortRestDialogState extends State<ShortRestDialog> {
  ...
  int get _totalDiceSelected =>
      _spentDice.values.fold(0, (sum, count) => sum + count);
  ...
  void _executeShortRest() {
    ...
    final conMod =
        character.effectiveAbilityScores.getModifier(AbilityType.constitution);
    ...
    for (final group in rollResult.groupResults) {
      for (final roll in group.rolls) {
        ...
        individualRollStrings
            .add('${group.entry.dieLabel}($roll) + $conMod = $healFromDie');
      }
    }
    ...
    A11yService.announce(
        'Short rest completed. Recovered $totalHealing hit points.');
                ...
                children: [
                  ...
                  ConstrainedBox(
                    ...
                    child: FilledButton.icon(
                      ...
                      label: Text(_totalDiceSelected > 0
                          ? 'Roll & Heal'
                          : 'Complete Rest'),
