...
class _SkillsSavesMatrixState extends State<SkillsSavesMatrix> {
  ...
  void _dispatchRoll({
    ...
    String rollDetail = '';
    if (rollResult.droppedRolls != null &&
        rollResult.droppedRolls!.isNotEmpty) {
    ...
    final critText = rollResult.isCrit
        ? ' 🌟 NATURAL 20!'
        : (rollResult.isFumble ? ' 💀 NATURAL 1!' : '');
  ...
  Widget _buildSkillsList(BuildContext context, int profBonus) {
    ...
    return Column(
      children: SkillType.values.map((skill) {
        ...
        final skillMod = stats.skillModifiers[skill] ??
            (baseAbilityMod + (profBonus * profLevel.multiplier).toInt());
        ...
        final pipButton = Tooltip(
          message:
              '${skill.displayName} (${profLevel.name}). Tap to cycle proficiency.',
          child: InkWell(
        ...
        return Padding(
          ...
          child: Material(
            ...
            shape: RoundedRectangleBorder(
              ...
              side: isExpertise
                  ? BorderSide(
                      color: Colors.amber.withValues(alpha: 0.45), width: 1.2)
                  : (isProf
                      ? BorderSide(
                          color:
                ...
                children: [
                  ...
                  Expanded(
                    child: Semantics(
                      button: true,
                      label:
                          '${skill.displayName} (${ability.shortName}) modifier $modStr, ${profLevel.name}',
                      child: InkWell(
