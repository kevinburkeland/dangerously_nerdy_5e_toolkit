...
class _CharacterSheetTabsState extends State<CharacterSheetTabs>
  ...
  String _selectedActionCategory =
      'all'; // 'all', 'action', 'bonusAction', 'reaction', 'special', 'standard'
      ...
      children: [
        ...
        SingleChildScrollView(
          ...
          child: Row(
            children: [
              ...
              const SizedBox(width: 6),
              _buildCategoryChip(
                  'action', 'Actions (${resolved.actions.length})'),
              const SizedBox(width: 6),
              _buildCategoryChip('bonusAction',
                  'Bonus Actions (${resolved.bonusActions.length})'),
              const SizedBox(width: 6),
              _buildCategoryChip(
                  'reaction', 'Reactions (${resolved.reactions.length})'),
              if (resolved.specialActions.isNotEmpty) ...[
                const SizedBox(width: 6),
                _buildCategoryChip(
                    'special', 'Special (${resolved.specialActions.length})'),
              ],
              const SizedBox(width: 6),
              _buildCategoryChip(
                  'standard', 'Rules (${resolved.standardActions.length})'),
            ],
          ),
        ),
        ...
        if (_selectedActionCategory == 'all') ...[
          if (resolved.actions.isNotEmpty) ...[
            _buildSectionHeader(
                'ACTIONS (${resolved.actions.length})', Colors.redAccent),
          ...
          if (resolved.bonusActions.isNotEmpty) ...[
            _buildSectionHeader(
                ...
                Colors.orangeAccent),
            const SizedBox(height: 6),
            ...resolved.bonusActions
                .map((a) => _buildCombatActionCard(context, a)),
            const SizedBox(height: 12),
          ],
          if (resolved.reactions.isNotEmpty) ...[
            _buildSectionHeader(
                'REACTIONS (${resolved.reactions.length})', Colors.blueAccent),
            const SizedBox(height: 6),
            ...resolved.reactions
                .map((a) => _buildCombatActionCard(context, a)),
            const SizedBox(height: 12),
          ...
          if (resolved.specialActions.isNotEmpty) ...[
            _buildSectionHeader(
                'SPECIAL & MAGIC ITEMS (${resolved.specialActions.length})',
                Colors.purpleAccent),
            const SizedBox(height: 6),
            ...resolved.specialActions
                .map((a) => _buildCombatActionCard(context, a)),
            const SizedBox(height: 12),
          ],
  ...
  Widget _buildEconomyTrackerChip({
    ...
    return InkWell(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: isSpent ? Colors.black26 : color.withValues(alpha: 0.15),
          borderRadius: BorderRadius.circular(8),
          border: Border.all(
  ...
  Widget _buildCombatActionCard(
      BuildContext context, CharacterCombatAction action) {
    final theme = Theme.of(context);
    ...
    return ListView(
      ...
      children: [
        ...
        Builder(
          builder: (context) {
            ...
            final linked = memberships.cast<CampaignMembership?>().firstWhere(
                  (m) =>
                      m?.characterId == character.id.slug ||
                      m?.characterId == character.name,
                  orElse: () => null,
                );
              ...
              child: InkWell(
                ...
                onTap: () {
                  Navigator.push(
                    ...
                    MaterialPageRoute(
                        builder: (_) =>
                            PartyRoomScreen(roomCode: linked.roomCode)),
                  );
                },
                child: Container(
                  padding:
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  _buildCoinPill('PP', character.purse.pp,
                      const Color(0xFFCBD5E1), theme, 'pp'),
                  const SizedBox(width: 6),
                  _buildCoinPill('GP', character.purse.gp,
                      const Color(0xFFEAB308), theme, 'gp'),
                  const SizedBox(width: 6),
                  _buildCoinPill('EP', character.purse.ep,
                      const Color(0xFF94A3B8), theme, 'ep'),
                  const SizedBox(width: 6),
                  _buildCoinPill('SP', character.purse.sp,
                      const Color(0xFF94A3B8), theme, 'sp'),
                  const SizedBox(width: 6),
                  _buildCoinPill('CP', character.purse.cp,
                      const Color(0xFFB45309), theme, 'cp'),
                ],
              ),
            ],
          ),
          ...
          child: Row(
            ...
            children: [
              Row(
                children: [
                  Icon(
                    ...
                    size: 16,
                    color:
                        stats.attunedItemCount > 0 ? Colors.amber : Colors.grey,
                  ),
                  const SizedBox(width: 8),
                  Text(
                    'Attunement Slots',
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: stats.attunedItemCount >=
                          stats.effectiveMaxAttunementSlots
        ...
        if (character.inventory.isEmpty)
          ...
          ...character.inventory
              .map((item) => _buildInventoryItemCard(context, item)),
      ],
    );
  }

  Future<void> _showAdjustCoinsDialog(BuildContext context,
      {String? initialDenom}) async {
        ...
        return AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                _buildCoinInputField(
                    'Platinum Pieces (PP)', ppCtrl, const Color(0xFFCBD5E1)),
                const SizedBox(height: 8),
                _buildCoinInputField(
                    'Gold Pieces (GP)', gpCtrl, const Color(0xFFEAB308)),
                const SizedBox(height: 8),
                _buildCoinInputField(
                    'Electrum Pieces (EP)', epCtrl, const Color(0xFF94A3B8)),
                const SizedBox(height: 8),
                _buildCoinInputField(
                    'Silver Pieces (SP)', spCtrl, const Color(0xFF94A3B8)),
                const SizedBox(height: 8),
                _buildCoinInputField(
                    'Copper Pieces (CP)', cpCtrl, const Color(0xFFB45309)),
              ],
            ),
          ),
          actions: [
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  _buildCoinPill('PP', stashPurse.pp, const Color(0xFFCBD5E1),
                      theme, 'pp'),
                  const SizedBox(width: 6),
                  _buildCoinPill('GP', stashPurse.gp, const Color(0xFFEAB308),
                      theme, 'gp'),
                  const SizedBox(width: 6),
                  _buildCoinPill('EP', stashPurse.ep, const Color(0xFF94A3B8),
                      theme, 'ep'),
                  const SizedBox(width: 6),
                  _buildCoinPill('SP', stashPurse.sp, const Color(0xFF94A3B8),
                      theme, 'sp'),
                  const SizedBox(width: 6),
                  _buildCoinPill('CP', stashPurse.cp, const Color(0xFFB45309),
                      theme, 'cp'),
                ],
              ),
              const SizedBox(height: 10),
              Row(
                ...
                children: [
                  OutlinedButton.icon(
                    ...
                    label: const Text('Deposit to Stash',
                        style: TextStyle(fontSize: 12)),
                    onPressed: () =>
                        _showDepositToStashDialog(context, membership),
                  ),
                  const SizedBox(width: 8),
                  OutlinedButton.icon(
                    style: OutlinedButton.styleFrom(
                    ...
                    label: const Text('Withdraw from Stash',
                        style: TextStyle(fontSize: 12)),
                    onPressed: () => _showWithdrawFromStashDialog(
                        context, membership, stashPurse),
                  ),
                ],
              ),
            ],
  ...
  Future<void> _showDepositToStashDialog(
      BuildContext context, CampaignMembership membership) async {
    final cur = widget.controller.character.purse;
        ...
        return AlertDialog(
          title: const Row(
            children: [
              ...
              SizedBox(width: 8),
              Text('Deposit to Campaign Stash',
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 12),
                _buildCoinInputField('Platinum (Max ${cur.pp})', ppCtrl,
                    const Color(0xFFCBD5E1)),
                const SizedBox(height: 8),
                _buildCoinInputField(
                    'Gold (Max ${cur.gp})', gpCtrl, const Color(0xFFEAB308)),
                const SizedBox(height: 8),
                _buildCoinInputField('Electrum (Max ${cur.ep})', epCtrl,
                    const Color(0xFF94A3B8)),
                const SizedBox(height: 8),
                _buildCoinInputField(
                    'Silver (Max ${cur.sp})', spCtrl, const Color(0xFF94A3B8)),
                const SizedBox(height: 8),
                _buildCoinInputField(
                    'Copper (Max ${cur.cp})', cpCtrl, const Color(0xFFB45309)),
              ],
            ),
          ),
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                final pp =
                    (int.tryParse(ppCtrl.text.trim()) ?? 0).clamp(0, cur.pp);
                final gp =
                    (int.tryParse(gpCtrl.text.trim()) ?? 0).clamp(0, cur.gp);
                final ep =
                    (int.tryParse(epCtrl.text.trim()) ?? 0).clamp(0, cur.ep);
                final sp =
                    (int.tryParse(spCtrl.text.trim()) ?? 0).clamp(0, cur.sp);
                final cp =
                    (int.tryParse(cpCtrl.text.trim()) ?? 0).clamp(0, cur.cp);
                ...
                if (ctx.mounted) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                        content: Text('Coins deposited to campaign stash!')),
                  );
                }
              },
      ...
      builder: (ctx) {
        return AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 12),
                _buildCoinInputField('Platinum (Max ${stashPurse.pp})', ppCtrl,
                    const Color(0xFFCBD5E1)),
                const SizedBox(height: 8),
                _buildCoinInputField('Gold (Max ${stashPurse.gp})', gpCtrl,
                    const Color(0xFFEAB308)),
                const SizedBox(height: 8),
                _buildCoinInputField('Electrum (Max ${stashPurse.ep})', epCtrl,
                    const Color(0xFF94A3B8)),
                const SizedBox(height: 8),
                _buildCoinInputField('Silver (Max ${stashPurse.sp})', spCtrl,
                    const Color(0xFF94A3B8)),
                const SizedBox(height: 8),
                _buildCoinInputField('Copper (Max ${stashPurse.cp})', cpCtrl,
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                final pp = (int.tryParse(ppCtrl.text.trim()) ?? 0)
                    .clamp(0, stashPurse.pp);
                final gp = (int.tryParse(gpCtrl.text.trim()) ?? 0)
                    .clamp(0, stashPurse.gp);
                final ep = (int.tryParse(epCtrl.text.trim()) ?? 0)
                    .clamp(0, stashPurse.ep);
                final sp = (int.tryParse(spCtrl.text.trim()) ?? 0)
                    .clamp(0, stashPurse.sp);
                final cp = (int.tryParse(cpCtrl.text.trim()) ?? 0)
                    .clamp(0, stashPurse.cp);
                ...
                if (ctx.mounted) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                        content: Text('Coins withdrawn from campaign stash!')),
                  );
                }
              },
  ...
  Widget _buildCoinPill(
      String denom, int count, Color color, ThemeData theme, String coinKey) {
    return Expanded(
      child: InkWell(
        onTap: () {
          HapticService.selectionTick(context);
  ...
  Widget _buildInventoryItemCard(
      BuildContext context, InventoryItemInstance item) {
    final theme = Theme.of(context);
    final stats = widget.controller.stats;
                ...
                OutlinedButton.icon(
                  ...
                  onPressed: () async {
                    ...
                    final ok = await widget.controller
                        .toggleAttuneItem(item.instanceId);
                    if (!mounted) return;
                    if (!ok) {
                      ...
                      _showAttunementLimitDialog(
                          this.context, stats.effectiveMaxAttunementSlots);
                    }
                  },
                ),
                const SizedBox(width: 8),
  ...
  Widget _buildSpellsTab(BuildContext context) {
    ...
    final hasAnySpells = cantrips.isNotEmpty ||
        knownSpells.isNotEmpty ||
        pool.maxSlots.isNotEmpty ||
        pool.pactMagicMax > 0;
    ...
    return ListView(
      ...
      children: [
              ...
              Row(
                ...
                children: [
                  ...
                  TextButton.icon(
                    ...
                    label:
                        const Text('Long Rest', style: TextStyle(fontSize: 11)),
                    onPressed: () {
                      HapticService.heavyImpact(context);
                      widget.controller.restoreAllSpellSlots();
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                            content: Text('All spell slots restored to full!')),
                      );
                    },
                  ),
                ],
                          ...
                          children: List.generate(max, (i) {
                            ...
                            return InkWell(
                              onTap: () {
                                HapticService.selectionTick(context);
                                widget.controller
                                    .toggleSpellSlot(lvl, isAvailable);
                              },
                              child: Padding(
                                padding:
                                    const EdgeInsets.symmetric(horizontal: 4),
        ...
        if (cantrips.isNotEmpty) ...[
          ...
          const SizedBox(height: 8),
          ...cantrips.map((c) => _buildSpellItemCard(context, c,
              isCantrip: true, edition: edition)),
          const SizedBox(height: 14),
        ],
        ...
        FilledButton.icon(
          ...
          label: const Text('Add / Manage Spells',
  ...
  Widget _buildSpellStatPill(ThemeData theme,
  ...
  Widget _buildSpellItemCard(
    ...
    final domainSpell = spellItem != null
        ? Spell(
            id: EntityId(
              slug: spellItem.id,
              ruleset: edition == DmRulesEdition.v2024
            ...
            descriptionMarkdown: rules != null
                ...
                ? [
                    EvaluationMath(
                      ...
                      damageType: DamageType.values.firstWhere(
                        (d) =>
                            d.name.toLowerCase() ==
                            (rules.damageOrHealType ?? '').toLowerCase(),
        ...
        : Spell(
            ...
            school: 'evocation',
            castingTime:
                const CastingTime(cost: 1, actionType: ActionType.action),
              ...
              if (action == 'cast_slot') {
                ...
                if (isPact) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                          'Cast $spellName (Expended Level ${pool.pactMagicSlotLevel} Pact slot)'),
                      behavior: SnackBarBehavior.floating,
                    ),
                  );
                } else {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content:
                          Text('Cast $spellName (Expended Level $lvl slot)'),
                      behavior: SnackBarBehavior.floating,
                    ),
                  );
                }
      ...
      builder: (ctx) {
        return StatefulBuilder(
          builder: (modalCtx, setModalState) {
            ...
            final existingCantripSlugs =
                widget.controller.character.cantrips.map((c) => c.slug).toSet();
            final existingKnownSlugs = widget.controller.character.spellsKnown
                .map((s) => s.slug)
                .toSet();
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (listCtx, idx) {
                        ...
                        return ListTile(
                          ...
                          title: Text(spell.getName(edition),
                              style:
                                  const TextStyle(fontWeight: FontWeight.bold)),
                          subtitle: Text(
                              '${spell.levelLabel} • ${spell.school.name}'),
