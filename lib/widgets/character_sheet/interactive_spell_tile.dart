...
class InteractiveSpellTile extends StatelessWidget {
  ...
  SpellSchool _resolveSchool(String schoolStr) {
    return SpellSchool.values.firstWhere(
      (s) =>
          s.name.toLowerCase() == schoolStr.toLowerCase() ||
          s.displayName.toLowerCase() == schoolStr.toLowerCase(),
  ...
  Future<void> _executeSpellAction(BuildContext context) async {
    ...
    if (spell.level > 0) {
      ...
      final hasRegularSlots =
          pool.maxSlots.entries.any((e) => e.key >= spell.level && e.value > 0);
      final hasPactSlots =
          pool.pactMagicMax > 0 && pool.pactMagicSlotLevel >= spell.level;
    ...
    final upcastTag = isPactMagic
        ...
        : ((castLevel != null && castLevel > spell.level)
            ? ' (Cast Lvl $castLevel)'
            : '');
    ...
    if (hasSpellAttack && dealsDamage) {
      ...
      final critStr =
          atk.isCrit ? ' (CRIT!)' : (atk.isFumble ? ' (FUMBLE!)' : '');
      final dmgText = rollResult != null
          ? ' | Damage ${rollResult.total} (${rollResult.formulaString})'
          : '';
      final summary =
          '${spell.name}$upcastTag: Attack ${atk.total}$critStr$dmgText';
      ...
      final critStr =
          atk.isCrit ? ' (CRIT!)' : (atk.isFumble ? ' (FUMBLE!)' : '');
      final summary =
          '${spell.name}$upcastTag: Attack ${atk.total}$critStr (1d20+${controller.stats.spellAttackBonus})';
            ...
            child: Column(
              ...
              children: [
                  ...
                  child: Row(
                    children: [
                      DndGlyph.spell(
                        ...
                        level: spell.level,
                        actionRings: spell.getGlyphActionRings(
                            controller.character.rulesEdition),
                        size: 40,
                        isDarkMode: true,
                      ),
                ...
                Wrap(
                  ...
                  runSpacing: 6,
                  children: [
                    _buildMetaBadge(theme, Icons.timer_outlined,
                        spell.castingTime.triggerCondition ?? '1 Action'),
                    _buildMetaBadge(theme, Icons.straighten, spell.range),
                    _buildMetaBadge(
                        theme,
                        Icons.hourglass_empty,
                        spell.duration.rawText ??
                ...
                if (spell.higherLevelsMarkdown != null &&
                    spell.higherLevelsMarkdown!.trim().isNotEmpty) ...[
                  const SizedBox(height: 14),
                  Text(
                    'AT HIGHER LEVELS',
                    style: theme.textTheme.labelSmall?.copyWith(
                ...
                ConstrainedBox(
                  ...
                  child: FilledButton.icon(
                    ...
                    label: Text(hasSpellAttack || dealsDamage
                        ? 'Roll ${spell.name}'
                        : 'Cast ${spell.name}'),
                    onPressed: () {
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final attackStr =
        hasSpellAttack ? '+${controller.stats.spellAttackBonus} Atk' : null;
    ...
    if (dealsDamage) {
      ...
      if (isEB &&
        ...
        final chaMod = controller.character.effectiveAbilityScores
            ...
            leading: isCantrip
                ...
                : (onTogglePrepared != null
                    ? Semantics(
                        button: true,
                        label:
                            isPrepared ? 'Prepared spell' : 'Unprepared spell',
                        child: ConstrainedBox(
                          constraints:
                              const BoxConstraints(minWidth: 48, minHeight: 48),
                          child: IconButton(
