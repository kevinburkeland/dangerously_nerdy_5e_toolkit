...
class LanguagesToolsSection extends StatelessWidget {
  ...
  void _dispatchToolRoll(BuildContext context, String tool) {
    ...
    final modBreakdown =
        'd20 ($sign [PB +$profBonus, ${ability.shortName} ${abilityMod >= 0 ? "+$abilityMod" : "$abilityMod"}])';
    final baseRoll = rollResult.individualRolls.isNotEmpty
        ? rollResult.individualRolls.first
        : (rollResult.total - totalModifier);
  ...
  Widget build(BuildContext context) {
    ...
    final primaryClassSlug =
        controller.character.progression.classes.firstOrNull?.classRef.slug;
              ...
              if (languages.isEmpty)
                ...
                Wrap(
                  ...
                  children: languages.map((lang) {
                    final category =
                        SrdProficienciesLibrary.getLanguageCategory(lang);
              ...
              Row(
                ...
                children: [
                  ...
                  TextButton.icon(
                    ...
                    icon: const Icon(Icons.add, size: 16),
                    label:
                        const Text('Add Tool', style: TextStyle(fontSize: 12)),
                    style: TextButton.styleFrom(
              ...
              if (tools.isEmpty)
                ...
                ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final category =
                        SrdProficienciesLibrary.getToolCategory(tool);
                    final recAbility =
                        SrdProficienciesLibrary.getRecommendedAbility(tool);
                    final abilityMod = controller.stats.effectiveScores
                        .getModifier(recAbility);
                    final totalMod =
                        controller.stats.proficiencyBonus + abilityMod;
...
class _AddLanguageDialogState extends State<_AddLanguageDialog> {
  ...
  @override
  Widget build(BuildContext context) {
    final existingLangs = widget.controller.character.languages
        .map((l) => l.toLowerCase())
        .toSet();
...
class _AddToolDialogState extends State<_AddToolDialog> {
  ...
  @override
  Widget build(BuildContext context) {
    final existingTools = widget.controller.character.toolProficiencies
        .map((t) => t.toLowerCase())
        .toSet();
