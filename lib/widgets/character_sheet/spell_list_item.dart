...
class SpellListItem extends StatelessWidget {
  ...
  String _formatComponents(SpellComponents comp) {
    ...
    if (comp.m) {
      if (comp.materialDescription != null &&
          comp.materialDescription!.isNotEmpty) {
                    ...
                    child: Column(
                      children: [
                        Row(
                          children: [
                            Expanded(
                                child: _buildMetaPill(context, 'CASTING TIME',
                                    _formatCastingTime(spell.castingTime))),
                        ...
                        Row(
                          children: [
                            Expanded(
                                child: _buildMetaPill(context, 'COMPONENTS',
                                    _formatComponents(spell.components))),
                            const SizedBox(width: 10),
                            Expanded(
                                child: _buildMetaPill(context, 'DURATION',
                                    _formatDuration(spell.duration))),
                          ],
                        ),
                      ],
                    ),
  ...
  Widget build(BuildContext context) {
    ...
    return Semantics(
      button: true,
      label:
          'Spell: ${spell.name}, ${spell.level == 0 ? "Cantrip" : "Level ${spell.level}"}. Tap for details.',
      child: Material(
                ...
                children: [
                  ...
                  if (spell.level > 0 && onTogglePrepared != null) ...[
                    Semantics(
                      button: true,
                      label:
                          '${isPrepared ? "Unprepare" : "Prepare"} ${spell.name}',
                      child: InkWell(
                      ...
                      children: [
                        Text(
                          ...
                          style: theme.textTheme.bodyMedium?.copyWith(
                            fontWeight:
                                isPrepared ? FontWeight.bold : FontWeight.w500,
                          ),
                        ),
                        Row(
                          children: [
                  ...
                  if (onCast != null)
                    Semantics(
                      ...
                      child: FilledButton.tonal(
                        ...
                        onPressed: onCast,
                        child:
                            const Text('Cast', style: TextStyle(fontSize: 11)),
                      ),
                    )
                  else
                    Icon(
