  ...
  Widget build(BuildContext context) {
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        final maxHp =
            (character.customProperties['maxHp'] as num?)?.toInt() ?? currentHp;
        final finalScores =
            character.baseScores.withBonus(character.bonusScores);
        final dexMod = (finalScores.dexterity - 10) ~/ 2;
        final wisMod = (finalScores.wisdom - 10) ~/ 2;
        final ac =
            (character.customProperties['armorClass'] as num?)?.toInt() ??
                (10 + dexMod);
        final pp = (character.customProperties['passivePerception'] as num?)
                ?.toInt() ??
            (10 + wisMod);
        final level = character.progression.totalLevel;
        final startingClass =
            character.progression.startingClass?.classRef.displayName ??
                'Adventurer';
