...
class _HomebrewImportDialogState extends State<HomebrewImportDialog> {
  ...
  void _handleAnalyze() {
    ...
    final result =
        _pipeline.ingestJsonString(input, forceRuleset: _selectedRuleset);
  ...
  Future<void> _pickFile() async {
    ...
    if (file == null) return;
    final result = _pipeline.ingestJsonString(file.content,
        forceRuleset: _selectedRuleset);
    setState(() {
      _loadedFile = file;
  ...
  Future<void> _handleCommitImport() async {
    ...
    try {
      ...
      final subracesToSave = [
        for (final r in _previewResult!.races) ...r.subraces,
      ];
      if (subracesToSave.isNotEmpty) {
        await persistence.saveCustomSubracesBatch(subracesToSave);
      }
      ...
      if (_previewResult!.backgrounds.isNotEmpty) {
        await persistence
            .saveCustomBackgroundsBatch(_previewResult!.backgrounds);
      }
      if (_previewResult!.otherEntries.isNotEmpty) {
        await persistence
            .saveCustomOtherEntriesBatch(_previewResult!.otherEntries);
      }
      if (_previewResult!.fluff.isNotEmpty) {
        await persistence.saveCustomFluffBatch(_previewResult!.fluff);
      }
  ...
  Widget build(BuildContext context) {
    ...
    return AppDialogFrame(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (preview.classes.isNotEmpty)
                      Text(
                          '• Classes: ${preview.classes.length} (${preview.classes.map((c) => c.name).take(3).join(', ')}${preview.classes.length > 3 ? '...' : ''})'),
                    if (preview.subclasses.isNotEmpty)
                      Text(
                          '• Subclasses: ${preview.subclasses.length} (${preview.subclasses.map((s) => s.name).take(3).join(', ')}${preview.subclasses.length > 3 ? '...' : ''})'),
                    if (preview.races.isNotEmpty)
                      Text(
                          '• Races / Species: ${preview.races.length} (${preview.races.map((r) => r.name).take(3).join(', ')}${preview.races.length > 3 ? '...' : ''})'),
                    if (preview.feats.isNotEmpty)
                      Text(
                          '• Feats: ${preview.feats.length} (${preview.feats.map((f) => f.name).take(3).join(', ')}${preview.feats.length > 3 ? '...' : ''})'),
                    if (preview.backgrounds.isNotEmpty)
                      Text(
                          '• Backgrounds: ${preview.backgrounds.length} (${preview.backgrounds.map((b) => b.name).take(3).join(', ')}${preview.backgrounds.length > 3 ? '...' : ''})'),
                    if (preview.spells.isNotEmpty)
                      Text(
                          '• Spells: ${preview.spells.length} (${preview.spells.map((s) => s.name).take(3).join(', ')}${preview.spells.length > 3 ? '...' : ''})'),
                    if (preview.monsters.isNotEmpty)
                      Text(
                          '• Monsters: ${preview.monsters.length} (${preview.monsters.map((m) => m.name).take(3).join(', ')}${preview.monsters.length > 3 ? '...' : ''})'),
                    if (preview.items.isNotEmpty)
                      Text(
                          '• Items: ${preview.items.length} (${preview.items.map((i) => i.name).take(3).join(', ')}${preview.items.length > 3 ? '...' : ''})'),
                    if (preview.otherEntries.isNotEmpty)
                      Text(
                          '• Other / Rules / Tables: ${preview.otherEntries.length} (${preview.otherEntries.map((o) => o.name).take(3).join(', ')}${preview.otherEntries.length > 3 ? '...' : ''})'),
      ...
      actions: [
        ...
        FilledButton.icon(
          onPressed:
              (preview != null && preview.totalEntities > 0 && !_isImporting)
                  ? _handleCommitImport
                  : null,
          icon: _isImporting
              ? const SizedBox(
