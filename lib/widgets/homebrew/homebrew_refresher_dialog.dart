...
class HomebrewRefresherDialog extends StatefulWidget {
  ...
  @override
  State<HomebrewRefresherDialog> createState() =>
      _HomebrewRefresherDialogState();
}
  ...
  Future<void> _loadStats() async {
    ...
    for (final type in [
      ...
      final entities = switch (type) {
        ...
        EntityType.classDefinition =>
          (await _persistence.loadCustomClasses()).length,
        EntityType.subclass =>
          (await _persistence.loadCustomSubclasses()).length,
        EntityType.species => (await _persistence.loadCustomRaces()).length,
        EntityType.feat => (await _persistence.loadCustomFeats()).length,
        EntityType.background =>
          (await _persistence.loadCustomBackgrounds()).length,
        EntityType.custom =>
          (await _persistence.loadCustomOtherEntries()).length,
        _ => 0,
      };
      total += entities;
    }
      ...
      child: Column(
        ...
        children: [
          ...
          _buildReportItem(
            ...
            subtitle:
                'Re-parsed through AST node pipeline with grants and math formulas.',
            theme: theme,
          ),
          ...
          _buildReportItem(
            ...
            iconColor:
                result.srdRemovedCount > 0 ? Colors.orangeAccent : Colors.grey,
          ...
          if (result.noPayloadCount > 0) ...[
            ...
            _buildReportItem(
              ...
              subtitle:
                  'Entities without raw JSON were safely retained without modification.',
              theme: theme,
            ),
          ],
        ],
