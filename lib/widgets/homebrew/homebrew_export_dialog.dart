...
class _HomebrewExportDialogState extends State<HomebrewExportDialog> {
  ...
  Future<void> _loadCounts() async {
    ...
    if (mounted) {
      setState(() {
        _categoryCounts = {
          EntityType.spell: bundle.spells.length,
          EntityType.monster: bundle.monsters.length,
          EntityType.equipment: bundle.items.length,
          EntityType.classDefinition: bundle.classes.length,
          EntityType.subclass: bundle.subclasses.length,
          EntityType.species: bundle.races.length + bundle.subraces.length,
          EntityType.feat: bundle.feats.length,
          EntityType.background: bundle.backgrounds.length,
          EntityType.custom: bundle.otherEntries.length,
        };
        _isLoading = false;
      });
    }
  ...
  Future<void> _generateExport() async {
    ...
    try {
      ...
      final jsonStr =
          const JsonEncoder.withIndent('  ').convert(bundle.toMap());
  ...
  void _copyToClipboard() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
          content: Text('Homebrew Bundle JSON copied to clipboard!')),
    );
  }
  ...
  Future<void> _saveToFile() async {
    ...
    if (json == null) return;
    final fileName =
        CompendiumFilePickerService.toSafeFileName(_nameController.text.trim());
  ...
  Widget build(BuildContext context) {
    ...
    return AppDialogFrame(
      ...
      title:
          _exportedJson == null ? 'Export Homebrew Pack' : 'Bundle Generated',
      maxWidth: 620,
      content: _isLoading
          ? const Center(
              child: Padding(
      ...
      actions: [
        ...
        if (_exportedJson == null)
          ElevatedButton.icon(
            onPressed: _selectedCategories.isNotEmpty && !_isExporting
                ? _generateExport
                : null,
            icon: _isExporting
                ? const SizedBox(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              TextButton(
                ...
                child: Text(
                  _selectedCategories.length == _categoryCounts.length
                      ? 'Deselect All'
                      : 'Select All',
          ...
          _buildCategoryCheckbox(
              EntityType.spell, 'Spells', Icons.auto_awesome),
          _buildCategoryCheckbox(
              EntityType.monster, 'Monsters & NPCs', Icons.pest_control),
          _buildCategoryCheckbox(
              EntityType.equipment, 'Equipment & Magic Items', Icons.shield),
          _buildCategoryCheckbox(
              EntityType.classDefinition, 'Classes', Icons.school),
          _buildCategoryCheckbox(
              EntityType.subclass, 'Subclasses', Icons.alt_route),
          _buildCategoryCheckbox(
              EntityType.species, 'Races & Species', Icons.people),
          _buildCategoryCheckbox(EntityType.feat, 'Feats', Icons.military_tech),
          _buildCategoryCheckbox(
              EntityType.background, 'Backgrounds', Icons.history_edu),
          _buildCategoryCheckbox(
              EntityType.custom, 'Rules & Tables', Icons.table_chart),
        ],
      ),
    );
  }
  ...
  Widget _buildCategoryCheckbox(EntityType type, String label, IconData icon) {
    ...
    return CheckboxListTile(
      ...
      secondary: Icon(icon,
          size: 20, color: isSelected ? Colors.amberAccent : Colors.grey),
      ...
      children: [
        ...
        Row(
          ...
          children: [
            ...
            _buildStatChip(
                Icons.format_list_numbered, 'Entities', '$totalEntities'),
          ],
        ),
        const SizedBox(height: 12),
        // ── Preview snippet ──────────────────────────────────────────────
