...
class LootConflictResolutionDialog extends StatefulWidget {
  ...
  @override
  State<LootConflictResolutionDialog> createState() =>
      _LootConflictResolutionDialogState();
}

class _LootConflictResolutionDialogState
    extends State<LootConflictResolutionDialog> {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                'Overwrote cloud with Local Version of "${widget.item.name}".'),
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                'Kept both versions ("${_cloudItem.name}" and "${widget.item.name} (Copy)").'),
  ...
  Widget build(BuildContext context) {
    ...
    final attuneDiff = local.requiresAttunement != cloud.requiresAttunement ||
        local.isAttuned != cloud.isAttuned;
      ...
      title: Row(
        children: [
          Container(
            ...
            child:
                const Icon(Icons.sync_problem, color: Colors.amber, size: 24),
          ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildFieldRow(
              'Value', '${item.gpValue} GP (x${item.count})', gpDiff),
          _buildFieldRow(
            'Attunement',
          ...
          _buildFieldRow(
              'Claimed By',
              item.isClaimed ? item.claimedByPlayer! : 'Vault (Unclaimed)',
              claimDiff),
