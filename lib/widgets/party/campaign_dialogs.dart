...
class CreateCampaignDialog extends StatefulWidget {
  ...
  static Future<void> show(BuildContext context,
      {Function(String roomCode)? onCreated}) {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                'Campaign "${session.campaignName}" created! Room Code: ${session.roomCode}'),
...
class JoinCampaignDialog extends StatefulWidget {
  ...
  static Future<void> show(BuildContext context,
      {String? initialRoomCode, Function(String roomCode)? onJoined}) {
    return showDialog<void>(
      context: context,
      builder: (ctx) => JoinCampaignDialog(
          initialRoomCode: initialRoomCode, onJoined: onJoined),
    );
  }
...
class _JoinCampaignDialogState extends State<JoinCampaignDialog> {
  ...
  bool _isLoading = false;
  final PartyRoomService _partyService = PartyRoomService();
  final CharacterPersistenceService _characterService =
    ...
    if (cached != null && cached.characterRoster.isNotEmpty) {
      if (mounted) {
        setState(() {
          ...
          if (_selectedExistingSlot == null &&
              _detectedRosterSlots.isNotEmpty) {
  ...
  Future<void> _pasteFromClipboard() async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('DM Passkey pasted from clipboard!'),
  ...
  Future<void> _submit() async {
    ...
    final Character? linkedChar =
        (!_isDmJoin && _useSavedCharacter) ? _selectedCharacter : null;
    final String targetExistingSlot =
        _selectedExistingSlot ?? _existingSlotController.text.trim();
    ...
    try {
      final session = await _partyService.joinCampaign(
        ...
        existingRosterName: (!_importAsNew &&
                linkedChar != null &&
                targetExistingSlot.isNotEmpty)
            ? targetExistingSlot
            : null,
    ...
    return AlertDialog(
      ...
      title: Row(
        children: [
          ...
          const SizedBox(width: 10),
          const Text('Join Existing Campaign',
              style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
        ],
      ),
      content: SingleChildScrollView(
          ...
          children: [
            ...
            TextField(
              ...
              autofocus: widget.initialRoomCode == null ||
                  widget.initialRoomCode!.isEmpty,
              maxLength: 30,
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    DropdownButtonFormField<Character?>(
                      initialValue:
                          _useSavedCharacter ? _selectedCharacter : null,
                      isExpanded: true,
                      decoration: InputDecoration(
                            ...
                            children: [
                              ...
                              Expanded(
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    const Text('Import as new character',
                                        style: TextStyle(
                                            fontSize: 13,
                            ...
                            children: [
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    const Text(
                                        'Link to existing character slot',
                                        style: TextStyle(
                                            fontSize: 13,
                                            fontWeight: FontWeight.w600)),
                                    Text(
                      ...
                      if (!_importAsNew) ...[
                        ...
                        if (_detectedRosterSlots.isNotEmpty)
                          DropdownButtonFormField<String>(
                            initialValue: _detectedRosterSlots
                                    .contains(_selectedExistingSlot)
                            ...
                            items: _detectedRosterSlots
                                ...
                                .toList(),
                            onChanged: (val) =>
                                setState(() => _selectedExistingSlot = val),
                          )
                        else
                          TextField(
            ...
            if (!_useSavedCharacter || _isDmJoin) ...[
              TextField(
                ...
                decoration: InputDecoration(
                  labelText: _isDmJoin
                      ? 'Your DM / Display Name'
                      : 'Your Character / Player Name',
                  hintText: _isDmJoin
                      ? 'e.g. Dungeon Master'
                      : 'e.g. Rogar the Barbarian',
                  border: OutlineInputBorder(
    ...
    return AlertDialog(
      ...
      title: Row(
        children: [
          ...
          const SizedBox(width: 10),
          const Flexible(
            child: Text('DM Passkey Delegation',
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
          ),
        ],
      ),
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            const Text('6-Word Mnemonic Passkey:',
            ...
            const SizedBox(height: 12),
            const Text('Raw Cryptographic Host Key:',
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
            const SizedBox(height: 4),
            SelectableText(
              hostKey,
    ...
    return AlertDialog(
      ...
      title: Row(
        children: [
          ...
          const SizedBox(width: 10),
          const Flexible(
            child: Text('Claim DM / Co-DM Status',
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
          ),
        ],
      ),
            ...
            Row(
              children: [
                ...
                ChoiceChip(
                  ...
                  onSelected: (selected) {
                    if (selected)
                      setState(() => _selectedRole = CampaignRole.host);
                  },
                ),
                const SizedBox(width: 8),
                ChoiceChip(
                  ...
                  onSelected: (selected) {
                    if (selected)
                      setState(() => _selectedRole = CampaignRole.coDm);
                  },
                ),
              ],
            ),
...
class AddLootItemDialog extends StatefulWidget {
  ...
  static Future<void> show(BuildContext context,
    ...
    return showDialog<void>(
      context: context,
      builder: (ctx) =>
          AddLootItemDialog(roomCode: roomCode, playerName: playerName),
    );
  }
...
class _AddLootItemDialogState extends State<AddLootItemDialog> {
  ...
  void _submit() {
    ...
    final itemId =
        'loot_${DateTime.now().millisecondsSinceEpoch}_${name.replaceAll(RegExp(r'\W+'), '_').toLowerCase()}';
          ...
          children: [
            TextField(
              ...
              decoration: const InputDecoration(
                  labelText: 'Item Name', hintText: 'e.g. Cloak of Protection'),
            ),
            const SizedBox(height: 10),
            DropdownButtonFormField<String>(
              ...
              items: const [
                DropdownMenuItem(
                    value: 'gear', child: Text('Adventuring Gear / Equipment')),
                DropdownMenuItem(
                    value: 'magicItem', child: Text('✨ Magic Item')),
                DropdownMenuItem(value: 'gem', child: Text('💎 Gemstone')),
                DropdownMenuItem(value: 'art', child: Text('🎨 Art Object')),
                DropdownMenuItem(
                    value: 'currency', child: Text('💰 Currency / Ingot')),
              ],
              ...
              children: [
                ...
                Expanded(
                  child: TextField(
                    ...
                    decoration:
                        const InputDecoration(labelText: 'GP Value Each'),
                  ),
                ),
              ],
            ),
            if (_category == 'magicItem') ...[
              ...
              CheckboxListTile(
                ...
                onChanged: (val) =>
                    setState(() => _requiresAttunement = val ?? false),
...
class CoinTransactionDialog extends StatefulWidget {
  ...
  static Future<void> show(BuildContext context,
    ...
    return showDialog<void>(
      context: context,
      builder: (ctx) => CoinTransactionDialog(
          roomCode: roomCode, playerName: playerName, isDeposit: isDeposit),
    );
  }
      ...
      title: Row(
        children: [
          Icon(isDeposit ? Icons.arrow_downward : Icons.arrow_upward,
              color: color),
          const SizedBox(width: 10),
          Text(isDeposit ? 'Deposit Coins' : 'Withdraw Coins',
        ...
        child: Column(
          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                ActionChip(
                    label: const Text('+10 GP'),
                    onPressed: () => _addQuickGp(10)),
                ActionChip(
                    label: const Text('+50 GP'),
                    onPressed: () => _addQuickGp(50)),
                ActionChip(
                    label: const Text('+100 GP'),
                    onPressed: () => _addQuickGp(100)),
                ActionChip(
                    label: const Text('+500 GP'),
                    onPressed: () => _addQuickGp(500)),
              ],
            ),
            const SizedBox(height: 14),
            Row(
              children: [
                Expanded(
                    child: TextField(
                        ...
                        keyboardType: TextInputType.number,
                        decoration:
                            const InputDecoration(labelText: 'PP (Plat)'))),
                const SizedBox(width: 8),
                Expanded(
                    child: TextField(
                        controller: _gp,
                        keyboardType: TextInputType.number,
                        decoration:
                            const InputDecoration(labelText: 'GP (Gold)'))),
                const SizedBox(width: 8),
                Expanded(
                    child: TextField(
                        controller: _ep,
                        keyboardType: TextInputType.number,
                        decoration:
                            const InputDecoration(labelText: 'EP (Elec)'))),
              ],
            ),
              ...
              children: [
                Expanded(
                    child: TextField(
                        ...
                        keyboardType: TextInputType.number,
                        decoration:
                            const InputDecoration(labelText: 'SP (Silv)'))),
                const SizedBox(width: 8),
                Expanded(
                    child: TextField(
                        controller: _cp,
                        keyboardType: TextInputType.number,
                        decoration:
                            const InputDecoration(labelText: 'CP (Copp)'))),
              ],
            ),
            const SizedBox(height: 10),
            TextField(
              controller: _note,
              decoration: const InputDecoration(
                  labelText: 'Transaction Note / Source',
                  hintText: 'e.g. Goblin Lair chest'),
            ),
          ],
        ),
      ),
...
class SwitchActiveCharacterDialog extends StatefulWidget {
  ...
  @override
  State<SwitchActiveCharacterDialog> createState() =>
      _SwitchActiveCharacterDialogState();
}

class _SwitchActiveCharacterDialogState
    extends State<SwitchActiveCharacterDialog> {
  late final TextEditingController _nameController;
  final PartyRoomService _partyService = PartyRoomService();
    ...
    return AlertDialog(
      ...
      title: const Row(
        children: [
          ...
          SizedBox(width: 10),
          Expanded(
            child: Text('Select Active Character',
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
          ),
        ],
      ),
          ...
          children: [
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                hintText: 'e.g. Thorek Ironbreaker (Fighter)',
                ...
                children: widget.roster.map((rName) {
                  ...
                  return ActionChip(
                    avatar: Icon(Icons.shield,
                        size: 14, color: isCurrent ? Colors.white : null),
                    label: Text(rName),
            ...
            CheckboxListTile(
              ...
              title: const Text('Save to campaign party roster',
...
class ManagePartyRosterDialog extends StatefulWidget {
  ...
  @override
  State<ManagePartyRosterDialog> createState() =>
      _ManagePartyRosterDialogState();
}
...
class _ManagePartyRosterDialogState extends State<ManagePartyRosterDialog> {
  ...
  Future<void> _openSheetForRosterCharacter(String name) async {
    ...
    Character? targetChar = localChars
        .where((c) =>
            c.name.toLowerCase() == name.toLowerCase() || c.id.slug == name)
        .firstOrNull;
    ...
    if (targetChar == null &&
        session != null &&
        session.sharedCharacters.isNotEmpty) {
    ...
    final localCharsAfter =
        await CharacterPersistenceService().loadCharacters();
    final latestChar = localCharsAfter
            .where((c) =>
                ...
                child: ListView.separated(
                  ...
                  itemBuilder: (context, idx) {
                    ...
                    return ListTile(
                      ...
                      subtitle: isCurrent
                          ? const Text('Active Session Character',
                              style:
            ...
            ConstrainedBox(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, idx) {
                  ...
                  return ListTile(
                    dense: true,
                    leading: Icon(isMe ? Icons.star : Icons.person_outline,
...
class _DisperseLootDialogState extends State<DisperseLootDialog> {
    ...
    _partyService.streamSession(_selectedRoomCode).first.then((session) {
      if (mounted && session != null) {
        setState(() {
          ...
          final roster =
              session.characterRoster.where((s) => s.trim().isNotEmpty).toSet();
          final currentMemberChar =
              _registry.getMembership(_selectedRoomCode)?.characterId;
          final allCandidates = roster.isNotEmpty
              ? roster
              : (currentMemberChar != null && currentMemberChar.isNotEmpty
                  ? {currentMemberChar}
                  : <String>{});
  ...
  Future<void> _submitDispersal() async {
    ...
    if (_selectedRecipients.isEmpty && !_includePartyReserve) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text(
                'Please select at least one party member or include the party reserve.')),
      );
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('Failed to disperse coins: $e'),
              backgroundColor: Colors.redAccent),
        );
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    final roster = _sessionState?.characterRoster
    ...
    final shareCount =
        _selectedRecipients.length + (_includePartyReserve ? 1 : 0);
    ...
    if (shareCount > 0) {
      if (_includeLiquidated && widget.liquidatedGemsAndArtGp > 0) {
        final totalGp =
            widget.purse.totalGpEquivalent + widget.liquidatedGemsAndArtGp;
        final perShareGp = (totalGp / shareCount).floor();
        final remGp = (totalGp - (perShareGp * shareCount)).round();
        perShareText = '~$perShareGp GP';
        reserveText = _includePartyReserve
            ? '~${perShareGp + remGp} GP'
            : (remGp > 0 ? '~$remGp GP (Remainder)' : '0 GP');
      } else {
          ...
          Expanded(
            child: Text(
              widget.sourceTitle != null
                  ? 'Disperse ${widget.sourceTitle}'
                  : 'Disperse Loot to Party',
        ...
        child: Column(
          ...
          children: [
            ...
            if (memberships.length > 1) ...[
              const Text('Destination Campaign:',
                  style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
              const SizedBox(height: 4),
              DropdownButtonFormField<String>(
                initialValue: _selectedRoomCode.isNotEmpty
                    ? _selectedRoomCode
                    : memberships.first.roomCode,
                decoration: InputDecoration(
                  contentPadding:
                ...
                items: memberships.map((m) {
                  return DropdownMenuItem(
                    value: m.roomCode,
                    child: Text('${m.campaignName} (${m.roomCode})',
            ...
            CheckboxListTile(
              ...
              onChanged: (val) =>
                  setState(() => _includePartyReserve = val ?? true),
              title: const Text('A share for the Party Reserve',
                  style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
              subtitle: const Text(
                  'Deposits 1 equal share + any coin remainders into the shared Vault',
            ...
            if (widget.liquidatedGemsAndArtGp > 0) ...[
              CheckboxListTile(
                ...
                onChanged: (val) =>
                    setState(() => _includeLiquidated = val ?? false),
                title: Text(
                    'Liquidate gems & art (+${widget.liquidatedGemsAndArtGp.toStringAsFixed(0)} GP)',
                    style: const TextStyle(
                        fontWeight: FontWeight.bold, fontSize: 13)),
                subtitle: const Text(
                    'Converts art and gemstone values directly into gold split',
                    style: TextStyle(fontSize: 11)),
                controlAffinity: ListTileControlAffinity.leading,
                contentPadding: EdgeInsets.zero,
                dense: true,
            ...
            Row(
              ...
              children: [
                ...
                TextButton(
                  ...
                  child: Text(
                    _selectedRecipients.length == candidates.length
                        ? 'Deselect All'
                        : 'Select All',
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      const Text('Each Character Gets:',
                          style: TextStyle(fontSize: 12)),
                      Text(perShareText,
                          style: const TextStyle(
                              fontSize: 12,
...
class MemberCoinTransactionDialog extends StatefulWidget {
  ...
  @override
  State<MemberCoinTransactionDialog> createState() =>
      _MemberCoinTransactionDialogState();
}

class _MemberCoinTransactionDialogState
    extends State<MemberCoinTransactionDialog> {
    ...
    try {
      ...
      await _partyService.updateMemberPurse(
        ...
        note: note.isNotEmpty
            ? note
            : (widget.isDeposit ? 'Deposit' : 'Withdrawal'),
      );
    ...
    return AlertDialog(
      ...
      title: Row(
        children: [
          Icon(isDeposit ? Icons.add_circle : Icons.remove_circle,
              color: isDeposit ? Colors.green : Colors.amber),
          const SizedBox(width: 10),
          Expanded(
            child: Text(
      ...
      actions: [
        ...
        ElevatedButton(
          style: ElevatedButton.styleFrom(
            backgroundColor:
                isDeposit ? Colors.green.shade700 : Colors.amber.shade800,
            foregroundColor: Colors.white,
          ),
          onPressed: _isLoading ? null : _submit,
      ...
      title: Row(
        children: [
          ...
          Expanded(
            child: Text(
              widget.toReserve
                  ? 'Transfer to Party Reserve'
                  : 'Withdraw from Party Reserve',
              style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
            ),
          ),
        ],
...
class LinkCampaignCharacterDialog extends StatefulWidget {
  ...
  static Future<void> show(BuildContext context,
      {required String roomCode, VoidCallback? onLinked}) {
    return showDialog<void>(
      context: context,
      builder: (ctx) =>
          LinkCampaignCharacterDialog(roomCode: roomCode, onLinked: onLinked),
    );
  }

  @override
  State<LinkCampaignCharacterDialog> createState() =>
      _LinkCampaignCharacterDialogState();
}

class _LinkCampaignCharacterDialogState
    extends State<LinkCampaignCharacterDialog> {
  final PartyRoomService _partyService = PartyRoomService();
  final CharacterPersistenceService _characterService =
      CharacterPersistenceService();
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content:
                Text('Linked "${_selectedCharacter!.name}" to campaign room!'),
            backgroundColor: Colors.green.shade700,
          ),
        );
      }
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('Failed linking character: $e'),
              backgroundColor: Colors.redAccent),
        );
      }
    }
    ...
    return AlertDialog(
      ...
      title: Row(
        children: [
          ...
          const SizedBox(width: 10),
          const Text('Link Character to Campaign',
        ...
        child: Column(
          ...
          children: [
            ...
            if (_savedCharacters.isEmpty)
              ...
              const SizedBox(height: 14),
              const Text('Campaign Roster Mode:',
                  style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
              const SizedBox(height: 6),
              InkWell(
                borderRadius: BorderRadius.circular(8),
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text('Link to existing character slot',
                                style: TextStyle(
                                    fontSize: 13, fontWeight: FontWeight.w600)),
                            Text(
      ...
      actions: [
        ...
        ElevatedButton.icon(
          ...
          onPressed:
              (_isLoading || _selectedCharacter == null) ? null : _submit,
        ),
      ],
    );
  }
