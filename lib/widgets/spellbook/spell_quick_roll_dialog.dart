...
class SpellQuickRollDialog extends StatefulWidget {
  ...
  final void Function(SpellRollResult result, String spellName, int castLevel)?
      onRollCompleted;
  ...
  static Future<SpellRollResult?> show(
    ...
    void Function(SpellRollResult result, String spellName, int castLevel)?
        onRollCompleted,
  }) {
...
class _SpellQuickRollDialogState extends State<SpellQuickRollDialog> {
  ...
  int _selectedCantripTier =
      1; // 1: Lvl 1-4, 2: Lvl 5-10, 3: Lvl 11-16, 4: Lvl 17-20
  ...
  String get _currentFormula {
    ...
    if (widget.spell.level == 0) {
      ...
      if (rules.scalingFormula != null) {
        return rules.scalingFormula!
            .getFormulaForSlot(widget.spell.level, _selectedSlotLevel);
      }
      return baseFormula;
    }
  }
  ...
  bool get _hasAbilityMod {
    ...
    return formula.contains('+ mod') ||
        formula.contains('- mod') ||
        (rules.scalingFormula?.addsAbilityMod ?? false);
  }
  ...
  void _executeRoll() {
    ...
    final announceText =
        '$spellName ($levelLabel): ${result.total} [Dice: ${result.individualDice.join(", ")}${result.modifier != 0 ? " + Mod ${result.modifier}" : ""}]';
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          _hasAbilityMod
                              ? _currentFormula.replaceAll(
                                  '+ mod', '+ $_selectedAbilityMod')
                              : _currentFormula,
                          style: TextStyle(
                            color: schoolColor,
                            fontSize: 15,
