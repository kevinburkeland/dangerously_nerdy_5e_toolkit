...
class _RaceDetailDialogState extends State<RaceDetailDialog>
  ...
  void initState() {
    ...
    final existingNotes =
        _fluffService.getUserNotes('race', widget.race.id.slug) ?? '';
  ...
  Widget build(BuildContext context) {
    ...
    final resolvedSpeciesType = SpeciesType.tryParse(widget.race.id.slug) ??
        SpeciesType.tryParse(widget.race.name);
      ...
      children: [
        ...
        Container(
          ...
          child: () {
            ...
            final resolvedSpeed =
                widget.race.getSpeedForEdition(resolvedEdition);
            ...
            return Column(
              ...
              children: [
                ...
                Text('• Size: ${widget.race.size}',
                    style: const TextStyle(fontSize: 12.5)),
                Text(
                    '• Speed: $resolvedSpeed (${resolvedEdition == DmRulesEdition.v2014 ? "2014 RAW" : "2024 Revised"})',
                    style: const TextStyle(fontSize: 12.5)),
                if (widget.race.abilityScoreSummary != null &&
                    widget.race.abilityScoreSummary!.isNotEmpty)
                  Text('• Ability Scores: ${widget.race.abilityScoreSummary}',
                      style: const TextStyle(fontSize: 12.5)),
                if (widget.race.bonusFeatCount > 0)
                  Text(
                      '• Bonus Feats: +${widget.race.bonusFeatCount} Feat choice',
                      style: const TextStyle(fontSize: 12.5)),
              ],
            );
          }(),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if ((subrace.abilityScoreSummary?.isNotEmpty == true) ||
                        (subrace.speed?.isNotEmpty == true) ||
                        (subrace.darkvision != null &&
                            subrace.darkvision! > 0)) ...[
                      const SizedBox(height: 6),
                      Wrap(
                        spacing: 6,
                        runSpacing: 4,
  ...
  Widget _buildLoreNotesTab(BuildContext context, ThemeData theme, bool isDark,
      EntityFluff? importedFluff) {
    return ListView(
      padding: const EdgeInsets.all(16),
      children: [
