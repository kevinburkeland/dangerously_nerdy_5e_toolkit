...
class _CreatureDprViewState extends State<CreatureDprView> {
  ...
  void _applyTurnMode(CreatureCombatTurnMode mode) {
    _turnMode = mode;
    final rechargeActions = _attacks
        .where((a) => a.rechargeRoll != null && !a.isLegendaryAction)
        .toList();
    final regularTurnAttacks = _attacks
        .where((a) => a.rechargeRoll == null && !a.isLegendaryAction)
        .toList();

    if (mode == CreatureCombatTurnMode.rechargeBurst &&
        rechargeActions.isNotEmpty) {
      ...
      final resolved = widget.statBlock
          .extractDprAttacks()
          .where((a) => !a.isLegendaryAction)
          .toList();
  ...
  Widget build(BuildContext context) {
    ...
    final hasRecharge =
        _attacks.any((a) => a.rechargeRoll != null && !a.isLegendaryAction);
    ...
    for (final attack in _attacks) {
      ...
      if (!attack.isLegendaryAction && count > 0) {
        if (hasRecharge &&
            _turnMode == CreatureCombatTurnMode.amortized3Round &&
    ...
    if (hasRecharge && _turnMode == CreatureCombatTurnMode.amortized3Round) {
      final primaryRecharge = _attacks
          .firstWhere((a) => a.rechargeRoll != null && !a.isLegendaryAction);
      final freq =
          primaryRecharge.rechargeRoll == 5 ? (1.667 / 3.0) : (1.333 / 3.0);
      double multiDpr = 0.0;
                  ...
                  Wrap(
                    ...
                    children: [
                      ChoiceChip(
                        ...
                        onSelected: (sel) {
                          if (sel)
                            setState(() => _applyTurnMode(
                                CreatureCombatTurnMode.amortized3Round));
                        },
                      ),
                      ChoiceChip(
                        label: const Text('Breath Burst Turn'),
                        selected:
                            _turnMode == CreatureCombatTurnMode.rechargeBurst,
                        onSelected: (sel) {
                          if (sel)
                            setState(() => _applyTurnMode(
                                CreatureCombatTurnMode.rechargeBurst));
                        },
                      ),
                      ChoiceChip(
                        label: const Text('Multiattack Only'),
                        selected:
                            _turnMode == CreatureCombatTurnMode.multiattackOnly,
                        onSelected: (sel) {
                          if (sel)
                            setState(() => _applyTurnMode(
                                CreatureCombatTurnMode.multiattackOnly));
                        },
                      ),
                    ],
                  ),
  ...
  Widget _buildAttackActionRow(
    ...
    final hitChancePct =
        point != null ? (point.hitChance * 100).toStringAsFixed(0) : '0';
    final critChancePct =
        point != null ? (point.critChance * 100).toStringAsFixed(1) : '0.0';
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        if (attack.deliveryType ==
                            DprActionDeliveryType.savingThrow) ...[
                          Container(
