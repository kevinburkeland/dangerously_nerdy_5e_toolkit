...
class SquadBuilderBottomSheet extends StatefulWidget {
  ...
  @override
  State<SquadBuilderBottomSheet> createState() =>
      _SquadBuilderBottomSheetState();
}
...
class _SquadBuilderBottomSheetState extends State<SquadBuilderBottomSheet> {
  ...
  void _showFeedback(String message,
      {bool isError = false, Color? customColor, String? prefixIcon}) {
    ...
    final displayContent =
        prefixIcon != null ? '$prefixIcon $message' : message;
    final bgColor =
        customColor ?? (isError ? Colors.redAccent : const Color(0xFF3F2B96));
          ...
          children: [
            ...
            if (_selectedPreset.isRandomTable) ...[
              ElevatedButton.icon(
                ...
                label: const Text('🎲 Roll & Pull Random Animal from Bag',
                    style:
            ...
            if (_selectedPreset.id.startsWith('horn_of_valhalla') ||
                _selectedPreset.id == 'horn_of_valhalla') ...[
              Text(
              ...
              Wrap(
                ...
                runSpacing: 8,
                children: [
                  _buildChip('Silver (2d4+2)',
                      () => _rollHornOfValhalla('silver', 'Silver Horn')),
                  _buildChip('Brass (3d4+3)',
                      () => _rollHornOfValhalla('brass', 'Brass Horn')),
                  _buildChip('Bronze (4d4+4)',
                      () => _rollHornOfValhalla('bronze', 'Bronze Horn')),
                  _buildChip('Iron (5d4+5)',
                      () => _rollHornOfValhalla('iron', 'Iron Horn')),
                ],
              ),
              const SizedBox(height: 16),
            ],
            ...
            if (_selectedPreset.id == 'animate_objects') ...[
              Wrap(
                ...
                children: [
                  _buildChip(
                      '10x Tiny (Coins)',
                      () => _addPresetObjects(ObjectSize.tiny, 10,
                          customName: 'Animated Coin')),
                  _buildChip(
                      '5x Small (Daggers)',
                      () => _addPresetObjects(ObjectSize.small, 5,
                          customName: 'Animated Dagger')),
                  _buildChip(
                      '5x Medium (Swords)',
                      () => _addPresetObjects(ObjectSize.medium, 5,
                          customName: 'Animated Sword')),
                  _buildChip(
                      '2x Large (Statues)',
                      () => _addPresetObjects(ObjectSize.large, 2,
                          customName: 'Animated Statue')),
                  _buildChip(
                      '1x Huge (Boulders)',
                      () => _addPresetObjects(ObjectSize.huge, 1,
                          customName: 'Animated Boulder')),
                ],
              ),
            ] else ...[
              Wrap(
            ...
            Column(
              children: _selectedPreset.effectiveStatBlocks.map((sb) {
                final isHb = MonsterCodexLibrary.homebrewMonsters
                    .any((m) => m.id == sb.id);
                return Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(10),
                    border: Border.all(
                        color: sb.accentColor.withValues(alpha: 0.4)),
                  ),
                  child: Material(
