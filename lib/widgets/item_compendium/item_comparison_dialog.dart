            ...
            Row(
              children: [
                ...
                IconButton(
                  ...
                  tooltip:
                      _pinned ? 'Remove from Reliquary' : 'Pin to Reliquary',
            ...
            Expanded(
              child: SingleChildScrollView(
                child: showDualView
                    ...
                    : _buildSingleEditionView(
                        context, item, _activeEdition, isDark, rarityColor),
              ),
            ),
  ...
  Widget _buildSingleEditionView(
    ...
    return Column(
      ...
      children: [
        ...
        _buildStatRow(context, 'Crafting Cost',
            item.getCraftingDetails(edition).goldCostDisplay),
        _buildStatRow(
            context,
            'Crafting Time',
            edition == DmRulesEdition.v2024
                ? item.getCraftingDetails(edition).craftingTime2024Display
                : item.getCraftingDetails(edition).craftingTime2014Display),
        _buildStatRow(context, 'Primary Tool',
            item.getCraftingDetails(edition).primaryTool),
        _buildStatRow(
            context, 'Activation', rules.activation ?? 'Standard Action'),
        ...
        LayoutBuilder(
          builder: (context, constraints) {
            ...
            if (isNarrow) {
              return Column(
                children: [
                  _buildEditionColumn(context, item, DmRulesEdition.v2014,
                      '2014 RAW Rules', Colors.blueGrey, isDark),
                  const SizedBox(height: 16),
                  _buildEditionColumn(context, item, DmRulesEdition.v2024,
                      '2024 Revised Rules', Colors.amber, isDark),
                ],
              );
            }
            return Row(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildStatRow(
              context, 'Market Price', item.getEffectivePrice(edition)),
          _buildStatRow(context, 'Primary Tool',
              item.getCraftingDetails(edition).primaryTool),
          _buildStatRow(
              context, 'Activation', rules.activation ?? 'Standard Action'),
      ...
      child: Row(
        ...
        children: [
          ...
          Text(
            '${ring.ringType.displayName}: ',
            style: TextStyle(
                fontWeight: FontWeight.bold, fontSize: 12, color: ringColor),
          ),
