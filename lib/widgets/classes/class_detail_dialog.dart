...
class _ClassDetailDialogState extends State<ClassDetailDialog>
  ...
  void initState() {
    ...
    final existing =
        _fluffService.getUserNotes('class', widget.characterClass.id.slug) ??
            '';
  ...
  void _initSpells([DmRulesEdition? resolvedEdition]) {
    ...
    final spellClass = SpellClass.values
        .where(
          (sc) =>
              sc.name.toLowerCase() == slug ||
              sc.label.toLowerCase() == cls.name.toLowerCase().trim(),
        )
        .firstOrNull;
    ...
    for (final sub in cls.subclasses) {
      ...
      for (final g in sub.grants) {
        if (g.type == GrantType.bonusSpell) {
          final name = g.payload['displayName']?.toString() ??
              g.label ??
              g.payload['slug']?.toString();
          if (name != null && name.isNotEmpty) {
            bonusNames.add(name.toLowerCase());
          }
        }
      }
      final addSpells = sub.customProperties['additionalSpells'] ??
          sub.customProperties['subclassSpells'];
      if (addSpells != null) {
        final names = FeatureGrant.extractSpellNames(addSpells);
        bonusNames.addAll(names.map((e) => e.toLowerCase()));
      }
      bonusNames.addAll(
          SubclassSpellsLibrary.getExpandedSpells(cls.id.slug, sub.id.slug));
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (cls.primaryAbility != null)
            _buildMetaRow('Primary Ability', cls.primaryAbility!),
          if (cls.savingThrows.isNotEmpty)
            _buildMetaRow('Saving Throws', cls.savingThrows.join(', ')),
          if (cls.armorProficiencies.isNotEmpty)
            _buildMetaRow('Armor Training', cls.armorProficiencies.join(', ')),
          if (cls.weaponProficiencies.isNotEmpty)
            _buildMetaRow(
                'Weapon Training', cls.weaponProficiencies.join(', ')),
          if (cls.spellcastingAbility != null)
            _buildMetaRow(
                'Spellcasting', '${cls.spellcastingAbility} modifier'),
          _buildMetaRow(
            'Subclass Level',
  ...
  String _getSubclassLevelString(CharacterClass cls, DmRulesEdition edition) {
    final ruleset = edition == DmRulesEdition.v2024
  ...
  Widget build(BuildContext context) {
    ...
    final resolvedClassType =
        DndClassType.tryParse(slug) ?? DndClassType.tryParse(cls.name);
              ...
              children: [
                ...
                IconButton(
                  icon: Icon(
                    ...
                    color:
                        _pinned ? pinColor : theme.colorScheme.onSurfaceVariant,
                  ),
                  tooltip:
                      _pinned ? 'Remove from Bookmarks' : 'Pin to Bookmarks',
            ...
            TabBar(
              ...
              tabs: [
                ...
                if (cls.subclasses.isNotEmpty)
                  Tab(text: 'Subclasses (${cls.subclasses.length})'),
            ...
            Expanded(
              child: TabBarView(
                ...
                children: [
                  ...
                  if (cls.subclasses.isNotEmpty)
                    _buildSubclassesTab(context, accentColor),
        ...
        children: [
          ...
          if (pipeMatches.isNotEmpty)
            ...pipeMatches.map((m) {
              ...
              final fallback =
                  _wellKnownClassFeatureDescriptions[name.toLowerCase()];
              final desc = fallback ??
      ...
      children: [
        ...
        TextField(
          ...
          decoration: InputDecoration(
            hintText:
                'Add class build notes, multiclass synergy, favored spells...',
            hintStyle: TextStyle(
                fontSize: 12,
                color:
  ...
  Widget _buildSubclassSpellsChips(
      BuildContext context, Subclass sub, Color accentColor) {
    final theme = Theme.of(context);
    final isDivineSoul =
        widget.characterClass.id.slug.toLowerCase() == 'sorcerer' &&
            (sub.id.slug.contains('divine') ||
                sub.name.toLowerCase().contains('divine'));
    ...
    if (!isDivineSoul) {
      final libExpanded = SubclassSpellsLibrary.getExpandedSpells(
          widget.characterClass.id.slug, sub.id.slug);
    ...
    final sortedSpells = (isDivineSoul && directGrantNames.isNotEmpty
            ? directGrantNames
            : bonusSpellNames)
        .toList()
      ..sort();
          ...
          children: [
            Row(
              children: [
                ...
                Text(
                  isDivineSoul
                      ? 'DIVINE MAGIC (FULL CLERIC SPELL LIST)'
                  ...
                  onPressed: () {
                    if (foundSpell != null) {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(
                              '$spName — expanded spell granted by ${sub.name}. Full text available in spellbook.'),
    ...
    final filtered = _allClassSpells.where((s) {
      ...
      if (_spellLevelFilter != null) {
        if (_spellLevelFilter == -1) {
          if (!_spellToSubclassMap.containsKey(s.id.toLowerCase()))
            return false;
      ...
      children: [
        ...
        Container(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              hintText:
                  'Search ${widget.characterClass.name} spells, schools, or subclasses...',
              hintStyle: TextStyle(
                  fontSize: 12,
                  color: theme.colorScheme.onSurfaceVariant
            ...
            onChanged: (val) =>
                setState(() => _spellSearchQuery = val.trim().toLowerCase()),
          ),
        ),
        const SizedBox(height: 8),
            ...
            children: [
              ...
              if (_spellToSubclassMap.isNotEmpty) ...[
                FilterChip(
                  ...
                  onSelected: (_) => setState(() =>
                      _spellLevelFilter = _spellLevelFilter == -1 ? null : -1),
                ),
                const SizedBox(width: 6),
              ],
              for (final lvl in sortedLevels) ...[
                FilterChip(
                  ...
                  visualDensity: VisualDensity.compact,
                  onSelected: (_) => setState(() => _spellLevelFilter =
                      _spellLevelFilter == lvl ? null : lvl),
                ),
                const SizedBox(width: 6),
              ],
            ],
              ...
              : ListView.separated(
                  ...
                  itemBuilder: (context, idx) {
                    ...
                    final isPinned = SettingsScope.maybeOf(context)
                            ...
                            .contains(spell.id) ??
                        false;
                    final isHomebrew = spell.id.startsWith('custom_') ||
                        spell.tags.contains('homebrew');
