...
class _TreasureHoardViewState extends State<TreasureHoardView> {
  ...
  Future<void> _disperseCoinsToParty() async {
    ...
    final totalGemsAndArtGp =
        drop.gemstones.fold<double>(0.0, (sum, g) => sum + g.totalGp) +
    ...
    await DisperseLootDialog.show(
      ...
      sourceTitle: drop.isHoard
          ? 'Treasure Hoard (${drop.tierLabel})'
          : 'Monster Loot (${drop.tierLabel})',
    );
  }
  ...
  Future<void> _depositToCampaignVault() async {
    ...
    if (memberships.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text(
              'No campaign rooms found. Create or join a campaign room first!'),
    ...
    for (int i = 0; i < drop.magicItemNames.length; i++) {
      ...
      final item = PartyLootItem(
        ...
        count: 1,
        gpValue: 0.0,
        requiresAttunement: name.toLowerCase().contains('attunement') ||
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
              'Deposited treasure into "${target.campaignName}" (${target.roomCode})!'),
  ...
  Widget _buildResultCard(
      BuildContext context, TreasureDropResult drop, bool isDark) {
    return Container(
      decoration: BoxDecoration(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(
                        drop.isHoard
                            ? 'Treasure Hoard (${drop.tierLabel})'
                            : 'Individual Monster Drop (${drop.tierLabel})',
                    ...
                    children: drop.artObjects.map((art) {
                      return Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(8),
                          border: Border.all(
                              color: const Color(0xFFC084FC).withAlpha(90)),
                        ),
                        child: Text(
                ...
                if (drop.magicItemNames.isNotEmpty) ...[
                  ...
                  Column(
                    children: List.generate(drop.magicItemNames.length, (idx) {
                      ...
                      final itemId = idx < drop.magicItemIds.length
                          ? drop.magicItemIds[idx]
                          : null;
                          ...
                          children: [
                            ...
                            if (itemId != null)
                              IconButton(
                                ...
                                onPressed: () {
                                  final item =
                                      MagicItemLibrary.findById(itemId) ??
              ...
              children: [
                ...
                if (_liquidateGemsAndArt) ...[
                  ...
                  Wrap(
                    ...
                    runSpacing: 4,
                    children: [
                      if (shares.ppPerPlayer > 0)
                        Text('${shares.ppPerPlayer} PP',
                            style: const TextStyle(
                                fontWeight: FontWeight.bold, fontSize: 13)),
                      if (shares.gpPerPlayer > 0)
                        Text('${shares.gpPerPlayer} GP',
                            style: const TextStyle(
                                fontWeight: FontWeight.bold,
                                fontSize: 13,
                                color: Color(0xFFF59E0B))),
                      if (shares.epPerPlayer > 0)
                        Text('${shares.epPerPlayer} EP',
                            style: const TextStyle(
                                fontWeight: FontWeight.bold, fontSize: 13)),
                      if (shares.spPerPlayer > 0)
                        Text('${shares.spPerPlayer} SP',
                            style: const TextStyle(
                                fontWeight: FontWeight.bold, fontSize: 13)),
                      if (shares.cpPerPlayer > 0)
                        Text('${shares.cpPerPlayer} CP',
                            style: const TextStyle(
                                fontWeight: FontWeight.bold, fontSize: 13)),
                      if (shares.ppPerPlayer == 0 &&
                          shares.gpPerPlayer == 0 &&
                          shares.epPerPlayer == 0 &&
                          shares.spPerPlayer == 0 &&
                          shares.cpPerPlayer == 0)
                        const Text('0 Coins (Only items/gems in hoard)',
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: const Text('Disperse Coins to Characters...',
                        style: TextStyle(
