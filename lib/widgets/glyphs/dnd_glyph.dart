...
class DndGlyph extends StatefulWidget {
  ...
  final int
      tierLevel; // Spell Level (0-9), Monster CR Tier (1-4), Item Rarity (0-5), Feat Tier (1-4), Hit Die (6-12)
  ...
  factory DndGlyph.item({
    ...
    if (actionRings != null && actionRings.isNotEmpty) {
      ...
      if (requiresAttunement &&
          !rings.any((r) => r.ringType == ActionRingType.attunement)) {
        rings.insert(
          0,
          const ActionTraitRing(
  ...
  factory DndGlyph.classFeature({
    ...
    return DndGlyph._(
      ...
      classType: classType,
      themeData: effectiveTheme,
      tierLevel: classType.hitDieSides <= 6
          ? 1
          : (classType.hitDieSides <= 8
              ? 2
              : (classType.hitDieSides <= 10 ? 3 : 4)),
      actionRings: rings,
      size: size,
      isDarkMode: isDarkMode,
      isActive: isActive,
      onTap: onTap,
      tooltip: tooltip ??
          '${classType.displayName} (d${classType.hitDieSides}, ${classType.primaryResource})',
    );
  }
  ...
  factory DndGlyph.feat({
    ...
    final rings = actionRings ??
        [
          ActionTraitRing(
            ringType: category == FeatCategory.epicBoon
                ? ActionRingType.legendary
                : ActionRingType.passive,
  ...
  factory DndGlyph.species({
    ...
    return DndGlyph._(
      ...
      onTap: onTap,
      tooltip: tooltip ??
          '${speciesType.displayName} (${speciesType.size}, ${speciesType.speed} ft)',
    );
  }
  ...
  factory DndGlyph.fromRenderable(
    ...
    switch (item.glyphCategory) {
      ...
      case GlyphCategory.creature:
        final cType =
            meta['creatureType'] as CreatureType? ?? CreatureType.beast;
      ...
      case GlyphCategory.item:
        final cat =
            meta['category'] as ItemCategory? ?? ItemCategory.wondrousItem;
      ...
      case GlyphCategory.classFeature:
        final classType =
            meta['classType'] as DndClassType? ?? DndClassType.fighter;
      ...
      case GlyphCategory.feat:
        final featCat =
            meta['featCategory'] as FeatCategory? ?? FeatCategory.general;
      ...
      case GlyphCategory.genericUi:
        final uiType =
            meta['uiType'] as GenericUiGlyphType? ?? GenericUiGlyphType.d20;
...
class _DndGlyphState extends State<DndGlyph> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final isInteractiveFocused =
        _isFocused || (Focus.maybeOf(context)?.hasFocus ?? false);
    final isFocusedOrHovered = (canAnimateHover && _isHovered) ||
        (!reduceMotion && isInteractiveFocused);
...
class _DndHolographicWireframePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (animateMotifPulse) {
      ...
      final scanWave = 0.5 + 0.5 * sin(t * 2.0 * pi * 2.0);
