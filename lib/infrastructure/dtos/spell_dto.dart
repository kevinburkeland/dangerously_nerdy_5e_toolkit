...
class SpellDto {
  ...
  factory SpellDto.fromJson(Map<String, dynamic> json) {
    ...
    final resolvedLevel =
        (json['level'] is num ? (json['level'] as num).toInt() : 0).clamp(0, 9);
    ...
    if (json['duration'] is String) {
      ...
      resolvedDuration = durMap['rawText']?.toString() ??
          durMap['type']?.toString() ??
          'Instantaneous';
    }

    final resolvedRange = json['range']?.toString() ?? 'Self';
    final resolvedDescription = json['descriptionMarkdown']?.toString() ??
    ...
    if (json['components'] is Map) {
      resolvedComponents = SpellComponents.fromMap(
    ...
    if (json['damageMath'] is List) {
      for (final dm in json['damageMath'] as List) {
        if (dm is Map) {
          try {
            resolvedDamageMath
                .add(EvaluationMath.fromMap(Map<String, dynamic>.from(dm)));
          } catch (_) {}
        }
      }
    } else if (json['damageMath'] is Map) {
      try {
        resolvedDamageMath.add(EvaluationMath.fromMap(
            Map<String, dynamic>.from(json['damageMath'] as Map)));
      } catch (_) {}
    }
    ...
    final extractedScaling =
        HomebrewIngestor.extractHigherLevelsDice(resolvedHigherLevels);
      ...
      for (var i = 0; i < resolvedDamageMath.length; i++) {
        ...
        if (dm.scalingFormula == null || dm.scalingFormula!.isEmpty) {
          ...
          if (mentionsType || resolvedDamageMath.length == 1) {
            resolvedDamageMath[i] =
                dm.copyWith(scalingFormula: extractedScaling);
            applied = true;
          }
        }
      }
    ...
    if (json['relatedEntityRefs'] is List) {
      for (final ref in json['relatedEntityRefs'] as List) {
        if (ref is Map) {
          try {
            resolvedRefs
                .add(EntityReference.fromMap(Map<String, dynamic>.from(ref)));
          } catch (_) {}
        }
      }
    }
  ...
  factory SpellDto.fromDomain(Spell spell) {
    return SpellDto(
      ...
      level: spell.level,
      school: spell.school,
      castingTime: spell.castingTime.triggerCondition ??
          '${spell.castingTime.cost} ${spell.castingTime.actionType.name}',
      ...
      damageType: spell.damageType ??
          (spell.damageMath.isNotEmpty
              ? spell.damageMath.first.damageType.name
              : 'untyped'),
  ...
  Map<String, dynamic> toMap() {
    ...
    if (relatedEntityRefs.isNotEmpty) {
      map['relatedEntityRefs'] =
          relatedEntityRefs.map((r) => r.toMap()).toList();
    }
