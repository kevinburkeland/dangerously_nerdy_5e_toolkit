...
class ClassLevelPointerDto {
  ...
  factory ClassLevelPointerDto.fromMap(Map<String, dynamic> map) {
    ...
    return ClassLevelPointerDto(
      classSlug:
          (map['c'] ?? map['classSlug'] ?? '').toString().trim().toLowerCase(),
...
class CharacterTelemetryDto {
  ...
  factory CharacterTelemetryDto.fromMap(Map<String, dynamic> map) {
    ...
    if (map.containsKey('progression') ||
        map.containsKey('baseScores') ||
        map.containsKey('resources')) {
    ...
    final backgroundSlug = map['bg'] != null
        ? map['bg'].toString()
        : (map['backgroundSlug']?.toString());
    ...
    if (rawClasses is List) {
      for (final item in rawClasses) {
        if (item is Map) {
          classPointers.add(
              ClassLevelPointerDto.fromMap(Map<String, dynamic>.from(item)));
        }
      }
    }

    final currentHp =
        _asInt(map['hp'] ?? map['currentHp'], fallback: 10).clamp(0, 999);
    final maxHp =
        _asInt(map['mhp'] ?? map['maxHp'], fallback: 10).clamp(1, 999);
    final tempHp =
        _asInt(map['thp'] ?? map['tempHp'], fallback: 0).clamp(0, 999);
    final armorClass =
        _asInt(map['ac'] ?? map['armorClass'], fallback: 10).clamp(1, 50);
    final speed =
        _asInt(map['spd'] ?? map['speed'], fallback: 30).clamp(0, 300);
    final level = _asInt(map['lvl'] ?? map['level'], fallback: 1).clamp(1, 20);
    final passivePerception =
        _asInt(map['pp'] ?? map['passivePerception'], fallback: 10)
            .clamp(1, 35);
    final exhaustionLevel =
        _asInt(map['exh'] ?? map['exhaustionLevel'], fallback: 0).clamp(0, 6);
    final deathSaveSuccesses =
        _asInt(map['dss'] ?? map['deathSaveSuccesses'], fallback: 0)
            .clamp(0, 3);
    final deathSaveFailures =
        _asInt(map['dsf'] ?? map['deathSaveFailures'], fallback: 0).clamp(0, 3);
    ...
    final rulesEdition =
        (map['re'] ?? map['rulesEdition'] ?? 'v2024').toString();
    final timestamp = _asInt(map['ts'] ?? map['timestamp'],
  ...
  factory CharacterTelemetryDto.fromLegacyCharacterMap(
    ...
    final speciesSlug = (speciesRef is Map
                ? speciesRef['slug']
                : (speciesRef is String ? speciesRef : null))
            ?.toString() ??
        '';

    final bgRef = map['backgroundRef'] ?? map['background'];
    final backgroundSlug =
        (bgRef is Map ? bgRef['slug'] : (bgRef is String ? bgRef : null))
            ?.toString();

    final List<ClassLevelPointerDto> classPointers = [];
    int totalLevel = 0;
    final progression = map['progression'];
    final rawClasses = (progression is Map ? progression['classes'] : null) ??
        map['classes'] ??
        map['classPointers'];
    if (rawClasses is List) {
      for (final item in rawClasses) {
        if (item is Map) {
          ...
          final cSlug =
              (cRef is Map ? cRef['slug'] : (cRef is String ? cRef : ''))
                      ?.toString() ??
                  '';
          final scSlug =
              (scRef is Map ? scRef['slug'] : (scRef is String ? scRef : null))
                  ?.toString();
          final lvl =
              _asInt(item['level'] ?? item['l'], fallback: 1).clamp(1, 20);
          totalLevel += lvl;
    ...
    final resources =
        map['resources'] is Map ? map['resources'] as Map : const {};
    final currentHp = _asInt(
            resources['currentHp'] ?? map['currentHp'] ?? map['hp'],
            fallback: 10)
        .clamp(0, 999);
    final rawMaxHp =
        map['maxHp'] ?? map['mhp'] ?? resources['maxHp'] ?? resources['mhp'];
    final maxHp =
        _asInt(rawMaxHp, fallback: math.max(10, currentHp)).clamp(1, 999);
    final tempHp =
        _asInt(resources['tempHp'] ?? map['tempHp'] ?? map['thp'], fallback: 0)
            .clamp(0, 999);
    final armorClass = _asInt(
            resources['armorClassOverride'] ?? map['armorClass'] ?? map['ac'],
            fallback: 10)
        .clamp(1, 50);
    final exhaustionLevel = _asInt(
            resources['exhaustionLevel'] ??
                map['exhaustionLevel'] ??
                map['exh'],
            fallback: 0)
        .clamp(0, 6);

    final deathSaves = resources['deathSaves'] is Map
        ? resources['deathSaves'] as Map
        : const {};
    final deathSaveSuccesses = _asInt(
            resources['deathSaveSuccesses'] ??
                deathSaves['successes'] ??
                map['dss'],
            fallback: 0)
        .clamp(0, 3);
    final deathSaveFailures = _asInt(
            resources['deathSaveFailures'] ??
                deathSaves['failures'] ??
                map['dsf'],
            fallback: 0)
        .clamp(0, 3);
...
extension CharacterTelemetryDtoX on Character {
  ...
  CharacterTelemetryDto toTelemetryDto() {
    ...
    final wisMod = effectiveAbilityScores.getModifier('wisdom');
    final profMultiplier = getTraitProficiency('perception');
    final percBonus =
        profMultiplier > 0 ? (proficiencyBonus * profMultiplier).floor() : 0;
