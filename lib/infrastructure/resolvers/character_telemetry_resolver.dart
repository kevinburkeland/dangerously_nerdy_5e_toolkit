  ...
  static Future<ResolvedCharacterDisplay> resolve(
    ...
    if (spSlug.isNotEmpty) {
      ...
      if (srdSpecies != null) {
        ...
        final match = customRaces
            .where((r) => r.id.slug.toLowerCase() == spSlug)
            .firstOrNull;
        if (match != null) {
    ...
    if (bgSlug != null && bgSlug.isNotEmpty) {
      ...
      if (srdBg != null) {
        ...
        final match = customBgs
            .where((b) => b.id.slug.toLowerCase() == bgSlug)
            .firstOrNull;
        if (match != null) {
    ...
    for (final pointer in dto.classPointers) {
      ...
      if (srdClass != null) {
        ...
        final customClassMatch = customClasses
            .where((c) => c.id.slug.toLowerCase() == cSlug)
            .firstOrNull;
      ...
      String? subclassName;
      if (pointer.subclassSlug != null &&
          pointer.subclassSlug!.trim().isNotEmpty) {
        final scSlug = pointer.subclassSlug!.trim().toLowerCase();
        final matchedSub =
            SrdClassesLibrary.findSubclass(scSlug, classSlug: cSlug) ??
                customSubclasses
                    .where((s) =>
                        s.id.slug.toLowerCase() == scSlug ||
                        s.name.toLowerCase() == scSlug)
                    .firstOrNull;
        if (matchedSub != null) {
    ...
    final classSummary = classParts.isNotEmpty
        ? classParts.join(' / ')
        : 'Adventurer ${dto.level}';
    ...
    for (final fSlug in dto.featSlugs) {
      ...
      if (srdFeat != null) {
        ...
        final customFeatMatch = customFeats
            .where((f) => f.id.slug.toLowerCase() == clean)
            .firstOrNull;
    ...
    for (final eqSlug in dto.equippedItemSlugs) {
      ...
      final armorBase =
          cleanNormalized.replaceAll(RegExp(r'^armor_|_armor$'), '');
      final weaponBase =
          cleanNormalized.replaceAll(RegExp(r'^weapon_|_weapon$'), '');
      String? itemName;
      final magicItem = MagicItemLibrary.findById(clean) ??
          ...
          MagicItemLibrary.allItems.where((m) {
            ...
            final cleanMName =
                mName.replaceAll(RegExp(r'\s*\([^)]*\)'), '').trim();
            return mId == clean ||
                mId == cleanNormalized ||
                mName == clean ||
                cleanMName == clean ||
      ...
      if (magicItem != null) {
        ...
        if (eqMatch != null) {
          ...
          final customItemMatch = customItems
              .where((i) => i.id.slug.toLowerCase() == clean)
              .firstOrNull;
