...
class FirebaseSignalingAdapter {
  ...
  Map<String, Set<String>> get peerTrackedDocPaths => Map.unmodifiable(
      _peerTrackedDocPaths.map((k, v) => MapEntry(k, Set.unmodifiable(v))));
  ...
  Future<void> initialize({
    ...
    final pruningThreshold =
        DateTime.now().millisecondsSinceEpoch - slidingTtlMs;
    ...
    if (isFirebaseAvailable) {
      ...
      void handleSnapshot(QuerySnapshot<Map<String, dynamic>> snapshot) {
        ...
        for (final change in snapshot.docChanges) {
          if (change.type == DocumentChangeType.added) {
            ...
            if (data != null) {
              final message =
                  SignalingMessage.fromMap(data, docId: change.doc.id);
      ...
      _subscriptions.add(
          localMailbox.snapshots().listen(handleSnapshot, onError: (_) {}));
      _subscriptions.add(
          broadcastMailbox.snapshots().listen(handleSnapshot, onError: (_) {}));
  ...
  Future<String> _sendSignal({
    ...
    if (_roomCode == null || _localNodeId == null) {
      throw StateError(
          'FirebaseSignalingAdapter must be initialized before sending signals.');
    }
  ...
  void emitIncomingSignal(SignalingMessage message) {
    ...
    final path =
        'rooms/${message.roomCode}/nodes/${message.toNodeId}/signals/${message.id}';
  ...
  Future<void> _deletePath(String path, String docId) async {
    ...
    try {
      ...
      final trackedEntries = List<MapEntry<String, Set<String>>>.from(
          _peerTrackedDocPaths.entries);
      for (final entry in trackedEntries) {
        entry.value.remove(path);
      }
      _peerTrackedDocPaths.removeWhere((_, set) => set.isEmpty);

      final authoredEntries = List<MapEntry<String, Set<String>>>.from(
          _peerAuthoredDocPaths.entries);
