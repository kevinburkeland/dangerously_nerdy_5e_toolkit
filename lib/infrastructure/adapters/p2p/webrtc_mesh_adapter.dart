...
class WebRtcMeshAdapter implements IP2pTransportPort {
  ...
  WebRtcMeshAdapter({
    ...
    IRtcPeerConnectionFactory connectionFactory =
        const DefaultRtcPeerConnectionFactory(),
  ...
  @override
  TransportState get currentState => _isDisposed
      ? TransportState.offline
      : (_dataChannels.isNotEmpty
          ? TransportState.webRtc
  ...
  Future<void> broadcastPayload(String jsonPayload) async {
    ...
    if (errors.length == _dataChannels.length) {
      throw StateError(
          'Failed to broadcast payload to any WebRTC peer: $errors');
    }
  }
