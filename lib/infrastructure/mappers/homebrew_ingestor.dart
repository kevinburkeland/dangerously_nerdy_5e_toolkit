...
class HomebrewIngestor {
  ...
  static Map<String, dynamic> normalizeRange(dynamic rawRange,
      [String? description]) {
    if (rawRange == null) {
    ...
    if (rawRange is Map) {
      ...
      if (lowerRange.contains('touch')) {
        ...
        type = 'cone';
      } else if (lowerRange.contains('radius') ||
          lowerRange.contains('sphere')) {
        type = 'radius';
    ...
    if (description != null && description.isNotEmpty) {
      ...
      if (type == 'ranged' || type == 'self') {
        if (descLower.contains('line of') ||
            RegExp(r'\b\d+[- ]foot line\b').hasMatch(descLower)) {
          type = 'line';
          if (distance == 0) {
            final m = RegExp(r'(\d+)[- ]foot line').firstMatch(descLower);
            if (m != null) distance = int.tryParse(m.group(1) ?? '0') ?? 0;
          }
        } else if (descLower.contains('cone of') ||
            RegExp(r'\b\d+[- ]foot cone\b').hasMatch(descLower)) {
          type = 'cone';
          if (distance == 0) {
          ...
          type = 'radius';
          if (distance == 0) {
            final m = RegExp(r'(\d+)[- ]foot (?:radius|sphere)')
                .firstMatch(descLower);
            if (m != null) distance = int.tryParse(m.group(1) ?? '0') ?? 0;
          }
        }
      }
  ...
  static List<EvaluationMath> correctDamageTypesFromDescription(
    ...
    return damageMath.map((dm) {
      ...
      if (formulaToType.containsKey(cleanFormula)) {
        ...
        if (dm.damageType != expectedType &&
            dm.damageType != DamageType.variable) {
  ...
  static String resolveDamageType(
      String rawDescription, String firstMatchedType) {
  ...
  static String? extractHigherLevelsDice(String? higherLevelsMarkdown) {
    if (higherLevelsMarkdown == null || higherLevelsMarkdown.isEmpty)
      return null;
  ...
  static List<EvaluationMath> enrichDamageDelivery(
    ...
    final alreadyAllFlagged =
        damageMath.every((dm) => dm.isAttackRoll || dm.requiresSave);
    ...
    if (damageMath.length >= 2 && hasAttack && hasSave) {
      ...
      return damageMath.map((dm) {
        ...
        if (itemIdx != -1) {
          final distToAttack =
              attackIdx != -1 ? (itemIdx - attackIdx).abs() : 999999;
    ...
    if (hasAttack && !hasSave) {
      return damageMath
          .map((dm) => dm.isAttackRoll || dm.requiresSave
              ? dm
              : dm.copyWith(isAttackRoll: true))
          .toList();
    } else if (hasSave && !hasAttack) {
      return damageMath
          .map((dm) => dm.isAttackRoll || dm.requiresSave
              ? dm
              : dm.copyWith(requiresSave: true))
          .toList();
    }
    ...
    for (final item in rawList) {
      ...
      try {
        ...
        LoggingService().logNonFatal(
          ...
          st,
          reason:
              'Failed to ingest homebrew character ${map['name']}. Skipping.',
        );
      }
    }
  ...
  static List<AnimatedObjectDto> parseCustomAnimatedObjects(
    ...
    for (final item in rawList) {
      ...
      try {
        ...
        LoggingService().logNonFatal(
          ...
          st,
          reason:
              'Failed to ingest homebrew animated object ${map['name']}. Skipping.',
        );
      }
    }
  ...
  static Race mapRaceFromDto(HomebrewEntityDto dto) {
    ...
    int? flexBonus;
    final flexData =
        normalized['flexibleAbilities'] ?? raw['flexibleAbilities'];
    if (flexData is Map) {
      ...
      for (final rawSub in rawSubList) {
        if (rawSub is Map) {
          ...
          if (subName.isNotEmpty) {
            final subTraits =
                subMap['entries'] ?? subMap['traits'] ?? subMap['desc'] ?? '';
            ...
            if (subFlexData is Map) {
              if (subFlexData['count'] is num)
                subFlexCount = (subFlexData['count'] as num).toInt();
              if (subFlexData['amount'] is num)
                subFlexBonus = (subFlexData['amount'] as num).toInt();
              if (subFlexData['from'] is List) {
                subFlexPool = (subFlexData['from'] as List)
            ...
            if (subRawAb is List) {
              for (final item in subRawAb) {
                if (item is Map &&
                    item.containsKey('choose') &&
                    item['choose'] is Map) {
                  final ch = item['choose'] as Map;
                  if (ch['count'] is num)
                    subFlexCount = (ch['count'] as num).toInt();
                  if (ch['amount'] is num)
                    subFlexBonus = (ch['amount'] as num).toInt();
                  if (ch['from'] is List) {
                    subFlexPool = (ch['from'] as List)
                        .map((e) => e.toString().toLowerCase().trim())
                        .toList();
                  }
                }
    ...
    final abilitySummary =
        summaryParts.isNotEmpty ? summaryParts.join(', ') : null;
  ...
  static Background mapBackgroundFromDto(HomebrewEntityDto dto) {
    ...
    if (rawSkills is List) {
      ...
      skillList.addAll(
          rawSkills.split(',').map((s) => s.trim()).where((s) => s.isNotEmpty));
    }
    ...
    if (rawTools is List) {
      ...
      toolList.addAll(
          rawTools.split(',').map((t) => t.trim()).where((t) => t.isNotEmpty));
    }

    // 3. Languages
    final langList = <String>[];
    final rawLangs =
        normalized['languages'] ?? raw['languages'] ?? raw['language'];
    if (rawLangs is List) {
      ...
      langList.addAll(
          rawLangs.split(',').map((l) => l.trim()).where((l) => l.isNotEmpty));
    }
    ...
    for (final item in rawList) {
      ...
      try {
        ...
        LoggingService().logNonFatal(
          ...
          st,
          reason:
              'Failed to ingest homebrew background ${map['name']}. Skipping.',
        );
      }
    }
  ...
  static CharacterClass mapClassFromDto(HomebrewEntityDto dto) {
    ...
    final rawSaves =
        raw['savingThrows'] ?? raw['proficiency'] ?? raw['proficiencies'];
    if (rawSaves is List) {
    ...
    if (sp is Map) {
      if (sp['armor'] is List) {
        armorProficiencies
            .addAll((sp['armor'] as List).map((e) => e.toString().trim()));
      }
      if (sp['weapons'] is List) {
        weaponProficiencies
            .addAll((sp['weapons'] as List).map((e) => e.toString().trim()));
      }
    }
