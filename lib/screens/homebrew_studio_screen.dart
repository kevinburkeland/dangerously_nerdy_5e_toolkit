...
class _HomebrewStudioScreenState extends State<HomebrewStudioScreen>
  ...
  Future<void> _openBulkDeleterDialog() async {
    ...
    if (deleted != null && deleted > 0) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('Successfully deleted $deleted custom entities.')),
        );
      }
    }
  }
  ...
  Future<void> _deleteSelectedInActiveTab() async {
    ...
    if (confirmed == true && mounted) {
      HapticService.selectionTick(context);
      await _persistence.deleteCustomEntitiesBatch(
          type, _selectedSlugs.toList());
      _selectedSlugs.clear();
  ...
  Future<void> _toggleOtherEntryEnabled(
      HomebrewCompendiumEntry entry, bool isEnabled) async {
    await _persistence.toggleOtherEntryEnabled(entry.id.slug,
        isEnabled: isEnabled);
    await _loadAll();
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _openImportDialog() async {
    final count = await showDialog<int>(
      context: context,
      builder: (ctx) =>
          HomebrewImportPreviewDialog(useIsolate: widget.useIsolate),
    );
    ...
    return Scaffold(
        ...
        bottom: TabBar(
          ...
          tabs: [
            ...
            Tab(
                ...
                text: 'Classes (${_classes.length + _subclasses.length})'),
            Tab(
                icon: const Icon(Icons.person),
                text: 'Races (${_races.length})'),
            Tab(
                icon: const Icon(Icons.stars),
                text: 'Feats (${_feats.length})'),
            Tab(
                icon: const Icon(Icons.menu_book),
                text: 'Backgrounds (${_backgrounds.length})'),
            Tab(
                icon: const Icon(Icons.category_outlined),
                text: 'Codex & Rules (${_otherEntries.length})'),
          ],
        ),
      ),
      body: _isLoading
      ...
      floatingActionButton: _isSelectionMode
          ...
          : FloatingActionButton.extended(
              ...
              icon:
                  Icon(_tabController.index <= 2 ? Icons.add : Icons.download),
              label: Text(
  ...
  Widget _buildSpellsTab(ThemeData theme) {
    ...
    return Column(
      children: [
        _buildTabToolbar(
            totalCount: _spells.length, allSlugs: slugs, theme: theme),
        Expanded(
  ...
  Widget _buildMonstersTab(ThemeData theme) {
    ...
    return Column(
      children: [
        _buildTabToolbar(
            totalCount: _monsters.length, allSlugs: slugs, theme: theme),
        Expanded(
          child: ListView.builder(
            padding: const EdgeInsets.all(16),
  ...
  Widget _buildItemsTab(ThemeData theme) {
    ...
    return Column(
      children: [
        _buildTabToolbar(
            totalCount: _items.length, allSlugs: slugs, theme: theme),
        Expanded(
          child: ListView.builder(
            padding: const EdgeInsets.all(16),
  ...
  Widget _buildClassesTab(ThemeData theme) {
    ...
    return Column(
      children: [
        _buildTabToolbar(
            totalCount: combinedCount, allSlugs: allSlugs, theme: theme),
        Expanded(
          child: ListView(
            padding: const EdgeInsets.all(16),
            children: [
                    ...
                    child: ListTile(
                      onTap: _isSelectionMode
                          ...
                          : () => _showDetailModal(
                                ...
                                metadataLines: [
                                  'Hit Die: ${cl.hitDie}',
                                  if (cl.savingThrows.isNotEmpty)
                                    'Saving Throws: ${cl.savingThrows.join(", ")}',
                                  if (cl.primaryAbility != null)
                                    'Primary Ability: ${cl.primaryAbility}',
                                ],
                              ),
                      leading: _isSelectionMode
                          ? Checkbox(
                      ...
                      title: Text(cl.name,
                          style: const TextStyle(fontWeight: FontWeight.bold)),
                      subtitle: Text(
                          'Hit Die: ${cl.hitDie} • Primary: ${cl.primaryAbility ?? "Custom"} • Subclasses: ${cl.subclasses.length}'),
                      trailing: _isSelectionMode
                          ? null
                          : IconButton(
                              icon: const Icon(Icons.delete_outline,
              ...
              if (_subclasses.isNotEmpty) ...[
                ...
                Text('Subclasses (${_subclasses.length})',
                    style: theme.textTheme.titleMedium
                        ?.copyWith(fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                  ...
                  Card(
                    ...
                    child: ListTile(
                      ...
                      title: Text(sub.name,
                          style: const TextStyle(fontWeight: FontWeight.bold)),
                      subtitle:
                          Text('Class: ${sub.classSlug} • ${sub.shortName}'),
                      trailing: _isSelectionMode
                          ? null
                          : IconButton(
                              icon: const Icon(Icons.delete_outline,
  ...
  Widget _buildRacesTab(ThemeData theme) {
    ...
    return Column(
      children: [
        _buildTabToolbar(
            totalCount: _races.length, allSlugs: slugs, theme: theme),
        Expanded(
          child: ListView.builder(
            padding: const EdgeInsets.all(16),
              ...
              return Card(
                ...
                child: ListTile(
                  onTap: _isSelectionMode
                      ...
                      : () => _showDetailModal(
                            ...
                            metadataLines: [
                              'Size: ${race.size}',
                              'Speed: ${race.speed}',
                              if (race.abilityScoreSummary != null)
                                'Abilities: ${race.abilityScoreSummary}',
                            ],
                          ),
                  leading: _isSelectionMode
                      ? Checkbox(
                  ...
                  title: Text(race.name,
                      style: const TextStyle(fontWeight: FontWeight.bold)),
                  subtitle: Text(
                      '${race.size} • Speed: ${race.speed} • Subraces: ${race.subraces.length}'),
                  trailing: _isSelectionMode
                      ? null
                      : IconButton(
                          icon: const Icon(Icons.delete_outline,
  ...
  Widget _buildFeatsTab(ThemeData theme) {
    ...
    return Column(
      children: [
        _buildTabToolbar(
            totalCount: _feats.length, allSlugs: slugs, theme: theme),
        Expanded(
          child: ListView.builder(
            padding: const EdgeInsets.all(16),
              ...
              return Card(
                ...
                child: ListTile(
                  ...
                  title: Text(feat.name,
                      style: const TextStyle(fontWeight: FontWeight.bold)),
                  subtitle: Text(
                      '${feat.category}${feat.prerequisite != null ? " • Prereq: ${feat.prerequisite}" : ""}'),
                  trailing: _isSelectionMode
                      ? null
                      : IconButton(
                          icon: const Icon(Icons.delete_outline,
  ...
  Widget _buildBackgroundsTab(ThemeData theme) {
    ...
    return Column(
      children: [
        _buildTabToolbar(
            totalCount: _backgrounds.length, allSlugs: slugs, theme: theme),
        Expanded(
          child: ListView.builder(
            padding: const EdgeInsets.all(16),
              ...
              return Card(
                ...
                child: ListTile(
                  ...
                  title: Text(bg.name,
                      style: const TextStyle(fontWeight: FontWeight.bold)),
                  subtitle: Text(
                      'Skills: ${bg.skillProficiencies.join(", ")}${bg.originFeat != null ? " • Feat: ${bg.originFeat}" : ""}'),
                  trailing: _isSelectionMode
                      ? null
                      : IconButton(
                          icon: const Icon(Icons.delete_outline,
  ...
  Widget _buildOtherEntriesTab(ThemeData theme) {
    if (_otherEntries.isEmpty) {
      return _buildEmptyState(
        ...
        subtitle:
            'Import custom Deities, Tables, Vehicles, Traps, Invocations, or rules via the JSON Importer.',
        onAction: _openImportDialog,
      );
    }
    ...
    final filtered = _otherEntries.where((e) {
      if (_selectedStatusFilter != null &&
          e.isEnabled != _selectedStatusFilter) {
        return false;
      }
    ...
    return Column(
      children: [
            ...
            children: [
              ...
              if (disabledCount > 0) ...[
                ...
                FilterChip(
                  ...
                  onSelected: (selected) {
                    setState(
                        () => _selectedStatusFilter = selected ? true : null);
                  },
                ),
                const SizedBox(width: 8),
                FilterChip(
                  ...
                  onSelected: (selected) {
                    setState(
                        () => _selectedStatusFilter = selected ? false : null);
                  },
                ),
              ],
        ...
        _buildTabToolbar(
            totalCount: filtered.length, allSlugs: slugs, theme: theme),
        Expanded(
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (ctx, idx) {
                    ...
                    if (cat == HomebrewOtherCategory.deities) {
                      final pantheon =
                          entry.customProperties['pantheon']?.toString();
                      if (pantheon != null && pantheon.isNotEmpty) {
                        sub += ' • $pantheon';
                      }
                    } else if (cat == HomebrewOtherCategory.vehicles) {
                      final vType =
                          entry.customProperties['vehicleType']?.toString();
                      if (vType != null && vType.isNotEmpty) {
                        sub += ' • $vType';
                      }
                    } else if (cat == HomebrewOtherCategory.trapsAndHazards) {
                      final tType = (entry.customProperties['trapType'] ??
                              entry.customProperties['hazardType'])
                          ?.toString();
                      ...
                      child: ListTile(
                        onTap: _isSelectionMode
                            ...
                            : () => _showDetailModal(
                                  ...
                                  onToggleEnabled: (val) =>
                                      _toggleOtherEntryEnabled(entry, val),
                                ),
                        leading: _isSelectionMode
                            ? Checkbox(
                                value: isSelected,
                            ...
                            : Row(
                                ...
                                children: [
                                  ...
                                  IconButton(
                                    ...
                                    onPressed: () =>
                                        _deleteOtherEntry(entry.id.slug),
                                  ),
                                ],
                              ),
                      ),
