...
class _MonsterCodexScreenState extends State<MonsterCodexScreen> {
  ...
  void _openFilterSheet(BuildContext context) {
    MonsterFilterSheet.show(
      ...
      onSpellSummonsToggled: (val) =>
          setState(() => _showOnlySpellSummons = val),
      onMagicItemsToggled: (val) => setState(() => _showOnlyMagicItems = val),
      onMultiattackToggled: (val) => setState(() => _showOnlyMultiattack = val),
      onSpellcastersToggled: (val) =>
          setState(() => _showOnlySpellcasters = val),
  ...
  Widget build(BuildContext context) {
    ...
    final filteredMonsters = allMonsters.where((monster) {
      ...
      if (_viewMode == MonsterCodexViewMode.homebrew && !monster.isHomebrew) {
        return false;
      }
      ...
      if (_showOnlyMultiattack &&
          !statBlock.actions
              .any((a) => a.name.toLowerCase().contains('multiattack'))) {
        return false;
      }
      if (_showOnlySpellcasters &&
          !statBlock.traits
              .any((t) => t.name.toLowerCase().contains('spellcasting'))) {
        return false;
      }
      ...
      if (_showOnlyResistances &&
          (statBlock.damageResistances == null ||
              statBlock.damageResistances!.isEmpty) &&
          (statBlock.damageImmunities == null ||
              statBlock.damageImmunities!.isEmpty)) {
        return false;
      }
  ...
  List<Widget> _buildGroupedSlivers(
    ...
    switch (_sortMode) {
      case MonsterSortMode.crAscending:
        return _buildGroupedCrSlivers(context, monsters, edition, pinnedIds,
            ascending: true);
      ...
      case MonsterSortMode.nameAscending:
        return _buildGroupedAlphabeticalSlivers(
            context, monsters, edition, pinnedIds);
    }
  }
  ...
  List<Widget> _buildGroupedDprSlivers(
    ...
    final tiers = <({String title, Color color, List<MonsterItem> items})>[
      (
        ...
        items: sorted
            .where((m) => m.calculateBaselineDpr(edition) >= 40.0)
            .toList(),
      ),
      (
      ...
      (
        ...
        items: sorted
            .where((m) => m.calculateBaselineDpr(edition) < 10.0)
            .toList(),
      ),
    ];
    ...
    for (int i = 0; i < tiers.length; i++) {
      ...
      slivers.add(
          _buildSliverMonsterCards(context, tier.items, edition, pinnedIds));
    }
  ...
  List<Widget> _buildGroupedAlphabeticalSlivers(
    ...
    for (int i = 0; i < letters.length; i++) {
      ...
      slivers.add(_buildSliverMonsterCards(
          context, letterMonsters, edition, pinnedIds));
    }
