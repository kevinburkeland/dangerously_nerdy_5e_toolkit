...
class _DiceRollerScreenState extends State<DiceRollerScreen> {
  ...
  Future<void> _showCustomDieDialog() async {
    HapticService.selectionTick(context);
    final sides = await ValueInputDialog.showCustomDie(context,
        initialSides: _customSides);
    if (sides != null) {
      setState(() {
        _customSides = sides;
  ...
  void _rollDice() {
    ...
    final currentPlayerName = (_roomService.playerName != null &&
            _roomService.playerName!.trim().isNotEmpty)
  ...
  Future<void> _showSavePresetDialog() async {
    ...
    if (_dicePool.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Cannot save preset with an empty dice pool.')),
      );
      return;
    }
    final name = await SavePresetDialog.show(context,
        formulaText: _currentFormulaString);
    if (name != null && name.isNotEmpty && mounted) {
      final messenger = ScaffoldMessenger.of(context);
      final uniqueId =
          '${DateTime.now().microsecondsSinceEpoch}_${secureRandom.nextInt(1000000)}';
      ...
      try {
        ...
        if (mounted) {
          messenger.showSnackBar(
            const SnackBar(
              content: Text(
                  'Failed to import presets. Please verify the JSON payload format.'),
          ...
          Scaffold(
            appBar: AppBar(
              ...
              actions: [
                IconButton(
                  icon: Icon(
                    settings.enable3dDiceOverlays
                        ? Icons.auto_awesome_motion
                        : Icons.auto_awesome_motion_outlined,
                    color: settings.enable3dDiceOverlays
                        ? theme.colorScheme.primary
                        : theme.colorScheme.onSurface.withValues(alpha: 0.5),
                  ),
                  tooltip: settings.enable3dDiceOverlays
                      ? 'Animated Dice: Enabled'
                      : 'Animated Dice: Disabled',
                  onPressed: () {
                    HapticService.selectionTick(context);
                    settingsProvider
                        .set3dDiceOverlays(!settings.enable3dDiceOverlays);
                  },
                ),
                ...
                PopupMenuButton<String>(
                  ...
                  onSelected: (val) {
                    ...
                    if (val == 'conditions')
                      ConditionReferenceDialog.show(context);
                  },
                  itemBuilder: (ctx) => [
                    const PopupMenuItem(
                      value: 'economy',
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      DicePoolBuilder(
                        ...
                        onUpdateDicePool: (pool) =>
                            setState(() => _dicePool = pool),
                        onUpdateModifier: (mod) =>
                            setState(() => _modifier = mod),
                        onUpdateRollMode: (mode) =>
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              _buildThumbModeButton(Icons.shield_outlined,
                                  RollMode.normal, 'Normal',
                                  theme: theme),
                              _buildThumbModeButton(Icons.arrow_upward,
                                  RollMode.advantage, 'Advantage',
                                  color: Colors.greenAccent, theme: theme),
                              _buildThumbModeButton(Icons.arrow_downward,
          ...
          if (_showAnimatedRoll && _animatingResult != null)
            Positioned.fill(
              child: AnimatedDiceRollOverlay(
                key: ValueKey(
                    'dice_anim_${_animatingResult!.timestamp.microsecondsSinceEpoch}'),
  ...
  Widget _buildThumbModeButton(IconData icon, RollMode mode, String tooltip,
