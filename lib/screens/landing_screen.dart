...
class _LandingScreenState extends State<LandingScreen> {
                ...
                children: [
                  ...
                  Center(
                    child: Column(
                      children: [
                        Text(
                          'DangerouslyNerdy 5e Toolkit • 5E / SRD-Compatible Combat System',
                          style: TextStyle(
                            color: theme.colorScheme.onSurface
                                .withValues(alpha: 0.5),
                            fontSize: 12,
  ...
  Widget _buildToolCardFromItem(BuildContext context, LandingToolItem item) {
    ...
    if (glyphWidget == null) {
      final summonPreset =
          SrdSummonsLibrary.allPresets.cast<SummonPreset?>().firstWhere(
                (p) => p?.id == item.id,
                orElse: () => null,
              );
      ...
      switch (item.id) {
        ...
        case 'character_builder':
          glyphWidget = DndGlyph.classFeature(
            ...
            actionRings: const [
              ActionTraitRing(
                  ringType: ActionRingType.hitDie, label: 'd10 Hit Die'),
              ActionTraitRing(
                  ringType: ActionRingType.resource, label: 'Second Wind'),
            ],
            size: 42,
            isDarkMode: isDark,
          );
        ...
        case 'dm_dashboard':
          glyphWidget = DndGlyph.genericUi(
            ...
            actionRings: const [
              ActionTraitRing(
                  ringType: ActionRingType.legendary, label: 'Tactical DM HUD'),
              ActionTraitRing(
        ...
        case 'party_room_vault':
          glyphWidget = DndGlyph.species(
            ...
            actionRings: const [
              ActionTraitRing(
                  ringType: ActionRingType.passive, label: 'Party Sync'),
            ],
            size: 42,
            isDarkMode: isDark,
          );
        ...
        case 'monster_arena':
          glyphWidget = DndGlyph.monster(
            ...
            actionRings: const [
              ActionTraitRing(
                  ringType: ActionRingType.legendary,
                  damageType: DamageAccent.slashing,
                  label: 'Pit Fight Simulator'),
              ActionTraitRing(
                  ringType: ActionRingType.recharge,
                  label: 'Monte Carlo Win Odds'),
            ],
            size: 42,
            isDarkMode: isDark,
          );
  ...
  Widget _buildCampaignCard(
      BuildContext context, CampaignMembership m, bool isDark) {
    final theme = Theme.of(context);
    final colorScheme = theme.colorScheme;
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: PopupMenuButton<String>(
                      ...
                      onSelected: (val) =>
                          _handleCampaignOption(context, m, val),
                      itemBuilder: (ctx) => [
                        const PopupMenuItem(
                          value: 'open',
                          child: Row(
                        ...
                        Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (_) => DmDashboardScreen(
                                initialCampaignId: 'campaign_${m.roomCode}'),
                          ),
                        );
                      },
  ...
  Future<void> _handleCampaignOption(
      BuildContext context, CampaignMembership m, String option) async {
    HapticService.selectionTick(context);
    switch (option) {
      case 'open':
        Navigator.push(
      ...
      case 'dmScreen':
        Navigator.push(
          ...
          MaterialPageRoute(
            builder: (_) =>
                DmDashboardScreen(initialCampaignId: 'campaign_${m.roomCode}'),
          ),
        );
      case 'shareKey':
        ShareDmPasskeyDialog.show(context, m);
          ...
          try {
            ...
            if (context.mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                    content: Text(
                        'Campaign "${m.campaignName}" permanently deleted')),
              );
            }
          } catch (e) {
            if (context.mounted) {
