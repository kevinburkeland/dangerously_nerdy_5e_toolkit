...
/// Standalone Magic Item Compendium — searchable, filterable browser of SRD 5.1 & 5.2.1
...
class _ItemCompendiumScreenState extends State<ItemCompendiumScreen> {
  ...
  void _openFilterSheet(BuildContext context) {
    ItemFilterSheet.show(
      ...
      onDamageAccentChanged: (dmg) =>
          setState(() => _selectedDamageAccent = dmg),
  ...
  void _showItemComparison(
      MagicItem item, DmRulesEdition edition, bool isPinned) {
  ...
  Widget _buildSliverItemCards(
    ...
    return SliverResponsiveCardGrid<MagicItem>(
      ...
      itemBuilder: (context, item) => ItemCard(
        ...
        onTogglePin: () => _togglePinItem(context, item.id),
        onTap: () =>
            _showItemDetails(item, edition, pinnedIds.contains(item.id)),
        onCompare: () =>
            _showItemComparison(item, edition, pinnedIds.contains(item.id)),
      ),
    );
  }
  ...
  Widget _buildEmptyState(ThemeData theme, bool isReliquaryEmpty) {
    return EmptyStateCard(
      ...
      actionLabel:
          isReliquaryEmpty ? 'Browse All Magic Items' : 'Reset All Filters',
