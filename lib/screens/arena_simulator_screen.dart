...
class _ArenaSimulatorScreenState extends State<ArenaSimulatorScreen> {
  ...
  void didChangeDependencies() {
    ...
    if (!_hasLoadedInitialPreset) {
      ...
      _loadPreset(
          ArenaPresetMatchup.defaultPresets.first, _resolveEdition(context));
    }
  }
  ...
  void _addMonsterToTeam(MonsterItem monster, int count, ArenaTeam team,
    ...
    setState(() {
      final targetList = team == ArenaTeam.teamA ? _teamA : _teamB;
      final existingCount =
          targetList.where((c) => c.monster.id == monster.id).length;
  ...
  void _startBattle() {
    if (_teamA.isEmpty || _teamB.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Both teams must have at least 1 monster to fight!')),
      );
      return;
    }
  ...
  void _togglePlayPause() {
    if (_status == ArenaSimulationStatus.playing) {
      ...
      _startPlaybackTimer();
    } else if (_status == ArenaSimulationStatus.setup ||
  ...
  void _advanceSingleStep() {
    ...
    final livingTeamA = _activeCombatants
        .where((c) => c.team == ArenaTeam.teamA && c.isAlive)
        .length;
    final livingTeamB = _activeCombatants
    ...
    final target =
        _engine.selectTarget(nextAttacker, _activeCombatants, _strategy);
    ...
    final postA = _activeCombatants
        .where((c) => c.team == ArenaTeam.teamA && c.isAlive)
        .length;
    final postB = _activeCombatants
        .where((c) => c.team == ArenaTeam.teamB && c.isAlive)
        .length;
  ...
  void _concludeBattle() {
    ...
    final livingA = _activeCombatants
        .where((c) => c.team == ArenaTeam.teamA && c.isAlive)
        .length;
    final livingB = _activeCombatants
        .where((c) => c.team == ArenaTeam.teamB && c.isAlive)
        .length;
    ...
    final result = ArenaSimulationResult(
      ...
      teamAKills: _activeCombatants
          .where((c) => c.team == ArenaTeam.teamA)
          .fold(0, (sum, c) => sum + c.kills),
      teamBKills: _activeCombatants
          .where((c) => c.team == ArenaTeam.teamB)
          .fold(0, (sum, c) => sum + c.kills),
    );
          ...
          children: [
            ...
            if (result.mvpCombatant != null) ...[
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border:
                      Border.all(color: const Color(0xFFFFD700).withAlpha(100)),
                ),
                child: Row(
                  children: [
            ...
            Row(
              ...
              children: [
                ...
                Text(
                    '${result.teamATotalDamage} dmg (${result.teamAKills} kills)'),
              ],
            ),
            ...
            Row(
              ...
              children: [
                ...
                Text(
                    '${result.teamBTotalDamage} dmg (${result.teamBKills} kills)'),
              ],
            ),
          ],
        ),
  ...
  void _openMonteCarlo() {
    if (_teamA.isEmpty || _teamB.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text(
                'Add monsters to both teams to calculate win probabilities!')),
      );
      return;
    }
            ...
            child: Column(
              ...
              children: ArenaEnvironment.values.map((env) {
                ...
                return Container(
                  ...
                  decoration: BoxDecoration(
                    color: env.themeColor.withAlpha(
                        isSelected ? (isDark ? 30 : 20) : (isDark ? 12 : 8)),
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Wrap(
                        spacing: 6,
                        runSpacing: 4,
                        children: env.mechanicTags
                            .map((tag) => Container(
                                  padding: const EdgeInsets.symmetric(
                                      horizontal: 6, vertical: 2),
                                  decoration: BoxDecoration(
                                    color: env.themeColor
                                        .withAlpha(isSelected ? 30 : 18),
                                    borderRadius: BorderRadius.circular(4),
                                    border: Border.all(
                                        color: env.themeColor.withAlpha(60)),
                                  ),
                                  child: Text(
                                    tag,
                                    style: TextStyle(
        ...
        appBar: AppBar(
          ...
          actions: [
            ...
            PopupMenuButton<ArenaPresetMatchup>(
              ...
              itemBuilder: (context) =>
                  ArenaPresetMatchup.defaultPresets.map((preset) {
                          ...
                          Row(
                            ...
                            children: [
                              PopupMenuButton<ArenaEnvironment>(
                                ...
                                itemBuilder: (context) =>
                                    ArenaEnvironment.values.map((env) {
                                  return PopupMenuItem(
                                    value: env,
                                    child: Row(
                                      children: [
                                        Icon(env.icon,
                                            color: env.themeColor, size: 18),
                                        const SizedBox(width: 8),
                                        Expanded(
                                          child: Text(
                                            env.label,
                                ...
                                child: Container(
                                  ...
                                  decoration: BoxDecoration(
                                    color:
                                        _environment.themeColor.withAlpha(25),
                              ...
                              IconButton(
                                icon: Icon(Icons.info_outline,
                                    size: 16, color: _environment.themeColor),
                                tooltip: 'Arena Rules & Descriptors Guide',
                                visualDensity: VisualDensity.compact,
                                padding: EdgeInsets.zero,
                                constraints: const BoxConstraints(),
                                onPressed: () =>
                                    _showEnvironmentDetails(context),
                              ),
                            ],
                          ),
                      ...
                      InkWell(
                        ...
                        child: Container(
                          ...
                          child: Row(
                            children: [
                              Icon(_environment.icon,
                                  size: 14, color: _environment.themeColor),
                              const SizedBox(width: 6),
                              Expanded(
                                child: Text(
                    ...
                    if (isSetup && combatants.isNotEmpty) ...[
                      ...
                      IconButton(
                        icon: Icon(Icons.delete_sweep_outlined,
                            size: 16, color: team.color.withAlpha(200)),
              ...
              ? Center(
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        Icon(team.icon,
                            size: 36, color: Colors.grey.withAlpha(80)),
                        const SizedBox(height: 8),
                        Text(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ArenaCombatantCard(
                      ...
                      onRemove: () {
                        setState(() {
                          final list =
                              team == ArenaTeam.teamA ? _teamA : _teamB;
                        ...
                        setState(() {
                          ...
                          list.add(
                            ArenaCombatant.fromMonster(
                              ...
                              team: team,
                              customName:
                                  '${fighter.monster.getName(edition)} #${list.length + 1}',
                              edition: edition,
                            ),
                          );
                        });
