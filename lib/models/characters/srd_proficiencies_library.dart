...
class SrdProficienciesLibrary {
  ...
  static AbilityType getRecommendedAbility(String tool) {
    ...
    if (clean.contains('smith') || clean.contains('mason'))
      return AbilityType.strength;
    if (clean.contains('cook') || clean.contains('brewer'))
      return AbilityType.wisdom;
    if (musicalInstruments.any((i) => i.toLowerCase() == clean))
      return AbilityType.charisma;
    if (gamingSets.any((g) => g.toLowerCase() == clean))
      return AbilityType.wisdom;
    if (vehicles.any((v) => v.toLowerCase() == clean))
