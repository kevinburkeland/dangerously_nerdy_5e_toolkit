...
class SubclassSpellsLibrary {
  ...
  static List<SpellItem> resolveFilterSpells(String filterString,
    ...
    return SpellbookLibrary.allSpells.where((spell) {
      ...
      if (targetLevels != null && !targetLevels.contains(spell.level))
        return false;

      if (targetSchools != null && !targetSchools.contains(spell.school))
        return false;

      if (requiresRitual &&
          !spell.rules2014.ritual &&
          !spell.rules2024.ritual) {
        return false;
      }
      ...
      if (targetSources != null) {
        ...
        final matchesSource = spell.tags.any((t) {
              ...
              final stripped = lt.startsWith('source:') ? lt.substring(7) : lt;
              return sources.contains(lt) || sources.contains(stripped);
            }) ||
      ...
      if (targetClasses != null) {
        final classes2014 =
            spell.rules2014.classes.map((c) => c.name.toLowerCase()).toSet();
        final classes2024 =
            spell.rules2024.classes.map((c) => c.name.toLowerCase()).toSet();
  ...
  static Set<String> getExpandedSpells(String classSlug, String? subclassSlug) {
    ...
    final cleanSubHyphen =
        subclassSlug.toLowerCase().replaceAll('_', '-').trim();
    ...
    final isDivineSoul = classSlug.toLowerCase() == 'sorcerer' &&
        (cleanSub.contains('divine_soul') ||
            cleanSub.contains('divine-soul') ||
            cleanSub == 'divinesoul' ||
            cleanSub.contains('divine'));
    if (isDivineSoul) {
      for (final s in SpellbookLibrary.allSpells) {
        if (s.rules2014.classes.contains(SpellClass.cleric) ||
            s.rules2024.classes.contains(SpellClass.cleric)) {
      ...
      ...SrdClassesLibrary.allSubclasses.where((s) {
        ...
        return s != matchedSub &&
            (sSlug == cleanSubHyphen ||
                ...
                sSlug.contains(cleanSubHyphen) ||
                sSlug.contains(cleanSub) ||
                (cleanSubHyphen.isNotEmpty &&
                    sName.toLowerCase().contains(cleanSubHyphen)));
      }),
    ];
  ...
  static bool isExpandedSpell(
    ...
    if (classSlug.toLowerCase() == 'rogue' &&
        cleanSub.contains('arcane_trickster')) {
      final rules = spell.getRules(edition);
      if (rules.classes.contains(SpellClass.wizard)) return true;
    }
    if (classSlug.toLowerCase() == 'fighter' &&
        cleanSub.contains('eldritch_knight')) {
      final rules = spell.getRules(edition);
      if (rules.classes.contains(SpellClass.wizard)) return true;
    }
  ...
  static bool isAlwaysPreparedSubclass(String classSlug,
    ...
    if (subclassSlug != null && subclassSlug.isNotEmpty) {
      final match =
          SrdClassesLibrary.findSubclass(subclassSlug, classSlug: classSlug);
      if (match != null) {
  ...
  static List<SpellItem> getAlwaysPreparedSpellsForLevel({
    ...
    if (!isAlwaysPreparedSubclass(classSlug, subclassSlug) ||
        subclassSlug == null ||
        subclassSlug.isEmpty) {
      return const [];
    }
