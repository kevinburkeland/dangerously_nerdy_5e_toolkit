...
class SrdBackgroundsLibrary {
  ...
  /// Extracts the starting equipment list for 2014 backgrounds.
  static List<String> extractStartingEquipment(String slug) {
    ...
    final desc = get2014Description(clean);
    final match = RegExp(r'\*\*Starting Equipment:\*\*\s*([^\n]+)', caseSensitive: false).firstMatch(desc);
    if (match != null) {
      final itemsText = match.group(1)!;
      return itemsText
          .split(RegExp(r',|\band\b'))
          .map((s) => s.trim().replaceAll(RegExp(r'^\s*a\s+|^\s*an\s+', caseSensitive: false), ''))
  ...
  /// Returns 2014 official background feature if known.
  static ({String name, String description})? get2014Feature(String slug) {
    ...
    final desc = get2014Description(clean);
    final match = RegExp(r'\*\*Feature:\s*([^*]+)\*\*\n+([^\n]+(?:\n\n[^\n]+)*)', caseSensitive: false).firstMatch(desc);
    if (match != null) {
      return (name: match.group(1)!.trim(), description: match.group(2)!.trim());
    }
    return null;
  }
  ...
  static Background? findBySlug(String slug) {
    final clean = slug.toLowerCase().trim();
    final bg = allBackgrounds.where((b) => b.id.slug == clean || b.name.toLowerCase() == clean).firstOrNull;
    if (bg == null) return null;
    final feature = get2014Feature(clean);
    ...
    final skills = bg.skillProficiencies.isNotEmpty
        ...
        : (clean == 'acolyte' ? const ['Insight', 'Religion'] : const <String>[]);
    return bg.copyWith(
