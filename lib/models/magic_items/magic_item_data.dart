...
class ItemEditionDetails {
  ...
  final String?
      activation; // e.g. "1 Action", "1 Bonus Action", "Passive", "Special"
...
class MagicItem {
  ...
  Color? get effectiveGlyphColor {
    ...
    if (lowerName.contains('clear spindle') ||
        lowerName.contains('pearl') ||
        lowerName.contains('white dragon') ||
        lowerName.contains('(white)') ||
        lowerName.contains('ivory') ||
        lowerName.contains('diamond') ||
        lowerTags.contains('pearl')) {
      return const Color(0xFFF1F5F9); // Luminous Pearl White
    }
    // Black / Obsidian / Ebony
    if (lowerName.contains('black dragon') ||
        lowerName.contains('(black)') ||
        lowerName.contains('obsidian') ||
        lowerName.contains('ebony')) {
      return const Color(0xFF475569); // Dark Obsidian Slate
    }
    // Gold / Solar / Radiant / Sun
    if (lowerName.contains('gold dragon') ||
        lowerName.contains('golden') ||
        lowerName.contains('sun blade') ||
        lowerName.contains('radiant')) {
      return const Color(0xFFF59E0B); // Solar Radiant Gold
    }
    // Silver / Lunar / Moon
    if (lowerName.contains('silver dragon') ||
        lowerName.contains('silver') ||
        lowerName.contains('moon sickle') ||
        lowerName.contains('mithral')) {
  ...
  List<ActionTraitRing> getGlyphActionRings(
    ...
    void addRing(ActionRingType type,
        {DamageAccent? damageType,
        List<DamageAccent> damageTypes = const [],
        String? label}) {
  ...
  DamageAccent? getGlyphPrimaryDamageAccent(
      [DmRulesEdition edition = DmRulesEdition.v2024]) {
  ...
  List<DamageAccent> getGlyphDamageAccents(
      [DmRulesEdition edition = DmRulesEdition.v2024]) {
  ...
  String getEffectivePrice([DmRulesEdition edition = DmRulesEdition.v2024]) {
    ...
    return switch (rarity) {
      ...
      ItemRarity.veryRare =>
        consumable ? '10,000–25,000 gp' : '5,000–50,000 gp',
      ItemRarity.legendary =>
        consumable ? '50,000–100,000+ gp' : '50,000–200,000+ gp',
  ...
  ItemCraftingDetails getCraftingDetails(
      [DmRulesEdition edition = DmRulesEdition.v2024]) {
    final lowerName = name.toLowerCase();
    final lowerTags = tags.map((t) => t.toLowerCase()).toSet();
    final rules = getRules(edition);
    final text =
        '${rules.summary} ${rules.description} ${rules.properties.join(" ")}'
            .toLowerCase();
    final consumable = isConsumable;
    ...
    if (rarity == ItemRarity.nonmagical) {
      ...
      final basePrice = match != null
          ? int.tryParse(match.group(1)!.replaceAll(',', '')) ?? 10
          : 10;
      ...
      time2024Display =
          '$days2024 ${days2024 == 1 ? 'day' : 'days'} (at 10 gp/day progress rate)';
      time2014Display =
          '$days2014 ${days2014 == 1 ? 'day' : 'days'} (at 5 gp/day RAW progress)';
      minLevel = null;
      ...
      notes.add(
          '2024 Rules: Progress is made at 10 gp per day during downtime.');
    } else if (rarity == ItemRarity.common) {
      goldCost = consumable ? 25 : 50;
      goldCostDisplay =
          '$goldCost gp${consumable ? " (Consumable half-cost)" : ""}';
      ...
      if (consumable) {
        notes.add(
            'Consumable item: Halves crafting time and gold material costs.');
      }
      ...
      time2014Display =
          consumable ? '1 workweek (5 days)' : '2 workweeks (10 days)';
      minLevel = 3;
      ...
      time2014Display =
          consumable ? '5 workweeks (25 days)' : '10 workweeks (50 days)';
      minLevel = 6;
      exoticCr =
          'CR 9–12 monster component (e.g., Young Dragon, Salamander, Chimera)';
      notes.add('Requires Arcane Formula / Blueprint.');
      notes.add('Character must be at least 6th level.');
      notes.add(
          'Assistant crafters with tool proficiency can divide the crafting time.');
      if (consumable) {
        notes.add(
            'Consumable item: Halves crafting time and gold material costs.');
      }
      ...
      time2014Display =
          consumable ? '12.5 workweeks (62.5 days)' : '25 workweeks (125 days)';
      minLevel = 11;
      exoticCr =
          'CR 13–18 monster component (e.g., Adult Dragon, Iron Golem, Marilith, Purple Worm)';
      notes.add('Requires Arcane Formula / Master Blueprint.');
      notes.add('Character must be at least 11th level.');
      notes.add(
          'Typically crafted in a high-tier Bastion or with an organized artisan guild.');
      if (consumable) {
        notes.add(
            'Consumable item: Halves crafting time and gold material costs.');
      }
      ...
      time2014Display =
          consumable ? '25 workweeks (125 days)' : '50 workweeks (250 days)';
      minLevel = 17;
      exoticCr =
          'CR 19+ monster component (e.g., Ancient Dragon, Pit Fiend, Kraken, Lich)';
      notes.add('Requires Master Arcane Formula.');
      notes.add('Character must be at least 17th level.');
      notes.add(
          'Requires mythic planar forging or grand guild/Bastion enterprise.');
      if (consumable) {
        notes.add(
            'Consumable item: Halves crafting time and gold material costs.');
      }
      ...
      goldCost = 0;
      goldCostDisplay =
          'Priceless / Cosmic (Cannot be crafted via standard downtime)';
      days2024 = 0;
      ...
      formulaRequired = false;
      notes.add(
          'Artifacts cannot be forged through ordinary mortal downtime crafting.');
      notes.add(
          'Forging an artifact requires a cosmic quest, planar furnace, or divine patron.');
    }

    if (spells.isNotEmpty) {
      notes.add(
          'Spellcasting Prerequisite: Must be capable of casting ${spells.join(", ")} or expend the requisite spell slot daily during crafting.');
    }
  ...
  bool matches(
    ...
    if (damageAccentFilter != null && damageAccent != damageAccentFilter)
      return false;
...
enum CurrencyCoinType {
  ...
  const CurrencyCoinType(
      this.displayName, this.suffix, this.darkColor, this.lightColor);
