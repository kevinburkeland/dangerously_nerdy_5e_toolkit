...
extension DprAttackActionSerialization on DprAttackAction {
  Map<String, dynamic> toMap() {
    return {
      ...
      'rechargeRoll': rechargeRoll,
      'isLegendaryAction': isLegendaryAction,
  ...
  static DprAttackAction fromMap(Map<String, dynamic> map) {
    ...
    final secDCount =
        ((map['secondaryDiceCount'] as num?)?.toInt() ?? 0).clamp(0, 100);
    final secDSides =
        ((map['secondaryDiceSides'] as num?)?.toInt() ?? 0).clamp(0, 100);
    final critThresh =
        ((map['critThreshold'] as num?)?.toInt() ?? 20).clamp(1, 20);
    ...
    return DprAttackAction(
      ...
      deliveryType: DprActionDeliveryType.values.firstWhere(
        ...
        orElse: () => DprActionDeliveryType.attackRoll,
      ),
      saveAbility: map['saveAbility']?.toString(),
      ...
      legendaryCost:
          ((map['legendaryCost'] as num?)?.toInt() ?? 1).clamp(1, 10),
      ...
      abilityModForAgonizing:
          (map['abilityModForAgonizing'] as num?)?.toInt() ?? 0,
      ...
      extraCritDiceCount:
          ((map['extraCritDiceCount'] as num?)?.toInt() ?? 0).clamp(0, 100),
      extraCritDiceSides:
          ((map['extraCritDiceSides'] as num?)?.toInt() ?? 0).clamp(0, 100),
      attackBuffDiceSides:
          ((map['attackBuffDiceSides'] as num?)?.toInt() ?? 0).clamp(0, 100),
  ...
  static DprCombatantProfile fromMap(Map<String, dynamic> map) {
    ...
    if (rawAttacks is List) {
      parsedAttacks = rawAttacks
          .whereType<Map>()
          .map((a) => DprAttackActionSerialization.fromMap(
    ...
    final abilityScore =
        ((map['abilityScore'] as num?)?.toInt() ?? 18).clamp(1, 30);
    final profBonus =
        ((map['proficiencyBonus'] as num?)?.toInt() ?? 3).clamp(1, 10);
    final sneakCount =
        ((map['sneakAttackDiceCount'] as num?)?.toInt() ?? 0).clamp(0, 50);
    final sneakSides =
        ((map['sneakAttackDiceSides'] as num?)?.toInt() ?? 6).clamp(0, 100);
    ...
    return DprCombatantProfile(
      ...
      hasHalflingLuck: map['hasHalflingLuck'] as bool? ?? false,
      attacks: parsedAttacks.isNotEmpty
          ? parsedAttacks
          : const [
              DprAttackAction(
                id: 'attack_default',
