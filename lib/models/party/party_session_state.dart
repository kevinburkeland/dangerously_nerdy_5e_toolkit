  ...
  factory PartySessionState.fromMap(Map<String, dynamic> map) {
    ...
    if (rawMemberPurses is Map) {
      rawMemberPurses.forEach((k, v) {
        if (v is Map) {
          memberPurses[k.toString()] =
              PartyPurse.fromMap(Map<String, dynamic>.from(v));
        }
      });
    }
    ...
    if (rawTelemetry is Map) {
      rawTelemetry.forEach((k, v) {
        if (v is Map) {
          partyTelemetry[k.toString()] =
              CharacterTelemetryDto.fromMap(Map<String, dynamic>.from(v));
        }
      });
    }
      ...
      rawShared.forEach((k, v) {
        if (v is Map) {
          ...
          if (!partyTelemetry.containsKey(k.toString())) {
            try {
              partyTelemetry[k.toString()] =
                  CharacterTelemetryDto.fromLegacyCharacterMap(m);
            } catch (_) {}
          }
        }
      });
