...
class SummonPreset {
  ...
  final String
      levelDisplay; // e.g. "5th-level Transmutation", "Wondrous Item (Rare)"
  ...
  SpellItem? get sourceSpell =>
      spellId != null ? SpellbookLibrary.getSpellById(spellId!) : null;
  ...
  List<MinionStatBlock> get effectiveStatBlocks {
    ...
    for (final monsterItem in MonsterCodexLibrary.homebrewMonsters) {
      ...
      if (existingIds.contains(sb.id) ||
          existingNames.contains(sb.name.toLowerCase())) {
        continue;
      }
      ...
      if (id == 'conjure_animals') {
        ...
        if ((typeLower.contains('beast') ||
                typeLower.contains('monstrosity')) &&
...
extension SummonPresetGlyphExt on SummonPreset {
  SpellSchool get glyphSchool {
    ...
    if (lower.contains('transmutation') || lower.contains('animate objects'))
  ...
  ItemCategory get glyphItemCategory {
    ...
    if (lower.contains('armor') || lower.contains('shield'))
      return ItemCategory.armor;
    if (lower.contains('sword') ||
        lower.contains('weapon') ||
        lower.contains('bow')) return ItemCategory.weapon;
    ...
    for (final sb in statBlocks) {
      ...
      for (final r in dmgAccent) {
        if (r.ringType != ActionRingType.concentration &&
            ...
            !rings.any((existing) =>
                existing.ringType == r.ringType &&
                existing.damageType == r.damageType)) {
          rings.add(r);
        }
      }
    }
