...
class SpellSession {
  ...
  int getMaxAllowedCount(String statBlockId) {
    if (activePreset.id == 'create_undead') {
      return switch (statBlockId) {
        'undead_ghoul' => 3 +
            (spellLevel - 6).clamp(0, 3), // 3 (6th), 4 (7th), 5 (8th), 6 (9th)
        'undead_ghast' || 'undead_wight' => spellLevel < 7
            ? 0
            : 2 + (spellLevel - 7).clamp(0, 2), // 2 (7th), 3 (8th), 4 (9th)
        'undead_mummy' => spellLevel < 8
            ? 0
            : 2 + (spellLevel - 8).clamp(0, 1), // 2 (8th), 3 (9th)
        _ => 0,
      };
    }
  ...
  bool canAddMinion(MinionStatBlock statBlock) {
    ...
    if (activePreset.id == 'giant_insect') {
      final maxAllowed = getMaxAllowedCount(statBlock.id);
      final currentCount = activeObjects
          .where((o) =>
              o.name.toLowerCase().contains(statBlock.name.toLowerCase()))
          .length;
  ...
  void addObject(ObjectSize size,
      {String? customName, String damageType = 'Bludgeoning'}) {
  ...
  void addMinionFromStatBlock(MinionStatBlock statBlock, {String? customName}) {
    ...
    if (activeObjects.length >= 50) return;
    int count =
        activeObjects.where((o) => o.name.startsWith(statBlock.name)).length +
            1;
  ...
  MinionStatBlock rollBagOfTricks() {
    ...
    addMinionFromStatBlock(selected,
        customName: '${activePreset.name}: ${selected.name}');
    return selected;
  }
  ...
  void healAll() {
    activeObjects =
        activeObjects.map((obj) => obj.copyWith(currentHp: obj.maxHp)).toList();
  }

  void applyGroupDamage(int amount) {
    activeObjects =
        activeObjects.map((obj) => obj.applyDamage(amount)).toList();
  }

  void updateObject(String id,
      AnimatedObjectInstance Function(AnimatedObjectInstance) update) {
    final index = activeObjects.indexWhere((o) => o.id == id);
    if (index != -1) {
  ...
  ({List<int> rolls, List<int>? maxedRolls, int damage}) _rollDiceDamage({
    ...
    return (
      rolls: poolResult.individualRolls,
      maxedRolls: null,
      damage: poolResult.total
    );
  }
  ...
  AttackRollResult resolveIndividualAttack({
    ...
    final roll2 =
        (effectiveAdv != RollMode.normal) ? _rng.nextInt(20) + 1 : null;
    ...
    if (isHit) {
      ...
      if (object.secondaryDamageDiceCount > 0 &&
          object.secondaryDamageDiceSides > 0) {
  ...
  BatchAttackSummary performBatchAttack({
    ...
    final livingObjects =
        activeObjects.where((o) => !o.isDead).toList(growable: false);
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      'activeObjects': activeObjects
          .map((o) => AnimatedObjectDto.fromDomain(o).toMap())
          .toList(),
    };
  }
  ...
  factory SpellSession.fromMap(Map<String, dynamic> map) {
    ...
    if (rawObjects is List) {
      objects = rawObjects
          .whereType<Map>()
          .map((m) => AnimatedObjectDto.fromMap(Map<String, dynamic>.from(m))
              .toDomain())
          .toList();
    }
    return SpellSession(
