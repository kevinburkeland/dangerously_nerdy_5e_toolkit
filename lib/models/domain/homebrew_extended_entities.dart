...
class FeatureOption {
  ...
  final Map<String, dynamic>
      grants; // e.g. {'acBonus': 1, 'bonusSpells': [...], 'proficiencies': [...]}
  ...
  FeaturePrerequisite get prerequisite =>
      FeaturePrerequisite.fromFeatureOption(this);
...
class FeaturePrerequisite {
  final int? minLevel;
  final String?
      requiredPact; // e.g. 'blade', 'tome', 'chain', or dynamic custom pact slug
  ...
  factory FeaturePrerequisite.fromFeatureOption(FeatureOption option) {
    ...
    final rawList = option.customProperties['prerequisite'] ??
        option.customProperties['prerequisites'];
    if (rawList is List && rawList.isNotEmpty) {
      for (final item in rawList) {
        if (item is Map) {
          ...
          if (item['spell'] != null) {
            final spList =
                item['spell'] is List ? item['spell'] as List : [item['spell']];
    ...
    if (requiredPact == null &&
        option.customProperties['requiredPact'] is String) {
      requiredPact = option.customProperties['requiredPact']
          .toString()
          .toLowerCase()
          .trim();
    }
    if (requiredSpell == null &&
        option.customProperties['requiredSpell'] is String) {
      requiredSpell = option.customProperties['requiredSpell']
          .toString()
          .toLowerCase()
          .trim();
    }
    ...
    if (prereqHeaderMatch != null) {
      ...
      if (requiredPact == null) {
        final pactMatch =
            RegExp(r'Pact of the ([A-Za-z]+)', caseSensitive: false)
                .firstMatch(prereqText);
        if (pactMatch != null) {
      ...
      if (requiredSpell == null) {
        if (prereqText.toLowerCase().contains('eldritch blast')) {
          ...
          final spellMatch =
              RegExp(r'([A-Za-z ]+)\s+(?:cantrip|spell)', caseSensitive: false)
                  .firstMatch(prereqText);
          if (spellMatch != null) {
  ...
  FeaturePrerequisiteEvaluation evaluate({
    ...
    if (requiredPact != null) {
      ...
      if (!hasPact) {
        final pactName = switch (req) {
          ...
          _ =>
            'Pact of the ${req.isNotEmpty ? req[0].toUpperCase() + req.substring(1) : req}',
        };
    ...
    // Check spell
    if (requiredSpell != null) {
      final reqSpellNorm = requiredSpell!
          .toLowerCase()
          .replaceAll(' ', '-')
          .replaceAll('_', '-')
          .trim();
      final hasSpell = knownSpellSlugs.any((s) {
        ...
        return norm == reqSpellNorm ||
            norm.contains(reqSpellNorm) ||
            reqSpellNorm.contains(norm);
      });
      if (!hasSpell) {
...
class CharacterClass extends DomainEntity {
  ...
  List<ClassFeatureDecision> getDecisionsForLevel(int level,
    ...
    return featureDecisions.where((d) {
      ...
      if (ruleset != null &&
          d.ruleset != null &&
          d.ruleset != ruleset &&
          d.ruleset != RulesetVersion.homebrew) {
        return false;
      }
      return true;
    }).toList();
  ...
  List<SkillType> get allowedSkills {
    ...
    if (cleanSlug.contains('warrior sidekick') ||
        cleanSlug == 'warrior-sidekick' ||
        cleanName.contains('warrior sidekick')) {
    ...
    } else if (cleanSlug.contains('spellcaster sidekick') ||
        cleanSlug == 'spellcaster-sidekick' ||
        cleanName.contains('spellcaster sidekick')) {
      archetypeFallback = const [
        SkillType.arcana,
        SkillType.history,
        SkillType.insight,
    ...
    if (rawList is List && rawList.isNotEmpty) {
      ...
      if (parsed.length == SkillType.values.length &&
          archetypeFallback != null) {
    ...
    final sp = customProperties['startingProficiencies'] ??
        (customProperties['rawJson'] is Map
            ? customProperties['rawJson']['startingProficiencies']
            : null) ??
        (customProperties['proficiency'] is Map
            ? customProperties['proficiency']
            : null);
    if (sp is Map) {
  ...
  factory CharacterClass.fromMap(Map<String, dynamic> map) {
    ...
    if (grants.isEmpty) {
      ...
      if (addSpells != null) {
        final slug =
            map['id'] is Map ? (map['id']['slug']?.toString() ?? '') : '';
    ...
    return CharacterClass(
      ...
      subclassSelectionLevel:
          (map['subclassSelectionLevel'] as num?)?.toInt() ?? 3,
      featureDecisions: (map['featureDecisions'] as List? ?? [])
          .whereType<Map>()
          .map(
              (d) => ClassFeatureDecision.fromMap(Map<String, dynamic>.from(d)))
          .toList(),
      grants: grants,
      customProperties: customProperties,
    );
  ...
  CharacterClass copyWith({
    ...
    return CharacterClass(
      ...
      subclassSelectionLevel:
          subclassSelectionLevel ?? this.subclassSelectionLevel,
...
class Subclass extends DomainEntity {
  ...
  factory Subclass.fromMap(Map<String, dynamic> map) {
    ...
    var classSlug = (map['classSlug'] ??
                map['className'] ??
                map['class'] ??
                customProperties['classSlug'] ??
                customProperties['className'])
            ?.toString()
            .trim() ??
        '';
    if (classSlug.contains('|')) {
      classSlug = classSlug.split('|').first.trim();
    }
    classSlug = classSlug.toLowerCase().replaceAll(' ', '-');

    if (classSlug.isEmpty && map['id'] is Map) {
      final sSlug = (map['id']['slug']?.toString() ?? '').toLowerCase().trim();
      const srdClasses = [
        'barbarian',
        'bard',
        'cleric',
      ...
      for (final core in srdClasses) {
        if (sSlug.startsWith('$core-')) {
          classSlug = core;
          break;
        }
      }
      if (classSlug.isEmpty) {
        final hyphenIndex = sSlug.indexOf('-');
        if (hyphenIndex > 0) {
          classSlug = sSlug.substring(0, hyphenIndex);
        }
      }
    }
...
class Race extends DomainEntity {
  ...
  Race({
    ...
    this.customProperties = const {},
            ...
            bonusFeatCount ?? _resolveBonusFeatCount(id.slug, customProperties),
        flexibleAbilityCount = flexibleAbilityCount ??
            _resolveFlexibleAbilityCount(id.slug, customProperties),
        flexibleAbilityBonus = flexibleAbilityBonus ??
            _resolveFlexibleAbilityBonus(id.slug, customProperties),
        fixedAbilityBonuses = fixedAbilityBonuses ??
            _resolveFixedAbilityBonuses(id.slug, customProperties);
  ...
  static int _resolveBonusFeatCount(String slug, Map<String, dynamic> custom) {
    ...
    if (custom['isVariantHuman'] == true || custom['isCustomLineage'] == true)
      return 1;
  ...
  static int _resolveFlexibleAbilityCount(
      String slug, Map<String, dynamic> custom) {
    if (custom['abilityChoiceCount'] is num)
      return (custom['abilityChoiceCount'] as num).toInt();
    if (custom['flexibleAbilityCount'] is num)
      return (custom['flexibleAbilityCount'] as num).toInt();
  ...
  static int _resolveFlexibleAbilityBonus(
      String slug, Map<String, dynamic> custom) {
    if (custom['abilityChoiceBonus'] is num)
      return (custom['abilityChoiceBonus'] as num).toInt();
    if (custom['flexibleAbilityBonus'] is num)
      return (custom['flexibleAbilityBonus'] as num).toInt();
    if (slug == 'custom-lineage' || custom['isCustomLineage'] == true) return 2;
    return 1;
  }

  static Map<String, int> _resolveFixedAbilityBonuses(
      String slug, Map<String, dynamic> custom) {
  ...
  /// Returns whether this race / lineage grants a starting bonus feat in 2014 (e.g. Variant Human or homebrew).
  bool get grantsBonusFeat => bonusFeatCount > 0;

  /// Number of flexible ability score increases player can choose (e.g. 2 for Variant Human).
  int get flexibleAbilityChoiceCount => flexibleAbilityCount;

  /// The bonus value added to each chosen flexible ability score (default 1).
  ...
  List<String>? get flexibleAbilityPool =>
      (customProperties['flexibleAbilityPool'] as List?)
          ?.map((e) => e.toString())
          .toList();
  ...
  factory Race.fromMap(Map<String, dynamic> map) {
    ...
    return Race(
      ...
      fixedAbilityBonuses: map['fixedAbilityBonuses'] is Map
          ? (map['fixedAbilityBonuses'] as Map)
...
class Subrace extends DomainEntity {
  ...
  Map<String, dynamic> toMap() => {
        ...
        'raceSlug': raceSlug,
        'traitsMarkdown': traitsMarkdown,
        if (abilityScoreSummary != null)
          'abilityScoreSummary': abilityScoreSummary,
        if (fixedAbilityBonuses.isNotEmpty)
          'fixedAbilityBonuses': fixedAbilityBonuses,
        if (flexibleAbilityCount > 0)
          'flexibleAbilityCount': flexibleAbilityCount,
        if (flexibleAbilityBonus > 0)
          'flexibleAbilityBonus': flexibleAbilityBonus,
        if (flexibleAbilityPool != null && flexibleAbilityPool!.isNotEmpty)
          'flexibleAbilityPool': flexibleAbilityPool,
  ...
  factory Subrace.fromMap(Map<String, dynamic> map) {
    ...
    final rawDarkvision =
        map['darkvision'] ?? map['customProperties']?['darkvision'];
    final darkvisionVal = rawDarkvision is num
        ? rawDarkvision.toInt()
        : (rawDarkvision == true ? 60 : null);

    final flexProps =
        map['customProperties'] is Map ? map['customProperties'] as Map : null;
    final flexAbilities = flexProps?['flexibleAbilities'] is Map
        ? flexProps!['flexibleAbilities'] as Map
        : null;
    ...
    final flexiblePool =
        poolRaw?.map((e) => e.toString().toLowerCase()).toList();
    ...
    return Subrace(
      ...
      speed: map['speed']?.toString() ??
          map['customProperties']?['speed']?.toString(),
...
class Feat extends DomainEntity {
  ...
  final String
      category; // e.g. "Origin", "General", "Fighting Style", "Epic Boon"
  final String descriptionMarkdown;
...
extension FeatAsiExtension on Feat {
  ...
  static ({List<AbilityType> selectableAbilities, int amount})
      parseFeatAbilityData(dynamic rawAbility) {
  ...
  List<AbilityType> get selectableAbilities {
    ...
    final rawList = customProperties['selectableAbilities'] ??
        customProperties['statChoicePool'];
    if (rawList is List && rawList.isNotEmpty) {
    ...
    // 3. Fallback to slug-based defaults for standard SRD / common tabletop feats if not explicitly declared
    return switch (id.slug.toLowerCase()) {
      'tenacious' => AbilityType.values,
  ...
  int get statIncreaseAmount {
    ...
    final rawAbility =
        customProperties['ability'] ?? customProperties['abilities'];
    if (rawAbility != null) {
      final parsed = parseFeatAbilityData(rawAbility);
  ...
  /// Whether this feat grants a skill proficiency choice.
  bool get hasSkillProficiencyChoice {
    if (customProperties['hasSkillChoice'] == true) return true;
    final sp = customProperties['skillProficiencies'];
    if (sp is List) {
      return sp.any(
          (e) => e is Map && (e.containsKey('choose') || e.containsKey('any')));
    }
    return false;
  }
  ...
  /// Whether this feat grants an expertise choice.
  bool get hasExpertiseChoice {
    if (customProperties['hasExpertiseChoice'] == true) return true;
    final exp = customProperties['expertise'];
    if (exp is List && exp.isNotEmpty) return true;
    return false;
  }

  /// Whether this feat grants an Eldritch Invocation choice.
  bool get hasInvocationChoice {
    if (customProperties['hasInvocationChoice'] == true) return true;
    final ofp = customProperties['optionalfeatureProgression'];
    if (ofp is List) {
      for (final item in ofp) {
        if (item is Map) {
          final ft = item['featureType'];
          if (ft is List && ft.any((e) => e.toString().toUpperCase() == 'EI'))
            return true;
  ...
  /// Whether this feat grants a Fighting Style choice.
  bool get hasFightingStyleChoice {
    if (customProperties['hasFightingStyleChoice'] == true) return true;
    final ofp = customProperties['optionalfeatureProgression'];
    if (ofp is List) {
      for (final item in ofp) {
        if (item is Map) {
          final ft = item['featureType'];
          if (ft is List &&
              ft.any((e) => e.toString().toUpperCase().startsWith('FS')))
            return true;
          final n = item['name']?.toString().toLowerCase() ?? '';
    ...
    final ft = customProperties['featureType'];
    if (ft is List &&
        ft.any((e) => e.toString().toUpperCase().startsWith('FS'))) return true;
    return false;
  }

  /// Evaluates whether a character can select a specific [FeatureOption] invocation under Eldritch Adept RAW rules.
  ///
  /// Under tabletop rules: "If the invocation has a prerequisite of any kind, you can choose that invocation
  ...
  FeaturePrerequisiteEvaluation evaluateInvocationPrerequisite(
    ...
    if (!isWarlock) {
      return const FeaturePrerequisiteEvaluation(
        isMet: false,
        unmetReasons: [
          'Requires Warlock class to select an invocation with prerequisites'
        ],
      );
    }
...
class HomebrewCompendiumEntry extends DomainEntity {
  ...
  final String name;
  final String
      category; // e.g. "Table", "Rule", "Optional Feature", "Condition", "Hazard", "Reward"
  final String descriptionMarkdown;
