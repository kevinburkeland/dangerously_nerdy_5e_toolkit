...
enum HomebrewOtherCategory {
  ...
  static bool isRollingTable({
    ...
    if (customProperties != null) {
      ...
      if (colLabels is List && colLabels.isNotEmpty) {
        final diceRegex = RegExp(r'\b(?:d\d+|d%|roll|die|dice|result)\b',
            caseSensitive: false);
        for (final col in colLabels) {
          final colStr =
              col is Map ? (col['entry'] ?? col.toString()) : col.toString();
      ...
      if (rows is List && rows.isNotEmpty) {
        ...
        if (firstRow is List && firstRow.isNotEmpty) {
          ...
          if (firstCell is Map &&
              (firstCell.containsKey('min') || firstCell.containsKey('roll'))) {
            return true;
          }
          final cellStr = firstCell.toString().trim();
          if (RegExp(r'^\s*(?:\d{1,3}\s*[-–—]\s*\d{1,3}|\d{1,3}|d\d+)\s*$')
              .hasMatch(cellStr)) {
            return true;
          }
  ...
  static HomebrewOtherCategory classify({
    ...
    if (cat.contains('table') ||
        (customProperties != null && customProperties.containsKey('rows'))) {
      if (isRollingTable(
          category: cat, name: n, customProperties: customProperties)) {
    ...
    // Deities
    if (cat.contains('deity') ||
        (customProperties != null &&
            customProperties.containsKey('pantheon'))) {
    ...
    if (cat.contains('vehicle') ||
        (customProperties != null &&
            (customProperties.containsKey('vehicleType') ||
                customProperties.containsKey('upgradeType')))) {
    ...
    if (cat.contains('trap') ||
        cat.contains('hazard') ||
        (customProperties != null &&
            (customProperties.containsKey('trapType') ||
                customProperties.containsKey('hazardType')))) {
