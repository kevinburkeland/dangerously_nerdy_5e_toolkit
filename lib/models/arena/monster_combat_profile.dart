...
class MonsterCombatProfile {
  ...
  factory MonsterCombatProfile.fromStatBlock(
    ...
    final canBurrow =
        parsed.canBurrow || sb.speed.toLowerCase().contains('burrow');
    final canClimb =
        parsed.canClimb || sb.speed.toLowerCase().contains('climb');
    final maxLegendaryActions = math.max(
        sb.legendaryActions.isNotEmpty ? 3 : 0, parsed.maxLegendaryActions);
    final maxLegendaryResistances = math.max(
        sb.hasLegendaryResistance ? 3 : 0, parsed.maxLegendaryResistances);
