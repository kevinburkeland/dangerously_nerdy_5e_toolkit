...
enum ArenaTargetingStrategy {
  focusLowestHp('Focus Fire (Lowest HP)',
      'Focuses attacks on the weakest living enemy to eliminate them quickly.'),
  randomEnemy(
      'Random Target', 'Attacks a random living enemy combatant each round.'),
  highestThreat('Highest Threat (Highest CR)',
      'Targets the most dangerous enemy with the highest Challenge Rating or max HP.');
...
class ArenaSimulationResult {
  ...
  List<ArenaCombatant> get survivingTeamA => finalCombatants
      .where((c) => c.team == ArenaTeam.teamA && c.isAlive)
      .toList();
  List<ArenaCombatant> get survivingTeamB => finalCombatants
...
class ArenaMonteCarloResult {
  ...
  double get teamAWinRate =>
      iterations > 0 ? (teamAWins / iterations) * 100 : 0.0;
  double get teamBWinRate =>
      iterations > 0 ? (teamBWins / iterations) * 100 : 0.0;
