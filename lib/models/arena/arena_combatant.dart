...
export 'package:vtt_engine_core/simulation/combat_rider.dart';
...
enum ArenaTeam {
  ...
  ArenaTeam get opponent =>
      this == ArenaTeam.teamA ? ArenaTeam.teamB : ArenaTeam.teamA;
}
...
class ArenaCombatant {
  ...
  ArenaCombatant({
    ...
    this.isHealingSuppressed = false,
        ...
        knownSpellIds =
            knownSpellIds != null ? List<String>.from(knownSpellIds) : [],
  ...
  static ISet<ArenaCondition> _parseConditions(
      dynamic conditions, dynamic activeConditions) {
    if (conditions != null) {
      if (conditions is ISet<ArenaCondition>) return conditions;
      if (conditions is Iterable<ArenaCondition>) return conditions.toISet();
      if (conditions is Iterable)
        return ISet<ArenaCondition>(conditions.cast<ArenaCondition>());
    }
  ...
  static IList<ActiveCondition> _parseActiveConditions(
      dynamic activeConditions, dynamic conditions) {
    if (activeConditions != null) {
      if (activeConditions is IList<ActiveCondition>) return activeConditions;
      if (activeConditions is Iterable<ActiveCondition>)
        return activeConditions.toIList();
      if (activeConditions is Iterable)
        return IList<ActiveCondition>(activeConditions.cast<ActiveCondition>());
    }
    if (conditions != null) {
  ...
  bool get isSpellcaster =>
      knownSpellIds.isNotEmpty || maxSpellSlots.isNotEmpty;
  ...
  double get hpPercent =>
      effectiveMaxHp > 0 ? (currentHp / effectiveMaxHp).clamp(0.0, 1.0) : 0.0;
  ...
  ({int fallDamage, bool fell, String? log}) applyActiveCondition(
    ...
    final idx = activeConditions
        .indexWhere((a) => a.condition == activeCondition.condition);
    if (idx >= 0) {
      ...
      if (!hasHover) {
        ...
        if (!activeConditions.any((a) => a.condition == ArenaCondition.prone)) {
          activeConditions = activeConditions
              .add(const ActiveCondition(condition: ArenaCondition.prone));
        }
        ...
        return (
          ...
          fell: true,
          log:
              '$displayName fell $fallHeight ft. from the air, taking $rawFallDamage bludgeoning damage and landing Prone!',
        );
      } else {
        return (
          fallDamage: 0,
          fell: false,
          log:
              '$displayName hovered in place despite suffering ${condition.label}!',
        );
      }
    }
  ...
  ({bool broken, int saveRoll, int dc, String? lostSpellId}) checkConcentration(
    ...
    return (
      ...
      saveRoll: totalSave,
      dc: dc,
      lostSpellId: activeConcentrationSpellId
    );
  }
  ...
  int applyDamage(int damage) {
    ...
    hitPoints =
        hitPoints.copyWith(tempHp: newTempHp).takeDamage(remainingDamage);
  ...
  void applyAttributeDrain(AbilityType ability, int drain,
      {bool deathAtZero = true}) {
    if (drain <= 0) return;
  ...
  ({List<String> logs, List<ArenaCondition> conditionsApplied}) applyRider(
    ...
    switch (rider) {
      case ConditionRider(
          ...
          :final requiresSave,
          :final saveDc,
          :final saveAttributeKey
        ):
        final saveAbility = saveAttributeKey != null
            ? AbilityType.fromLooseString(saveAttributeKey)
            : null;
        if (requiresSave && saveDc != null && saveAbility != null) {
          ...
          if (roll < saveDc) {
            applyActiveCondition(ActiveCondition(condition: condition),
                edition: edition);
            conditions.add(condition);
            final actionVerb =
                condition == ArenaCondition.prone ? 'fell' : 'became';
            logs.add(
                '$displayName failed DC $saveDc ${saveAbility.shortName.toUpperCase()} save ($roll) and $actionVerb ${condition.label}!');
          } else {
            logs.add(
                '$displayName succeeded on DC $saveDc ${saveAbility.shortName.toUpperCase()} save ($roll) against ${condition.label}.');
          }
        } else {
          applyActiveCondition(ActiveCondition(condition: condition),
              edition: edition);
      ...
      case AttributeDrainRider(
        ...
        final targetAbility = AbilityType.fromLooseString(targetAttributeKey);
        applyAttributeDrain(targetAbility, drain, deathAtZero: deathAtZero);
        logs.add(
            '$displayName had ${targetAbility.shortName.toUpperCase()} drained by $drain!');
        if (getAbilityScore(targetAbility) <= 0) {
          logs.add(
              "$displayName's ${targetAbility.shortName.toUpperCase()} was reduced to 0! $displayName dies instantly!");
        }
      case MaxHpReductionRider(
        ...
        final reduction =
            reductionEqualsDamage ? damageDealt : (flatReduction ?? 0);
  ...
  ({
    int damageDealt,
    List<String> riderLogs,
    List<ArenaCondition> conditionsApplied
  }) applyAttackHit(
  ...
  int getAbilitySavingThrowBonus(AbilityType ability,
      [DmRulesEdition edition = DmRulesEdition.v2024]) {
    final key = ability.shortName.toLowerCase();
    final sb = getStatBlock(edition);
    ...
    final profileBonus =
        monster.getCombatProfile(edition).savingThrowBonuses[key];
  ...
  int getSavingThrowBonus(String ability,
      [DmRulesEdition edition = DmRulesEdition.v2024]) {
    return getAbilitySavingThrowBonus(
        AbilityType.fromLooseString(ability), edition);
  }
}
